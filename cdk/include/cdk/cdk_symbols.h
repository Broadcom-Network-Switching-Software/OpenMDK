 ...
 * Real max is encoded as ((1 << min) * max) - 1)
 * Real min is always zero
 * Ex.: (max,min)=(17,12) => real max = ((1 << 12) * 17) - 1) = 0x10fff
 *
 * VAR
...
#define CDK_SYMBOL_INDEX_MAX_GET(w) \
    ((CDK_SYMBOL_INDEX_FLAGS_GET(w) & CDK_SYMBOL_INDEX_F_EXP) ? \
       (CDK_SYMBOL_INDEX_MAX_GETRAW(w) * (1 << CDK_SYMBOL_INDEX_MIN_GETRAW(w))) - 1 : \
...
typedef struct cdk_symbol_s {
    ...
    const char *ufname; 
#endif
#if  CDK_CONFIG_INCLUDE_RESET_VALUES == 1
