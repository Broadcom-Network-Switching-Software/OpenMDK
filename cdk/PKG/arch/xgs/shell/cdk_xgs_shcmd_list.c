...
_print_sym(const cdk_symbol_t *s, void *vptr)
{
    ...
    if (s->flags & CDK_SYMBOL_FLAG_MEMORY) {
        max = cdk_xgs_mem_maxidx(si->unit, s->addr, max);
    }
    ...
    if (s->flags & CDK_SYMBOL_FLAG_MEMORY) {
        ...
        CDK_PRINTF("Size:     %s-bit\n", 
                   (s->flags & CDK_SYMBOL_FLAG_R64) ? "64" : "32"); 
    }
    CDK_PRINTF("Flags:    "); 
    
    for (mask = 1; mask; mask <<= 1) {
        if (s->flags & mask) {
            flagstr = cdk_xgs_shell_symflag_type2name(si->unit, mask);
            if (flagstr != NULL) {
    ...
    if (s->fields) {
        ...
        cdk_symbol_show_fields(s, CDK_XGS_SYMBOLS(si->unit)->field_names,
...
int
cdk_shcmd_xgs_list(int argc, char *argv[])
{
    ...
    i = 0; 
    if ((argc == 0) || 
        ((i = cdk_xgs_shell_parse_args(argc, argv,
                                       csts, COUNTOF(csts))) >= 0)) {
        /* Error in argument i */
