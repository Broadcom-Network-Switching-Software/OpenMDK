...
_seti_reg(int argc, char *argv[], void *context)
{
    ...
    return cdk_xgs_shell_regops(unit, NULL, &sid, size, and_masks, or_masks); 
}
...
_seti_mem(int argc, char *argv[], void *context)
{
    ...
    return cdk_xgs_shell_memops(unit, NULL, &sid, size, and_masks, or_masks); 
}
...
_seti_miim(int argc, char *argv[], void *context)
{
    ...
    for (i = sid.port.start; i <= sid.port.end; i++) {
        if (cdk_xgs_miim_write(unit, sid.addr.name32, i, data) < 0) {
...
int
cdk_shcmd_xgs_seti(int argc, char* argv[])
{
    int unit;
