           ...
           uint32_t *and_masks, uint32_t *or_masks)
{
    ...
    for (port = sid->port.start; port <= sid->port.end; port++) {
        ...
        cdk_shell_id_t sid2; 
        int blktype; 
        const cdk_xgs_block_t *blkp; 

        /*
         * Iterate through all blocks of this symbol
         */
        for (blktype = 0; blktype < CDK_XGS_INFO(unit)->nblktypes; blktype++) {
             ...
             */
            for (b = 0; b < CDK_XGS_INFO(unit)->nblocks; b++) {

                /* Get the block pointer for this block */
                blkp = CDK_XGS_INFO(unit)->blocks + b; 
                CDK_ASSERT(blkp); 
                ...
                if (!sid2.block.valid) {
                    ...
                    CDK_STRCPY(sid2.block.name, cdk_xgs_shell_block_type2name(unit, blktype)); 
                    ...
                    if (port != -1) {
                        ...
                        if (cdk_xgs_port_block(unit, port, &pb, blkp->type) == 0 && 
                    ...
                    if (CDK_STRCMP(sid2.block.name, cdk_xgs_shell_block_type2name(unit, blktype)) == 0) {
                        ...
                        if (sid->block.start == -1) {
                            ...
                            sid2.block.start = cdk_xgs_block_number(unit, blktype, sid->block.start); 
                            sid2.block.end = cdk_xgs_block_number(unit, blktype, sid->block.end); 
                        }                               
                    }
                    else {                      
            ...
            cdk_xgs_shell_regops(unit, symbol, &sid2, wsize, and_masks, or_masks); 
        }
...
{    
    ...
    if (sid->addr.start == -1) {
        ...
        sid->addr.start = CDK_SYMBOL_INDEX_MIN_GET(symbol->index); 
        maxidx = CDK_SYMBOL_INDEX_MAX_GET(symbol->index); 
        sid->addr.end = cdk_xgs_mem_maxidx(unit, sid->addr.name32, maxidx);
    }
    ...
    for (blktype = 0; blktype < CDK_XGS_INFO(unit)->nblktypes; blktype++) {
        ...
        wsize = CDK_BYTES2WORDS(CDK_SYMBOL_INDEX_SIZE_GET(symbol->index));
        cdk_xgs_shell_memops(unit, symbol, &sid2, wsize, and_masks, or_masks); 
    }
...
cdk_xgs_shell_symop(int unit, const cdk_symbol_t *symbol, cdk_shell_id_t *sid, 
                    uint32_t *and_masks, uint32_t *or_masks)
{
    int cmic_blktype = cdk_xgs_shell_block_name2type(unit, "cmic");
