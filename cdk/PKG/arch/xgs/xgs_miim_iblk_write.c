...
cdk_xgs_miim_iblk_write(int unit, uint32_t phy_addr, uint32_t reg, uint32_t val)
{
    ...
    /* Select block */
    ioerr += cdk_xgs_miim_read(unit, phy_addr, 0x1f, &blk);
    if ((reg >> 8) ^ blk) {
        ioerr += cdk_xgs_miim_write(unit, phy_addr, 0x1f, reg >> 8);
    }

    /* Write register value */
    ioerr += cdk_xgs_miim_write(unit, phy_addr, reg & 0x1f, val);

    /* Restore block */
    if ((reg >> 8) ^ blk) {
        ioerr += cdk_xgs_miim_write(unit, phy_addr, 0x1f, blk);
    }
