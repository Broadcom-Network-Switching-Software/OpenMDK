...
_seti_reg(int argc, char *argv[], void *context)
{
    ...
    /* Default size is the number of words specified (in bytes) */
    size = CDK_WORDS2BYTES(cst.argc); 

    /* Optional third argument is the size of the register (in bytes) */
    if (argc > 0 && cdk_shell_parse_int(*argv, &size) < 0) {
        return cdk_shell_parse_error("size", *argv); 
    }

    return cdk_robo_shell_regops(unit, NULL, &sid, size, and_masks, or_masks); 
}
...
_seti_mem(int argc, char *argv[], void *context)
{
    ...
    return cdk_robo_shell_memops(unit, NULL, &sid, size, and_masks, or_masks); 
}
...
_seti_miim(int argc, char *argv[], void *context)
{
    ...
    for (i = sid.port.start; i <= sid.port.end; i++) {
        if (cdk_robo_miim_write(unit, sid.addr.name32, i, data) < 0) {
...
int
cdk_shcmd_robo_seti(int argc, char* argv[])
{
    int unit;
