...
_iter_op(const cdk_symbol_t *symbol, void *vptr)
{
    ...
    cdk_robo_shell_symop(unit, symbol, &sid, NULL, NULL);
...
int
cdk_shcmd_robo_get(int argc, char *argv[])
{       
    ...
    i = 0; 
    if ((argc == 0) || 
        ((i = cdk_robo_shell_parse_args(argc, argv, csts, COUNTOF(csts))) >= 0)) {
        /* Error in argument i */
