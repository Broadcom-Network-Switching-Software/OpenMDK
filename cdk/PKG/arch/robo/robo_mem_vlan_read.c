...
cdk_robo_mem_vlan_read(int unit, uint32_t addr, uint32_t idx, void *vptr, int size)
{
    ...
    ROBO_ARLA_VTBL_RWCTRLr_ARLA_VTBL_RW_CLRf_SET(vtbl_rwctrl, ROBO_MEM_OP_READ);
    ...
    while (cnt < MAX_POLL) {
        ...
        if (ioerr == 0 && 
            ...
            /* Read 32-bit VLAN entry */
            ioerr += cdk_robo_reg_read(unit, entry_reg, &vtbl_entry, bsize);
            break;
        }
    }
    ...
    for (cnt = 0; cnt < wsize; cnt++) {
        wdata[cnt] = ROBO_ARLA_VTBL_ENTRYr_GET(vtbl_entry, cnt);
