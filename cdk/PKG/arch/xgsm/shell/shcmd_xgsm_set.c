...
_iter_op(const cdk_symbol_t *symbol, void *vptr)
{
    ...
    cdk_shell_id_t sid;
    cdk_shell_tokens_t *csts = xgsm_iter->csts; 
    const char **fnames = CDK_XGSM_SYMBOLS(xgsm_iter->unit)->field_names;
    ...
    cdk_xgsm_shell_symop(unit, symbol, &sid, and_masks, or_masks);
...
int
cdk_shcmd_xgsm_set(int argc, char *argv[])
{
    ...
    if ((i = cdk_xgsm_shell_parse_args(argc, argv, csts, COUNTOF(csts))) >= 0) {
        /* Error in argument i */
    ...
    CDK_XGSM_CHIP_TREX_SET(unit, trex);

    sid.flags = sid_flags;

    xgsm_iter.unit = unit; 
    xgsm_iter.csts = &csts[data_start]; 
    xgsm_iter.sid = &sid; 
