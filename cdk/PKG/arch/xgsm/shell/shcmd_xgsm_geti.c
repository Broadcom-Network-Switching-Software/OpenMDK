...
_geti_reg(int argc, char *argv[], void *context)
{
    ...
    return cdk_xgsm_shell_regops(unit, NULL, &sid, size, NULL, NULL); 
}
...
_geti_mem(int argc, char *argv[], void *context)
{
     ...
     */
    return cdk_xgsm_shell_memops(unit, NULL, &sid, size, NULL, NULL); 
}
...
_geti_miim(int argc, char *argv[], void *context)
{
    ...
    for (i = sid.port.start; i <= sid.port.end; i++) {
        ...
        if (cdk_xgsm_miim_read(unit, sid.addr.name32, regaddr, &data) < 0) {
...
int
cdk_shcmd_xgsm_geti(int argc, char *argv[])
{
    int unit;
