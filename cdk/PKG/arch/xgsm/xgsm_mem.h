...
extern int
cdk_xgsm_mem_clear(int unit, uint32_t adext, uint32_t addr,
                   uint32_t si, uint32_t ei, int size); 

#define CDK_XGSM_MEM_CLEAR(_u, _m) \
    cdk_xgsm_mem_blocks_clear(_u, _m##_BLKACC, _m, _m##_MIN, _m##_MAX, (_m##_SIZE + 3) >> 2)
...
extern int
cdk_xgsm_mem_block_read(int unit, uint32_t blkacc, int block,
...
extern int
cdk_xgsm_mem_blocks_read(int unit, uint32_t blkacc, int port, uint32_t offset,
...
extern int
cdk_xgsm_mem_op(int unit, cdk_xgsm_mem_op_info_t *mem_op_info);
