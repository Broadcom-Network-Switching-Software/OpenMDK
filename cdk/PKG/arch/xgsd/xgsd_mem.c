                  ...
                  void* vptr, int size)
{
    ...
    /* Simulator hooks */
    if (cdk_simhook_read) {
        CDK_DEBUG_MEM(("cdk_xgsd_mem_read[%d]: "
    ...
    /* Issue SCHAN op */
    rv = cdk_xgsd_schan_op(unit, &schan_msg, 2, 1 + entry_dw);
    if (CDK_FAILURE(rv)) {
        CDK_ERR(("cdk_xgsd_mem_read[%d]: S-channel error "
    ...
    if (SCMH_OPCODE_GET(schan_msg.readresp.header) != READ_MEMORY_ACK_MSG) {
        CDK_ERR(("cdk_xgsd_mem_read[%d]: Invalid S-channel ACK: %"PRIu32""
                   ...
                   void* vptr, int size)
{
    ...
    if (cdk_simhook_write) {
        CDK_DEBUG_MEM(("cdk_xgsd_mem_write[%d]: "
                       "addr=0x%03"PRIx32"%08"PRIx32" idx=%"PRIx32"\n",
                       unit, adext, addr, idx));
     ...
     */
    rv = cdk_xgsd_schan_op(unit, &schan_msg, 2 + entry_dw, 0); 
    if (CDK_FAILURE(rv)) {
        CDK_ERR(("cdk_xgsd_mem_write[%d]: S-channel error "
                 "addr=0x%03"PRIx32"%08"PRIx32"\n",
                 unit, adext, addr));
    }
    return rv;
