                  ...
                  uint32_t *data, int size)
{
    ...
    /* Simulator hooks */
    if (cdk_simhook_read) {
        CDK_DEBUG_REG(("cdk_xgsd_reg_read[%d]: "
    ...
    rv = cdk_xgsd_schan_op(unit, &schan_msg, 2, 1+size);
    if (CDK_FAILURE(rv)) {
        CDK_ERR(("cdk_xgsd_reg_read[%d]: S-channel error "
                 "addr=0x%03"PRIx32"%08"PRIx32"\n",
                 unit, adext, addr));
        return rv;
    }
    ...
    if (SCMH_OPCODE_GET(schan_msg.readresp.header) != READ_REGISTER_ACK_MSG) {
        CDK_ERR(("cdk_xgsd_reg_read[%d]: Invalid S-channel ACK: %"PRIu32""
                   ...
                   uint32_t *data, int size)
{
    ...
    if (cdk_simhook_write) {
        CDK_DEBUG_REG(("cdk_xgsd_reg_write[%d]: "
                       "addr=0x%03"PRIx32"%08"PRIx32"\n",
                       unit, adext, addr));
    ...
    rv = cdk_xgsd_schan_op(unit, &schan_msg, 2+size, 0); 
    if (CDK_FAILURE(rv)) {
        CDK_ERR(("cdk_xgsd_reg_write[%d]: S-channel error "
                 "addr=0x%03"PRIx32"%08"PRIx32"\n",
                 unit, adext, addr));
    }
