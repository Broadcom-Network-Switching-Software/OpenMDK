                   ...
                   uint32_t offset, int idx, uint32_t *adext)
{
     ...
     */
    for (bdx = 0; bdx < CDK_XGSD_INFO(unit)->nblocks; bdx++) {  
    ...
    CDK_PBMP_ITER(blkp->pbmps, p) {
        if (p == port) {
            ...
            *adext = CDK_XGSD_BLKACC2ADEXT(blkacc);
            CDK_XGSD_ADEXT_BLOCK_SET(*adext, block);
            return cdk_xgsd_blockport_addr(unit, block, blk_port, offset, idx);
        }
        blk_port++;
    }
     ...
     */
    CDK_WARN(("cdk_xgsd_port_addr[%d]: invalid port %d "
