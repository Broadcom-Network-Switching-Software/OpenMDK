...
_print_sym(const cdk_symbol_t *s, void *vptr)
{
    ...
    flags = s->flags;
    acc_type = (flags >> 18 ) & 0x1f;
    flags &= ~(0x1f << 18);

    /* Max index may depend on current chip configuration */
    if (flags & CDK_SYMBOL_FLAG_MEMORY) {
        enum_val = cdk_symbols_index(CDK_XGSD_SYMBOLS(si->unit), s);
        max = cdk_xgsd_mem_maxidx(si->unit, enum_val, max);
    }
    ...
    for (mask = 1; mask; mask <<= 1) {
        if (flags & mask) {
            flagstr = cdk_xgsd_shell_symflag_type2name(si->unit, mask);
            if (flagstr != NULL) {
    ...
    if (s->fields) {
        ...
        cdk_symbol_show_fields(s, CDK_XGSD_SYMBOLS(si->unit)->field_names,
...
int
cdk_shcmd_xgsd_list(int argc, char *argv[])
{
    ...
    i = 0; 
    if ((argc == 0) || 
        ((i = cdk_xgsd_shell_parse_args(argc, argv,
                                       csts, COUNTOF(csts))) >= 0)) {
        /* Error in argument i */
