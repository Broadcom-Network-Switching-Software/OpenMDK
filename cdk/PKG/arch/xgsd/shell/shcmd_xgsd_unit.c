 ...
 */
int
cdk_shcmd_xgsd_unit(int argc, char *argv[])
{
    int unit;
    int u, b; 
    ...
    for (u = 0; u < CDK_CONFIG_MAX_UNITS; u++) {
        if (CDK_DEV_EXISTS(u) && (unit == -1 || unit == u)) {
            CDK_PRINTF("unit %d:\n", u); 
            if ((CDK_DEV_FLAGS(u) & CDK_DEV_ARCH_XGSD) == 0) {
                CDK_PRINTF("  Not XGSD architecture\n"); 
                continue;
            }
            ...
            cdk_shell_port_bitmap(tmp, sizeof(tmp),
                                  ...
                                  &CDK_XGSD_INFO(u)->valid_pbmps);
            CDK_PRINTF("  Valid Ports:         %s\n", tmp); 
            CDK_PRINTF("  Flags: 0x%"PRIx32"\n", CDK_XGSD_INFO(u)->flags); 
            if (unit == -1) {
                continue;
            }
            CDK_PRINTF("  Block  Name          Ports\n"); 
            for (b = 0; b < CDK_XGSD_INFO(u)->nblocks; b++) {  
                const cdk_xgsd_block_t *blkp = CDK_XGSD_INFO(u)->blocks+b; 
                if (cdk_xgsd_shell_block_name(u, blkp->blknum, -1, tmp)) {
