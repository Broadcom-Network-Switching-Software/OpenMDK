...
#ifndef __CDK_XGSD_SHELL_H__
#define __CDK_XGSD_SHELL_H__
...
/*******************************************************************************
 *
 * Register and Memory Output Functions (cdk_xgsd_shell_memregs.c)
 */

extern int
cdk_xgsd_shell_regops(int unit, const cdk_symbol_t *symbol, cdk_shell_id_t *sid, 
		      uint32_t size, uint32_t inst_mask,
                      uint32_t *and_masks, uint32_t *or_masks); 

extern int
cdk_xgsd_shell_memops(int unit, const cdk_symbol_t *symbol, cdk_shell_id_t *sid, 
		      uint32_t size, uint32_t inst_mask, int sect_size,
                      uint32_t *and_masks, uint32_t *or_masks); 

/*******************************************************************************
 *
 * Symbolic Register and Memory operations (cdk_xgsd_shell_symops.c)
 */
...
/*******************************************************************************
 *
 * Block functions (cdk_xgsd_shell_blocks.c)
 */
...
extern const char *
cdk_xgsd_shell_symflag_type2name(int unit, uint32_t flag); 

extern uint32_t
cdk_xgsd_shell_symflag_name2type(int unit, const char *name); 

extern int
cdk_xgsd_shell_symflag_cst2flags(int unit, const cdk_shell_tokens_t *cst,
                                uint32_t *present, uint32_t *absent); 

/*******************************************************************************
 *
 * Input parsing utilities (cdk_xgsd_shell_parse.c)
 */

extern int 
cdk_xgsd_shell_parse_args(int argc, char *argv[], cdk_shell_tokens_t *cst, int max);

/*******************************************************************************
 *
 * Field dump with XGSD filtering
 */

extern int 
cdk_xgsd_shell_show_fields(int unit, const cdk_symbol_t *symbol,
                          uint32_t *data, int skip_zeros);

/*******************************************************************************
 *
 * Shell macros for XGSD chips
 */

/* Iterate over all blocktypes */
#define CDK_SHELL_BLKTYPE_ITER(flags, blktype) \
        for(blktype = CDK_XGSD_BLOCK_START; blktype <= CDK_XGSD_BLOCK_LAST; blktype <<= 1) \
           if(flags & blktype) 


#endif /* __CDK_XGSD_SHELL_H__ */
