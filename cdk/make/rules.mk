...
#
# CDK default make rules. These can optionally be overridden by
# letting CDK_MAKE_RULES point to a different rules file.
#

ifdef CDK_MAKE_RULES

include $(CDK_MAKE_RULES)
...
#
ifeq ($(CDK_MAKE_LIBRARY_MODE),)

ifneq ($(word 600,$(BOBJS)),)
CDK_MAKE_LIBRARY_MODE=chunk
else
CDK_MAKE_LIBRARY_MODE=normal
endif
...
#
ifeq ($(CDK_MAKE_LIBRARY_MODE),normal)
...
ifeq ($(CDK_MAKE_LIBRARY_MODE),serial)
...
ifndef CDK_MAKE_LIBRARY_SERIAL_SECOND_STAGE
...
#
$(LIBDIR)/$(LIBNAME).$(LIBSUFFIX): $(BOBJS) $(BLDDIR)/.tree 
	$(MAKE) $(LIBDIR)/$(LIBNAME).$(LIBSUFFIX) CDK_MAKE_LIBRARY_SERIAL_SECOND_STAGE=1 CDK_MAKE_LIBRARY_MODE=serial
...
ifeq ($(CDK_MAKE_LIBRARY_MODE),chunk)
...
$(error Too many files in chunk library build $(LIBNAME). Add more chunks @ $$CDK/make/rules.mk)
endif
