...
#endif
/* PCIe link status control */
#define PCIE_LNKSTA_CTRLr                       0xbc
#define PCIE_LNKSTA_CTRLr_NEG_LNKWIDTHf_SHFT    20
#define PCIE_LNKSTA_CTRLr_NEG_LNKWIDTHf_MASK    0x3f
...
int
shbde_iproc_pcie_common_clk_set(shbde_hal_t *shbde, void *iproc_regs,
                                shbde_iproc_config_t *icfg, void *pci_dev)
{
    shbde_mdio_ctrl_t mdio_ctrl, *smc = &mdio_ctrl;
    unsigned int phy_addr;
    unsigned int lnksta_ctrl, lnkwidth;

    if (!icfg || !icfg->adjust_pcie_common_clk) {
        return -1;
    }

    /* Check if the link width is in x2 mode */
    if (!shbde || !shbde->pcic32_read || !pci_dev) {
        return -1;
    }
    lnksta_ctrl = shbde->pcic32_read(pci_dev, PCIE_LNKSTA_CTRLr);
    lnkwidth = REG_FIELD_GET(PCIE_LNKSTA_CTRLr, NEG_LNKWIDTHf, lnksta_ctrl);
    if (lnkwidth < 2) {
        /* Do nothing if the negotiated link width is x1. */
        return 0;
    }
    ...
    /* Disable PCIe SerDes clock compensation events. */
    shbde_iproc_mdio_write(smc, phy_addr, 0x1f, 0x8630);
    shbde_iproc_mdio_write(smc, phy_addr, 0x1e, 0x0801);
