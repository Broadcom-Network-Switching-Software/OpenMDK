...
# Create clean destination for phymod chip files
my $dest_phymod_chip_inc = "$dest_base_inc/phymod/chip";
if (-e $dest_phymod_chip_inc) {
    $pkginst->remove_file($dest_phymod_chip_inc);
}
$pkginst->make_path($dest_phymod_chip_inc);
...
foreach my $chip (@phy_chips_uniq) {
    ...
    while (my $file = readdir(CHIPDIR)) {
        ...
        if ($file =~ /^phymod_/) {
            $file =~ s/^phymod_//g;
            $pkginst->install_file($thisfile, "$dest_phymod_chip_inc/$file");
            next;
        }
        if ($file =~ /sym\.c$/) {
        ...
        if (-d $thisfile && $file !~ /^\./) {
            if (opendir(SUBDIR, $thisfile)) {
                $pkginst->make_path("$dest_chip_src/$file");
                while (my $subfile = readdir(SUBDIR)) {
                    $subpath = "$thisfile/$subfile";
                    if (!opendir(TMPDIR, $subpath)) {
                        $pkginst->install_file($subpath, "$dest_chip_src/$file/$subfile");
                    }
                    next;
                }
            }
