...
_phy_addr(int port)
{
    ...
    if (port > 40) {
        return (port - 28) + CDK_XGSM_MIIM_EBUS(1);
    }
    if (port > 36) {
        return (port - 8) + CDK_XGSM_MIIM_EBUS(1);
    }
    if (port > 24) {
        return (port - 24) + CDK_XGSM_MIIM_EBUS(1);
    }
...
phy_bus_t phy_bus_bcm956640k_miim_ext = {
    "bcm956640k_miim_ext",
    _phy_addr,
    _read,
    _write,
...
int bcm956640k_miim_ext_not_empty;
