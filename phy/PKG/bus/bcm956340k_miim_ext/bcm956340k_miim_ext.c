...
_phy_addr(int port)
{
    ...
    if (port > 24) {
        return (port - 24) + (((port - 24) - 1) >> 3) + CDK_XGSM_MIIM_EBUS(1);
    } 
    return port + ((port - 1) >> 3) + CDK_XGSM_MIIM_EBUS(0);
}
...
phy_bus_t phy_bus_bcm956340k_miim_ext = {
    "bcm956340k_miim_ext",
    _phy_addr,
    _read,
    _write,
...
int bcm956340k_miim_ext_not_empty;
