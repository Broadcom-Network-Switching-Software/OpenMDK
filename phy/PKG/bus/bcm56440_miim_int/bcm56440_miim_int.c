...
_phy_addr(int port)
{
    if (port < 25) {
        ...
        return (port - 25) + CDK_XGSM_MIIM_IBUS(1);
    } else if (port < 32) {
        return (port - 22) + CDK_XGSM_MIIM_IBUS(1);
    } else if (port < 35) {
        return (port - 29) + CDK_XGSM_MIIM_IBUS(1);
    }
    return 0;
}
...
phy_bus_t phy_bus_bcm56440_miim_int = {
    "bcm56440_miim_int",
    _phy_addr,
    _read,
    _write,
...
int bcm56440_miim_int_not_empty;
