...
extern unsigned char bcm8486x_ucode[];
extern unsigned int bcm8486x_ucode_len;
...
#define BCM84868_PMA_PMD_ID0                    0xae02
#define BCM84868_PMA_PMD_ID1                    0x5060
 ...
 */
static int
_bcm84868_top_level_cmd_set(phy_ctrl_t *pc, uint32_t cmd, uint32_t arg[], int size)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data, cnt;
    ...
    if (cmd == BCM8484X_DIAG_CMD_SET_PAIR_SWAP) {
        ioerr += PHY_BUS_WRITE(pc, TOPLVL_MDIO_CMD_STATUS_REG, 
                                        BCM8484X_CMD_OPEN_OVERRIDE);
    }
 ...
 */
static int
_bcm84868_top_level_cmd_get(phy_ctrl_t *pc, uint32_t cmd, uint32_t arg[], int size)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data, cnt;
...
_bcm84868_halt(phy_ctrl_t *pc)
{
    ...
    if (data & (0x1 << 13)) {
        ...
        ioerr += PHY_BUS_WRITE(pc, PMA_PMD_DOWNLOAD_DATA_LOW_REG, 0x001f);
    }
    
    ioerr += PHY_BUS_WRITE(pc, PMA_PMD_DOWNLOAD_DATA_HIGH_REG, 0x0000);
    data = (0x2 << DOWNLOAD_CTRL_SIZE_SHIFT) | (0x1 << DOWNLOAD_CTRL_WR_SHIFT);
 ...
 */
static int
_bcm84868_firmware_download(phy_ctrl_t *pc, uint8_t *fw_data, uint32_t fw_size)
{
    int ioerr = 0;
    uint32_t data;
    int n_writes, idx;
 ...
 */
static int
_bcm84868_auto_negotiate_gcd(phy_ctrl_t *pc, int *speed, int *duplex)
{
    int ioerr = 0;
 ...
 */
static int
_bcm84868_mdi_pair_remap(phy_ctrl_t *pc, uint32_t mapping)
{
    int ioerr = 0;
    uint16_t value;
    uint32_t args[5];
    ...
    ioerr += _bcm84868_top_level_cmd_set(pc, 
 ...
 */
static int
_bcm84868_eee_mode_set(phy_ctrl_t *pc, uint32_t mode)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t args[5];
    ...
    if (mode == PHY_EEE_802_3) {
        ...
        rv = _bcm84868_top_level_cmd_set(pc, 
        ...
        if (CDK_SUCCESS(rv)) {
            rv = _bcm84868_top_level_cmd_get(pc, 
 ...
 */
static int
_bcm84868_eee_mode_get(phy_ctrl_t *pc, uint32_t *mode)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t args[5];
...
static int
_bcm84868_master_set(phy_ctrl_t *pc, uint32_t master)
{
    int ioerr = 0;
 ...
 */
static int
_bcm84868_master_get(phy_ctrl_t *pc, uint32_t *master)
{
    int ioerr = 0;
    uint32_t mii_gb_stat, mii_gb_ctrl;
 ...
 */
static int
_bcm84868_ability_local_get(phy_ctrl_t *pc, uint32_t *ability)
{
    if (!ability) {
        return CDK_E_PARAM;
    }
 ...
 */
static int
_bcm84868_ability2_local_get(phy_ctrl_t *pc, uint32_t *ability2)
{
    uint32_t abil1;
 ...
 */
static int
_bcm84868_copper_ability_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
...
static int
_bcm84868_copper_ability2_remote_get(phy_ctrl_t *pc, uint32_t *ability2)
{
    if (!ability2) {
        return CDK_E_PARAM;
    }
...
static int
_bcm84868_fiber_ability_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t mii_stat;
...
static int
_bcm84868_fiber_ability2_remote_get(phy_ctrl_t *pc, uint32_t *ability2)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t mii_stat;
    ...
    *ability2 = 0;
    ...
    if (!((data & GP_STATUS_XGXS3_LINK_MASK) && (mii_stat & MII_STAT_AN_DONE))) {
        ...
        rv = _bcm84868_ability2_local_get(pc, ability2);
    }
...
static int
_bcm84868_ability_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int rv = CDK_E_NONE;
    
    if (IS_COPPER_MODE(pc)) {
        rv = _bcm84868_copper_ability_remote_get(pc, ability);
    } else {
        rv = _bcm84868_fiber_ability_remote_get(pc, ability);
    }
...
static int
_bcm84868_ability2_remote_get(phy_ctrl_t *pc, uint32_t *ability2)
{
    int rv = CDK_E_NONE;
    
    if (IS_COPPER_MODE(pc)) {
        rv = _bcm84868_copper_ability2_remote_get(pc, ability2);
    } else {
        rv = _bcm84868_fiber_ability2_remote_get(pc, ability2);
    }
...
/*
 * Function:    
 *      _bcm84868_ability_eee_remote_get
 * Purpose:     
 ...
 */
static int
_bcm84868_ability_eee_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    uint32_t an, an_done, link;
    uint32_t mii_stat, mii_ctrl;
...
/*
 * Function:    
 *      _bcm84868_copper_ability_advert_set
 * Purpose:     
 ...
 */
static int
_bcm84868_copper_ability_advert_set(phy_ctrl_t *pc, uint32_t ability)
{
    int ioerr = 0;
    uint32_t data;
    ...
    if (ability & PHY_ABIL_10MB_HD)  {
        data |= MII_ANA_HD_10;
    }
    if (ability & PHY_ABIL_100MB_HD) {
        data |= MII_ANA_HD_100;
    }
    if (ability & PHY_ABIL_10MB_FD)  {
        data |= MII_ANA_FD_10;
    }
 ...
 */
static int
_bcm84868_copper_ability2_advert_set(phy_ctrl_t *pc, uint32_t ability2)
{
    int ioerr = 0;
    uint32_t data;

    /* 5G / 2.5G Specific values */
    ioerr += PHY_BUS_READ(pc, TOPLVL_AN_CTRL_REG, &data);
    data = (ability2 & PHY_ABIL2_5000MB)
            ? (MGBASE_AN_CTRL_ADV_5G   | MGBASE_AN_CTRL_MG_ENABLE) : 0;
    /* MGBASE-T AN Control Reg.(30.0x0000), Advert_5G/2.5G_capability[2:1] */
    ioerr += PHY_BUS_WRITE(pc, TOPLVL_AN_CTRL_REG, data);
    
    return ioerr ? CDK_E_IO : CDK_E_NONE;
}

static int
_bcm84868_fiber_ability_advert_set(phy_ctrl_t *pc, uint32_t ability)
{
    int ioerr = 0;
    uint32_t data;
...
static int
_bcm84868_ability_advert_set(phy_ctrl_t *pc, uint32_t ability)
{
    int rv = CDK_E_NONE;
    
    if (IS_COPPER_MODE(pc)) {
        rv = _bcm84868_copper_ability_advert_set(pc, ability);
    } else {
        rv = _bcm84868_fiber_ability_advert_set(pc, ability);
    }
 ...
 */
static int
_bcm84868_copper_ability_advert_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    uint32_t data;
    ...
    *ability = 0;
    ...
    /* MGBASE-T AN Control Reg.(30.0x0000), Advert_5G/2.5G_capability[2:1] */
    ioerr += PHY_BUS_READ(pc, TOPLVL_AN_CTRL_REG, &data);
    if ( data & MGBASE_AN_CTRL_ADV_2P5G ) {
        *ability |= PHY_ABIL_2500MB;
    }
    if ( data & MGBASE_AN_CTRL_ADV_5G ) {
...
STATIC int
_bcm84868_fiber_ability_advert_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    uint32_t data;
...
static int
_bcm84868_ability2_advert_set(phy_ctrl_t *pc, uint32_t ability2)
{
    int rv = CDK_E_NONE;
    
    if (IS_COPPER_MODE(pc)) {
        rv = _bcm84868_copper_ability2_advert_set(pc, ability2);
    }
 ...
 */
static int
_bcm84868_medium_change(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    if (changed) {
        ...
        rv = _bcm84868_ability_local_get(pc, &ability);
        if (CDK_SUCCESS(rv)) {
            rv = _bcm84868_ability_advert_set(pc, ability);
        }
        
        PHY_CONFIG_GET(pc, PhyConfig_AdvLocal2, &ability, NULL);
        rv = _bcm84868_ability2_advert_set(pc, ability);
    }
 ...
 */
static int
_bcm84868_init_stage_0(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data;
 ...
 */
static int
_bcm84868_init_stage_1(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
_bcm84868_init_stage_2(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    if (PHY_CTRL_FLAGS(pc) & PHY_F_FW_MDIO_LOAD) {
        ...
        rv = _bcm84868_firmware_download(pc, bcm8486x_ucode, bcm8486x_ucode_len);
 ...
 */
static int
_bcm84868_init_stage_3(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data;
 ...
 */
static int
_bcm84868_init_stage_4(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    PHY_VERB(pc, ("BCM84868: port: %d, chip rev:%d, version:%d.%d\n", 
    ...
    PHY_VERB(pc, ("BCM84868: port: %d, SPIROM CRC check status:%d\n", 
                                        pc->port, crc_status));
    return ioerr ? CDK_E_IO : rv;
}
 ...
 */
static int
_bcm84868_init_stage_5(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
_bcm84868_init_stage(phy_ctrl_t *pc, int stage)
{
    switch (stage) {
    case 0:
        return _bcm84868_init_stage_0(pc);
    case 1:
        return _bcm84868_init_stage_1(pc);
    case 2:
        return _bcm84868_init_stage_2(pc);
    case 3:
        return _bcm84868_init_stage_3(pc);
    case 4:
        return _bcm84868_init_stage_4(pc);
    case 5:
        return _bcm84868_init_stage_5(pc);
    default:
        break;
    }
 ...
 */
static int
bcm84868_phy_probe(phy_ctrl_t *pc)
{
    int ioerr = 0;
    ...
    if ((phyid0 == BCM84868_PMA_PMD_ID0) &&
        ((phyid1 & ~PHY_ID1_REV_MASK) == BCM84868_PMA_PMD_ID1)) {
 ...
 */
static int
bcm84868_phy_notify(phy_ctrl_t *pc, phy_event_t event)
{
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84868_phy_reset(phy_ctrl_t *pc)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcm84868_phy_init(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    int stage;
    ...
    for (stage = 0; CDK_SUCCESS(rv); stage++) {
        rv = _bcm84868_init_stage(pc, stage);
    }
 ...
 */
static int
bcm84868_phy_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84868_phy_duplex_set(phy_ctrl_t *pc, int duplex)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84868_phy_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t mii_stat, mii_ctrl;
        ...
        if (mii_ctrl & MII_CTRL_AE) {     /* Auto-negotiation enabled */
            if (!(mii_stat & MII_STAT_AN_DONE)) { /* Auto-neg NOT complete */
                *duplex = FALSE;
                ...
                rv = _bcm84868_auto_negotiate_gcd(pc, NULL, duplex);
            }
 ...
 */
static int
bcm84868_phy_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84868_phy_speed_get(phy_ctrl_t *pc, uint32_t *speed)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data;
 ...
 */
static int
bcm84868_phy_autoneg_set(phy_ctrl_t *pc, int autoneg)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84868_phy_autoneg_get(phy_ctrl_t *pc, int *autoneg)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t mii_ctrl;
 ...
 */
static int
bcm84868_phy_loopback_set(phy_ctrl_t *pc, int enable)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84868_phy_loopback_get(phy_ctrl_t *pc, int *enable)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t speed, data;
 ...
 */
static int
bcm84868_phy_ability_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int rv = CDK_E_NONE;
    ...
    if (IS_COPPER_MODE(pc)) { /* Connected at copper mode */
        rv = _bcm84868_copper_ability_advert_get(pc, ability);
    } else { /* Connected at fiber mode */
        rv = _bcm84868_fiber_ability_advert_get(pc, ability);
    }
 ...
 */
static int
bcm84868_phy_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    int rv = CDK_E_NONE;
    ...
    case PhyConfig_MdiPairRemap:
        rv = _bcm84868_mdi_pair_remap(pc, val);
        break;
    case PhyConfig_InitStage:
        if (PHY_CTRL_FLAGS(pc) & PHY_F_STAGED_INIT) {
            rv = _bcm84868_init_stage(pc, val);
        }
        break;
    case PhyConfig_EEE:
        rv = _bcm84868_eee_mode_set(pc, val);
        break;
    case PhyConfig_Master:
        rv = _bcm84868_master_set(pc, val);
        break;
    case PhyConfig_PHY_SERDES:
        rv = _phy8486x_xfi_2p5g_mode_set(pc, val);
        return rv;
    case PhyConfig_AdvLocal2:
        rv = _bcm84868_ability2_advert_set(pc, val);
        return rv;
    default:
        rv = CDK_E_UNAVAIL;
        break;
 ...
 */
static int
bcm84868_phy_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    int rv = CDK_E_NONE;
    uint32_t abil;
    PHY_CTRL_CHECK(pc);
    ...
    case PhyConfig_BcastAddr:
        *val = PHY_CTRL_BUS_ADDR(pc) & ~0x1f;
        return CDK_E_NONE;
    case PhyConfig_EEE:
        rv = _bcm84868_eee_mode_get(pc, val);
        return rv;
    case PhyConfig_Master:
        rv = _bcm84868_master_get(pc, val);
        return rv;
    case PhyConfig_AdvLocal:
        rv = _bcm84868_ability_local_get(pc, val);
        return rv;
    case PhyConfig_AdvRemote:
        rv = _bcm84868_ability_remote_get(pc, val);
        return rv;
    case PhyConfig_AdvEEERemote:
        rv = _bcm84868_ability_eee_remote_get(pc, val);
        return rv;
    case PhyConfig_Ability2:
        ...
        *val = PHY_ABIL2_5000MB;
        return rv;
    case PhyConfig_AdvLocal2:
        rv = _bcm84868_ability2_local_get(pc, val);
        return rv;
    case PhyConfig_AdvRemote2:
        rv = _bcm84868_ability2_remote_get(pc, val);
        return rv;
    default:
        break;
    }
 ...
 */
static int
bcm84868_phy_status_get(phy_ctrl_t *pc, phy_status_t st, uint32_t *val)
{
    int ioerr = 0;
    uint32_t data;
...
/*
 * Variable:    bcm84868_drv
 * Purpose:     PHY Driver for BCM84868.
 */
phy_driver_t bcm84868_drv = {
    "bcm84868",
    "BCM84868 10GbE PHY Driver",  
    0,
    bcm84868_phy_probe,                  /* pd_probe */
    bcm84868_phy_notify,                 /* pd_notify */
    bcm84868_phy_reset,                  /* pd_reset */
    bcm84868_phy_init,                   /* pd_init */
    bcm84868_phy_link_get,               /* pd_link_get */
    bcm84868_phy_duplex_set,             /* pd_duplex_set */
    bcm84868_phy_duplex_get,             /* pd_duplex_get */
    bcm84868_phy_speed_set,              /* pd_speed_set */
    bcm84868_phy_speed_get,              /* pd_speed_get */
    bcm84868_phy_autoneg_set,            /* pd_autoneg_set */
    bcm84868_phy_autoneg_get,            /* pd_autoneg_get */
    bcm84868_phy_loopback_set,           /* pd_loopback_set */
    bcm84868_phy_loopback_get,           /* pd_loopback_get */
    bcm84868_phy_ability_get,            /* pd_ability_get */
    bcm84868_phy_config_set,             /* pd_config_set */
    bcm84868_phy_config_get,             /* pd_config_get */
    bcm84868_phy_status_get,             /* pd_status_get */
    NULL                                 /* pd_cable_diag */
};
