 ...
 *
 * PHY driver for BCM84846.
 */

#include <phy/phy.h>
#include <phy/ge_phy.h>

extern unsigned char bcm8484x_ucode[];
extern unsigned int bcm8484x_ucode_len;
...
#define BCM84846_PMA_PMD_ID0                    0x600d
#define BCM84846_PMA_PMD_ID1                    0x84c0
 ...
 */
static int
_bcm84846_top_level_cmd_set(phy_ctrl_t *pc, uint32_t cmd, uint32_t arg[], int size)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data, cnt;
    ...
    for (cnt = 0; cnt < PHY_CMD_POLL_MAX; cnt++) {
        ...
        if ((data & BCM8484X_CMD_OPEN_FOR_CMDS) != 0) {
            break;
        }
    ...
    for (cnt = 0; cnt < PHY_CMD_POLL_MAX; cnt++) {
        ioerr += PHY_BUS_READ(pc, TOPLVL_MDIO_CMD_STATUS_REG, &data);
        if ((data & (BCM8484X_CMD_COMPLETE_PASS | BCM8484X_CMD_COMPLETE_ERROR)) != 0) {
            break;
        }
        PHY_SYS_USLEEP(10000);
    }
 ...
 */
static int
_bcm84846_top_level_cmd_get(phy_ctrl_t *pc, uint32_t cmd, uint32_t arg[], int size)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data, cnt;
 ...
 */
static int
_bcm84846_firmware_download(phy_ctrl_t *pc, uint8_t *fw_data, uint32_t fw_size)
{
    int ioerr = 0;
    uint32_t data;
    int n_writes, idx;
 ...
 */
static int
_bcm84846_auto_negotiate_gcd(phy_ctrl_t *pc, int *speed, int *duplex)
{
    int ioerr = 0;
 ...
 */
static int
_bcm84846_mdi_pair_remap(phy_ctrl_t *pc, uint32_t mapping)
{
    int ioerr = 0;
    uint32_t data;
    uint16_t value;
    ...
    ioerr += PHY_BUS_READ(pc, TOPLVL_MIDO_CMD_REG, &data);
    data &= ~(1U << 1);
    ioerr += PHY_BUS_WRITE(pc, TOPLVL_MIDO_CMD_REG, data);

    PHY_SYS_USLEEP(35000);

    ioerr += PHY_BUS_READ(pc, TOPLVL_PAIR_SWAP_CTRL_REG, &data);
    data &= ~0xff;
    data |= value;
    ioerr += PHY_BUS_WRITE(pc, TOPLVL_PAIR_SWAP_CTRL_REG, data);
 ...
 */
static int
_bcm84846_eee_mode_set(phy_ctrl_t *pc, uint32_t mode)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t args[5];
    ...
    if (mode == PHY_EEE_802_3) {
        ...
        rv = _bcm84846_top_level_cmd_set(pc, 
        ...
        if (CDK_SUCCESS(rv)) {
            rv = _bcm84846_top_level_cmd_get(pc, 
 ...
 */
static int
_bcm84846_eee_mode_get(phy_ctrl_t *pc, uint32_t *mode)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t args[5];
...
static int
_bcm84846_master_set(phy_ctrl_t *pc, uint32_t master)
{
    int ioerr = 0;
 ...
 */
static int
_bcm84846_master_get(phy_ctrl_t *pc, uint32_t *master)
{
    int ioerr = 0;
    uint32_t mii_gb_stat, mii_gb_ctrl;
 ...
 */
static int
_bcm84846_ability_local_get(phy_ctrl_t *pc, uint32_t *ability)
{
    if (!ability) {
        return CDK_E_PARAM;
    }
 ...
 */
static int
_bcm84846_copper_ability_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    *ability = 0;
    ...
    if (an && an_done && link) {
        ...
        rv = _bcm84846_ability_local_get(pc, ability);
    }
...
static int
_bcm84846_fiber_ability_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t mii_stat;
...
static int
_bcm84846_ability_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int rv = CDK_E_NONE;
    
    if (IS_COPPER_MODE(pc)) {
        rv = _bcm84846_copper_ability_remote_get(pc, ability);
    } else {
        rv = _bcm84846_fiber_ability_remote_get(pc, ability);
    }
...
/*
 * Function:    
 *      _bcm84846_ability_eee_remote_get
 * Purpose:     
 ...
 */
static int
_bcm84846_ability_eee_remote_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    uint32_t an, an_done, link;
    uint32_t mii_stat, mii_ctrl;
 ...
 */
static int
_bcm84846_copper_ability_advert_set(phy_ctrl_t *pc, uint32_t ability)
{
    int ioerr = 0;
    uint32_t data;
    
    ioerr += PHY_BUS_READ(pc, D7_FFXX_MII_GB_CTRL_REG, &data);
    data &= ~(MII_GB_CTRL_PT | MII_GB_CTRL_ADV_1000HD | MII_GB_CTRL_ADV_1000FD);
    data |= MII_GB_CTRL_PT;
...
static int
_bcm84846_fiber_ability_advert_set(phy_ctrl_t *pc, uint32_t ability)
{
    int ioerr = 0;
    uint32_t data;
...
static int
_bcm84846_ability_advert_set(phy_ctrl_t *pc, uint32_t ability)
{
    int rv = CDK_E_NONE;
    
    if (IS_COPPER_MODE(pc)) {
        rv = _bcm84846_copper_ability_advert_set(pc, ability);
    } else {
        rv = _bcm84846_fiber_ability_advert_set(pc, ability);
    }
 ...
 */
static int
_bcm84846_copper_ability_advert_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    uint32_t data;
...
STATIC int
_bcm84846_fiber_ability_advert_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int ioerr = 0;
    uint32_t data;
 ...
 */
static int
_bcm84846_medium_change(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    if (changed) {
        ...
        rv = _bcm84846_ability_local_get(pc, &ability);
        if (CDK_SUCCESS(rv)) {
            rv = _bcm84846_ability_advert_set(pc, ability);
        }
    }
 ...
 */
static int
_bcm84846_init_stage_0(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data;
 ...
 */
static int
_bcm84846_init_stage_1(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    if (PHY_CTRL_FLAGS(pc) & PHY_F_FW_MDIO_LOAD) {
        ...
        ioerr += PHY_BUS_WRITE(pc, TOPLVL_0X411E_REG, 0xf003);
        ioerr += PHY_BUS_WRITE(pc, TOPLVL_0X410E_REG, 0x0401);

        rv = _bcm84846_halt(pc);
    }
 ...
 */
static int
_bcm84846_init_stage_2(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    if (PHY_CTRL_FLAGS(pc) & PHY_F_FW_MDIO_LOAD) {
        ...
        rv = _bcm84846_firmware_download(pc, bcm8484x_ucode, bcm8484x_ucode_len);
        if (CDK_SUCCESS(rv)) {
            ioerr += PHY_BUS_WRITE(pc, PMA_PMD_DOWNLOAD_ADDR_LOW_REG, 0x0000);
            ioerr += PHY_BUS_WRITE(pc, PMA_PMD_DOWNLOAD_ADDR_HIGH_REG, 0xc300);
            ioerr += PHY_BUS_WRITE(pc, PMA_PMD_DOWNLOAD_DATA_LOW_REG, 0x002c);
            ioerr += PHY_BUS_WRITE(pc, PMA_PMD_DOWNLOAD_DATA_HIGH_REG, 0x0000);
 ...
 */
static int
_bcm84846_init_stage_3(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data;
    ...
    if (PHY_CTRL_FLAGS(pc) & PHY_F_FW_MDIO_LOAD) {
        ...
        ioerr += PHY_BUS_WRITE(pc, TOPLVL_0X410E_REG, 0x0000);
 ...
 */
static int
_bcm84846_init_stage_4(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    PHY_VERB(pc, ("BCM84846: port: %d, chip rev:%d, version:%d.%d\n", 
    ...
    PHY_VERB(pc, ("BCM84846: port: %d, SPIROM CRC check status:%d\n", 
                                        pc->port, crc_status));
 ...
 */
static int
_bcm84846_init_stage_5(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data;
    ...
    /* Enable Higig2 support */
    ioerr += PHY_BUS_WRITE(pc, PMA_PMD_HG2_ENABLE_REG, 0x0183);
 ...
 */
static int
_bcm84846_init_stage(phy_ctrl_t *pc, int stage)
{
    switch (stage) {
    case 0:
        return _bcm84846_init_stage_0(pc);
    case 1:
        return _bcm84846_init_stage_1(pc);
    case 2:
        return _bcm84846_init_stage_2(pc);
    case 3:
        return _bcm84846_init_stage_3(pc);
    case 4:
        return _bcm84846_init_stage_4(pc);
    case 5:
        return _bcm84846_init_stage_5(pc);
    default:
        break;
    }
 ...
 */
static int
bcm84846_phy_probe(phy_ctrl_t *pc)
{
    int ioerr = 0;
    ...
    if ((phyid0 == BCM84846_PMA_PMD_ID0) &&
        ((phyid1 & ~PHY_ID1_REV_MASK) == BCM84846_PMA_PMD_ID1)) {
 ...
 */
static int
bcm84846_phy_notify(phy_ctrl_t *pc, phy_event_t event)
{
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84846_phy_reset(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t data;
 ...
 */
static int
bcm84846_phy_init(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    int stage;
    ...
    for (stage = 0; CDK_SUCCESS(rv); stage++) {
        rv = _bcm84846_init_stage(pc, stage);
    }
 ...
 */
static int
bcm84846_phy_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    *autoneg_done = 0;
    ...
    if (IS_COPPER_MODE(pc)) { /* Connected at copper mode */
        ...
        ioerr += PHY_BUS_READ(pc, PCS_CTRL_REG, &pcs_mii_stat);
 ...
 */
static int
bcm84846_phy_duplex_set(phy_ctrl_t *pc, int duplex)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84846_phy_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t mii_stat, mii_ctrl;
        ...
        if (mii_ctrl & MII_CTRL_AE) {     /* Auto-negotiation enabled */
            if (!(mii_stat & MII_STAT_AN_DONE)) { /* Auto-neg NOT complete */
                *duplex = FALSE;
                ...
                rv = _bcm84846_auto_negotiate_gcd(pc, NULL, duplex);
            }
 ...
 */
static int
bcm84846_phy_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84846_phy_speed_get(phy_ctrl_t *pc, uint32_t *speed)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
            ...
            if (an_done) {
                ...
                if ((teng_ctrl & TENG_CTRL_10GBASET_MASK) && 
                    ...
                    *speed = 10000;
                    ...
                    rv = _bcm84846_auto_negotiate_gcd(pc, (int *)speed, NULL);
                }
            } else {
                *speed = 0;
                return CDK_E_NONE;
 ...
 */
static int
bcm84846_phy_autoneg_set(phy_ctrl_t *pc, int autoneg)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84846_phy_autoneg_get(phy_ctrl_t *pc, int *autoneg)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t mii_ctrl;
 ...
 */
static int
bcm84846_phy_loopback_set(phy_ctrl_t *pc, int enable)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm84846_phy_loopback_get(phy_ctrl_t *pc, int *enable)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t speed, data;
 ...
 */
static int
bcm84846_phy_ability_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int rv = CDK_E_NONE;
    ...
    if (IS_COPPER_MODE(pc)) { /* Connected at copper mode */
        rv = _bcm84846_copper_ability_advert_get(pc, ability);
    } else { /* Connected at fiber mode */
        rv = _bcm84846_fiber_ability_advert_get(pc, ability);
    }
 ...
 */
static int
bcm84846_phy_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    int rv = CDK_E_NONE;
    ...
    case PhyConfig_MdiPairRemap:
        rv = _bcm84846_mdi_pair_remap(pc, val);
        break;
    case PhyConfig_InitStage:
        if (PHY_CTRL_FLAGS(pc) & PHY_F_STAGED_INIT) {
            rv = _bcm84846_init_stage(pc, val);
        }
        break;
    case PhyConfig_EEE:
        rv = _bcm84846_eee_mode_set(pc, val);
        break;
    case PhyConfig_Master:
        rv = _bcm84846_master_set(pc, val);
        break;
    default:
        rv = CDK_E_UNAVAIL;
        break;
 ...
 */
static int
bcm84846_phy_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    int rv = CDK_E_NONE;
    PHY_CTRL_CHECK(pc);
    ...
    case PhyConfig_BcastAddr:
        *val = PHY_CTRL_BUS_ADDR(pc) & ~0x1f;
        return CDK_E_NONE;
    case PhyConfig_EEE:
        rv = _bcm84846_eee_mode_get(pc, val);
        return rv;
    case PhyConfig_Master:
        rv = _bcm84846_master_get(pc, val);
        return rv;
    case PhyConfig_AdvLocal:
        rv = _bcm84846_ability_local_get(pc, val);
        return rv;
    case PhyConfig_AdvRemote:
        rv = _bcm84846_ability_remote_get(pc, val);
        return rv;
    case PhyConfig_AdvEEERemote:
        rv = _bcm84846_ability_eee_remote_get(pc, val);
        return rv;
    default:
        break;
    }
 ...
 */
static int
bcm84846_phy_status_get(phy_ctrl_t *pc, phy_status_t st, uint32_t *val)
{
    int ioerr = 0;
    uint32_t data;
...
/*
 * Variable:    bcm84846_drv
 * Purpose:     PHY Driver for BCM84846.
 */
phy_driver_t bcm84846_drv = {
    "bcm84846",
    "BCM84846 10GbE PHY Driver",  
    0,
    bcm84846_phy_probe,                  /* pd_probe */
    bcm84846_phy_notify,                 /* pd_notify */
    bcm84846_phy_reset,                  /* pd_reset */
    bcm84846_phy_init,                   /* pd_init */
    bcm84846_phy_link_get,               /* pd_link_get */
    bcm84846_phy_duplex_set,             /* pd_duplex_set */
    bcm84846_phy_duplex_get,             /* pd_duplex_get */
    bcm84846_phy_speed_set,              /* pd_speed_set */
    bcm84846_phy_speed_get,              /* pd_speed_get */
    bcm84846_phy_autoneg_set,            /* pd_autoneg_set */
    bcm84846_phy_autoneg_get,            /* pd_autoneg_get */
    bcm84846_phy_loopback_set,           /* pd_loopback_set */
    bcm84846_phy_loopback_get,           /* pd_loopback_get */
    bcm84846_phy_ability_get,            /* pd_ability_get */
    bcm84846_phy_config_set,             /* pd_config_set */
    bcm84846_phy_config_get,             /* pd_config_get */
    bcm84846_phy_status_get,             /* pd_status_get */
    NULL                                 /* pd_cable_diag */
};
