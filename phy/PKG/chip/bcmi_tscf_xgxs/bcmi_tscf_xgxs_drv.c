...
#include <phy/chip/bcmi_tscf_xgxs_defs.h>
...
extern cdk_symbols_t bcmi_tscf_xgxs_symbols;
#define SET_SYMBOL_TABLE(_pc) \
    PHY_CTRL_SYMBOLS(_pc) = &bcmi_tscf_xgxs_symbols
#else
#define SET_SYMBOL_TABLE(_pc)
#endif
 ...
 */
static int
_tscf_serdes_lane(phy_ctrl_t *pc)
{
 ...
 */
static int
_tscf_primary_lane(phy_ctrl_t *pc)
{
    uint32_t inst = PHY_CTRL_INST(pc);
 ...
 */
static int
_tscf_phymod_core_t_init(phy_ctrl_t *pc, phymod_core_access_t *core)
{
 ...
 */
static int
_tscf_phymod_phy_t_init(phy_ctrl_t *pc, phymod_phy_access_t *phy)
{
    uint32_t lane_map;
    ...
    phy->access.lane_mask = lane_map << _tscf_serdes_lane(pc);
 ...
 */
static int
_tscf_firmware_loader(const phymod_core_access_t *pm_core,
 ...
 */
static int
_tscf_serdes_stop(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    ...
    _PHY_DBG(pc, ("_tscf_serdes_stop: stop = %d\n", stop));
    ...
    _tscf_phymod_phy_t_init(pc, &pm_phy);
    if (CDK_SUCCESS(rv)) {
        rv = tscf_phy_tx_lane_control_set(&pm_phy, tx_control); 
    }
    if (CDK_SUCCESS(rv)) {
        rv = tscf_phy_rx_lane_control_set(&pm_phy, rx_control); 
    }
 ...
 */
static int
_tscf_ability_set(phy_ctrl_t *pc, uint32_t ability)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    _tscf_phymod_phy_t_init(pc, &pm_phy);
    rv = tscf_phy_autoneg_ability_set(&pm_phy, an_ablt);
 ...
 */
static int
_tscf_ability_get(phy_ctrl_t *pc, uint32_t *ability)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    phymod_autoneg_ability_t autoneg_ability; 
    ...
    _tscf_phymod_phy_t_init(pc, &pm_phy);
    rv = tscf_phy_autoneg_ability_get(&pm_phy, &autoneg_ability);
...
static int 
_tscf_ref_clk_convert(int port_refclk_int, phymod_ref_clk_t *ref_clk)
{
 ...
 */
static int
bcmi_tscf_xgxs_probe(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    ...
    _tscf_phymod_core_t_init(pc, &pm_core);
    
    found = 0;
    rv = tscf_core_identify(&pm_core, 0, &found);    
 ...
 */
static int
bcmi_tscf_xgxs_notify(phy_ctrl_t *pc, phy_event_t event)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcmi_tscf_xgxs_reset(phy_ctrl_t *pc)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcmi_tscf_xgxs_init(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    phymod_core_access_t pm_core;
    phymod_phy_access_t pm_phy;
    ...
    if (_tscf_primary_lane(pc)) {
        ...
        if (PHY_CTRL_FW_HELPER(pc)) {
            cic->firmware_loader = _tscf_firmware_loader;
        ...
        if (CDK_SUCCESS(rv)) {
            rv = tscf_core_init(&pm_core, cic, &core_status);
        }
    }
    ...
    if (CDK_SUCCESS(rv)) {
        rv = tscf_phy_init(&pm_phy, &phy_init_config);
    }
 ...
 */
static int
bcmi_tscf_xgxs_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    _tscf_phymod_phy_t_init(pc, &pm_phy);
    if (CDK_SUCCESS(rv)) {
        rv = tscf_phy_autoneg_get(&pm_phy, &an, (uint32_t *)&an_done);
    }
    ...
    *autoneg_done = 0;
    ...
    if (CDK_SUCCESS(rv)) {
        _tscf_phymod_phy_t_init(pc, &pm_phy);
        rv = tscf_phy_link_status_get(&pm_phy, (uint32_t *)link);
    }
 ...
 */
static int
bcmi_tscf_xgxs_duplex_set(phy_ctrl_t *pc, int duplex)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcmi_tscf_xgxs_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcmi_tscf_xgxs_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    /* Set autoneg = 0 before set force mode */
    rv = PHY_AUTONEG_SET(pc, 0);
    ...
    _tscf_phymod_phy_t_init(pc, &pm_phy);
    if (CDK_SUCCESS(rv)) {
        rv = tscf_phy_interface_config_set(&pm_phy, 0, if_cfg);
    }
...
static int
bcmi_tscf_xgxs_speed_get(phy_ctrl_t *pc, uint32_t *speed)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    phymod_phy_inf_config_t interface_config;
    ...
    rv = _tscf_ref_clk_convert(PORT_REFCLK_INT, &ref_clock);
    if (CDK_SUCCESS(rv)) {
        rv = tscf_phy_interface_config_get(&pm_phy,
 ...
 */
static int
bcmi_tscf_xgxs_autoneg_set(phy_ctrl_t *pc, int autoneg)
{
    int rv = CDK_E_NONE;
    phymod_autoneg_control_t an;
    phymod_phy_access_t pm_phy;
    ...
    /* change cl72 config */
    if (fs_cl72) {
        an.flags &= ~PHYMOD_AN_F_ALLOW_CL72_CONFIG_CHANGE;
    } else {
        an.flags |= PHYMOD_AN_F_ALLOW_CL72_CONFIG_CHANGE;
    }
    ...
    if (CDK_SUCCESS(rv)) {
        _tscf_phymod_phy_t_init(pc, &pm_phy);
        rv = tscf_phy_autoneg_set(&pm_phy, &an);
    }
 ...
 */
static int
bcmi_tscf_xgxs_autoneg_get(phy_ctrl_t *pc, int *autoneg)
{
    int rv = CDK_E_NONE;
    phymod_autoneg_control_t an;
    phymod_phy_access_t pm_phy;
    ...
    _tscf_phymod_phy_t_init(pc, &pm_phy);
    rv = tscf_phy_autoneg_get(&pm_phy, &an, (uint32_t *)&an_complete);
 ...
 */
static int
bcmi_tscf_xgxs_loopback_set(phy_ctrl_t *pc, int enable)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    if (CDK_SUCCESS(rv)) {
        rv = tscf_phy_loopback_set(&pm_phy, phymodLoopbackGlobal, enable); 
    }
 ...
 */
static int
bcmi_tscf_xgxs_loopback_get(phy_ctrl_t *pc, int *enable)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    uint32_t lb_enable;
    ...
    *enable = 0;
    ...
    _tscf_phymod_phy_t_init(pc, &pm_phy);
    rv = tscf_phy_loopback_get(&pm_phy, phymodLoopbackGlobal, &lb_enable); 
    if (lb_enable) {
        *enable = 1;
    }
 ...
 */
static int
bcmi_tscf_xgxs_ability_get(phy_ctrl_t *pc, uint32_t *abil)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcmi_tscf_xgxs_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    case PhyConfig_RemoteLoopback:
        {
            ...
            if (CDK_SUCCESS(rv)) {
                rv = tscf_phy_loopback_set(&pm_phy, phymodLoopbackRemotePMD, val); 
            }
            return rv;
        }
    ...
    case PhyConfig_EEE:
        {
            _tscf_phymod_phy_t_init(pc, &pm_phy);
            if (CDK_SUCCESS(rv)) {
                rv = tscf_phy_eee_set(&pm_phy, val);
            }
            return rv;
        }
 ...
 */
static int
bcmi_tscf_xgxs_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    int rv = CDK_E_NONE;
    phymod_core_access_t pm_core;
    phymod_phy_access_t pm_phy;
    ...
    case PhyConfig_RemoteLoopback:
        {
            _tscf_phymod_phy_t_init(pc, &pm_phy);
            rv = tscf_phy_loopback_get(&pm_phy, phymodLoopbackRemotePMD, val);
            return rv;
        }
#if PHY_CONFIG_INCLUDE_XAUI_TX_LANE_MAP_SET
    case PhyConfig_XauiTxLaneRemap:
        {
            ...
            _tscf_phymod_core_t_init(pc, &pm_core);
            rv = tscf_core_lane_map_get(&pm_core, &lane_map);
            if (CDK_SUCCESS(rv)) {
                *val = 0;
    ...
    case PhyConfig_XauiTxPolInvert:
        {
            ...
            _tscf_phymod_phy_t_init(pc, &pm_phy);
            if (CDK_SUCCESS(rv)) {
                rv = tscf_phy_polarity_get(&pm_phy, &polarity);
            }
    ...
    case PhyConfig_EEE:
        {
            ...
            if (CDK_SUCCESS(rv)) {
                *val = 0;
                rv = tscf_phy_eee_get(&pm_phy, val);
            }
            return rv;
        }
    case PhyConfig_TxPreemp:
 ...
 */
static int
bcmi_tscf_xgxs_status_get(phy_ctrl_t *pc, phy_status_t stat, uint32_t *val)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    case PhyStatus_LineInterface:
        {
            ...
            rv = _tscf_ref_clk_convert(PORT_REFCLK_INT, &ref_clock);
            if (CDK_SUCCESS(rv)) {
                rv = tscf_phy_interface_config_get(&pm_phy, 0, ref_clock, 
                                                   if_cfg);
            }
...
phy_driver_t bcmi_tscf_xgxs_drv = {
    "bcmi_tscf_xgxs", 
    "Internal TSC Falcon 40G SerDes PHY Driver",  
    PHY_DRIVER_F_INTERNAL,
    bcmi_tscf_xgxs_probe,                /* pd_probe */
    bcmi_tscf_xgxs_notify,               /* pd_notify */
    bcmi_tscf_xgxs_reset,                /* pd_reset */
    bcmi_tscf_xgxs_init,                 /* pd_init */
    bcmi_tscf_xgxs_link_get,             /* pd_link_get */
    bcmi_tscf_xgxs_duplex_set,           /* pd_duplex_set */
    bcmi_tscf_xgxs_duplex_get,           /* pd_duplex_get */
    bcmi_tscf_xgxs_speed_set,            /* pd_speed_set */
    bcmi_tscf_xgxs_speed_get,            /* pd_speed_get */
    bcmi_tscf_xgxs_autoneg_set,          /* pd_autoneg_set */
    bcmi_tscf_xgxs_autoneg_get,          /* pd_autoneg_get */
    bcmi_tscf_xgxs_loopback_set,         /* pd_loopback_set */
    bcmi_tscf_xgxs_loopback_get,         /* pd_loopback_get */
    bcmi_tscf_xgxs_ability_get,          /* pd_ability_get */
    bcmi_tscf_xgxs_config_set,           /* pd_config_set */
    bcmi_tscf_xgxs_config_get,           /* pd_config_get */
    bcmi_tscf_xgxs_status_get,           /* pd_status_get */
    NULL                                 /* pd_cable_diag */
};
