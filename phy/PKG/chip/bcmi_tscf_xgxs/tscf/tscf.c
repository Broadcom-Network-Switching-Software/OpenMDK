...
int tscf_phy_tx_set(const phymod_phy_access_t* phy, const phymod_tx_t* tx)
{
    ...
    if((tx->drivermode != -1) && 
       (phy->device_op_mode & PHYMOD_INTF_CONFIG_TX_FIR_DRIVERMODE_ENABLE)) {
        PHYMOD_IF_ERR_RETURN
            (falcon_tsc_write_tx_afe(&phy->access, TX_AFE_DRIVERMODE,  tx->drivermode));
    }
    return PHYMOD_E_NONE;
}
...
int tscf_phy_tx_get(const phymod_phy_access_t* phy, phymod_tx_t* tx)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (falcon_tsc_read_tx_afe(&phy->access, TX_AFE_DRIVERMODE, &tx->drivermode));
    return PHYMOD_E_NONE;
}
         ...
         case 10000:
            {
                ...
                rv_temp = READ_MAIN0_SERDESIDr(&phy->access, &serdesid);
                if (rv_temp != PHYMOD_E_NONE) {
                     return PHYMOD_E_CONFIG;
                }
