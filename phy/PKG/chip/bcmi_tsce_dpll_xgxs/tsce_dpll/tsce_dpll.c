...
#include "../../bcmi_eagle_dpll_xgxs/eagle_dpll/eagle2_tsc2pll_dependencies.h"
...
#ifdef VENDOR_BROADCOM
	PHYMOD_VDBG(TEMOD2PLL_DBG_CFG, pm_acc,("%-22s: core_id=%0d identified=%0d rv=%0d adr=%0"PRIx32" lmask=%0"PRIx32"\n",
...
int tsce_dpll_core_info_get(const phymod_core_access_t* core, phymod_core_info_t* info)
{
    ...
    if ((serdes_id & 0x3f) == TSCE_DPLL_MODEL) {
        info->core_version = phymodCoreVersionTsceDpll;
        PHYMOD_STRNCPY(info->name, "TsceGen3", PHYMOD_STRLEN("TsceGen3")+1);
    }
...
#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(TEMOD2PLL_DBG_TOP, pm_acc,
       ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" pcs_swap=%0"PRIx32" addr_index_swap=%0"PRIx32" tx_map=%0"PRIx32"\n",
          __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask,
        pcs_swap, addr_index_swap, pmd_tx_map));
#endif
...
{
    PHYMOD_IF_ERR_RETURN(temod2pll_eee_control_set(&phy->access,enable));
    return PHYMOD_E_NONE;
}
...
int tsce_dpll_phy_eee_get(const phymod_phy_access_t* phy, uint32_t* enable)
{
    ...
    PHYMOD_IF_ERR_RETURN(temod2pll_eee_control_get(&phy->access, &enable_local));
    *enable = enable_local;
    return PHYMOD_E_NONE;
}
...
int _tsce_dpll_phy_pll_config(const phymod_phy_access_t* phy, uint32_t new_pll_div, phymod_ref_clk_t ref_clk, uint8_t pll_index)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (_tsce_dpll_pll_multiplier_get(new_pll_div, &pll_multiplier));
...
int tsce_dpll_phy_interface_config_set(const phymod_phy_access_t* phy, uint32_t flags, const phymod_phy_inf_config_t* config)
{
    ...
    case 15750:
        if (PHYMOD_INTF_MODES_HIGIG_GET(config)) {
            spd_intf = TEMOD2PLL_SPD_15750_HI_DXGXS;
        }
        break;
    case 16000:
    ...
    if (!high_vco_1G) {
        PHYMOD_IF_ERR_RETURN
            (temod2pll_cl37_high_vco_set(&pm_phy_copy.access, 0));
    }
...
#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(TEMOD2PLL_DBG_SPD, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" spd=%0d(%s) media=%0d\n",
       __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, spd_intf, e2s_temod2pll_spd_intfc_type[spd_intf],
...
int tsce_dpll_phy_interface_config_get(const phymod_phy_access_t* phy, uint32_t flags, phymod_ref_clk_t ref_clock, phymod_phy_inf_config_t* config)
{
    ...
    {
        phymod_autoneg_control_t an_control_tmp;
        int an_complete_tmp;
        /* first check if an is enable or not */
        PHYMOD_IF_ERR_RETURN
        (tsce_dpll_phy_autoneg_get(phy, &an_control_tmp, (uint32_t *) &an_complete_tmp));
        if (!an_control_tmp.enable) {
...
#ifdef VENDOR_BROADCOM
#ifdef PHYMOD_DIAG
    PHYMOD_VDBG(TEMOD2PLL_DBG_LNK, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" sp_id=0x%0x rate=%0d phy_intf=%0d(%s) intf_md=%0x\n",
...
int tsce_dpll_phy_autoneg_ability_set(const phymod_phy_access_t* phy, const phymod_autoneg_ability_t* an_ability)
{
    ...
    const phymod_access_t *pm_acc;
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_AN, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" an_cap=%0"PRIx32" cap=%0"PRIx32"\n",
      __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, an_ability->an_cap, an_ability->capabilities));
#endif
    /* next program the tx fir taps and driver current based on the input */
    PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    ...
    value.cl73_adv.an_fec = PHYMOD_AN_FEC_CL74_GET(an_ability->an_fec);
...
#ifdef VENDOR_BROADCOM
    pm_acc = &phy->access;
    PHYMOD_VDBG(TEMOD2PLL_DBG_AN, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" an_cap=%0"PRIx32" bam=%0"PRIx32"\n",
        __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, an_ability_get_type->an_cap, an_ability_get_type->cl73bam_cap));
#endif
    return PHYMOD_E_NONE;
}
...
int tsce_dpll_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    const phymod_access_t *pm_acc;
    pm_acc = &phy->access;
    PHYMOD_VDBG(DBG_AN, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" an_en=%d flag=%0"PRIx32" mode=%x ln=%0d\n",
          __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, (int)an->enable, an->flags, an->an_mode, (int)an->num_lane_adv));
#endif
    /* next program the tx fir taps and driver current based on the input */
    PHYMOD_IF_ERR_RETURN
        (phymod_util_lane_config_get(&phy->access, &start_lane, &num_lane));
    ...
    /*if an_mode is not CL37BAM_10P9375G_VCO, then decide to if need re-configure pll0
     *if current PLL0 is not 10p3125, the pll0 need to be configured.
     */
    if (an->an_mode != phymod_AN_MODE_CL37BAM_10P9375G_VCO) {
        if (current_pll_div0 != TEMOD2PLL_PLL_MODE_DIV_66) {
            /*recofig pll0*/
            pll_change = 1;
   ...
   if ((an->num_lane_adv == 4) && !pll_change)  {
    ...
    PHYMOD_IF_ERR_RETURN
                (temod2pll_set_an_port_mode(&phy->access, an->enable, num_lane_adv_encoded, start_lane, ctrl_port));

    PHYMOD_IF_ERR_RETURN
        (temod2pll_autoneg_control(&phy_copy.access, &an_control));

#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(DBG_AN, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" an_en=%d ctrl_port=%0d\n",
         __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, (int)an->enable, ctrl_port));
#endif
    return PHYMOD_E_NONE;
...
#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(TEMOD2PLL_DBG_AN, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" an_en=%d an_done=%0"PRIx32"\n",
         __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, (int)an->enable, *an_done));
#endif
                          ...
                          const phymod_core_status_t* core_status)
{
  ...
  /* NEED TO REVISIT */
    ...
    PHYMOD_IF_ERR_RETURN
        (temod2pll_pll_config_set(&phy_access_copy.access, TEMOD2PLL_PLL_MODE_DIV_66, init_config->interface.ref_clock));
    ...
    PHYMOD_IF_ERR_RETURN
        (temod2pll_pll_config_set(&phy_access_copy.access, TEMOD2PLL_PLL_MODE_DIV_40, init_config->interface.ref_clock));
...
#ifdef VENDOR_BROADCOM
    PHYMOD_VDBG(TEMOD2PLL_DBG_LNK, pm_acc, ("%-22s: p=%p adr=%0"PRIx32" lmask=%0"PRIx32" link_status=%0d\n",
                __func__, (void *)pm_acc, pm_acc->addr, pm_acc->lane_mask, (int)*link_status));
#endif
    return PHYMOD_E_NONE;
}
