...
extern cdk_symbols_t bcmi_tsce_dpll_xgxs_symbols;
#define SET_SYMBOL_TABLE(_pc) \
    PHY_CTRL_SYMBOLS(_pc) = &bcmi_tsce_dpll_xgxs_symbols
#else
#define SET_SYMBOL_TABLE(_pc)
#endif
...
_tsce_serdes_stop(phy_ctrl_t *pc)
{
    ...
    if (CDK_SUCCESS(rv)) {
        rv = tsce_dpll_phy_tx_lane_control_set(&pm_phy, tx_control); 
    }
    if (CDK_SUCCESS(rv)) {
        rv = tsce_dpll_phy_rx_lane_control_set(&pm_phy, rx_control); 
    }
...
_tsce_ability_set(phy_ctrl_t *pc, uint32_t ability)
{
    ...
    for (idx = 0; idx < pc->num_phys; idx++) {
        ...
        rv = tsce_dpll_phy_autoneg_ability_set(&pm_phy, &autoneg_ability);
    }
...
_tsce_ability_get(phy_ctrl_t *pc, uint32_t *ability)
{
    ...
    _tsce_phymod_phy_t_init(pc, &pm_phy);
    rv = tsce_dpll_phy_autoneg_ability_get(&pm_phy, &autoneg_ability);
    if (!CDK_SUCCESS(rv)) {
        return CDK_E_FAIL;
    }
                   ...
                   phymod_phy_inf_config_t *if_cfg)
{
    ...
    for (idx = 0; idx < pc->num_phys; idx++) {
        ...
        _tsce_phymod_phy_t_init(lpc, &pm_phy);
        if (CDK_SUCCESS(rv)) {
            rv = tsce_dpll_phy_interface_config_set(&pm_phy, flags, if_cfg);
        }
    }
 ...
 */
static int
bcmi_tsce_dpll_xgxs_probe(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    ...
    found = 0;
    rv = tsce_dpll_core_identify(&pm_core, 0, &found);
 ...
 */
static int
bcmi_tsce_dpll_xgxs_notify(phy_ctrl_t *pc, phy_event_t event)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    case PhyEvent_ChangeToPassthru:
        ...
        if (CDK_SUCCESS(rv)) {
            rv = tsce_dpll_phy_interface_config_set(&pm_phy,
 ...
 */
static int
bcmi_tsce_dpll_xgxs_reset(phy_ctrl_t *pc)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcmi_tsce_dpll_xgxs_init(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    phymod_core_access_t pm_core;
    phymod_phy_access_t pm_phy;
    ...
    for (idx = 0; idx < pc->num_phys; idx++) {
        ...
        if (_tsce_primary_lane(lpc)) {
            ...
            if (CDK_SUCCESS(rv)) {
                rv = tsce_dpll_core_init(&pm_core, cic, &core_status);
            }
        }
        ...
        if (CDK_SUCCESS(rv)) {
            rv = tsce_dpll_phy_init(&pm_phy, &phy_init_config);
        }
    }
 ...
 */
static int
bcmi_tsce_dpll_xgxs_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    _tsce_phymod_phy_t_init(pc, &pm_phy);
    if (CDK_SUCCESS(rv)) {
        rv = tsce_dpll_phy_autoneg_get(&pm_phy, &an, (uint32_t *)&an_done);
    }
    *autoneg_done = 0;
    if (an.enable) {
    ...
    if (CDK_SUCCESS(rv)) {
        _tsce_phymod_phy_t_init(pc, &pm_phy);
        rv = tsce_dpll_phy_link_status_get(&pm_phy, (uint32_t *)link);
    }
...
/*
 * Function:    
 *      bcmi_tsce_dpll_xgxs_duplex_set
 * Purpose:     
 ...
 */
static int
bcmi_tsce_dpll_xgxs_duplex_set(phy_ctrl_t *pc, int duplex)
{
    PHY_CTRL_CHECK(pc);
...
/*
 * Function:    
 *      bcmi_tsce_dpll_xgxs_duplex_get
 * Purpose:     
 ...
 */
static int
bcmi_tsce_dpll_xgxs_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcmi_tsce_dpll_xgxs_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
    int rv = CDK_E_NONE;
    phymod_phy_inf_config_t interface_config;
    phymod_phy_inf_config_t *if_cfg;
...
static int
bcmi_tsce_dpll_xgxs_speed_get(phy_ctrl_t *pc, uint32_t *speed)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    phymod_phy_inf_config_t interface_config;
    ...
    if (CDK_SUCCESS(rv)) {
        rv = tsce_dpll_phy_interface_config_get(&pm_phy, 0, ref_clock, 
                                           &interface_config);
    }
...
/*
 * Function:    
 *      bcmi_tsce_dpll_xgxs_autoneg_set
 * Purpose:     
 ...
 */
static int
bcmi_tsce_dpll_xgxs_autoneg_set(phy_ctrl_t *pc, int autoneg)
{
    int rv = CDK_E_NONE;
    phymod_autoneg_control_t an;
    phymod_phy_access_t pm_phy;
    ...
    if (pc->num_phys == 3) {
        if (autoneg) {
            ...
            for (idx = 0; idx < pc->num_phys; idx++) {
                ...
                if (CDK_SUCCESS(rv)) {
                    _tsce_phymod_phy_t_init(lpc, &pm_phy);
                    rv = tsce_dpll_phy_autoneg_set(&pm_phy, &an);
                }
            }
            an.flags &= ~PHYMOD_AN_F_SET_PRIOR_ENABLE;            
        }
...
/*
 * Function:    
 *      bcmi_tsce_dpll_xgxs_autoneg_get
 * Purpose:     
 ...
 */
static int
bcmi_tsce_dpll_xgxs_autoneg_get(phy_ctrl_t *pc, int *autoneg)
{
    int rv = CDK_E_NONE;
    phymod_autoneg_control_t an;
    phymod_phy_access_t pm_phy;
    ...
    _tsce_phymod_phy_t_init(lpc, &pm_phy);
    rv = tsce_dpll_phy_autoneg_get(&pm_phy, &an, (uint32_t *)&an_complete);
...
/*
 * Function:    
 *      bcmi_tsce_dpll_xgxs_loopback_set
 * Purpose:     
 ...
 */
static int
bcmi_tsce_dpll_xgxs_loopback_set(phy_ctrl_t *pc, int enable)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    phy_ctrl_t *lpc;
    ...
    for (idx = 0; idx < pc->num_phys; idx++) {
        ...
        if (CDK_SUCCESS(rv)) {
            rv = tsce_dpll_phy_loopback_set(&pm_phy, phymodLoopbackGlobal, enable); 
        }
    }
...
/*
 * Function:    
 *      bcmi_tsce_dpll_xgxs_loopback_get
 * Purpose:     
 ...
 */
static int
bcmi_tsce_dpll_xgxs_loopback_get(phy_ctrl_t *pc, int *enable)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    uint32_t lb_enable;
    ...
    *enable = 0;
    ...
    _tsce_phymod_phy_t_init(pc, &pm_phy);
    rv = tsce_dpll_phy_loopback_get(&pm_phy, phymodLoopbackGlobal, &lb_enable); 
    if (lb_enable) {
        *enable = 1;
    }
...
/*
 * Function:    
 *      bcmi_tsce_dpll_xgxs_ability_get
 * Purpose:     
 ...
 */
static int
bcmi_tsce_dpll_xgxs_ability_get(phy_ctrl_t *pc, uint32_t *abil)
{
    PHY_CTRL_CHECK(pc);
...
/*
 * Function:
 *      bcmi_tsce_dpll_xgxs_config_set
 * Purpose:
 ...
 */
static int
bcmi_tsce_dpll_xgxs_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    int rv = CDK_E_NONE;
    phymod_core_access_t pm_core;
    phymod_phy_access_t pm_phy;
    ...
    case PhyConfig_RemoteLoopback:
        {
            for (idx = 0; idx < pc->num_phys; idx++) {
                ...
                if (CDK_SUCCESS(rv)) {
                    rv = tsce_dpll_phy_loopback_set(&pm_phy, phymodLoopbackRemotePMD, val); 
                }
            }
    ...
    case PhyConfig_XauiTxLaneRemap:
        {
            ...
            _tsce_phymod_core_t_init(pc, &pm_core);
            rv = tsce_dpll_core_lane_map_get(&pm_core, &lane_map);
            if (CDK_SUCCESS(rv)) {
                for (idx = 0; idx < NUM_LANES; idx++) {
                    /*4 bit per lane*/
                    lane_map.lane_map_tx[idx] = (val >> (idx * 4)) & 0xf;
                }
                rv = tsce_dpll_core_lane_map_set(&pm_core, &lane_map);
            }
    ...
    case PhyConfig_XauiTxPolInvert:
        {
            ...
            _tsce_phymod_phy_t_init(pc, &pm_phy);
            if (CDK_SUCCESS(rv)) {
                rv = tsce_dpll_phy_polarity_get(&pm_phy, &polarity);
            }
                    
            polarity.tx_polarity = val;
            if (CDK_SUCCESS(rv)) {
                rv = tsce_dpll_phy_polarity_set(&pm_phy, &polarity);
            }
    ...
    case PhyConfig_EEE:
        {
            for (idx = 0; idx < pc->num_phys; idx++) {
                ...
                _tsce_phymod_phy_t_init(lpc, &pm_phy);
                if (CDK_SUCCESS(rv)) {
                    rv = tsce_dpll_phy_eee_set(&pm_phy, val);
                }
            }
...
/*
 * Function:
 *      bcmi_tsce_dpll_xgxs_config_get
 * Purpose:
 ...
 */
static int
bcmi_tsce_dpll_xgxs_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    int rv = CDK_E_NONE;
    phymod_core_access_t pm_core;
    phymod_phy_access_t pm_phy;
    ...
    case PhyConfig_RemoteLoopback:
        {
            _tsce_phymod_phy_t_init(pc, &pm_phy);
            rv = tsce_dpll_phy_loopback_get(&pm_phy, phymodLoopbackRemotePMD, val);
            return rv;
        }
#if PHY_CONFIG_INCLUDE_XAUI_TX_LANE_MAP_SET
    case PhyConfig_XauiTxLaneRemap:
    ...
    case PhyConfig_EEE:
        {
            _tsce_phymod_phy_t_init(pc, &pm_phy);
            if (CDK_SUCCESS(rv)) {
                rv = tsce_dpll_phy_eee_get(&pm_phy, val);
            }
            return rv;
        }
    case PhyConfig_TxPreemp:
...
/*
 * Function:
 *      bcmi_tsce_dpll_xgxs_status_get
 * Purpose:
 ...
 */
static int
bcmi_tsce_dpll_xgxs_status_get(phy_ctrl_t *pc, phy_status_t stat, uint32_t *val)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    ...
    case PhyStatus_LineInterface:
        {
            ...
            {
                int an = 0;
                bcmi_tsce_dpll_xgxs_autoneg_get(pc, &an);
                if (an) {
                    *val = PHY_IF_KR;
                } else {
                    if (PHY_CTRL_FLAGS(pc) & PHY_F_FIBER_MODE) {
...
phy_driver_t bcmi_tsce_dpll_xgxs_drv = {
    "bcmi_tsce_dpll_xgxs", 
    "Internal TSC Eagle 40G SerDes PHY Driver",  
    PHY_DRIVER_F_INTERNAL,
    bcmi_tsce_dpll_xgxs_probe,                /* pd_probe */
    bcmi_tsce_dpll_xgxs_notify,               /* pd_notify */
    bcmi_tsce_dpll_xgxs_reset,                /* pd_reset */
    bcmi_tsce_dpll_xgxs_init,                 /* pd_init */
    bcmi_tsce_dpll_xgxs_link_get,             /* pd_link_get */
    bcmi_tsce_dpll_xgxs_duplex_set,           /* pd_duplex_set */
    bcmi_tsce_dpll_xgxs_duplex_get,           /* pd_duplex_get */
    bcmi_tsce_dpll_xgxs_speed_set,            /* pd_speed_set */
    bcmi_tsce_dpll_xgxs_speed_get,            /* pd_speed_get */
    bcmi_tsce_dpll_xgxs_autoneg_set,          /* pd_autoneg_set */
    bcmi_tsce_dpll_xgxs_autoneg_get,          /* pd_autoneg_get */
    bcmi_tsce_dpll_xgxs_loopback_set,         /* pd_loopback_set */
    bcmi_tsce_dpll_xgxs_loopback_get,         /* pd_loopback_get */
    bcmi_tsce_dpll_xgxs_ability_get,          /* pd_ability_get */
    bcmi_tsce_dpll_xgxs_config_set,           /* pd_config_set */
    bcmi_tsce_dpll_xgxs_config_get,           /* pd_config_get */
    bcmi_tsce_dpll_xgxs_status_get,           /* pd_status_get */
    NULL                                 /* pd_cable_diag */
};
