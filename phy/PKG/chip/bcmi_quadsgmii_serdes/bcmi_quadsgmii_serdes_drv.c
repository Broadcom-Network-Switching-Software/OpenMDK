...
bcmi_quadsgmii_serdes_probe(phy_ctrl_t *pc)
{
    ...
    if (phyid0 == BCM_SERDES_PHY_ID0 && phyid1 == BCM_SERDES_PHY_ID1) {
        ...
        if (model == SERDES_ID0_QUADSGMII) {
            /* The Viper SerDes and the Quad SGMII SerDes use the same SerDes 
               model number, so we use the TECH_PROC field to distinguish them. 
             */
            if (SERDES_ID0r_TECHf_GET(serdesid0) == 0x3) {
