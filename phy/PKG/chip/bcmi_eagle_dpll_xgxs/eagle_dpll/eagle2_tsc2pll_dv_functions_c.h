/**************************************************************************************
**************************************************************************************
*  File Name     :  eagle2_tsc2pll_dv_functions.c                                        *
*  Created On    :  22/05/2014                                                       *
*  Created By    :  Kiran Divakar                                                    *
*  Description   :  APIs for Serdes IPs ported over from DV                          *
*  Revision      :  $Id: eagle2_tsc2pll_dv_functions.c 1490 Broadcom SDK $ *
*                                                                                    *
...
/** @file eagle2_tsc2pll_dv_functions.c
...
err_code_t eagle2_tsc2pll_apply_txfir_cfg (const phymod_access_t *pa, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3){
	...
	err_code_t failcode = eagle2_tsc2pll_validate_txfir_cfg (pa, pre, main, post1, post2, post3);
...
err_code_t eagle2_tsc2pll_prbs_err_count_ll (const phymod_access_t *pa, uint32_t *prbs_err_cnt){
	...
	if (!prbs_err_cnt)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
	EFUN(eagle2_tsc2pll_pmd_rdt_reg(pa, TLB_RX_PRBS_CHK_ERR_CNT_MSB_STATUS, &rddata));
...
err_code_t eagle2_tsc2pll_prbs_err_count_state (const phymod_access_t *pa, uint32_t *prbs_err_cnt, uint8_t *lock_lost){
	if (!prbs_err_cnt || !lock_lost)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
	EFUN(eagle2_tsc2pll_prbs_err_count_ll (pa, prbs_err_cnt));
...
err_code_t eagle2_tsc2pll_loop_timing (const phymod_access_t *pa, uint8_t enable){
	...
	if (enable) {
		...
		EFUN(eagle2_tsc2pll_delay_us(25));              /* Wait for tclk to lock to CDR */
	} else {
...
err_code_t eagle2_tsc2pll_rmt_lpbk (const phymod_access_t *pa, uint8_t enable){
	if (enable) {
		EFUN(eagle2_tsc2pll_loop_timing (pa, enable));
		...
		EFUN(eagle2_tsc2pll_delay_us(50));      /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
...
err_code_t eagle2_tsc2pll_tx_pi_jitt_gen (const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp){
	...
	EFUN(eagle2_tsc2pll_tx_pi_freq_override (pa, enable, freq_override_val));
...
err_code_t eagle2_tsc2pll_uc_reset (const phymod_access_t *pa, uint8_t enable){
	if (enable) {
		...
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD200, 0x0000));
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD201, 0x0000));
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD202, 0x0000));
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD203, 0x0000));
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD207, 0x0000));
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD208, 0x0000));
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD20A, 0x080f));
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD20C, 0x0002));
		EFUN(eagle2_tsc2pll_pmd_wr_reg(pa, 0xD20D, 0x0000));
	}else  {
...
err_code_t eagle2_tsc2pll_core_pwrdn (const phymod_access_t *phy_pa, enum srds_core_pwrdn_mode_enum mode){
    phymod_access_t *pa;
	...
	switch (mode) {
	case PWR_ON:
		EFUN(_eagle2_tsc2pll_core_clkgate (pa, 0));
		EFUN(wrc_afe_s_pll_pwrdn(0x0));
		EFUN(wrc_core_dp_s_rstb(0x1));
		break;
	case PWRDN:
		EFUN(wrc_core_dp_s_rstb(0x0));
		EFUN(eagle2_tsc2pll_delay_ns(   500)); /* wait >50 comclk cycles  */
		EFUN(wrc_afe_s_pll_pwrdn(0x1));
		break;
	case PWRDN_DEEP:
	{   uint8_t lane_orig = eagle2_tsc2pll_get_lane(pa);
	    uint8_t lane;
	    for (lane = 0; lane < SERDES_NUM_LANES; ++lane) {
		    EFUN(eagle2_tsc2pll_set_lane(pa, lane));
		    EFUN(wr_ln_rx_s_pwrdn(  0x1));
		    EFUN(wr_ln_tx_s_pwrdn(  0x1));
		    EFUN(_eagle2_tsc2pll_lane_clkgate (pa, 1));
		    EFUN(wr_ln_dp_s_rstb(   0x0));
	    }
	    EFUN(eagle2_tsc2pll_set_lane(pa,lane_orig)); }
		EFUN(wrc_core_dp_s_rstb(0x0));
		EFUN(eagle2_tsc2pll_delay_ns(   500)); /* wait >50 comclk cycles  */
		{   uint8_t pll_orig = eagle2_tsc2pll_get_pll(pa);
		    uint8_t pll;
		    for (pll = 0; pll < SERDES_NUM_PLLS; ++pll) {
			    EFUN(eagle2_tsc2pll_set_pll(pa,pll));
			    EFUN(wrc_afe_s_pll_pwrdn(0x1));
		    }
		    EFUN(eagle2_tsc2pll_set_pll(pa,pll_orig)); }
		EFUN(_eagle2_tsc2pll_core_clkgate (pa, 1));
		break;
	default:
...
err_code_t eagle2_tsc2pll_lane_pwrdn (const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode){
	...
	case PWR_ON:
		...
		EFUN(_eagle2_tsc2pll_lane_clkgate (pa, 0));
		break;
	case PWRDN:
...
err_code_t eagle2_tsc2pll_isolate_ctrl_pins (const phymod_access_t *pa, uint8_t enable){
	...
	EFUN(eagle2_tsc2pll_isolate_lane_ctrl_pins (pa, enable));
	EFUN(eagle2_tsc2pll_isolate_core_ctrl_pins (pa, enable));
...
err_code_t eagle2_tsc2pll_ucode_mdio_load (const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len){
	...
	EFUN(eagle2_tsc2pll_delay_us(300));                     /* Wait for Initialization to finish */
#ifndef ATE_LOG
...
err_code_t eagle2_tsc2pll_wrw_uc_ram (const phymod_access_t *pa, uint16_t addr, uint16_t wr_val){
	...
	EFUN(eagle2_tsc2pll_delay_ns(80));                      /* wait for 10 comclk cycles/ 80ns  */
	EFUN(wrc_micro_ram_wrdata(wr_val));                     /* RAM Write value */
	EFUN(eagle2_tsc2pll_delay_ns(80));                      /* Wait for Data RAM to be written */
	return ERR_CODE_NONE;
}
...
/* Micro RAM Word Read */
uint16_t eagle2_tsc2pll_rdw_uc_ram (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint16_t rddata;
	...
	*err_code_p = ERR_CODE_NONE;
	...
	EPFUN(eagle2_tsc2pll_delay_ns(80));                     /* Wait for the RAM read  */
...
/* Micro RAM Byte Read */
uint8_t eagle2_tsc2pll_rdb_uc_ram (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint8_t rddata;
...
err_code_t eagle2_tsc2pll_wrws_uc_ram (const phymod_access_t *pa, uint16_t addr, int16_t wr_val){
	return eagle2_tsc2pll_wrw_uc_ram (pa, addr, wr_val);
}

/* Micro RAM Byte Signed Write */
err_code_t eagle2_tsc2pll_wrbs_uc_ram (const phymod_access_t *pa, uint16_t addr, int8_t wr_val){
	return eagle2_tsc2pll_wrb_uc_ram (pa, addr, wr_val);
}

/* Micro RAM Word Signed Read */
int16_t eagle2_tsc2pll_rdws_uc_ram (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	return (int16_t)eagle2_tsc2pll_rdw_uc_ram (pa, err_code_p, addr);
}

/* Micro RAM Byte Signed Read */
int8_t eagle2_tsc2pll_rdbs_uc_ram (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	return (int8_t)eagle2_tsc2pll_rdb_uc_ram (pa, err_code_p, addr);
}
...
err_code_t eagle2_tsc2pll_rdblk_uc_ram (const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt){
	...
	while (cnt--) {
		...
		uint8_t tmp = eagle2_tsc2pll_rdb_uc_ram (pa, &error_code, addr++); /* Value read from the required Data RAM Address */
		if (error_code)
