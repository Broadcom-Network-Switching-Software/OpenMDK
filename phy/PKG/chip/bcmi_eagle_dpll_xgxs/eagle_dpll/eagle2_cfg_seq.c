 ...
 *  $Id: 67cb93301b185240069ac54f8d207f7f6f97929c $
*/


#include <phymod/phymod.h>
#include "eagle2_cfg_seq.h"
#include <phy/chip/bcmi_eagle_dpll_xgxs_defs.h>
...
{
    CORE_PLL_TOP_USER_CTLr_t reg;

    CORE_PLL_TOP_USER_CTLr_CLR(reg);
    CORE_PLL_TOP_USER_CTLr_CORE_DP_S_RSTBf_SET(reg, enable);
    MODIFY_CORE_PLL_TOP_USER_CTLr(pa, reg);
    return ERR_CODE_NONE;
}
...
err_code_t eagle2_tsc2pll_pram_flop_set(const phymod_access_t *pa, int value)   /* release the pmd core soft reset */
{
...
err_code_t eagle2_tsc2pll_pmd_force_signal_detect(const phymod_access_t *pa, int enable)
{
    SIGDET_CTL1r_t reg;
...
err_code_t eagle2_tsc2pll_uc_init(const phymod_access_t *pa)
{
  ...
  eagle2_tsc2pll_pram_flop_set(pa, 0x0);
...
err_code_t eagle2_tsc2pll_lane_hard_soft_reset_release(const phymod_access_t *pa, uint32_t enable)   /* release the pmd core soft reset */
{
  err_code_t __err;
  __err=ERR_CODE_NONE;
...
err_code_t eagle2_tsc2pll_clause72_control(const phymod_access_t *pa, uint32_t cl_72_en)                /* CLAUSE_72_CONTROL */
{
  err_code_t __err;
...
err_code_t eagle2_tsc2pll_clause72_control_get(const phymod_access_t *pa, uint32_t *cl_72_en)                /* CLAUSE_72_CONTROL */
{
  err_code_t __err;
  __err=ERR_CODE_NONE;
...
err_code_t eagle2_tsc2pll_pll_config_get(const phymod_access_t *pa,enum  eagle2_tsc2pll_pll_enum *pll_mode)
{
  ...
  if(fracn_sel == 1) {
     switch(fracn_ndiv) {
       case 0x52: *pll_mode = EAGLE2_TSC2PLL_pll_div_82p5x;
                  break;
       case 0x57: *pll_mode = EAGLE2_TSC2PLL_pll_div_87p5x;
                  break;
     ...
     switch(pll_div){
       case 0x1: *pll_mode = EAGLE2_TSC2PLL_pll_div_72x;
                 break;
       case 0xc: *pll_mode = EAGLE2_TSC2PLL_pll_div_70x;
                 break;
       case 0xa: *pll_mode = EAGLE2_TSC2PLL_pll_div_66x;
                 break;
       case 0x9: *pll_mode = EAGLE2_TSC2PLL_pll_div_64x;
                 break;
       case 0x8: *pll_mode = EAGLE2_TSC2PLL_pll_div_60x;
                 break;
       case 0x6: *pll_mode = EAGLE2_TSC2PLL_pll_div_52x;
                 break;
       case 0x3: *pll_mode = EAGLE2_TSC2PLL_pll_div_42x;
                 break;
       case 0x2: *pll_mode = EAGLE2_TSC2PLL_pll_div_40x;
                 break;
       case 0xf: *pll_mode = EAGLE2_TSC2PLL_pll_div_100x;
                 break;
       case 0xe: *pll_mode = EAGLE2_TSC2PLL_pll_div_92x;
                 break;
       case 0xb: *pll_mode = EAGLE2_TSC2PLL_pll_div_68x;
                 break;
       case 0x5: *pll_mode = EAGLE2_TSC2PLL_pll_div_50x;
                 break;
       case 0x0: *pll_mode = EAGLE2_TSC2PLL_pll_div_46x;
                 break;
       case 0xd: *pll_mode = EAGLE2_TSC2PLL_pll_div_80x;
                 break;
       default: return ((ERR_CODE_INVALID_PLL_CFG));
                break;
     }
  ...
  if(config->ref_clock == phymodRefClk156Mhz) {
    switch (config->data_rate) {
      ...
      case 1250 :   /* speed 1.25G ; pll_div= 40    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_40x; *new_os_mode = 5 /*= OS5 */; *vco_rate = 1250; /* vco_rate = */
        break;

      case 3125 :   /* speed 3.125G ; pll_div= 60    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_40x; *new_os_mode = 1 /*= OS2 */; *vco_rate = 6250; /* vco_rate = */
        break;

      case 5750 :   /* speed 5.75G ; fracn pll= 73.6    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_73p6x; *new_os_mode = 1 /*= OS2 */; *vco_rate = 11500; /* vco_rate = 24 */
        break;

      case 6250:   /* speed 6.25G: pll_div=40  */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_40x;  *new_os_mode = 0 /*= OS2 */; *vco_rate = 6250; /* vco_rate=  */
        break;

      case 10312:   /* speed 10.3G; pll_div = 66  */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_66x; *new_os_mode = 0 /*= OS1 */; *vco_rate = 10312; /* vco_rate = 19 */
        break;

      case 10937:   /* speed 10.937G ; pll_div= 70    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_70x; *new_os_mode = 0 /*= OS1 */; *vco_rate = 109375; /* vco_rate = */
        break;

      case 11250:   /* speed 11.25G ; pll_div= 72    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_72x; *new_os_mode = 0 /*= OS1 */; *vco_rate = 11250; /* vco_rate = */
        break;

      case 11500:   /* speed 11.5G ; fracn pll= 73.6    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_73p6x; *new_os_mode = 0 /*= OS1 */; *vco_rate = 11500; /* vco_rate = 24 */
        break;

      case 12500:   /* speed 12.5G ; pll_div = 80    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_80x; *new_os_mode = 0 /*= OS1 */; *vco_rate = 12500; /* vco_rate =  */
        break;
    ...
    switch (config->data_rate) {
      ...
      case 1250 :   /* speed 1.25G ; pll_div= 50    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_50x; *new_os_mode = 5 /*= OS5 */; *vco_rate = 6250; /* vco_rate = */
        break;

      case 3125 :   /* speed 3.125G ; pll_div= 50    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_50x; *new_os_mode = 1 /*= OS2 */; *vco_rate = 6250; /* vco_rate = */
        break;

      case 5750 :   /* speed 5.75G ; pll_div= 46    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_46x; *new_os_mode = 0 /*= OS1 */; *vco_rate =5750; /* vco_rate =  */
        break;

      case 6250:   /* speed 6.25G: pll_div=50  */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_50x;  *new_os_mode = 0 /*= OS1 */; *vco_rate = 6250; /* vco_rate= 3 */
        break;

      case 10312:   /* speed 10.3G; fracn pll = 82.5  */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_82p5x; *new_os_mode = 0 /*= OS1 */; *vco_rate = 10312; /* vco_rate = 19 */
        break;

      case 11500:   /* speed 11.5G ; pll_div= 92    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_92x; *new_os_mode = 0 /*= OS1 */; *vco_rate = 11500; /* vco_rate = 24 */
        break;

      case 12500:   /* speed 12.5G ; pll_div= 100    */
        *new_pll_div = EAGLE2_TSC2PLL_pll_div_100x; *new_os_mode = 0 /*= OS1 */; *vco_rate = 12500; /* vco_rate = */
        break;
...
err_code_t eagle2_tsc2pll_config_shared_tx_pattern_idx_set( const phymod_access_t *pa, const uint32_t *pattern_len ) {
  ...
  if(mode_sel !=0)
    /* coverity[on_same_line] */
    __err = wr_patt_gen_start_pos(mode_sel); if(__err) return(__err);
/*
...
err_code_t eagle2_tsc2pll_osr_mode_to_enum(int osr_mode, phymod_osr_mode_t* osr_mode_en)
{
    switch(osr_mode) {
        ...
        default:
            *osr_mode_en = phymodOversampleMode1;break;
    }
    return ERR_CODE_NONE;
}
