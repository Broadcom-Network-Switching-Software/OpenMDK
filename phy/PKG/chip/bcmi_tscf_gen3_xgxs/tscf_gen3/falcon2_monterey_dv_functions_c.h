/**************************************************************************************
**************************************************************************************
*  File Name     :  falcon2_monterey_dv_functions.c                                        *
*  Created On    :  22/05/2014                                                       *
*  Created By    :  Kiran Divakar                                                    *
*  Description   :  APIs for Serdes IPs ported over from DV                          *
*  Revision      :  $Id: falcon2_monterey_dv_functions.c 1560 Broadcom SDK $ *
*                                                                                    *
...
/** @file falcon2_monterey_dv_functions.c
...
err_code_t falcon2_monterey_config_shared_tx_pattern (const phymod_access_t *pa, uint8_t patt_length, const char pattern[]){
...
err_code_t falcon2_monterey_tx_shared_patt_gen_en (const phymod_access_t *pa, uint8_t enable, uint8_t patt_length){
	uint8_t zero_pad_len = 0;       /* suppress warnings, changed by _calc_patt_gen_mode_sel (pa, ) */
	uint8_t mode_sel     = 0;       /* suppress warnings, changed by _calc_patt_gen_mode_sel (pa, ) */
...
err_code_t falcon2_monterey_pmd_lock_status (const phymod_access_t *pa, uint8_t *pmd_rx_lock){
	if (!pmd_rx_lock)
...
err_code_t falcon2_monterey_display_cl93n72_status (const phymod_access_t *pa){
	EFUN_PRINTF(("\n\n***************************\n"                                                                                                                                    ));
...
err_code_t falcon2_monterey_tx_disable (const phymod_access_t *pa, uint8_t enable){
...
err_code_t falcon2_monterey_rx_restart (const phymod_access_t *pa, uint8_t enable){
...
err_code_t falcon2_monterey_write_rx_afe (const phymod_access_t *pa, enum srds_rx_afe_settings_enum param, int8_t val){
...
err_code_t falcon2_monterey_read_rx_afe (const phymod_access_t *pa, enum srds_rx_afe_settings_enum param, int8_t *val){
	/* Assumes the micro is not actively tuning */
	if (!val)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
...
err_code_t falcon2_monterey_validate_txfir_cfg (const phymod_access_t *pa, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3){
...
err_code_t falcon2_monterey_write_tx_afe (const phymod_access_t *pa, enum srds_tx_afe_settings_enum param, int8_t val){
...
err_code_t falcon2_monterey_read_tx_afe (const phymod_access_t *pa, enum srds_tx_afe_settings_enum param, int8_t *val){
	if (!val)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
...
err_code_t falcon2_monterey_apply_txfir_cfg (const phymod_access_t *pa, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3){
	...
	err_code_t failcode = falcon2_monterey_validate_txfir_cfg (pa, pre, main, post1, post2, post3);
...
err_code_t falcon2_monterey_config_tx_prbs (const phymod_access_t *pa, enum srds_prbs_polynomial_enum prbs_poly_mode, uint8_t prbs_inv){
...
err_code_t falcon2_monterey_get_tx_prbs_config (const phymod_access_t *pa, enum srds_prbs_polynomial_enum *prbs_poly_mode, uint8_t *prbs_inv){
	uint8_t val;
...
err_code_t falcon2_monterey_tx_prbs_en (const phymod_access_t *pa, uint8_t enable){
...
err_code_t falcon2_monterey_get_tx_prbs_en (const phymod_access_t *pa, uint8_t *enable){
...
err_code_t falcon2_monterey_tx_prbs_err_inject (const phymod_access_t *pa, uint8_t enable){
	/* PRBS Error Insert.
...
err_code_t falcon2_monterey_config_rx_prbs (const phymod_access_t *pa, enum srds_prbs_polynomial_enum prbs_poly_mode, enum srds_prbs_checker_mode_enum prbs_checker_mode, uint8_t prbs_inv){
...
/* get PRBS Checker */
err_code_t falcon2_monterey_get_rx_prbs_config (const phymod_access_t *pa, enum srds_prbs_polynomial_enum *prbs_poly_mode, enum srds_prbs_checker_mode_enum *prbs_checker_mode, uint8_t *prbs_inv){
	uint8_t val;
...
err_code_t falcon2_monterey_rx_prbs_en (const phymod_access_t *pa, uint8_t enable){
...
err_code_t falcon2_monterey_get_rx_prbs_en (const phymod_access_t *pa, uint8_t *enable){
...
err_code_t falcon2_monterey_prbs_chk_lock_state (const phymod_access_t *pa, uint8_t *chk_lock){
	if (!chk_lock)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
...
err_code_t falcon2_monterey_prbs_err_count_ll (const phymod_access_t *pa, uint32_t *prbs_err_cnt){
	uint16_t rddata;

	if (!prbs_err_cnt)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, TLB_RX_PRBS_CHK_ERR_CNT_MSB_STATUS, &rddata));
...
err_code_t falcon2_monterey_prbs_err_count_state (const phymod_access_t *pa, uint32_t *prbs_err_cnt, uint8_t *lock_lost){
	if (!prbs_err_cnt || !lock_lost)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
	EFUN(falcon2_monterey_prbs_err_count_ll (pa, prbs_err_cnt));
	*lock_lost    = (*prbs_err_cnt >> 31);
	*prbs_err_cnt = (*prbs_err_cnt & 0x7FFFFFFF);
	return ERR_CODE_NONE;
}

err_code_t falcon2_monterey_display_detailed_prbs_state_hdr (const phymod_access_t *pa){
...
err_code_t falcon2_monterey_display_detailed_prbs_state (const phymod_access_t *pa){
	uint32_t err_cnt = 0;
	uint8_t lock_lost = 0, enabled = 0;

	ESTM_PRINTF(("  %d ", falcon2_monterey_get_lane (pa )));
	...
	EFUN(falcon2_monterey_prbs_err_count_state (pa, &err_cnt, &lock_lost));
...
err_code_t falcon2_monterey_dig_lpbk (const phymod_access_t *pa, uint8_t enable){
...
err_code_t falcon2_monterey_dig_lpbk_rptr (const phymod_access_t *pa, uint8_t enable, enum srds_rptr_mode_enum mode){
	switch (mode) {
	case DATA_IN_SIDE:
		/* DATA_IN_SIDE  - The side where data is fed into the RX serial input */
		if (enable) {
			EFUN(falcon2_monterey_loop_timing (pa, 0));
			EFUN(falcon2_monterey_loop_timing (pa, 1)); /* Program DATA_IN_SIDE side TX_PI for loop-timing mode to lock SYS TX to SYS TX clock freq. */
		} else
			EFUN(falcon2_monterey_loop_timing (pa, 0));
		break;
	...
	{ uint8_t osr_mode;
	  ...
	  if ((osr_mode == FALCON2_MONTEREY_OSX16P5) || (osr_mode == FALCON2_MONTEREY_OSX20P625))
		  EFUN(wr_rx_pi_manual_mode(0x0)); }
		EFUN(falcon2_monterey_tx_rptr_mode_timing (pa, 1));  /* Program DIG_LPBK_SIDE side TX_PI for repeater mode to lock LINE TX to SYS RX clock freq. */
		EFUN(wr_dig_lpbk_en(enable));                   /* 0 = diabled, 1 = enabled */
...
err_code_t falcon2_monterey_ull_from_dig_lpbk (const phymod_access_t *pa, enum srds_rptr_mode_enum mode){
	switch (mode) {
	case DATA_IN_SIDE:
		EFUN(falcon2_monterey_dig_lpbk_rptr (pa, 0, mode));
		EFUN(falcon2_monterey_tx_ull_config (pa, 1));
		break;
	case DIG_LPBK_SIDE:
		EFUN(falcon2_monterey_dig_lpbk_rptr (pa, 0, mode));
		EFUN(falcon2_monterey_rx_ull_config (pa, 1));
		break;
	default: return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
		break;
...
err_code_t falcon2_monterey_nl_from_dig_lpbk (const phymod_access_t *pa, enum srds_rptr_mode_enum mode){
	EFUN(falcon2_monterey_dig_lpbk_rptr (pa, 0, mode));
	return ERR_CODE_NONE;
...
err_code_t falcon2_monterey_loop_timing (const phymod_access_t *pa, uint8_t enable){
	uint8_t osr_mode;
	...
	if (enable) {
		...
		EFUN(falcon2_monterey_delay_us(25));    /* Wait for tclk to lock to CDR */
	} else {
...
err_code_t falcon2_monterey_rmt_lpbk (const phymod_access_t *pa, uint8_t enable){
	if (enable) {
		EFUN(falcon2_monterey_loop_timing (pa, enable));
		...
		EFUN(falcon2_monterey_delay_us(50));    /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
...
err_code_t falcon2_monterey_rmt_lpbk_from_ull (const phymod_access_t *pa){
	EFUN(falcon2_monterey_rmt_lpbk (pa, 0));     /* Reset TX_PI */
	EFUN(falcon2_monterey_rmt_lpbk (pa, 1));     /* Enable RMT LPBK */
	return ERR_CODE_NONE;
}


/* Switch to remote loopback from NL mode in repeater */
err_code_t falcon2_monterey_rmt_lpbk_from_nl (const phymod_access_t *pa){
	EFUN(falcon2_monterey_rmt_lpbk (pa, 0));     /* Reset TX_PI */
	EFUN(falcon2_monterey_rmt_lpbk (pa, 1));     /* Enable RMT LPBK */
	return ERR_CODE_NONE;
}
...
err_code_t falcon2_monterey_ull_from_rmt_lpbk (const phymod_access_t *pa, enum srds_rptr_mode_enum mode){
	switch (mode) {
	case DATA_IN_SIDE:
		EFUN(falcon2_monterey_rx_rptr_mode_timing (pa, 1)); /* Enable jitter filter enable */
		EFUN(_ull_rx_mode_setup (pa, 1));
		break;
	case RMT_LPBK_SIDE:
		EFUN(falcon2_monterey_rmt_lpbk (pa, 0));             /* Reset TX_PI */
		EFUN(falcon2_monterey_tx_rptr_mode_timing (pa, 1));  /* Enable jitter filter enable */
...
err_code_t falcon2_monterey_nl_from_rmt_lpbk (const phymod_access_t *pa, enum srds_rptr_mode_enum mode){
	switch (mode) {
	case DATA_IN_SIDE:
		EFUN(falcon2_monterey_rx_rptr_mode_timing (pa, 1)); /* Enable RX PI transfer */
		break;
	case RMT_LPBK_SIDE:
		EFUN(falcon2_monterey_rmt_lpbk (pa, 0));             /* Reset TX_PI */
		EFUN(falcon2_monterey_tx_rptr_mode_timing (pa, 1));  /* Enable jitter filter enable */
...
err_code_t falcon2_monterey_tx_pi_freq_override (const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val){
	if (enable) {
...
err_code_t falcon2_monterey_tx_pi_jitt_gen (const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp){
	...
	EFUN(falcon2_monterey_tx_pi_freq_override (pa, enable, freq_override_val));
...
err_code_t falcon2_monterey_core_config_for_iddq (const phymod_access_t *pa){

	return ERR_CODE_NONE;
}


err_code_t falcon2_monterey_lane_config_for_iddq (const phymod_access_t *pa){
...
err_code_t falcon2_monterey_uc_active_enable (const phymod_access_t *pa, uint8_t enable){
...
err_code_t falcon2_monterey_uc_reset (const phymod_access_t *pa, uint8_t enable){
	if (enable) {
		...
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD200, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD201, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD202, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD204, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD205, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD206, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD207, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD208, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD209, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD20A, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD20B, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD20C, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD20D, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD20E, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD211, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD212, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD213, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD214, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD215, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD216, 0x0007));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD217, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD218, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD219, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD21A, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD21B, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD220, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD221, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD224, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD226, 0x0000));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD225, 0x8803));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD228, 0x0101));
		EFUN(falcon2_monterey_pmd_wr_reg (pa, 0xD229, 0x0000));
		...
		EFUN(falcon2_monterey_poll_micro_ra_initdone (pa, 250));     /* Poll   status of  data RAM initialization */
...
err_code_t falcon2_monterey_map_lanes (const phymod_access_t *pa, uint8_t num_lanes, uint8_t const *tx_lane_map, uint8_t const *rx_lane_map){
	uint8_t rd_val = 0;
...
/*#else */                  /* defined(wrc_tx_lane_addr_0) || defined(wrc_tx_lane_map_0)*/
			...
			if ((tx_lane_map[index1] != rx_lane_map[index1])
				...
				EFUN_PRINTF(("Warning:  In core %d, TX lane %d is mapped to %d, while RX lane %d is mapped to %d.\n          Digital and remote loopback will not operate as expected.\n          Further warnings are suppressed.\n",
					     EFUN(falcon2_monterey_get_core()), index1, tx_lane_map[index1], index1, rx_lane_map[index1]));
...
err_code_t falcon2_monterey_core_pwrdn (const phymod_access_t *pa_phy, enum srds_core_pwrdn_mode_enum mode){
        phymod_access_t *pa;
	...
	switch (mode) {
	case PWR_ON:
		EFUN(_falcon2_monterey_core_clkgate (pa, 0));
		EFUN(wrc_afe_s_pll_pwrdn(0x0));
		EFUN(wrc_core_dp_s_rstb(0x1));
		break;
	case PWRDN:
		EFUN(wrc_core_dp_s_rstb(0x0));
		EFUN(falcon2_monterey_delay_ns(   500)); /* wait >50 comclk cycles  */
		EFUN(wrc_afe_s_pll_pwrdn(0x1));
		break;
	case PWRDN_DEEP:
	{   uint8_t lane_orig = falcon2_monterey_get_lane (pa );
	    uint8_t lane;
	    for (lane = 0; lane < SERDES_NUM_LANES; ++lane) {
		    EFUN(falcon2_monterey_set_lane(pa, lane));
		    EFUN(wr_ln_rx_s_pwrdn(  0x1));
		    EFUN(wr_ln_tx_s_pwrdn(  0x1));
		    EFUN(_falcon2_monterey_lane_clkgate (pa, 1));
		    EFUN(wr_ln_dp_s_rstb(   0x0));
	    }
	    EFUN(falcon2_monterey_set_lane(pa, lane_orig)); }
		EFUN(wrc_core_dp_s_rstb(0x0));
		EFUN(falcon2_monterey_delay_ns(   500)); /* wait >50 comclk cycles  */
		{   uint8_t pll_orig = falcon2_monterey_get_pll(pa);
		    uint8_t pll;
		    for (pll = 0; pll < SERDES_NUM_PLLS; ++pll) {
			    EFUN(falcon2_monterey_set_pll(pa, pll));
			    EFUN(wrc_afe_s_pll_pwrdn(0x1));
		    }
		    EFUN(falcon2_monterey_set_pll(pa, pll_orig)); }
		EFUN(_falcon2_monterey_core_clkgate (pa, 1));
		break;
	default:
...
err_code_t falcon2_monterey_lane_pwrdn (const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode){
	switch (mode) {
	case PWR_ON:
		EFUN(wr_ln_tx_s_pwrdn(0x0));
		EFUN(wr_ln_rx_s_pwrdn(0x0));
		EFUN(_falcon2_monterey_lane_clkgate (pa, 0));
		break;
	case PWRDN:
...
err_code_t falcon2_monterey_isolate_ctrl_pins (const phymod_access_t *pa, uint8_t enable){
	...
	EFUN(falcon2_monterey_isolate_lane_ctrl_pins (pa, enable));
	EFUN(falcon2_monterey_isolate_core_ctrl_pins (pa, enable));
...
err_code_t falcon2_monterey_ucode_mdio_load (const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len){
...
err_code_t falcon2_monterey_ucode_load_verify (const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len){
...
/* Micro RAM Word Read */
uint16_t falcon2_monterey_rdw_uc_ram (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint16_t rddata;
...
/* Micro RAM Byte Read */
uint8_t falcon2_monterey_rdb_uc_ram (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint8_t rddata;
...
err_code_t falcon2_monterey_wrws_uc_ram (const phymod_access_t *pa, uint16_t addr, int16_t wr_val){
	return falcon2_monterey_wrw_uc_ram (pa, addr, wr_val);
}

/* Micro RAM Byte Signed Write */
err_code_t falcon2_monterey_wrbs_uc_ram (const phymod_access_t *pa, uint16_t addr, int8_t wr_val){
	return falcon2_monterey_wrb_uc_ram (pa, addr, wr_val);
}

/* Micro RAM Word Signed Read */
int16_t falcon2_monterey_rdws_uc_ram (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	return (int16_t)falcon2_monterey_rdw_uc_ram (pa, err_code_p, addr);
}

/* Micro RAM Byte Signed Read */
int8_t falcon2_monterey_rdbs_uc_ram (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	return (int8_t)falcon2_monterey_rdb_uc_ram (pa, err_code_p, addr);
}
...
err_code_t falcon2_monterey_rdblk_uc_ram (const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt){
	...
	while (cnt--) {
		...
		uint8_t tmp = falcon2_monterey_rdb_uc_ram (pa, &error_code, addr++); /* Value read from the required Data RAM Address */
		if (error_code)
...
err_code_t falcon2_monterey_configure_fast_link_recovery (const phymod_access_t *pa, uint8_t enable, uint8_t enable_timeout, enum falcon2_monterey_flr_timeout_enum timeout_time, uint8_t enable_2ms_yield, uint8_t filter_count){
	...
	if (reset_state < 7) {
		EFUN_PRINTF(("ERROR: falcon2_monterey_configure_fast_link_recovery (pa, ..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n", falcon2_monterey_get_lane (pa ), reset_state));
		return _error(ERR_CODE_LANE_DP_NOT_RESET);
	}
#endif

	if (timeout_time_val > 3) {
		ESTM_PRINTF(("timeout_time value for falcon2_monterey_configure_fast_link_recovery (pa, ) is too large:  %d\n", timeout_time_val));
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
	}
	if (filter_count > 7) {
		ESTM_PRINTF(("filter_count value for falcon2_monterey_configure_fast_link_recovery (pa, ) is too large:  %d\n", filter_count));
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
	}
