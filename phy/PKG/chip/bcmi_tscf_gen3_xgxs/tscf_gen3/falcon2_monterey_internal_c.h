/***********************************************************************************
***********************************************************************************
*  File Name     :  falcon2_monterey_internal.c                                         *
*  Created On    :  13/02/2014                                                    *
*  Created By    :  Justin Gaither                                                *
*  Description   :  APIs for Serdes IPs                                           *
*  Revision      :  $Id: falcon2_monterey_internal.c 1560 Broadcom SDK $ *
*                                                                                 *
...
/** @file falcon2_monterey_internal.c
...
err_code_t falcon2_monterey_log_full_pmd_state (const phymod_access_t *pa, struct falcon2_monterey_detailed_lane_status_st *lane_st){
	...
	EFUN(falcon2_monterey_read_tx_afe (pa, TX_AFE_AMP, &tmp)); lane_st->amp_ctrl = (uint8_t)tmp;
	EFUN(falcon2_monterey_read_tx_afe (pa, TX_AFE_PRE, &tmp)); lane_st->pre_tap = (uint8_t)tmp;
	EFUN(falcon2_monterey_read_tx_afe (pa, TX_AFE_MAIN, &tmp)); lane_st->main_tap = (uint8_t)tmp;
	EFUN(falcon2_monterey_read_tx_afe (pa, TX_AFE_POST1, &tmp)); lane_st->post1_tap = (uint8_t)tmp;
	EFUN(falcon2_monterey_read_tx_afe (pa, TX_AFE_POST2, &lane_st->post2_tap));
	EFUN(falcon2_monterey_read_tx_afe (pa, TX_AFE_POST3, &lane_st->post3_tap));
	...
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_D, &reg_data));
	lane_st->ddq_hoffset = (uint8_t)dist_ccw(((reg_data >> 8) & 0xFF), (reg_data & 0xFF));
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_P, &reg_data));
	lane_st->ppq_hoffset = (uint8_t)dist_ccw(((reg_data >> 8) & 0xFF), (reg_data & 0xFF));
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_L, &reg_data));
	lane_st->llq_hoffset = (uint8_t)dist_ccw(((reg_data >> 8) & 0xFF), (reg_data & 0xFF));
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_PD, &reg_data));
	lane_st->dp_hoffset = (uint8_t)dist_cw(((reg_data >> 8) & 0xFF), (reg_data & 0xFF));
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_LD, &reg_data));
	...
	EFUN(falcon2_monterey_prbs_chk_lock_state (pa, &lane_st->prbs_chk_lock));
	EFUN(falcon2_monterey_prbs_err_count_ll (pa, &lane_st->prbs_chk_errcnt));
...
static err_code_t _ull_tx_mode_setup (const phymod_access_t *pa, uint8_t enable){
	...
	EFUN(falcon2_monterey_delay_us(1024));                  /* Wait for phase detector path to settle */
	...
	EFUN(falcon2_monterey_poll_st_afe_tx_fifo_resetb_equals_1 (pa, 1));  /* Check if FIFO reset is reported to be released within 1ms time interval */
...
static uint8_t _falcon2_monterey_rdb_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	...
	EPSTM(rddata = falcon2_monterey_rdb_uc_ram (pa, err_code_p, addr)); /* Use Micro register interface for reading RAM */
...
#endif
	EPSTM(rddata = falcon2_monterey_rdw_uc_ram (pa, err_code_p, (addr))); /* Use Micro register interface for reading RAM */
...
static err_code_t _falcon2_monterey_wrb_uc_var (const phymod_access_t *pa, uint16_t addr, uint8_t wr_val){
	...
	return falcon2_monterey_wrb_uc_ram (pa, addr, wr_val);                              /* Use Micro register interface for writing RAM */
}
...
static err_code_t _falcon2_monterey_wrw_uc_var (const phymod_access_t *pa, uint16_t addr, uint16_t wr_val){
	...
	return falcon2_monterey_wrw_uc_ram (pa, addr, wr_val);                               /* Use Micro register interface for writing RAM */
}
...
static err_code_t _display_event (const phymod_access_t *pa, uint8_t event_id, uint8_t entry_len, uint8_t prev_cursor, uint8_t curr_cursor, uint8_t post_cursor, uint8_t *supp_info){
	...
	case EVENT_CODE_RELEASE_USER_CORE_PLL1_RESET:
		...
		break;
	case EVENT_CODE_ACTIVE_WAIT_FOR_SIG:
		EFUN_PRINTF(("  Active wait for signal"));
		break;
	case EVENT_CODE_EXIT_FROM_WAIT_FOR_SIG:
		EFUN_PRINTF(("  Exit from wait for signal"));
		break;
	case EVENT_CODE_START_VGA_TUNING:
		EFUN_PRINTF(("  Start to tune VGA"));
		break;
	case EVENT_CODE_START_FX_DFE_TUNING:
		EFUN_PRINTF(("  Start to tune fixed DFE tap"));
		break;
	case EVENT_CODE_START_PF_TUNING:
		EFUN_PRINTF(("  Start to tune peaking filter"));
		break;
	case EVENT_CODE_START_EYE_MEAS_TUNING:
		EFUN_PRINTF(("  Start to measure eyes"));
		break;
	case EVENT_CODE_START_MAIN_TAP_TUNING:
		EFUN_PRINTF(("  Start to tune main tap"));
		break;
	case EVENT_CODE_START_FL_TAP_TUNING:
		EFUN_PRINTF(("  Start to tune floating DFE taps"));
		break;
	case EVENT_CODE_RESTART_PMD_ON_SHORT_CHANNEL_DETECTED:
		EFUN_PRINTF(("  Restart Rx PMD due to Short Channel detected"));
		break;
	case EVENT_CODE_CL72_FCTTS_TF_TRN:
		EFUN_PRINTF(("  LP TX is operating with fixed TXFIR coefficients"));
		break;
	case EVENT_CODE_FLR_ACTIVATED:
		EFUN_PRINTF(("  Fast Lane Recovery activated"));
		break;
	case EVENT_CODE_FLR_SIGDET_DEASSERT:
		EFUN_PRINTF(("  Fast Lane Recovery SigDet deasserted (but masked)"));
		break;
	case EVENT_CODE_FLR_SIGDET_ASSERT:
		EFUN_PRINTF(("  Fast Lane Recovery SigDet asserted; tuning restarted"));
		break;
	case EVENT_CODE_FLR_DEACTIVATED_DUE_TO_TIMEOUT:
		EFUN_PRINTF(("  Fast Lane Recovery deactivated due to SigDet timeout"));
		break;
	default:
...
static err_code_t _get_tx_driver_mode (const phymod_access_t *pa, int8_t *val){
	ESTM(*val = rd_ams_tx_driver_mode());
	return ERR_CODE_NONE;
}
...
SDK_STATIC err_code_t _falcon2_monterey_get_osr_mode (const phymod_access_t *pa, falcon2_monterey_osr_mode_st *imode){
	falcon2_monterey_osr_mode_st mode;

	ENULL_MEMSET(&mode, 0, sizeof(falcon2_monterey_osr_mode_st));
...
SDK_STATIC err_code_t _falcon2_monterey_read_lane_state (const phymod_access_t *pa, falcon2_monterey_lane_state_st *istate){
	...
	falcon2_monterey_lane_state_st state;
	uint8_t ladder_range = 0;
	uint16_t reg_data;

	ENULL_MEMSET(&state, 0, sizeof(falcon2_monterey_lane_state_st));

	if (!istate)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);

	EFUN(_falcon2_monterey_pmd_lock_status (pa, &state.rx_lock, &state.rx_lock_chg));
	...
	{  falcon2_monterey_osr_mode_st osr_mode;
	   ENULL_MEMSET(&osr_mode, 0, sizeof(falcon2_monterey_osr_mode_st));
	   EFUN(_falcon2_monterey_get_osr_mode (pa, &osr_mode));
	...
	EFUN(_falcon2_monterey_sigdet_status (pa, &state.sig_det, &state.sig_det_chg));
	ESTM(state.rx_ppm = rd_cdr_integ_reg() / 84);
	EFUN(falcon2_monterey_get_clk90_offset (pa, &state.clk90));
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, DSC_E_RX_PI_CNT_BIN_LD, &reg_data));
...
static err_code_t _falcon2_monterey_display_lane_state_no_newline (const phymod_access_t *pa){
	...
	EFUN(_falcon2_monterey_read_lane_state (pa, &state));

	lane_idx = falcon2_monterey_get_lane (pa );
...
SDK_STATIC err_code_t _falcon2_monterey_read_core_state (const phymod_access_t *pa, falcon2_monterey_core_state_st *istate){
	falcon2_monterey_core_state_st state;
	struct falcon2_monterey_uc_core_config_st core_cfg;

	ENULL_MEMSET(&state, 0, sizeof(falcon2_monterey_core_state_st));
	ENULL_MEMSET(&core_cfg, 0, sizeof(struct falcon2_monterey_uc_core_config_st));
	...
	{  int16_t die_temp = 0;
	   EFUN(falcon2_monterey_read_die_temperature (pa, &die_temp));
	...
	EFUN(_falcon2_monterey_pll_lock_status (pa, &state.pll_lock, &state.pll_lock_chg));
...
static err_code_t _falcon2_monterey_display_core_state_no_newline (const phymod_access_t *pa_phy){
        phymod_access_t *pa;
        phymod_access_t pa_copy;
	falcon2_monterey_core_state_st state;
	falcon2_monterey_core_state_st state_pll1;
	uint8_t pll_orig;
	...
	ENULL_MEMSET(&state, 0, sizeof(state     ));
	{
		...
		EFUN(_falcon2_monterey_read_core_state (pa, &state));
		EFUN(falcon2_monterey_set_pll(pa, 1));
		EFUN(_falcon2_monterey_read_core_state (pa, &state_pll1));
		EFUN(falcon2_monterey_set_pll(pa, pll_orig));
	}
	EFUN_PRINTF((" %02d ",  falcon2_monterey_get_core()));
	...
	EFUN_PRINTF((" (%02d,%02d) ",  state.pll_div,
		     state_pll1.pll_div));
	EFUN(falcon2_monterey_set_pll(pa, 0));
	EFUN(_falcon2_monterey_display_pll_to_divider (pa,   state.pll_div));
	EFUN_PRINTF((","));
	EFUN(falcon2_monterey_set_pll(pa, 1));
	EFUN(_falcon2_monterey_display_pll_to_divider (pa,   state_pll1.pll_div));
...
SDK_STATIC err_code_t _falcon2_monterey_pll_lock_status (const phymod_access_t *pa, uint8_t *pll_lock, uint8_t *pll_lock_chg){
	...
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _falcon2_monterey_pmd_lock_status (const phymod_access_t *pa, uint8_t *pmd_lock, uint8_t *pmd_lock_chg){
	...
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, PMD_LOCK_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _falcon2_monterey_sigdet_status (const phymod_access_t *pa, uint8_t *sig_det, uint8_t *sig_det_chg){
	...
	EFUN(falcon2_monterey_pmd_rdt_reg (pa, SIGDET_STATUS_ADDR, &rddata));
...
static err_code_t _falcon2_monterey_display_ber_scan_data (const phymod_access_t *pa,  uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control){
	...
	uint8_t range250;
	falcon2_monterey_osr_mode_st osr_mode;
	struct falcon2_monterey_uc_core_config_st core_config;

	ENULL_MEMSET(&core_config, 0, sizeof(core_config));
	ENULL_MEMSET(&osr_mode, 0, sizeof(osr_mode));

	EFUN(falcon2_monterey_get_uc_core_config (pa, &core_config));
	EFUN(_falcon2_monterey_get_osr_mode (pa, &osr_mode));
	...
	for (i = 0; i < cnt; i++) {
		...
		EFUN(falcon2_monterey_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_WORD, 0, 100));
		...
		EFUN(falcon2_monterey_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_BYTE, 0, 100));
	...
	EFUN_PRINTF(("\n"));
	EFUN(falcon2_monterey_pmd_uc_cmd (pa, CMD_CAPTURE_BER_END, 0x00, 2000));
