...
#define BCM8727_PMA_PMD_ID0             0x0020
#define BCM8727_PMA_PMD_ID1             0x6036
#define BCM8727_CHIP_ID                 0x8727
...
#define PMA_PMD_CTRL2_TYPE_1G           0xd
...
/* Driver messages */
#define BCM8727_MSG(_pc, _str) \
    CDK_WARN(("bcm8727[%d.%d]: " _str "\n", \
               PHY_CTRL_UNIT(_pc), PHY_CTRL_PORT(_pc)));

/* Low level debugging (off by default) */
#ifdef BCM8727_DEBUG_ENABLE
#define BCM8727_DBG(_pc, _str) BCM8727_MSG(_pc, _str)
#else
#define BCM8727_DBG(_pc, _str)
#endif
 ...
 */
static int
bcm8727_phy_probe(phy_ctrl_t *pc)
{
    ...
    if (phyid0 == BCM8727_PMA_PMD_ID0 && 
        phyid1 == BCM8727_PMA_PMD_ID1) {
        ioerr += PHY_BUS_READ(pc, PMA_PMD_CHIP_ID_REG, &chip_id);
        if (chip_id == BCM8727_CHIP_ID) {
 ...
 */
static int
bcm8727_phy_notify(phy_ctrl_t *pc, phy_event_t event)
{
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm8727_phy_reset(phy_ctrl_t *pc)
{
    uint32_t pma_pmd_ctrl, pcs_ctrl, phy_xs_ctrl;
    int cnt;
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    for (cnt = 0; cnt < PHY_RESET_POLL_MAX; cnt++) {
        ...
        ioerr += PHY_BUS_READ(pc, PHY_XS_CTRL_REG, &phy_xs_ctrl);
        if ((pma_pmd_ctrl & MII_CTRL_RESET) == 0 &&
            (pcs_ctrl & MII_CTRL_RESET) == 0 &&
            (phy_xs_ctrl & MII_CTRL_RESET) == 0) {
            break;
        }
    }
    if (cnt >= PHY_RESET_POLL_MAX) {
        BCM8727_DBG(pc, "reset timeout");
        rv = CDK_E_TIMEOUT;
    }
 ...
 */
static int
bcm8727_phy_init(phy_ctrl_t *pc)
{
    uint32_t lane_stat;
    int sleep_time = 1000;
    int cnt;
    ...
    if (CDK_SUCCESS(rv)) {
        uint32_t gen_reg1, gen_reg2, gen_reg4;

        ioerr += PHY_BUS_WRITE(pc, PMA_PMD_EXT_BOOTROM_REG_2, 0x0001);
        PHY_SYS_USLEEP(sleep_time);
        ioerr += PHY_BUS_WRITE(pc, PMA_PMD_EXT_BOOTROM_REG_2, 0x008c);
        PHY_SYS_USLEEP(sleep_time);
        ...
        ioerr += PHY_BUS_READ(pc, PMA_PMD_GEN_REG_2, &gen_reg2);
        ...
        for (cnt = 0; cnt < PHY_ROM_LOAD_POLL_MAX; cnt++) {
            if (gen_reg2 < 0xd101) {
                ...
                if (ioerr == 0 && gen_reg1 == 0x0aa0) {
                    BCM8727_DBG(pc, "old rom ok");
                    break;
                }
            } else {
                ioerr += PHY_BUS_READ(pc, PMA_PMD_GEN_REG_4, &gen_reg4);
                if (ioerr == 0 && gen_reg4 == 0x600d) {
                    BCM8727_DBG(pc, "new rom ok");
                    break;
                }
            }
            if (ioerr) {
        ...
        if (cnt >= PHY_ROM_LOAD_POLL_MAX) {
            BCM8727_DBG(pc, "rom load timeout");
            rv = CDK_E_TIMEOUT;
        }
    }
#endif
    ...
    if (CDK_SUCCESS(rv)) {
        BCM8727_DBG(pc, "lane sync start");
        /* Make sure 8727 XAUI lanes are synchronized with the SOC XAUI */
        for (cnt = 0; cnt < PHY_LANES_POLL_MAX; cnt++) {
            ioerr += PHY_BUS_READ(pc, PHY_XS_XGXS_LANE_STAT, &lane_stat);
            if ((lane_stat & 0xf) == 0xf) {
                BCM8727_DBG(pc, "lane sync ok");
                break;
            }
            PHY_SYS_USLEEP(sleep_time);
        }
        if (cnt >= PHY_LANES_POLL_MAX) {
            ...
            BCM8727_MSG(pc, "lane sync timeout");
        }
    }
 ...
 */
static int
bcm8727_phy_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
    uint32_t stat;
    int ioerr = 0;
    ...
    *link = 0;
    ...
    if (stat & MII_STAT_LA) {
        ...
        if (stat & MII_STAT_LA) {
            ioerr += PHY_BUS_READ(pc, PHY_XS_STAT_REG, &stat);
            if (stat & MII_STAT_LA) {
                *link = 1;
            }
        }
 ...
 */
static int
bcm8727_phy_duplex_set(phy_ctrl_t *pc, int duplex)
{
 ...
 */
static int
bcm8727_phy_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    *duplex = 1;
 ...
 */
static int
bcm8727_phy_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
 ...
 */
static int
bcm8727_phy_speed_get(phy_ctrl_t *pc, uint32_t *speed)
{
    ...
    *speed = 0;
    ...
    if (CDK_SUCCESS(rv)) {
        if (an) {
            ...
            if (pma_pmd_ctrl2 == PMA_PMD_CTRL2_TYPE_1G) {
                *speed = 1000;
            } else {
                *speed = 10000;
            }
 ...
 */
static int
bcm8727_phy_autoneg_set(phy_ctrl_t *pc, int autoneg)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm8727_phy_autoneg_get(phy_ctrl_t *pc, int *autoneg)
{
    uint32_t ctrl;
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm8727_phy_loopback_set(phy_ctrl_t *pc, int enable)
{
    uint32_t pcs_ctrl;
    int ioerr = 0;
 ...
 */
static int
bcm8727_phy_loopback_get(phy_ctrl_t *pc, int *enable)
{
    uint32_t pcs_ctrl;
    int ioerr = 0;
 ...
 */
static int
bcm8727_phy_ability_get(phy_ctrl_t *pc, uint32_t *abil)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcm8727_phy_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcm8727_phy_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    PHY_CTRL_CHECK(pc);
...
/*
 * Variable:    bcm8727_drv
 * Purpose:     PHY Driver for BCM8727.
 */
phy_driver_t bcm8727_drv = {
    "bcm8727",
    "BCM8727 10-Gigabit PHY Driver",  
    0,
    bcm8727_phy_probe,                  /* pd_probe */
    bcm8727_phy_notify,                 /* pd_notify */
    bcm8727_phy_reset,                  /* pd_reset */
    bcm8727_phy_init,                   /* pd_init */
    bcm8727_phy_link_get,               /* pd_link_get */
    bcm8727_phy_duplex_set,             /* pd_duplex_set */
    bcm8727_phy_duplex_get,             /* pd_duplex_get */
    bcm8727_phy_speed_set,              /* pd_speed_set */
    bcm8727_phy_speed_get,              /* pd_speed_get */
    bcm8727_phy_autoneg_set,            /* pd_autoneg_set */
    bcm8727_phy_autoneg_get,            /* pd_autoneg_get */
    bcm8727_phy_loopback_set,           /* pd_loopback_set */
    bcm8727_phy_loopback_get,           /* pd_loopback_get */
    bcm8727_phy_ability_get,            /* pd_ability_get */
    bcm8727_phy_config_set,             /* pd_config_set */
    bcm8727_phy_config_get,             /* pd_config_get */
    NULL,                               /* pd_status_get */
    NULL                                /* pd_cable_diag */
};
