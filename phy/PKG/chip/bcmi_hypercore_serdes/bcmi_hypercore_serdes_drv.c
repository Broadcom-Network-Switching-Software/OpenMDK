...
#define XGXSBLK3_LOCALCONTROL0_REG      XGS_MIIM_REG(0x8111)
#define XGXSBLK3_RX_INBANDMDIO_RST_MASK 0x0008
...
#define DIGITAL6_MP5_NPCTRL_REG         XGS_MIIM_REG(0x8350)
#define DIGITAL6_UD_FIELD_REG           XGS_MIIM_REG(0x8357)
...
/* Vco frequency values for 10.5G/12.773G, 1G/2.5G and 10/100/1000 */
#define HC_VCO_FREQ_HD127               0x7800
#define HC_VCO_FREQ_HD25                0x7700
#define HC_VCO_FREQ_SGMII               0x7500

#define DTE_IEEESTATUS1                 XGS_MIIM_REG(0x28000001)
...
bcmi_hypercore_serdes_init(phy_ctrl_t *pc)
{
     ...
     */
    if (lane_num == 0) {
        /* keep mode_10g setting before reset */
        ioerr += _PHY_REG_READ(pc, XGXS_BLK0_XGXS_CTRL_REG, &data16);
        xgxs_ctrl = (data16 & XGXS_CTRL_MODE_10G_MASK) >> XGXS_CTRL_MODE_10G_SHIFT;
        ...
        if (PHY_CTRL_FLAGS(pc) & PHY_F_R2_MODE) { 
            ...
            vco_freq = HC_VCO_FREQ_SGMII;
            if (xgxs_ctrl == MODE_10G_IND_LN_OS5) {
    ...
    /* Disable in-band MDIO */
    mask16 = XGXSBLK3_RX_INBANDMDIO_RST_MASK;
    data16 = mask16;
    ioerr += _PHY_REG_MODIFY(pc, XGXSBLK3_LOCALCONTROL0_REG, data16, mask16);
    ...
    if (PHY_CTRL_FLAGS(pc) & PHY_F_PASSTHRU) {
        ...
        data16 = mask16;
        ioerr += _PHY_REG_MODIFY(pc, DIGITAL_MISC3_REG, data16, mask16);
    }
    ...
    ioerr += _PHY_REG_WRITE(pc, DIGITAL6_MP5_NPCTRL_REG, 0);
    ioerr += _PHY_REG_WRITE(pc, DIGITAL6_UD_FIELD_REG, 0);
    
    data16 = D1000X_CTRL2_DISABLE_FALSE_LINK |
             D1000X_CTRL2_FILTER_FORCE_LINK;
    mask16 = data16 | D1000X_CTRL2_PAR_DET_EN;   
    if (!(PHY_CTRL_FLAGS(pc) & PHY_F_PASSTHRU)) {
        data16 |= D1000X_CTRL2_PAR_DET_EN;
    }
   ...
   /* Configuring operating mode */
    ...
    if (!(PHY_CTRL_FLAGS(pc) & PHY_F_PASSTHRU)) {
        data16 |= D1000X_CTRL1_FIBER_MODE;
    }
...
bcmi_hypercore_serdes_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
    ...
    if (*link == 0) {
        ioerr += _PHY_REG_READ(pc, DTE_IEEESTATUS1, &mii_stat);
        *link = (mii_stat &  MII_STAT_LA) ? 1 : 0;
    }
    if (autoneg_done) {
...
bcmi_hypercore_serdes_duplex_set(phy_ctrl_t *pc, int duplex)
{
    ...
    uint32_t data16, mask16;
    
    ioerr += _PHY_REG_READ(pc, DIGITAL_1000X_STATUS1_REG, &sgmii_status);
    sgmii_status &= SERDESDIGITAL_STATUS1000X1_SGMII_MODE_MASK;
    if (!sgmii_status) {
        data16 = duplex ? FX100_CONTROL1_FULL_DUPLEX_MASK : 0;
        mask16 = FX100_CONTROL1_FULL_DUPLEX_MASK;
        ioerr += _PHY_REG_MODIFY(pc, FX100_CONTROL1_REG, data16, mask16);

        /* always be full duplex */
        duplex = 1;
    }

    data16 = duplex ? MII_CTRL_FD : 0;
    mask16 = MII_CTRL_FD;
    ioerr += _PHY_REG_MODIFY(pc, COMBO_IEEE0_MII_CTRL, data16, mask16);
...
bcmi_hypercore_serdes_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    ...
    *duplex = 1;
    ...
    if (sgmii_status) {
        ...
        ioerr += _PHY_REG_READ(pc, COMBO_IEEE0_MII_CTRL, &data16);
        *duplex = (data16 & MII_CTRL_FD) ? 1 : 0;
    }
