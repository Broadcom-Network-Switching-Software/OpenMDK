...
#ifndef TSC_CRC_DISABLE
#define TSC_CRC_DISABLE                 0
#endif
...
#ifdef BCM_TSC_DEBUG_ENABLE
#define BCM_TSC_DBG(_p) \
    CDK_WARN(_p)
#else
#define BCM_TSC_DBG(_p)
#endif
...
/*
 * Function:
 *      bcmi_tsc_xgxs_firmware_set
 * Purpose:
 *      Download firmware to TSC
 * Parameters:
                                 ...
                                 uint32_t fw_size, uint8_t *fw_data)
{
    ...
    if (PHY_CTRL_FW_HELPER(pc)) {
        /* Allow external access to TSC uC memory */
        ...
        /* Disable external access to TSC uC memory */
        ioerr += READ_COMMAND3r(pc, &cmd3);
