 ...
 *
 * PHY driver for internal Warpcore3 40G XGXS PHY.
 */
...
#define SERDES_ID0_XGXS_WARPCORE3       0x10
 ...
 */
static int
_warpcore3_serdes_lane(phy_ctrl_t *pc)
{
 ...
 */
static int
_warpcore3_primary_lane(phy_ctrl_t *pc)
{
 ...
 */
static int
_warpcore3_pll_lock_wait(phy_ctrl_t *pc)
{
    int ioerr = 0;
 ...
 */
static int
_warpcore3_rx_div_clk_set(phy_ctrl_t *pc)
{
    int ioerr = 0;
...
/*
 * Function:
 *      _warpcore3_clk_comp_adjust
 * Purpose:
 *      Adjust clock compensation for MAC loopback mode.
 * Parameters:
 *      pc - PHY control structure
 * Returns:
 *      CDK_E_xxx
 */
static int
_warpcore3_clk_comp_adjust(phy_ctrl_t *pc)
{
    int ioerr = 0;
    ...
    if (PHY_CTRL_FLAGS(pc) & PHY_F_MAC_LOOPBACK) {
 ...
 */
static int
_warpcore3_serdes_stop(phy_ctrl_t *pc)
{
    int ioerr = 0;
    ...
    if ((PHY_CTRL_FLAGS(pc) & f_any) ||
        ...
        lane = _warpcore3_serdes_lane(pc);
 ...
 */
static int
_warpcore3_init_stage_0(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv;
    ...
    if (!IS_4LANE_PORT(pc)) {
        ...
        if (IS_2LANE_PORT(pc)) {
            ...
            MISC1r_FORCE_PLL_MODE_AFEf_SET(misc1, 0);
    ...
    if (IS_1LANE_PORT(pc)) {
        /*
         * Set mdio override control to send out 312.5MHz clock on
         * txck_out[0].  This allows lane 1-3 to support 10G speed
         * while lane 0 runs at 1G.
         */
    ...
    /* Increase PLL clock buffer for stable locking */
    ioerr += READ_ANAPLLACONTROL1r(pc, &pll_ctrl);
    ANAPLLACONTROL1r_ICLKIBUF4f_SET(pll_ctrl, 1);
    ioerr += WRITE_ANAPLLACONTROL1r(pc, pll_ctrl);

    /* Select the analog Rx div/16 clock for digital lanes */
    ioerr += _warpcore3_rx_div_clk_set(pc);

    /* Load firmware and start uController */
    fw_data = wc3_ucode_bin;
    fw_size = wc3_ucode_bin_len;
    rv = bcmi_warpcore3_xgxs_firmware_set(pc, 0, fw_size, fw_data);
    if (CDK_FAILURE(rv)) {
 ...
 */
static int
_warpcore3_init_stage_1(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    ...
    /* Check firmware CRC */
    if (_warpcore3_primary_lane(pc)) {
        rv = bcmi_warpcore3_xgxs_firmware_check(pc);
    }
 ...
 */
static int
_warpcore3_init_stage_2(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    MISC1r_t misc1;
    ...
    (void)_warpcore3_pll_lock_wait(pc);
 ...
 */
static int
_warpcore3_init_stage(phy_ctrl_t *pc, int stage)
{
    switch (stage) {
    case 0:
        return _warpcore3_init_stage_0(pc);
    case 1:
        return _warpcore3_init_stage_1(pc);
    case 2:
        return _warpcore3_init_stage_2(pc);
    default:
        break;
    }
...
#define SET_SYMBOL_TABLE(_pc) \
    PHY_CTRL_SYMBOLS(_pc) = &bcmi_warpcore_xgxs_symbols
#else
#define SET_SYMBOL_TABLE(_pc)
#endif
 ...
 */
static int
bcmi_warpcore3_xgxs_probe(phy_ctrl_t *pc)
{
    ...
    if (phyid0 == BCM_SERDES_PHY_ID0 && phyid1 == BCM_SERDES_PHY_ID1) {
        ...
        if (model == SERDES_ID0_XGXS_WARPCORE3) {
 ...
 */
static int
bcmi_warpcore3_xgxs_notify(phy_ctrl_t *pc, phy_event_t event)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    CONTROL1000X1r_t ctrl1;
    ...
    rv = _warpcore3_serdes_stop(pc);

    /* Adjust clock compensation */
    _warpcore3_clk_comp_adjust(pc);
 ...
 */
static int
bcmi_warpcore3_xgxs_reset(phy_ctrl_t *pc)
{
    return CDK_E_NONE;
}
 ...
 */
static int
bcmi_warpcore3_xgxs_init(phy_ctrl_t *pc)
{
    int rv = CDK_E_NONE;
    int stage;
    ...
    for (stage = 0; CDK_SUCCESS(rv); stage++) {
        rv = _warpcore3_init_stage(pc, stage);
    }
 ...
 */
static int
bcmi_warpcore3_xgxs_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
...
/*
 * Function:    
 *      bcmi_warpcore3_xgxs_duplex_set
 * Purpose:     
 ...
 */
static int
bcmi_warpcore3_xgxs_duplex_set(phy_ctrl_t *pc, int duplex)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
...
/*
 * Function:    
 *      bcmi_warpcore3_xgxs_duplex_get
 * Purpose:     
 ...
 */
static int
bcmi_warpcore3_xgxs_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    *duplex = 1;
 ...
 */
static int
bcmi_warpcore3_xgxs_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    UNICOREMODE10Gr_t unicore_mode;
    ...
    if (IS_4LANE_PORT(pc)) {
        ...
        ioerr += READ_XGXSCONTROLr(pc, &xgxs_ctrl);
        XGXSCONTROLr_TXCKO_DIVf_SET(xgxs_ctrl, txcko_div);
        ioerr += WRITE_XGXSCONTROLr(pc, xgxs_ctrl);
 
        /* Set VCO (required for certain speeds) */
        ioerr += READ_MISC1r(pc, &misc1);
        MISC1r_FORCE_PLL_MODE_AFEf_SET(misc1, pll_mode);
        MISC1r_FORCE_PLL_MODE_AFE_SELf_SET(misc1, pll_mode ? 1 : 0);
        ioerr += WRITE_MISC1r(pc, misc1);
    } else {
...
static int
bcmi_warpcore3_xgxs_speed_get(phy_ctrl_t *pc, uint32_t *speed)
{
    int ioerr = 0;
    ...
    *speed = 0;
    ...
    if (*speed == 40000) {
        ioerr += READ_MISC1r(pc, &misc1);
        switch (MISC1r_FORCE_PLL_MODE_AFEf_GET(misc1)) {
        case FV_div70:
            *speed= 42000;
            break;
        default:
...
/*
 * Function:    
 *      bcmi_warpcore3_xgxs_autoneg_set
 * Purpose:     
...
static int
bcmi_warpcore3_xgxs_autoneg_set(phy_ctrl_t *pc, int autoneg)
{
    int ioerr = 0;
    int lane_num;
    int cl73_an;
...
/*
 * Function:    
 *      bcmi_warpcore3_xgxs_autoneg_get
 * Purpose:     
 ...
 */
static int
bcmi_warpcore3_xgxs_autoneg_get(phy_ctrl_t *pc, int *autoneg)
{
    int ioerr = 0;
    COMBO_MIICNTLr_t mii_ctrl;
...
/*
 * Function:    
 *      bcmi_warpcore3_xgxs_loopback_set
 * Purpose:     
 ...
 */
static int
bcmi_warpcore3_xgxs_loopback_set(phy_ctrl_t *pc, int enable)
{
    int ioerr = 0;
...
/*
 * Function:    
 *      bcmi_warpcore3_xgxs_loopback_get
 * Purpose:     
 ...
 */
static int
bcmi_warpcore3_xgxs_loopback_get(phy_ctrl_t *pc, int *enable)
{
    int ioerr = 0;
    LANECTRL2r_t lanectrl2;
    int lane_num;
...
/*
 * Function:    
 *      bcmi_warpcore3_xgxs_ability_get
 * Purpose:     
 ...
 */
static int
bcmi_warpcore3_xgxs_ability_get(phy_ctrl_t *pc, uint32_t *abil)
{
    PHY_CTRL_CHECK(pc);
...
/*
 * Function:
 *      bcmi_warpcore3_xgxs_config_set
 * Purpose:
 ...
 */
static int
bcmi_warpcore3_xgxs_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    PHY_CTRL_CHECK(pc);
    ...
    case PhyConfig_InitStage: {
        return _warpcore3_init_stage(pc, val);
    }
    default:
        break;
    }
...
/*
 * Function:
 *      bcmi_warpcore3_xgxs_config_get
 * Purpose:
 ...
 */
static int
bcmi_warpcore3_xgxs_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    PHY_CTRL_CHECK(pc);
...
/*
 * Function:
 *      bcmi_warpcore3_xgxs_status_get
 * Purpose:
 ...
 */
static int
bcmi_warpcore3_xgxs_status_get(phy_ctrl_t *pc, phy_status_t stat, uint32_t *val)
{
    PHY_CTRL_CHECK(pc);
...
phy_driver_t bcmi_warpcore3_xgxs_drv = {
    "bcmi_warpcore3_xgxs", 
    "Internal Warpcore3 40G XGXS PHY Driver",  
    PHY_DRIVER_F_INTERNAL,
    bcmi_warpcore3_xgxs_probe,          /* pd_probe */
    bcmi_warpcore3_xgxs_notify,         /* pd_notify */
    bcmi_warpcore3_xgxs_reset,          /* pd_reset */
    bcmi_warpcore3_xgxs_init,           /* pd_init */
    bcmi_warpcore3_xgxs_link_get,       /* pd_link_get */
    bcmi_warpcore3_xgxs_duplex_set,     /* pd_duplex_set */
    bcmi_warpcore3_xgxs_duplex_get,     /* pd_duplex_get */
    bcmi_warpcore3_xgxs_speed_set,      /* pd_speed_set */
    bcmi_warpcore3_xgxs_speed_get,      /* pd_speed_get */
    bcmi_warpcore3_xgxs_autoneg_set,    /* pd_autoneg_set */
    bcmi_warpcore3_xgxs_autoneg_get,    /* pd_autoneg_get */
    bcmi_warpcore3_xgxs_loopback_set,   /* pd_loopback_set */
    bcmi_warpcore3_xgxs_loopback_get,   /* pd_loopback_get */
    bcmi_warpcore3_xgxs_ability_get,    /* pd_ability_get */
    bcmi_warpcore3_xgxs_config_set,     /* pd_config_set */
    bcmi_warpcore3_xgxs_config_get,     /* pd_config_get */
    bcmi_warpcore3_xgxs_status_get,     /* pd_status_get */
    NULL                                /* pd_cable_diag */
};
