 ...
 *
 * PHY driver for BCM53314.
 */

#include <phy/phy.h>
#include <phy/ge_phy.h>

#define BCM53314_PHY_ID0                0x0143
#define BCM53314_PHY_ID1                0xbd20

#define PHY_ID1_REV_MASK                0x000f

/* Default LED control */
#define BCM53314_LED1_SEL(_pc)          0x0
#define BCM53314_LED2_SEL(_pc)          0x1
#define BCM53314_LED3_SEL(_pc)          0x3
#define BCM53314_LED4_SEL(_pc)          0x6
#define BCM53314_LEDCTRL(_pc)           0x8
#define BCM53314_LEDSELECT(_pc)         0x0
...
extern cdk_symbols_t bcm53314_symbols;
#endif
 ...
 */
static int
bcm53314_phy_probe(phy_ctrl_t *pc)
{
    ...
    if (phyid0 == BCM53314_PHY_ID0 && 
        (phyid1 & ~PHY_ID1_REV_MASK) == BCM53314_PHY_ID1) {

#if PHY_CONFIG_INCLUDE_CHIP_SYMBOLS == 1
        PHY_CTRL_SYMBOLS(pc) = &bcm53314_symbols;
#endif
 ...
 */
static int
bcm53314_phy_notify(phy_ctrl_t *pc, phy_event_t event)
{
    return CDK_E_NONE;
}
 ...
 */
static int
bcm53314_phy_reset(phy_ctrl_t *pc)
{
 ...
 */
static int
bcm53314_phy_init(phy_ctrl_t *pc)
{
    ...
    ioerr += PHY_BUS_WRITE(pc, 0x1c,
                           REG_1C_WR(0x0d, BCM53314_LED1_SEL(pc) |
                                     (BCM53314_LED2_SEL(pc) << 4)));
    ioerr += PHY_BUS_WRITE(pc, 0x1c,
                           REG_1C_WR(0x0e, BCM53314_LED3_SEL(pc) |
                                     (BCM53314_LED4_SEL(pc) << 4)));
    ioerr += PHY_BUS_WRITE(pc, 0x1c,
                           REG_1C_WR(0x09, BCM53314_LEDCTRL(pc)));
    ioerr += PHY_BUS_WRITE(pc, MII_EXP_SEL, MII_EXP_MAP_REG(0x4));
    ioerr += PHY_BUS_WRITE(pc, MII_EXP_REG, BCM53314_LEDSELECT(pc));
    ioerr += PHY_BUS_WRITE(pc, MII_EXP_SEL, MII_EXP_UNMAP);
    /* If using LED link/activity mode, disable LED traffic mode */
    if ((BCM53314_LEDCTRL(pc) & 0x10) || BCM53314_LEDSELECT(pc) == 0x01) {
        ioerr += PHY_BUS_READ(pc, MII_ECR_REG, &tmp);
        tmp &= ~0x0020;
        ioerr += PHY_BUS_WRITE(pc, MII_ECR_REG, tmp);
    }
 ...
 */
static int
bcm53314_phy_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
 ...
 */
static int
bcm53314_phy_duplex_set(phy_ctrl_t *pc, int duplex)
{
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm53314_phy_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcm53314_phy_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm53314_phy_speed_get(phy_ctrl_t *pc, uint32_t *speed)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcm53314_phy_autoneg_set(phy_ctrl_t *pc, int autoneg)
{
 ...
 */
static int
bcm53314_phy_autoneg_get(phy_ctrl_t *pc, int *autoneg)
{
 ...
 */
static int
bcm53314_phy_loopback_set(phy_ctrl_t *pc, int enable)
{
    uint32_t test;
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm53314_phy_loopback_get(phy_ctrl_t *pc, int *enable)
{
 ...
 */
static int
bcm53314_phy_ability_get(phy_ctrl_t *pc, uint32_t *abil)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcm53314_phy_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    int ioerr = 0;
    uint32_t next_abil;
 ...
 */
static int
bcm53314_phy_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    int ioerr = 0;
    uint32_t misc_test;
...
/*
 * Variable:    bcm53314_phy drv
 * Purpose:     PHY Driver for BCM53314.
 */
phy_driver_t bcm53314_drv = {
    "bcm53314",
    "BCM53314 Gigabit PHY Driver",  
    PHY_DRIVER_F_INTERNAL, 
    bcm53314_phy_probe,                 /* pd_probe */
    bcm53314_phy_notify,                /* pd_notify */
    bcm53314_phy_reset,                 /* pd_reset */
    bcm53314_phy_init,                  /* pd_init */
    bcm53314_phy_link_get,              /* pd_link_get */
    bcm53314_phy_duplex_set,            /* pd_duplex_set */
    bcm53314_phy_duplex_get,            /* pd_duplex_get */
    bcm53314_phy_speed_set,             /* pd_speed_set */
    bcm53314_phy_speed_get,             /* pd_speed_get */
    bcm53314_phy_autoneg_set,           /* pd_autoneg_set */
    bcm53314_phy_autoneg_get,           /* pd_autoneg_get */
    bcm53314_phy_loopback_set,          /* pd_loopback_set */
    bcm53314_phy_loopback_get,          /* pd_loopback_get */
    bcm53314_phy_ability_get,           /* pd_ability_get */
    bcm53314_phy_config_set,            /* pd_config_set */
    bcm53314_phy_config_get,            /* pd_config_get */
    NULL,                               /* pd_status_get */
    NULL                                /* pd_cable_diag */
};
