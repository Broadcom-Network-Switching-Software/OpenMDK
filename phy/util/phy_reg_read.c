 ...
 * and performs a register read accoding to the access method
...
phy_reg_read(phy_ctrl_t *pc, uint32_t addr, uint32_t *data)
{
    ...
    case PHY_REG_ACC_BRCM_1000X:
        return phy_brcm_1000x_read(pc, addr, data);
    case PHY_REG_ACC_XGS_IBLK:
        return phy_xgs_iblk_read(pc, addr, data);
    case PHY_REG_ACC_XAUI_IBLK:
        return phy_xaui_iblk_read(pc, addr, data);
    case PHY_REG_ACC_AER_IBLK:
        return phy_aer_iblk_read(pc, addr, data);
