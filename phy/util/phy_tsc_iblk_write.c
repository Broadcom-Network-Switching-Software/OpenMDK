...
phy_tsc_iblk_proxy_write(phy_ctrl_t *pc, uint32_t addr, uint32_t data)
{
    ...
    /* Write the data */
    ioerr += phy_tsc_iblk_mdio_write(pc, WRDATAr, data);

    /* Write command to mailbox */
    cmd = TSC_UC_SYNC_CMD_REQ | TSC_UC_SYNC_CMD_WR;
    if (reg_copies == 3) {
    ...
    if (done == 0) {
        PHY_VERB(pc, ("TSC proxy write did not complete\n"));
        return CDK_E_NONE;
    }
...
phy_tsc_iblk_write(phy_ctrl_t *pc, uint32_t addr, uint32_t data)
{
    if (PHY_CTRL_ADDR_TYPE(pc) && PHY_CTRL_ADDR_TYPE(pc)(pc, addr) != 0) {
        return phy_tsc_iblk_proxy_write(pc, addr, data);
    }
    return phy_tsc_iblk_mdio_write(pc, addr, data);
}
