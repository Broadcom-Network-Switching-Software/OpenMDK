...
int 
bcm53400_a0_bmd_port_mode_update(int unit, int port)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    int led_flags = 0;
    ...
    uint32_t flags;
    cdk_pbmp_t gxport_pbmp, xlport_pbmp;
    ...
    bcm53400_a0_xport_pbmp_get(unit, XPORT_FLAG_GXPORT, &gxport_pbmp);
    bcm53400_a0_xport_pbmp_get(unit, XPORT_FLAG_XLPORT, &xlport_pbmp);

    rv = bmd_link_update(unit, port, &status_change);
    if (CDK_SUCCESS(rv) && status_change) {
        /* Update xlport */
        if (CDK_PBMP_MEMBER(xlport_pbmp, port)) {
            rv = bcm53400_a0_bmd_port_mode_get(unit, port, &mode, &flags);
            if (CDK_SUCCESS(rv)) {
                rv = bmd_phy_autoneg_get(unit, port, &autoneg);
                if (CDK_SUCCESS(rv) && autoneg) {
                    if (mode == bmdPortMode10000KR || mode == bmdPortMode10000KX) {
                        mode = bmdPortMode10000fd;
                    } else if (mode == bmdPortMode1000KX) {
                        mode = bmdPortMode1000fd;
                    }

                    flags |= BMD_PORT_MODE_F_INTERNAL;
                    rv = bcm53400_a0_bmd_port_mode_set(unit, port, mode, flags);
                }
            }
        ...
        /* Update gport */
        if (CDK_PBMP_MEMBER(gxport_pbmp, port)) {
            rv = bcm53400_a0_bmd_port_mode_get(unit, port, &mode, &flags);
            if (CDK_SUCCESS(rv)) {
                flags |= BMD_PORT_MODE_F_INTERNAL;
                rv = bcm53400_a0_bmd_port_mode_set(unit, port, mode, flags);
