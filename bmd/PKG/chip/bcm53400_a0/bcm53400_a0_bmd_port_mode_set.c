...
int 
bcm53400_a0_bmd_port_mode_set(int unit, int port, 
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        } else if (CDK_PBMP_MEMBER(gxport_pbmp, port)) {
        ...
        if (cnt < 0) {
            CDK_WARN(("bcm53400_a0_bmd_port_mode_set[%d]: "
        ...
        if (CDK_PBMP_MEMBER(xlport_pbmp, port)) {
             ...
             */
            rv = bcm53400_a0_bmd_port_mode_get(unit, port, 
                                               &cur_mode, &cur_flags);
            if ((CDK_SUCCESS(rv) && 
                ...
                rv = bcm53400_a0_xlport_reset(unit, port);
