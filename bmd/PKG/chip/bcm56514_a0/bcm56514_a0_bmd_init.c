...
int
bcm56514_a0_xport_init(int unit, int port)
{
    int ioerr = 0;
...
bcm56514_a0_bmd_init(int unit)
{
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56514_a0_bmd_init[%d]: IPIPE reset timeout\n", unit));
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56514_a0_bmd_init[%d]: EPIPE reset timeout\n", unit));
        return ioerr ? CDK_E_IO : CDK_E_TIMEOUT;
    }
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ioerr += bcm56514_a0_xport_init(unit, port);
        if (CDK_SUCCESS(rv)) {
            rv = bmd_phy_attach(unit, port);
        }
    }
