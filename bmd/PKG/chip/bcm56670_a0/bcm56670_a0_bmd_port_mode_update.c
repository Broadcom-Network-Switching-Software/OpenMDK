...
int
bcm56670_a0_bmd_port_mode_update(int unit, int port)
{
    ...
    if (CDK_SUCCESS(rv) && status_change) {
        rv = bcm56670_a0_bmd_port_mode_get(unit, port, &mode, &flags);
        if (CDK_SUCCESS(rv)) {
            flags |= BMD_PORT_MODE_F_INTERNAL;
            rv = bcm56670_a0_bmd_port_mode_set(unit, port, mode, flags);
        }
