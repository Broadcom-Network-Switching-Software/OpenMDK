...
/**
@name: tdm_mn_filter_dither
@param:
 */
int
tdm_mn_filter_dither(int *tdm_tbl, int lr_idx_limit, int accessories, int **tsc, int threshold, enum port_speed_e *speed)
{
	int g, i, j, k=1, l=MN_NUM_EXT_PORTS, slice_idx, shift_cnt=0;
	...
	for (i=0; i<(lr_idx_limit+accessories); i++) {
		if (tdm_tbl[i]==MN_OVSB_TOKEN && tdm_mn_scan_slice_size_local(i,tdm_tbl,(lr_idx_limit+accessories), &slice_idx)==tdm_mn_slice_size(MN_OVSB_TOKEN,tdm_tbl,(lr_idx_limit+accessories))) {
			g=i; while( (tdm_tbl[g]==MN_OVSB_TOKEN) && (g<(lr_idx_limit+accessories)) ) {g++;}
			if ( tdm_mn_slice_prox_up(g,tdm_tbl,tsc,speed) &&
				 tdm_mn_slice_prox_dn((tdm_mn_slice_idx(MN_OVSB_TOKEN,tdm_tbl,(lr_idx_limit+accessories))-1),tdm_tbl,(lr_idx_limit+accessories),tsc,speed) ) {
				l=tdm_tbl[i];
				for (j=i; j<255; j++) {
	...
	if (dither_shift_done) {
		for (i=1; i<(lr_idx_limit+accessories); i++) {
			...
			while (tdm_tbl[i]!=MN_OVSB_TOKEN && tdm_mn_scan_slice_size_local(i,tdm_tbl,(lr_idx_limit+accessories),&slice_idx)==tdm_mn_slice_size(1,tdm_tbl,(lr_idx_limit+accessories)) && i<(lr_idx_limit+accessories) ) {
				if (++dither_slice_counter>=threshold && tdm_tbl[i-1]==MN_OVSB_TOKEN) {
					for (j=255; j>i; j--) {
						tdm_tbl[j]=tdm_tbl[j-1];
					}
					tdm_tbl[i+1]=MN_OVSB_TOKEN;
					dither_done=BOOL_TRUE;
					break;
				}
				i++; if (tdm_tbl[i]==MN_OVSB_TOKEN) {i++;}
			}
			if (dither_done) {
				break;
			}
...
/**
@name: tdm_mn_filter_fine_dither
@param:
 */
int
tdm_mn_filter_fine_dither(int port, int *tdm_tbl, int lr_idx_limit, int accessories, int **tsc)
{
	...
	for (j=2; j<(lr_idx_limit+accessories-4); j++) {
		if ( tdm_tbl[j]!=MN_OVSB_TOKEN && tdm_tbl[j-2]!=MN_OVSB_TOKEN && tdm_tbl[j+2]!=MN_OVSB_TOKEN && tdm_tbl[j+4]!=MN_OVSB_TOKEN && 
			 tdm_mn_scan_slice_size_local((j-2), tdm_tbl, (lr_idx_limit+accessories), &slice_idx)==1 && 
			 tdm_mn_scan_slice_size_local( j,    tdm_tbl, (lr_idx_limit+accessories), &slice_idx)==1 && 
			 tdm_mn_scan_slice_size_local((j+2), tdm_tbl, (lr_idx_limit+accessories), &slice_idx)==1 && 
			 tdm_mn_scan_slice_size_local((j+4), tdm_tbl, (lr_idx_limit+accessories), &slice_idx)==1 &&
			 tdm_mn_slice_prox_local( j,    tdm_tbl, (lr_idx_limit+accessories), tsc)>min_prox )
		{
...
/**
@name: tdm_mn_filter_shift_lr_port
@param:
 ...
 */
int
tdm_mn_filter_shift_lr_port(unsigned short port, int *tdm_tbl, int tdm_tbl_len, int dir)
{
...
/**
@name: tdm_mn_filter_migrate_os_slot
@param:

Migrate OVSB slot from src to dst in an array.
 */
int
tdm_mn_filter_migrate_os_slot(int idx_src, int idx_dst, int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	...
	if ( tdm_tbl[idx_src]!=MN_OVSB_TOKEN && tdm_tbl[idx_src]!=MN_ANCL_TOKEN) {
		check_pass = BOOL_FALSE;
	}
	...
	if (check_pass==BOOL_TRUE){
		...
		for (j=1; j<VBS_MIN_SPACING; j++){
			...
			MN_TOKEN_CHECK(tdm_tbl[idx0]){
				MN_TOKEN_CHECK(tdm_tbl[idx1]){
...
/**
@name: tdm_mn_filter_smooth_idle_slice
@param:

Smooth MMU TDM calendar by migrating IDLE slots
 */
int
tdm_mn_filter_smooth_idle_slice(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	...
	idle1_token = MN_IDL1_TOKEN;
	idle2_token = MN_IDL2_TOKEN;
	...
	if(idle_slot_cnt>0){
		...
		for (k=0; k<idle_slot_cnt; k++){
			...
			idle_slice_max = tdm_mn_scan_slice_max(idle1_token, tdm_tbl,tdm_tbl_len, &idle_slice_max_idx,0);
			lr_slice_max   = tdm_mn_scan_mix_slice_max(1,tdm_tbl,tdm_tbl_len, &lr_slice_max_idx,pos);
			...
			if ( (idle_slice_max>1 && lr_slice_max>0) &&
				...
				tsc_x = tdm_mn_legacy_which_tsc(tdm_tbl[idx_x],tsc);
				tsc_y = tdm_mn_legacy_which_tsc(tdm_tbl[idx_y],tsc);
				tsc1  = tdm_mn_legacy_which_tsc(tdm_tbl[idx1],tsc);
				tsc2  = tdm_mn_legacy_which_tsc(tdm_tbl[idx2],tsc);
...
/**
@name: tdm_mn_filter_smooth_os_slice
@param:
 ...
 */
int
tdm_mn_filter_smooth_os_slice(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed, int dir)
{
	...
	for (i=1; i<tdm_tbl_len; i++) {
		...
		lr_clump_max = tdm_mn_scan_mix_slice_max(1,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
		lr_clump_min = tdm_mn_scan_mix_slice_min(1,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
		if ( (lr_clump_max<=1) || 
		     (lr_clump_max==2 && lr_clump_min==1)){
			break;
		}
		if ( FAIL==tdm_mn_check_shift_cond_pattern(filter_port, tdm_tbl, tdm_tbl_len, tsc, dir) ) {
			continue;
		}
		else if ( FAIL==tdm_mn_check_shift_cond_local_slice(filter_port, tdm_tbl, tdm_tbl_len, tsc, dir) ) {
			continue;
		}
		else {
			tdm_mn_filter_shift_lr_port(filter_port, tdm_tbl, tdm_tbl_len, shift_dir);
...
/**
@name: tdm_mn_filter_smooth_os_slice_fine
@param:

Smooth MMU TDM calendar by migrating OVSB slot
 */
int
tdm_mn_filter_smooth_os_slice_fine(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	...
	for (k=0; k<tdm_tbl_len; k++){
		...
		slice_max= tdm_mn_scan_slice_max(ovsb_token,tdm_tbl,tdm_tbl_len, &slice_max_idx,k);
		slice_min= tdm_mn_scan_slice_min(ovsb_token,tdm_tbl,tdm_tbl_len, &slice_min_idx,pos);
...
/**
@name: tdm_mn_filter_smooth_os_os_up
@param:

Smooth MMU TDM calendar by transforming:
        --- _x_y_ovsb_ovsb_ -> _x_ovsb_y_ovsb_
 */
int tdm_mn_filter_smooth_os_os_up(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	int i, slot_tmp, filter_cnt=0;
	
	for (i=2; i<(tdm_tbl_len-1); i++) {
		if (tdm_tbl[i  ]==MN_OVSB_TOKEN &&
			tdm_tbl[i+1]==MN_OVSB_TOKEN ){
			MN_TOKEN_CHECK(tdm_tbl[i-1]){
				MN_TOKEN_CHECK(tdm_tbl[i-2]){
					if ( PASS==tdm_mn_check_slot_swap_cond((i-1), tdm_tbl, tdm_tbl_len, tsc, speed) ){
...
/**
@name: tdm_mn_filter_smooth_os_os_dn
@param:

Smooth MMU TDM calendar by transforming:
		--- _ovsb_ovsb_x_y_ -> _ovsb_x_ovsb_y_
 */
int tdm_mn_filter_smooth_os_os_dn(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	int i, slot_tmp, filter_cnt=0;
...
/**
@name: tdm_mn_filter_smooth_ancl
@param:

Smooth MMU TDM calendar with ANCILLARY slots
 */
int
tdm_mn_filter_smooth_ancl(int ancl_token, int *tdm_tbl, int tdm_tbl_len, int ancl_space_min)
{
	int i, j, g, l, pool, ancl_num=0, ancl_dist_up, ancl_dist_dn, lr_slice_size, lr_slice_min, slice_idx, timeout;
	
	lr_slice_min = tdm_mn_scan_slice_min(1, tdm_tbl, tdm_tbl_len, &slice_idx,0);
	...
	while( (pool>0) && ((--timeout)>0) ){
		for (i=1; i<tdm_tbl_len; i++) {
			...
			MN_TOKEN_CHECK(tdm_tbl[i]) {
				lr_slice_size = tdm_mn_scan_slice_size_local(i,tdm_tbl,tdm_tbl_len,&slice_idx);
				ancl_dist_up  = tdm_mn_check_same_port_dist_up_port(ancl_token,i,tdm_tbl,tdm_tbl_len);
				ancl_dist_dn  = tdm_mn_check_same_port_dist_dn_port(ancl_token,i,tdm_tbl,tdm_tbl_len);
...
/**
@name: tdm_mn_filter_chain
@param:

Smooth MMU main calendar in terms of linerate and/or oversub distribution
 */
int
tdm_mn_filter_chain( tdm_mod_t *_tdm )
{
	int i, k, timeout=TIMEOUT, filter_result, filter_cnt, one_slot_cnt=0, 
...
if (_tdm->_core_data.vars_pkg.lr_enable && _tdm->_core_data.vars_pkg.os_enable && (tdm_clk_freq==793 || tdm_clk_freq ==794)) {
		TDM_PRINT0("Filter applied: smoothing ovs slots in FB Configs\n");

	for (i=0; i<tdm_pipe_main_len; i++){
		 MN_TOKEN_CHECK(tdm_pipe_main[i]) one_slot_cnt++; 
  }

	if(one_slot_cnt>0){

		for (i=0; i<one_slot_cnt/2; i+=2){
		  for (k=200; k>i; k--) {
		  ...
		  tdm_pipe_main[i]=MN_OVSB_TOKEN;
		  tdm_pipe_main[i+1]=MN_OVSB_TOKEN;
	  }	
	}
}

	/* case 793: calendar length is 402*/
   /*Monterey does not have 402 deep MMU calendar as Per slot BW is 2.5 G and not 1.25 G in Apache. 
  if(tdm_clk_freq == 793 || tdm_clk_freq ==794 ) {
    ...
    for(i=0; i<512; i++) {
    if(tdm_pipe_main[i] == MN_NUM_EXT_PORTS) break;
    }
    size = i;
   
    while(size < tdm_pipe_main_len) {
      tdm_pipe_main[size] = MN_ANCL_TOKEN;
      size++;
    } 
  } */
	...
	if(_tdm->_core_data.vars_pkg.lr_enable && !(_tdm->_core_data.vars_pkg.os_enable) && !(_tdm->_core_data.vars_pkg.refactor_done)){
		/*filter_result = tdm_mn_filter_smooth_idle_slice(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed);
		TDM_PRINT1("\nFilter done: ---IDLE SLOT SMOOTH, filter applied <%d> times\n", filter_result);*/
              if(tdm_clk_freq == 705 && (tdm_pipe_main[0] == tdm_pipe_main[1])) {
                one_slot_cnt = tdm_pipe_main[0];
                tdm_pipe_main[0] = tdm_pipe_main[164];
                tdm_pipe_main[164] = one_slot_cnt; }
	...
	if (_tdm->_core_data.vars_pkg.lr_enable && _tdm->_core_data.vars_pkg.os_enable && !(_tdm->_core_data.vars_pkg.refactor_done)) {
		...
		filter_cnt= 0;
		max_size = tdm_mn_scan_mix_slice_max(ovsb_token,tdm_pipe_main,tdm_pipe_main_len, &max_slice_idx, 0);
		min_size = tdm_mn_scan_mix_slice_min(ovsb_token,tdm_pipe_main,tdm_pipe_main_len, &min_slice_idx, 0);
		if( (max_size-min_size)>=6 ){
			for (k=1; k<max_size-min_size; k++){
				filter_result = tdm_mn_filter_migrate_os_slot(max_slice_idx, min_slice_idx, tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed );
				if(filter_result==FAIL){break;}
				filter_cnt++;
				max_size_k = tdm_mn_scan_mix_slice_max(ovsb_token,tdm_pipe_main,tdm_pipe_main_len, &max_slice_idx, 0);
				min_size_k = tdm_mn_scan_mix_slice_min(ovsb_token,tdm_pipe_main,tdm_pipe_main_len, &min_slice_idx, 0);
		...
		do{
			filter_result = tdm_mn_filter_smooth_os_slice(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed, DN);
		...
		timeout = _tdm->_chip_data.soc_pkg.lr_idx_limit;
		do{
			filter_result = tdm_mn_filter_smooth_os_slice(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed, UP);
			TDM_PRINT2("\nFilter done: ---OVSB MAX SLICE REDUCTION (2) Shift Up (%2d), filter applied <%d> times\n", ++filter_cnt, filter_result);
			TDM_SML_BAR
		} while ( filter_result>0 && (timeout--)>0 );

		/* Smooth oversub distribution */
		filter_result = tdm_mn_filter_smooth_os_os_up(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed);
		TDM_PRINT1("\nFilter done: ---OVSB LOCAL SLICE REDUCTION (1) slot UP, X_Y_OVSB_OVSB, filter applied <%d> times \n", filter_result);
		TDM_SML_BAR
		filter_result = tdm_mn_filter_smooth_os_os_dn(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed);
		TDM_PRINT1("\nFilter done: ---OVSB LOCAL SLICE REDUCTION (2) slot DOWN, OVSB_OVSB_X_Y, filter applied <%d> times \n", filter_result);
		TDM_SML_BAR

		/* Smooth oversub distribution */
    /*filter_result = tdm_mn_filter_smooth_os_slice_fine(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed);
		...
		filter_result = 0;
		timeout=DITHER_PASS;
		while ( tdm_mn_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>1 && 
		        tdm_mn_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>=tdm_mn_slice_size(1,tdm_pipe_main,tdm_pipe_main_len) && 
		       ((--timeout)>0) ) {			
			filter_result += tdm_mn_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,tdm_port_pmap,DITHER_THRESHOLD,tdm_port_speed);
		}
		timeout=DITHER_PASS;
		while ( tdm_mn_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>1 && 
		        tdm_mn_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>=tdm_mn_slice_size(1,tdm_pipe_main,tdm_pipe_main_len) && 
				((--timeout)>0) ) {			
			filter_result += tdm_mn_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,tdm_port_pmap,(DITHER_THRESHOLD-DITHER_SUBPASS_STEP_SIZE),tdm_port_speed);
		}
		timeout=DITHER_PASS;
		while ( tdm_mn_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>1 && 
		        tdm_mn_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>=tdm_mn_slice_size(1,tdm_pipe_main,tdm_pipe_main_len) && 
				((--timeout)>0) ) {			
			filter_result += tdm_mn_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,tdm_port_pmap,(DITHER_THRESHOLD-DITHER_SUBPASS_STEP_SIZE-DITHER_SUBPASS_STEP_SIZE),tdm_port_speed);
		}
		...
		/* Smooth oversub distribution */
		filter_result = 0;
	/*	if (tdm_mn_slice_size(1, tdm_pipe_main, tdm_pipe_main_len)==2) {
			for (i=3; i<tdm_pipe_main_len; i++) {
				if (tdm_pipe_main[i-3]!=ovsb_token && 
					...
					tdm_mn_scan_slice_size_local((i-3), tdm_pipe_main, tdm_pipe_main_len, &slice_idx)==2 && 
					tdm_mn_scan_slice_size_local( i,    tdm_pipe_main, tdm_pipe_main_len, &slice_idx)==2 && 
					tdm_mn_scan_slice_size_local((i+3), tdm_pipe_main, tdm_pipe_main_len, &slice_idx)==2 && 
					tdm_mn_scan_slice_size_local((i+6), tdm_pipe_main, tdm_pipe_main_len, &slice_idx)==2) {
					if (tdm_mn_filter_fine_dither(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,tdm_port_pmap)==FAIL) {
						break;
					}
					filter_result++;
				}
