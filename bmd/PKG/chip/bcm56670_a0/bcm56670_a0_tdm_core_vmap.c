...
tdm_vmap_pm_t *pm, int vmap_idx, unsigned short **vmap)
{
    ...
    tmp_len = (*pm).pm_slots;
    /* malloc: tmp_vmap */
    tmp_vmap = (unsigned short**) TDM_MN_ALLOC(
               tmp_wid * sizeof(unsigned short*), "tmp_vmap_l1");
    for (i = 0; i < tmp_wid; i++) {
        tmp_vmap[i] = (unsigned short*) TDM_MN_ALLOC(
                      tmp_len * sizeof(unsigned short), "tmp_vmap_l2");
    }
    /* malloc: tmp_r_a_arr */
    tmp_r_a_arr = (int**) TDM_MN_ALLOC(tmp_wid * sizeof(int*), "tmp_r_a_arr_l1");
    for (i = 0; i < tmp_wid; i++) {
        tmp_r_a_arr[i] = (int*) TDM_MN_ALLOC(2 * sizeof(int), "tmp_r_a_arr_l2");
    }
    /* init */
    for (i = 0; i < tmp_wid; i++) {
        for (j = 0; j < tmp_len; j++) {
                  ...
                  int pmlist_size, unsigned short **vmap)
{
    ...
    /* allocate memory */
    r_a_arr = (int**) TDM_MN_ALLOC(param_vmap_wid * sizeof(int*), "r_a_arr_l1");
    for (i = 0; i < param_vmap_wid; i++) {
        r_a_arr[i] = (int*) TDM_MN_ALLOC(2 * sizeof(int), "r_a_arr_l2");
    }
    for (i=0; i<param_vmap_wid; i++) {
        r_a_arr[i][0] = 0;
        r_a_arr[i][1] = 0;
...
tdm_vmap_alloc(tdm_mod_t *_tdm)
{
    ...
    /* allocate memory */
    pmlist  = (tdm_vmap_pm_t *) TDM_MN_ALLOC(
                        pmlist_size * sizeof(tdm_vmap_pm_t), "pmlist");
    lr_buff = (int*) TDM_MN_ALLOC(lr_buff_size * sizeof(int), "lr_buff");
    vmap    = (unsigned short**) TDM_MN_ALLOC(
                        param_vmap_wid * sizeof(unsigned short*), "vmap_l1");
    for (i = 0; i < param_vmap_wid; i++) {
        vmap[i] = (unsigned short*)
                  TDM_MN_ALLOC(param_vmap_len * sizeof(unsigned short), "vmap_l2");
    }
