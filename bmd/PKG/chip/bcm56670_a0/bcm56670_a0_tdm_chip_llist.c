...
tdm_mn_ll_append(struct mn_ll_node *llist, unsigned short port_append, int *pointer)
{
	...
	if (tail) {
		mbox = ( (struct mn_ll_node *) TDM_MN_ALLOC(sizeof(struct mn_ll_node),"mn_ll_node") );
...
{
	int i, pool=0, skew=0, slices=0, split, count, set=BOOL_FALSE, div1, timeout, original_len;
	struct mn_ll_node *list = llist;
	...
	if (list!=NULL && tdm_mn_ll_len(list)>0) {
		...
		while (pool>0 && (--timeout)>0) {
			...
			if (set) {
				...
				for (i=((original_len/div1)-1); i<original_len; i+=(original_len/div1)) {
					tdm_mn_ll_insert(list,token,i);
					if ((--pool)<=0) {
						break;
					}
...
{
   int i, j, k, div1, pool=0, set=BOOL_FALSE, original_len, timeout=32000;
   struct mn_ll_node *list = llist;
   ...
   temp_port=cl_token;
    ...
    while (pool>0 && (--timeout)>0) {
      if (set) {
      	...
      	for (i=((original_len/div1)-1); i<original_len;) {
   			  tdm_mn_ll_insert(list,cl_token,i);
   		    if ((--pool)<=0) {
   			     break;
   		    }
           if(i<= original_len/2) { i+=((original_len/div1)+1);}
           else { i+=(original_len/div1);} 
   	    }
      }
    }
...
{
	int i, pool=0, div1, set=BOOL_FALSE, timeout, original_len;
	struct mn_ll_node *list = llist;
