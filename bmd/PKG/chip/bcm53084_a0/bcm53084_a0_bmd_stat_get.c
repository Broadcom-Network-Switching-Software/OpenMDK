...
int 
bcm53084_a0_bmd_stat_get(
    int unit, int port, bmd_stat_t stat, bmd_counter_t *counter)
{
    ...
    case bmdStatTxPackets:
        ...
        ioerr += READ_TXPAUSEPKTSr(unit, port, &txpausepkts);
        counter->v[0] += TXPAUSEPKTSr_GET(txpausepkts);
        break;
    case bmdStatTxBytes:
    ...
    case bmdStatTxErrors:
        ...
        ioerr += READ_TXLATECOLLISIONr(unit, port, &txlatecollision);
        counter->v[0] += TXLATECOLLISIONr_GET(txlatecollision);
        break;
    case bmdStatRxPackets:
        ...
        ioerr += READ_RXBROADCASTPKTSr(unit, port, &rxbroadcastpkts);
        counter->v[0] += RXBROADCASTPKTSr_GET(rxbroadcastpkts);
        ioerr += READ_RXPAUSEPKTSr(unit, port, &rxpausepkts);
        counter->v[0] += RXPAUSEPKTSr_GET(rxpausepkts);
        break;
    case bmdStatRxBytes:
