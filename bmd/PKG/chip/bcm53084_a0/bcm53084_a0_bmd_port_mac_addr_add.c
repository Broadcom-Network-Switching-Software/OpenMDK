...
int
bcm53084_a0_arl_write(
    int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr)
{
    int ioerr = 0;
    int rv;
    ...
    /* Create ARL data entry for writing */
    ...
    for (idx = 0; idx < NUM_BINS; idx++) {
        ...
        if (ARLA_FWD_ENTRY0r_ARL_VALIDf_GET(fwd_chk)) {
            if (ARLA_FWD_ENTRY0r_ARL_STATICf_GET(fwd_chk) == 0) {
                ...
                CDK_VVERB(("bcm53084_a0_arl_write: dynamic bin %d\n", idx));
                dyn_bin = idx;
            }
            reg = macvid_base + (idx * macvid_inc);
            ioerr += cdk_robo_reg_read(unit, reg, &macvid_chk, macvid_size);
            if (CDK_MEMCMP(&macvid_chk, 
				    &arla_macvid, sizeof(&macvid_chk)) == 0) {
                /* Found a matching key */
                CDK_VVERB(("bcm53084_a0_arl_write: matching bin %d\n", idx));
                bin_no = idx;
                break;
            }
        } else if (bin_no < 0) {
            /* First empty bin */
            CDK_VVERB(("bcm53084_a0_arl_write: empty bin %d\n", idx));
            bin_no = idx;
        }
    }
...
int 
bcm53084_a0_bmd_port_mac_addr_add(
    int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr)
{
    BMD_CHECK_UNIT(unit);
    ...
    return bcm53084_a0_arl_write(unit, port, vlan, mac_addr);
}
