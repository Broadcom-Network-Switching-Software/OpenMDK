...
int 
bcm53084_a0_bmd_port_vlan_get(int unit, int port, int *vlan)
{
    int ioerr = 0;
    ...
    if (port == 5) {
        ...
        *vlan = DEFAULT_1Q_TAG_P5r_VIDf_GET(default_tag_p5);

    } else if (port == 8) {
        ioerr += READ_DEFAULT_1Q_TAG_IMPr(unit, &default_tag_imp);
        *vlan = DEFAULT_1Q_TAG_IMPr_VIDf_GET(default_tag_imp);
