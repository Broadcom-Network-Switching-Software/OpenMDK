    ...
    for (idx = 0; idx < NUM_BINS; idx++) {
        ...
        if (ARLA_FWD_ENTRY0r_ARL_VALIDf_GET(fwd_chk)) {
            if (ARLA_FWD_ENTRY0r_ARL_STATICf_GET(fwd_chk) == 0) {
                ...
                CDK_VVERB(("bcm53101_a0_arl_write: dynamic bin %d\n", idx));
                dyn_bin = idx;
            }
            ...
            if (CDK_MEMCMP(&macvid_chk, &arla_macvid, sizeof(&macvid_chk)) == 0) {
                ...
                CDK_VVERB(("bcm53101_a0_arl_write: matching bin %d\n", idx));
                bin_no = idx;
                break;
            }
        } else if (bin_no < 0) {
            /* First empty bin */
            CDK_VVERB(("bcm53101_a0_arl_write: empty bin %d\n", idx));
            bin_no = idx;
        }
    }
...
int
bcm53101_a0_bmd_port_mac_addr_add(int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr)
{
    BMD_CHECK_UNIT(unit);
    BMD_CHECK_VLAN(unit, vlan);
    BMD_CHECK_PORT(unit, port);

    return bcm53101_a0_arl_write(unit, port, vlan, mac_addr);
}
