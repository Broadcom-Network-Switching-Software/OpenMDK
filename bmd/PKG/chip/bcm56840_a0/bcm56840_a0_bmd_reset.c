...
#define _INIT_SVK_CLK(_u) _init_svk_clk(_u)

static int
_init_svk_clk(int unit)
{
    int ioerr = 0;

    if (CDK_CHIP_CONFIG(unit) & DCFG_PLL_MOD) {
        CMIC_CORE_PLL0_CTRL_STAT_2r_t cpll2;
        CMIC_CORE_PLL0_CTRL_STAT_3r_t cpll3;
        CMIC_CORE_PLL0_CTRL_STAT_4r_t cpll4;
        CMIC_MISC_CONTROLr_t misc_ctrl;

        CDK_WARN(("bcm56840_a0_bmd_reset[%d]: adjust core PLL\n", unit));

        ioerr += READ_CMIC_CORE_PLL0_CTRL_STAT_3r(unit, &cpll3);
        CMIC_CORE_PLL0_CTRL_STAT_3r_I_NDIV_INTf_SET(cpll3, 99);
        CMIC_CORE_PLL0_CTRL_STAT_3r_CH0_MDIVf_SET(cpll3, 5);
        ioerr += WRITE_CMIC_CORE_PLL0_CTRL_STAT_3r(unit, cpll3);

        ioerr += READ_CMIC_CORE_PLL0_CTRL_STAT_4r(unit, &cpll4);
        CMIC_CORE_PLL0_CTRL_STAT_4r_SLV_I_NDIV_INTf_SET(cpll4, 28);
        CMIC_CORE_PLL0_CTRL_STAT_4r_SLV_CH0_MDIVf_SET(cpll4, 7);
        ioerr += WRITE_CMIC_CORE_PLL0_CTRL_STAT_4r(unit, cpll4);

        ioerr += READ_CMIC_CORE_PLL0_CTRL_STAT_2r(unit, &cpll2);
        CMIC_CORE_PLL0_CTRL_STAT_2r_FB_PHASE_ENf_SET(cpll2, 1);
        ioerr += WRITE_CMIC_CORE_PLL0_CTRL_STAT_2r(unit, cpll2);

        ioerr += READ_CMIC_MISC_CONTROLr(unit, &misc_ctrl);
        CMIC_MISC_CONTROLr_CMIC_TO_CORE_PLL_LOADf_SET(misc_ctrl, 1);
        ioerr += WRITE_CMIC_MISC_CONTROLr(unit, misc_ctrl);

        ioerr += READ_CMIC_CORE_PLL0_CTRL_STAT_4r(unit, &cpll4);
        CMIC_CORE_PLL0_CTRL_STAT_4r_I_LOAD_ENf_SET(cpll4, 1);
        ioerr += WRITE_CMIC_CORE_PLL0_CTRL_STAT_4r(unit, cpll4);
