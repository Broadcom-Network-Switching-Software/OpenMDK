...
int
bcm53570_a0_bmd_port_mode_update(int unit, int port)
{
    int rv = CDK_E_NONE;
    int ioerr = 0;
    int status_change;
    ...
    bcm53570_a0_tscq_pbmp_get(unit, &qpbmp);
    
    rv = bmd_link_update(unit, port, &status_change);
    if (CDK_SUCCESS(rv) && status_change) {
        rv = bcm53570_a0_bmd_port_mode_get(unit, port, &mode, &flags);
        if (CDK_PBMP_MEMBER(clpbmp, port)) {
            if (CDK_SUCCESS(rv)) {
                ...
                if (CDK_SUCCESS(rv) && autoneg) {
                    ...
                    rv = bcm53570_a0_bmd_port_mode_set(unit, port, mode, flags);
                }
            }
