...
typedef struct xgsd_rx_dscr_s {
    RX_DCB_t *dcb;      /* DMA Control Block */
    dma_addr_t bdcb;    /* RX_DCB bus address */
    bmd_pkt_t *pkt;     /* Packet associated with RX_DCB */
} xgsd_rx_dscr_t;

static xgsd_rx_dscr_t _rx_dscr[BMD_CONFIG_MAX_UNITS];

static int
_cpu_port_enable_set(int unit, int enable)
{
    int ioerr = 0;
    EPC_LINK_BMAP_LO_64r_t epc_link;
    uint32_t epc_pbm;

    ioerr += READ_EPC_LINK_BMAP_LO_64r(unit, &epc_link);
    CDK_ASSERT(CMIC_PORT < 32);
    epc_pbm = EPC_LINK_BMAP_LO_64r_GET(epc_link, 0);
    if (enable) {
        epc_pbm |= LSHIFT32(1, CMIC_PORT);
    } else {
        epc_pbm &= ~LSHIFT32(1, CMIC_PORT);
    }
    EPC_LINK_BMAP_LO_64r_SET(epc_link, 0, epc_pbm);
    ioerr += WRITE_EPC_LINK_BMAP_LO_64r(unit, epc_link);
...
int
bcm53570_a0_bmd_rx_start(int unit, bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    dma_addr_t bdcb;
    ...
    bmd_xgsd_dma_rx_start(unit, bdcb); 
...
int
bcm53570_a0_bmd_rx_poll(int unit, bmd_pkt_t **ppkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    bmd_pkt_t *pkt;
    ...
    if (bmd_xgsd_dma_rx_poll(unit, 1) < 0) {
        return CDK_E_TIMEOUT;
    }
    BMD_DMA_CACHE_INVAL(dcb, sizeof(*dcb));
    bmd_xgsd_dump_rx_dcb(unit, (uint32_t *)dcb,
    ...
    pkt->port = L2P(unit, RX_DCB_SRC_PORTf_GET(*dcb));

    bmd_xgsd_parse_higig2(unit, pkt, RX_DCB_MODULE_HEADERf_PTR(*dcb));
...
int
bcm53570_a0_bmd_rx_stop(int unit)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    int rv = CDK_E_NONE;
