...
int
bcm53570_a0_bmd_reset(int unit)
{
    ...
    int idx;
    uint32_t chip_cfg, opt, freq;
    uint32_t ring_map[] = { 0x11110100, 0x00430000, 0x00005064, 
    ...
    /* Using port configuration to get frequence */
    /* For cfg 1:2, port_cfg = 0x12
       For cfg 12:1, port_cfg = 0x121, etc .. */
    opt = chip_cfg >> OPT_START_BIT;
    if ((opt == 0x12) || (opt == 0x13)) {
        /* FREQ_125 */
        freq = 0x1c;
    } else if ((opt == 0x5) || (opt >= 0x8)) {
        /* FREQ_437 */
        freq = 0x8;
    } else {
        /* FREQ_583 */
    ...
    /* disable_tsc = 0~0x7f, disable_qtc = 0~1 */
    ...
    disable_tsc = ((strap_val >> 2) & 0x7f);
    disable_qtc = (strap_val & 0x3);
    if (disable_tsc || disable_qtc) {
        bcm53570_a0_block_disable(unit, disable_tsc, disable_qtc);
    }    
    disable_tsc = 0x7f;
    disable_qtc = 0x3;
