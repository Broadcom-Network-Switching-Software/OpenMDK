...
int
bcm56270_a0_bmd_port_mode_set(int unit, int port,
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        bcm56270_a0_mac_xl_drain_cells(unit, port);
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & ((BMD_PORT_HG) | (BMD_PORT_XE))) {
             ...
             */
            rv = bcm56270_a0_bmd_port_mode_get(unit, port,
                                               &cur_mode, &cur_flags);
   ...
   /* Update PHYs before MAC */
    ...
    if (mode == bmdPortModeDisabled) {
        ...
        if (CDK_PBMP_MEMBER(mxqport_pbmp, port)) {
            if (CDK_SUCCESS(rv)) {
                rv = bcm56270_a0_mxq_phy_mode_get(unit, port, speed_max, &phy_mode, &num_lanes);
        ...
        if (CDK_PBMP_MEMBER(xlport_pbmp, port)) {
            if (CDK_SUCCESS(rv)) {
                rv = bcm56270_a0_xl_phy_core_port_mode(unit, port,
