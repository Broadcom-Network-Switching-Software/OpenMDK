...
#include "bcm56685_a0_internal.h"
...
int
bcm56685_a0_xport_init(int unit, int port)
{
    int ioerr = 0;
...
bcm56685_a0_bmd_init(int unit)
{
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56685_a0_bmd_init[%d]: IPIPE reset timeout\n", unit));
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56685_a0_bmd_init[%d]: EPIPE reset timeout\n", unit));
        return ioerr ? CDK_E_IO : CDK_E_TIMEOUT;
    }
    ...
    for (idx = 0; idx < COUNTOF(tdmb685); idx++) {
        IARB_TDM_TABLEm_CLR(iarb_tdm);
        ARB_TDM_TABLEm_CLR(arb_tdm);
        IARB_TDM_TABLEm_PORT_NUMf_SET(iarb_tdm, tdmb685[idx]);
        ARB_TDM_TABLEm_PORT_NUMf_SET(arb_tdm, tdmb685[idx]);
        if (idx == (COUNTOF(tdmb685) - 1)) {
    ...
    IARB_TDM_CONTROLr_TDM_WRAP_PTRf_SET(iarb_tdm_ctrl, COUNTOF(tdmb685) - 1);
    ...
    for (idx = 0; idx < COUNTOF(xq_xports); idx++) {
        ...
        XQPORT_MODE_REGr_XQPORT_MODE_BITSf_SET(xqport_mode, xq_modes[idx]);
    ...
    CDK_PBMP_ITER(pbmp, port) {
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
            ioerr += bcm56685_a0_xport_init(unit, port);
        } else {
            ioerr += _gport_init(unit, port);
        }
        if (CDK_SUCCESS(rv)) {
