...
static int
bcm56685_a0_unicore_phy_init(int unit, int port)
{
    int ioerr = 0;
...
int
bcm56685_a0_xport_reset(int unit, int port)
{
    int ioerr = 0;
    int msec;
    int idx;
    ...
    if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
        ioerr += bcm56685_a0_unicore_phy_init(unit, port);
    }
#endif
    ...
    if (msec >= PLL_LOCK_MSEC) {
        CDK_WARN(("bcm56685_a0_xport_reset[%d]: "
...
static int
bcm56685_a0_lcpll_init(int unit)
{
    int ioerr = 0;
    int msec;
    ...
    if (msec >= PLL_LOCK_MSEC) {
        CDK_WARN(("bcm56685_a0_bmd_reset[%d]: "
...
int
bcm56685_a0_bmd_reset(int unit)
{
    int ioerr = 0;
    int wait_usec = 10000;
    int idx;
    ...
    /* Initialize LC PLLs */
    ioerr += bcm56685_a0_lcpll_init(unit);
    ...
    /* Reset XQPORTs and enable 1G Tx clocks */
    for (idx = 0; idx < COUNTOF(xq_xports); idx++) {
        port = xq_xports[idx];
        if (!CDK_XGS_PORT_VALID(unit, port)) {
            continue;
        }
        ioerr += bcm56685_a0_xport_reset(unit, port);
    ...
    {      
        ...
        cdk_pbmp_t pbmp;
        /* Reset GXPORTs and enable 1G Tx clocks */
