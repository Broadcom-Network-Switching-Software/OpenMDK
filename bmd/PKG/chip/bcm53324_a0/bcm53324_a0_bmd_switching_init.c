...
_config_e_port(int unit, int port, uint32_t vlan_flags)
{
    ...
    rv = bcm53324_a0_bmd_vlan_port_add(unit, BMD_CONFIG_DEFAULT_VLAN,
                                       port, vlan_flags);
    if (CDK_SUCCESS(rv)) {
        rv = bcm53324_a0_bmd_port_stp_set(unit, port, 
                                          bmdSpanningTreeForwarding);
    }
    if (CDK_SUCCESS(rv)) {
        rv = bcm53324_a0_bmd_port_mode_set(unit, port, bmdPortModeAuto, 0);
    }
...
int
bcm53324_a0_bmd_switching_init(int unit)
{
    int ioerr = 0;
    int rv;
    int port;
    ...
    rv = bcm53324_a0_bmd_reset(unit);

    if (CDK_SUCCESS(rv)) {
        rv = bcm53324_a0_bmd_init(unit);
    }

    if (CDK_SUCCESS(rv)) {
        rv = bcm53324_a0_bmd_vlan_create(unit, BMD_CONFIG_DEFAULT_VLAN);
    }
