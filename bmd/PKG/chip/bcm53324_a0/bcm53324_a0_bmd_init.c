...
bcm53324_a0_bmd_init(int unit)
{
    ...
    EGR_ENABLE_SELECTr_t egr_en_sel;
    CMIC_QGPHY_QSGMII_CONTROLr_t qgphy_qsgmii_ctrl;
    ING_MISC_CONFIGr_t ing_misc_cfg;
    ...
    READ_CMIC_QGPHY_QSGMII_CONTROLr(unit, &qgphy_qsgmii_ctrl);
    CMIC_QGPHY_QSGMII_CONTROLr_DISABLE_HW_IDLE_PWRDWNf_SET(qgphy_qsgmii_ctrl, 1);
    WRITE_CMIC_QGPHY_QSGMII_CONTROLr(unit, qgphy_qsgmii_ctrl);
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm53324_a0_bmd_init[%d]: IPIPE reset timeout\n", unit));
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm53324_a0_bmd_init[%d]: EPIPE reset timeout\n", unit));
        return ioerr ? CDK_E_IO : CDK_E_TIMEOUT;
    }
    ...
    /* EGR_ENABLE is ignored by default, so enable it */
    READ_EGR_ENABLE_SELECTr(unit, &egr_en_sel);
    EGR_ENABLE_SELECTr_ENABLE_SELECTf_SET(egr_en_sel, 1);
    WRITE_EGR_ENABLE_SELECTr(unit, egr_en_sel);
    ...
    /* Clear VLAN tables (all VLANs are enabled by default) */
    ioerr += CDK_XGS_MEM_CLEAR(unit, VLAN_TABm);
    ioerr += CDK_XGS_MEM_CLEAR(unit, EGR_VLANm);
    ioerr += CDK_XGS_MEM_CLEAR(unit, STG_TABm);
    ioerr += CDK_XGS_MEM_CLEAR(unit, EGR_VLAN_STGm);
    ...
    /* Turn off fixed learning VLAN ID */
    READ_VLAN_CTRLr(unit, &vlan_ctrl);
    VLAN_CTRLr_USE_LEARN_VIDf_SET(vlan_ctrl, 0);
    WRITE_VLAN_CTRLr(unit, vlan_ctrl);
   
    /* EPC_LINK_BMAP is ignored by default, so enable it */
    READ_ING_MISC_CONFIGr(unit, &ing_misc_cfg);
    ING_MISC_CONFIGr_USE_EPC_LINK_BMPf_SET(ing_misc_cfg, 1);
    WRITE_ING_MISC_CONFIGr(unit, ing_misc_cfg);
