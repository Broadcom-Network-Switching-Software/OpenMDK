...
static int _tdm_arbiter_table_4x[] = {
    5, 6, 7, 8, 9, 10, 11, 12,
    5, 6, 7, 8, 9, 10, 11, 12,
    5, 6, 7, 8, 9, 10, 11, 12,
    5, 6, 7, 8, 9, 10, 11, 12,
...
static int _tdm_arbiter_table_4y[] = {
...
_tdm_arbiter_init(int unit)
{
    ...
    if (CDK_XGS_FLAGS(unit) & CHIP_FLAG_TDM3_X) {
        ...
        x_ports = _tdm_arbiter_table_4x;
    }

    if (CDK_XGS_FLAGS(unit) & CHIP_FLAG_TDM3_Y) {
        y_ports = _tdm_arbiter_table_3y;
    } else {
        y_ports = _tdm_arbiter_table_4y;
    }

    /* Ingress pipe select X */
    bcm56725_a0_pipe_select(unit, 0);
    ...
    bcm56725_a0_pipe_select(unit, 1);
...
_mmu_init(int unit)
{
    ...
    int port, i;
    int num_ports;
    cdk_pbmp_t gxport_pbmp, mmu_pbmp;
    uint32_t pbm;
    ...
    CDK_XGS_BLKTYPE_PBMP_GET(unit, BLKTYPE_GXPORT, &gxport_pbmp);
    CDK_PBMP_OR(mmu_pbmp, gxport_pbmp);
    ...
    /* Note that only PG2 is being used */
    ...
    /* CMIC port is intentionally excluded from the following iteration */
    CDK_PBMP_ITER(gxport_pbmp, port) {
        ioerr += WRITE_PG_HDRM_LIMITr(unit, port, 2, pg_hdrm_limit);
    }
    ...
    OP_PORT_CONFIGr_OP_SHARED_LIMITf_SET(op_port_config, 0x800);
    OP_PORT_CONFIGr_OP_SHARED_RESET_VALUEf_SET(op_port_config, 0x733);
    OP_PORT_CONFIGr_PORT_LIMIT_ENABLEf_SET(op_port_config, 1);
    WRITE_OP_PORT_CONFIGr(unit, CMIC_PORT, op_port_config);

    OP_QUEUE_RESET_OFFSETr_CLR(opq_rst_offs);
    OP_QUEUE_RESET_OFFSETr_Q_RESET_OFFSETf_SET(opq_rst_offs, MMU_MTU_CELLS);
    ...
    /* Use fixed buffer size for CPU port */
    OP_QUEUE_CONFIGr_Q_LIMIT_ENABLEf_SET(op_queue_config, 0);
    OP_QUEUE_CONFIGr_Q_LIMIT_DYNAMICf_SET(op_queue_config, 0);
    OP_QUEUE_CONFIGr_Q_SHARED_LIMITf_SET(op_queue_config, LIMIT_DISABLE);
    for (i = 0; i < MMU_NUM_COS; i++) {
        ioerr += WRITE_OP_QUEUE_CONFIGr(unit, CMIC_PORT, i, op_queue_config);
    }
    for (i = 8; i <= 9; i++) {
        ioerr += WRITE_OP_QUEUE_CONFIGr(unit, CMIC_PORT, i, op_queue_config);
    }
    ...
    CFAPCONFIGr_CFAPPOOLSIZEf_SET(cfapconfig, 0x2fff);
    ioerr += WRITE_CFAPCONFIGr(unit, cfapconfig);

    CFAPFULLTHRESHOLDr_CLR(cfapfullthreshold);
    CFAPFULLTHRESHOLDr_CFAPFULLSETPOINTf_SET(cfapfullthreshold, 0x2f00);
    CFAPFULLTHRESHOLDr_CFAPFULLRESETPOINTf_SET(cfapfullthreshold, 0x2e50);
...
bcm56725_a0_bmd_init(int unit)
{
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56725_a0_bmd_init[%d]: IPIPE reset timeout\n", unit));
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56725_a0_bmd_init[%d]: EPIPE reset timeout\n", unit));
        return ioerr ? CDK_E_IO : CDK_E_TIMEOUT;
    }
    ...
    ioerr += WRITE_EGR_VLAN_TAG_ACTION_PROFILEm(unit, 0, egr_action);

    /* Configure GXPORTs */
    CDK_XGS_BLKTYPE_PBMP_GET(unit, BLKTYPE_GXPORT, &pbmp);
    CDK_PBMP_ITER(pbmp, port) {
        ioerr += bcm56725_a0_gxport_init(unit, port);
        if (CDK_SUCCESS(rv)) {
