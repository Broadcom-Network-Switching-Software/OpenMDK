...
int
bcm56260_a0_bmd_vlan_port_add(int unit, int vlan, int port, uint32_t flags)
{
    int ioerr = 0;
    int ppport;
    ...
    if (flags & BMD_VLAN_PORT_F_UNTAGGED) {
        pbmp = EGR_VLANm_UT_PORT_BITMAP_W0f_GET(egr_vlan);
        EGR_VLANm_UT_PORT_BITMAP_W0f_SET(egr_vlan, (uint32_t)(pbmp | mask));
    }
