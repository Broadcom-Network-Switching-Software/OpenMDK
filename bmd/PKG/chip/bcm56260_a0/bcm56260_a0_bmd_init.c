...
_mmu_init(int unit)
{
    ...
    if (idx >= LLS_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56260_a0_bmd_init[%d]: LLS INIT timeout\n", unit));
...
int bcm56260_a0_soc_port_enable_set(int unit, int port, int enable) 
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    int loop;
...
bcm56260_a0_mac_xl_drain_cells(int unit, int port)
{
    ...
    ioerr += bcm56260_a0_soc_port_enable_set(unit, port, 0);
    ioerr += bcm56260_a0_soc_port_enable_set(unit, port, 1);
    ...
    if (cnt < 0) {
        CDK_WARN(("bcm56260_a0_bmd_port_mode_set[%d]: "
...
bcm56260_a0_bmd_init(int unit)
{
    ...
    CDK_PBMP_ITER(pbmp_all, port) {
        ioerr += bcm56260_a0_xlport_init(unit, port);    
        ioerr += _port_init(unit, port);  
    }
