...
_lcpll_check(int unit)
{
    ...
    if (msec >= PLL_LOCK_MSEC) {
        CDK_WARN(("bcm56440_b0_bmd_reset[%d]: "
...
int
bcm56440_b0_bmd_reset(int unit)
{
    int ioerr = 0;
    int wait_usec = 10000;
    int port;
    ...
    TOP_MISC_CONTROL_2r_t misc_ctrl;;
    BMD_CHECK_UNIT(unit);
    ...
    if (TOP_SWITCH_FEATURE_ENABLE_2r_BOND_CES_ENABLEf_GET(feature_enable)) {
         /* Disable CES */
        ioerr += READ_TOP_MISC_CONTROL_2r(unit, &misc_ctrl);
        TOP_MISC_CONTROL_2r_BOND_CES_ENABLE_OVERRIDEf_SET(misc_ctrl , 1);
        WRITE_TOP_MISC_CONTROL_2r(unit, misc_ctrl);
    }
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ioerr += bcm56440_b0_xport_reset(unit, port);
