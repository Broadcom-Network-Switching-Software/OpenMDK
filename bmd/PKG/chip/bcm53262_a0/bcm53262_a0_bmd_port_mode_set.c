...
int
bcm53262_a0_bmd_port_mode_set(int unit, int port, 
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if(BMD_PORT_PROPERTIES(unit, port) == BMD_PORT_FE) {
        ...
        STS_OVERRIDE_Pr_SW_ORDf_SET(sts_override_p, 1);
        STS_OVERRIDE_Pr_SPEED_Rf_SET(sts_override_p, sp_sel);
        ...
        mac_disabled = TH_PCTLr_MIRX_DISf_GET(th_pctl);

        if (mode == bmdPortModeDisabled) {
            /* Disable MAC if enabled */
            if (!mac_disabled) {
                TH_PCTLr_MIRX_DISf_SET(th_pctl, 1);
                TH_PCTLr_MITX_DISf_SET(th_pctl, 1);
            ...
            if (mac_disabled) {
                TH_PCTLr_MIRX_DISf_SET(th_pctl, 0);
                TH_PCTLr_MITX_DISf_SET(th_pctl, 0);
                ioerr += WRITE_TH_PCTLr(unit, port, th_pctl);
            }
            if (phy_lb) {
        ...
        STS_OVERRIDE_GPr_SW_ORDf_SET(sts_override_gp, 1);
        STS_OVERRIDE_GPr_GIGA_SPEEDf_SET(sts_override_gp, sp_sel);
        ...
        mac_disabled = G_PCTLr_MIRX_DISf_GET(g_pctl);
    
        if (mode == bmdPortModeDisabled) {
            /* Disable MAC if enabled */
            if (!mac_disabled) {
                G_PCTLr_MIRX_DISf_SET(g_pctl, 1);
                G_PCTLr_MITX_DISf_SET(g_pctl, 1);
            ...
            /* Enable MAC if disabled */
            if (mac_disabled) {
                G_PCTLr_MIRX_DISf_SET(g_pctl, 0);
                G_PCTLr_MITX_DISf_SET(g_pctl, 0);
                ioerr += WRITE_G_PCTLr(unit, port, g_pctl);
            }
            if (phy_lb) {
                BMD_PORT_STATUS_SET(unit, port, BMD_PST_LINK_UP | BMD_PST_FORCE_LINK);
