...
int
bcm53262_a0_bmd_stat_get(int unit, int port, bmd_stat_t stat, bmd_counter_t *counter)
{
    ...
    case bmdStatRxErrors:
        ...
        ioerr += READ_RXEXCESSSIZEDISCr(unit, port, &rxexcesssizedisc);
        counter->v[0] += RXEXCESSSIZEDISCr_GET(rxexcesssizedisc);
        break;
    case bmdStatRxDrops:
        ioerr += READ_RXFWDDISCPKTSr(unit, port, &rxdiscard);
