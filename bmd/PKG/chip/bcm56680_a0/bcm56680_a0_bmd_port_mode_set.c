...
int
bcm56680_a0_bmd_port_mode_set(int unit, int port, 
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        if (cnt < 0) {
            CDK_WARN(("bcm56680_a0_bmd_port_mode_set[%d]: "
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
             ...
             */
            rv = bcm56680_a0_bmd_port_mode_get(unit, port, 
                                               &cur_mode, &cur_flags);
            if (CDK_SUCCESS(rv) && 
                ...
                rv = bcm56680_a0_xport_reset(unit, port);
                if (CDK_SUCCESS(rv)) {
                    rv = bcm56680_a0_xport_init(unit, port);
                }
            }
        }
#endif
    ...
    if (mode == bmdPortModeDisabled) {
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
            ...
            ICONTROL_OPCODE_BITMAPr_SET(opcode_bmap, hg ? 0x100 : 0);
            /* Set HiGig vs. HiGig2 */
