...
int
bcm56680_a0_xport_init(int unit, int port)
{
    int ioerr = 0;
...
bcm56680_a0_bmd_init(int unit)
{
    ...
    /* Configure egress scheduler for 2.5 Gbps */
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56680_a0_bmd_init[%d]: IPIPE reset timeout\n", unit));
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56680_a0_bmd_init[%d]: EPIPE reset timeout\n", unit));
        return ioerr ? CDK_E_IO : CDK_E_TIMEOUT;
    }
    ...
    for (idx = 0; idx < COUNTOF(xg_xports); idx++) {
        ...
        XGPORT_MODE_REGr_XGPORT_MODE_BITSf_SET(xgport_mode, 3);
    ...
    CDK_XGS_BLKTYPE_PBMP_GET(unit, BLKTYPE_GXPORT, &pbmp);
    CDK_PBMP_ITER(pbmp, port) {
        ioerr += bcm56680_a0_xport_init(unit, port);
        if (CDK_SUCCESS(rv)) {
            rv = bmd_phy_attach(unit, port);
        }
    }
