...
int
bcm53128_a0_bmd_port_mode_update(int unit, int port)
{
    int rv = CDK_E_NONE;
    int ioerr = 0;
    ...
    if (CDK_SUCCESS(rv) && status_change) {
        rv = bcm53128_a0_bmd_port_mode_get(unit, port, &mode, &flags);
        if (CDK_SUCCESS(rv)) {
            ...
            rv = bcm53128_a0_bmd_port_mode_set(unit, port, mode, flags);
