 ...
 */
int
tdm_td2p_filter_smooth_ancl(int ancl_token, int *tdm_tbl, int tdm_tbl_len, int ancl_space_min)
{
    int i, j, g, l, pool, ancl_num=0, ancl_dist_up, ancl_dist_dn, lr_slice_size, lr_slice_min, slice_idx, timeout;

    lr_slice_min = tdm_td2p_scan_slice_min(1, tdm_tbl, tdm_tbl_len, &slice_idx,0);
    ...
    while( (pool>0) && ((--timeout)>0) ){
        for (i=1; i<tdm_tbl_len; i++) {
            ...
            TD2P_TOKEN_CHECK(tdm_tbl[i]) {
                lr_slice_size = tdm_td2p_scan_slice_size_local(i,tdm_tbl,tdm_tbl_len,&slice_idx);
                ancl_dist_up  = tdm_td2p_check_same_port_dist_up_port(ancl_token,i,tdm_tbl,tdm_tbl_len);
                ancl_dist_dn  = tdm_td2p_check_same_port_dist_dn_port(ancl_token,i,tdm_tbl,tdm_tbl_len);
