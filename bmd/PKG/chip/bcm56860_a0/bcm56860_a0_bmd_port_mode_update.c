...
int 
bcm56860_a0_bmd_port_mode_update(int unit, int port)
{
    ...
    if (CDK_SUCCESS(rv) && status_change) {
        ...
        if (CDK_SUCCESS(rv) && autoneg) {
            rv = bcm56860_a0_bmd_port_mode_get(unit, port, &mode, &flags);
            if (CDK_SUCCESS(rv)) {
                flags |= BMD_PORT_MODE_F_INTERNAL;
                rv = bcm56860_a0_bmd_port_mode_set(unit, port, mode, flags);
            }
        }
