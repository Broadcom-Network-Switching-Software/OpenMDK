...
_dcb_init(int unit, TX_DCB_t *dcb, const bmd_pkt_t *pkt)
{
    ...
    if (pkt->port >= 0) {
        ...
        /* Get transmit queue */
        txq = bcm56860_a0_uc_queue_num(unit, pkt->port, 0);

        /* Fill out stream-of-bytes module header */
        sob = TX_DCB_MODULE_HEADERf_PTR(*dcb);
        sob[0] = 0x81000000;
        sob[1] = LSHIFT32(P2L(unit, pkt->port) & 0x7f, 2);
        sob[2] = LSHIFT32(txq & 0xfff, 8) | 0x00100000;
    }
}
...
int 
bcm56860_a0_bmd_tx(int unit, const bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    TX_DCB_t *dcb;
    dma_addr_t bdcb;
