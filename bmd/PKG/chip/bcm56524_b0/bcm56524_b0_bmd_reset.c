...
static int
bcm56524_b0_hypercore_phy_init(int unit, int port)
{
    int ioerr = 0;
...
int
bcm56524_b0_xport_reset(int unit, int port)
{
    int ioerr = 0;
    int msec;
    int idx;
    ...
    if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
        ioerr += bcm56524_b0_hypercore_phy_init(unit, port);
    }
#endif
    ...
    if (msec >= PLL_LOCK_MSEC) {
        CDK_WARN(("bcm56524_b0_xport_reset[%d]: "
...
static int
bcm56524_b0_lcpll_init(int unit)
{
    int ioerr = 0;
    int msec;
    ...
    if (msec >= PLL_LOCK_MSEC) {
        CDK_WARN(("bcm56524_b0_bmd_reset[%d]: "
...
int
bcm56524_b0_bmd_reset(int unit)
{
    int ioerr = 0;
    int wait_usec = 10000;
    int port;
    ...
    /* Initialize LC PLLs */
    ioerr += bcm56524_b0_lcpll_init(unit);
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ioerr += bcm56524_b0_xport_reset(unit, port);
    }
