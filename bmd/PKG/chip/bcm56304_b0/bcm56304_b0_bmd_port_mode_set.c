...
int
bcm56304_b0_bmd_port_mode_set(int unit, int port, 
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
            if (CDK_SUCCESS(rv)) {
                 ...
                 */
                rv = bcm56304_b0_bmd_port_mode_get(unit, port, 
                                                   &cur_mode, &cur_flags);
                if (CDK_SUCCESS(rv)) {
                    if (mode != cur_mode || 
                        ...
                        rv = bcm56304_b0_xport_reset(unit, port, speed);
                        if (CDK_SUCCESS(rv)) {
                            rv = bcm56304_b0_xport_init(unit, port);
                        }
                    }
                }
            }
