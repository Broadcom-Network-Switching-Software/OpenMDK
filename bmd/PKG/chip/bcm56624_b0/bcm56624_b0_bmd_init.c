...
int
bcm56624_b0_xport_init(int unit, int port)
{
    int ioerr = 0;
...
bcm56624_b0_bmd_init(int unit)
{
    ...
    /* Configure egress scheduler for 2.5 Gbps or 16 Gbps */
    ioerr += READ_ESTDMCONFIGr(unit, &estdmconfig);
    if (CDK_XGS_FLAGS(unit) & CHIP_FLAG_XG01_16G) {
        ESTDMCONFIGr_GROUP0_MODEf_SET(estdmconfig, tdm_mode_sel);
        ESTDMCONFIGr_GROUP1_MODEf_SET(estdmconfig, tdm_mode_sel);
        xg_modes[0] = xg_mode_sel;
        xg_modes[1] = xg_mode_sel;
    } else if (CDK_XGS_FLAGS(unit) & CHIP_FLAG_NO_ESM) {
        ESTDMCONFIGr_GROUP0_MODEf_SET(estdmconfig, 3);
        ESTDMCONFIGr_GROUP1_MODEf_SET(estdmconfig, 3);
        xg_modes[0] = 3;
        xg_modes[1] = 3;
    }
    if (CDK_XGS_FLAGS(unit) & CHIP_FLAG_XG23_16G) {
        ESTDMCONFIGr_GROUP2_MODEf_SET(estdmconfig, tdm_mode_sel);
        ESTDMCONFIGr_GROUP3_MODEf_SET(estdmconfig, tdm_mode_sel);
        xg_modes[2] = xg_mode_sel;
        xg_modes[3] = xg_mode_sel;
    } else if (CDK_XGS_FLAGS(unit) & CHIP_FLAG_NO_ESM) {
        ESTDMCONFIGr_GROUP2_MODEf_SET(estdmconfig, 3);
        ESTDMCONFIGr_GROUP3_MODEf_SET(estdmconfig, 3);
        xg_modes[2] = 3;
        xg_modes[3] = 3;
    }
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56624_b0_bmd_init[%d]: IPIPE reset timeout\n", unit));
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56624_b0_bmd_init[%d]: EPIPE reset timeout\n", unit));
        return ioerr ? CDK_E_IO : CDK_E_TIMEOUT;
    }
    ...
    CDK_PBMP_ITER(pbmp, port) {
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
            ioerr += bcm56624_b0_xport_init(unit, port);
        } else {
            ioerr += _gport_init(unit, port);
        }
        if (CDK_SUCCESS(rv)) {
