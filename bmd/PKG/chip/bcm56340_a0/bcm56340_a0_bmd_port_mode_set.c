...
int 
bcm56340_a0_bmd_port_mode_set(int unit, int port, 
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        if (cnt < 0) {
            CDK_WARN(("bcm56340_a0_bmd_port_mode_set[%d]: "
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
             ...
             */
            rv = bcm56340_a0_bmd_port_mode_get(unit, port, 
                                               &cur_mode, &cur_flags);
            if (CDK_SUCCESS(rv) && 
                ...
                rv = bcm56340_a0_warpcore_phy_init(unit, port);
            }
        }
#endif
    }
    ...
    if (mode == bmdPortModeDisabled) {
        ...
        if (phy_mode != old_phy_mode) {
            ...
            if (lanes == 2) {
                ...
                if (CDK_SUCCESS(rv)) {
                    rv = bcm56340_a0_xport_init(unit, np);
                }
                if (CDK_SUCCESS(rv)) {
