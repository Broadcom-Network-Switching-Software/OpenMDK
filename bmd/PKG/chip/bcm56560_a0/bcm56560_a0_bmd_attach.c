...
static phy_bus_t *bcm56560_phy_bus[] = {
#ifdef PHY_BUS_BCM56560_MIIM_INT_INSTALLED
    &phy_bus_bcm56560_miim_int,
#endif
    NULL
};
...
_mmu_ports_assign(int unit)
{
    ...
    CDK_XGSD_BLKTYPE_PBMP_GET(unit, BLKTYPE_XLPORTB, &pbmp_add);
    ...
    CDK_PBMP_ITER(pbmp, phy_port) {
        ...
        if (bcm56560_a0_port_in_eq_bmp(unit, phy_port)) {
    ...
    /* Assign MMU port for 1st lane OS phy_port with speed >= 50G */
    for (phy_port = 1; phy_port <= 72; phy_port++) {
        if (_mmu_port_map[unit].map[phy_port] != -1) {
            continue;
        }

        if (P2L(unit, phy_port) == -1 || ((phy_port - 1) % PORTS_PER_TSC)) {
            continue;
        }

        if ((CDK_PORT_CONFIG_PORT_FLAGS(unit, phy_port) & CDK_DCFG_PORT_F_OVERSUB) &&
            (SPEED_MAX(unit, phy_port) >= 50000)) {
            _mmu_port_map[unit].map[phy_port] = mmu_port;
            mmu_port++;
        }
    }

    /* Assign MMU port for all over-sub ports with speed = 40G/42G */
    for (phy_port = 1; phy_port <= 72; phy_port++) {
        if (_mmu_port_map[unit].map[phy_port]!= -1) {
            continue;
        }
    ...
    /* Assign MMU port for all line-rate ports with speed >= 40G */
    for (phy_port = 1; phy_port <= 72; phy_port++) {
        if (_mmu_port_map[unit].map[phy_port] != -1) {
            continue;
        }
    ...
    for (phy_port = 1; phy_port <= 72; phy_port+= PORTS_PER_TSC) {
        ...
        if (bcm56560_a0_port_is_falcon(unit, phy_port)) {
            _mmu_port_map[unit].map[phy_port] = mmu_port;
            mmu_port++;
        }
    }
    ...
    for (phy_port = 1; phy_port <= 72; phy_port+= PORTS_PER_TSC) {
        ...
        if (bcm56560_a0_p2l(unit, phy_port, 0) == -1) {
            continue;
        }
...
_q_num_config(int unit)
{
    ...
    for (mport_offset = 0; mport_offset < PORTS_PER_PIPE; mport_offset++) {
        ...
        mc_queues = bcm56560_a0_mmu_port_mc_queues(unit, lport);
        _mc_q_base[unit][lport] = mc_base + mport_offset * mc_queues;
        uc_queues = bcm56560_a0_mmu_port_uc_queues(unit, phy_port);
...
bcm56560_a0_port_lanes_get(int unit, int port)
{
    ...
    if (bcm56560_a0_port_is_falcon(unit, port)) {
...
bcm56560_a0_xlbport_pbmp_get(int unit, cdk_pbmp_t *pbmp)
{
    ...
    CDK_XGSD_BLKTYPE_PBMP_GET(unit, BLKTYPE_XLPORTB, pbmp);
...
bcm56560_a0_mmu_port_uc_queue_index(int unit, int port)
{
    ...
    if (bcm56560_a0_port_in_eq_bmp(unit, port)) {
        idx = bcm56560_a0_mmu_port_uc_queues(unit, port) * mport;
    } else {
        idx = bcm56560_a0_mmu_port_uc_queues(unit, port) * mport;
    }
...
int
bcm56560_a0_bmd_attach(int unit)
{
    int port;
    int port_mode;
    uint32_t speed_max;
...
#if BMD_CONFIG_INCLUDE_PHY == 1
    phy_bus_t **phy_bus = bcm56560_phy_bus;
#endif
    ...
    BMD_PORT_SPEED_MAX(unit) = bcm56560_a0_port_speed_max;
    BMD_PORT_P2L(unit) = bcm56560_a0_p2l;
    BMD_PORT_P2M(unit) = bcm56560_a0_p2m;
