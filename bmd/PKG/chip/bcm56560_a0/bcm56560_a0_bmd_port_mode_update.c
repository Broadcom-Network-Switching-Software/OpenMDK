...
int 
bcm56560_a0_bmd_port_mode_update(int unit, int port)
{
    ...
    if (CDK_SUCCESS(rv) && status_change) {
        rv = bcm56560_a0_bmd_port_mode_get(unit, port, &mode, &flags);
        if (CDK_SUCCESS(rv)) {
            rv = bmd_phy_autoneg_get(unit, port, &autoneg);
            if (CDK_SUCCESS(rv) && autoneg) {
                flags |= BMD_PORT_MODE_F_INTERNAL;
                rv = bcm56560_a0_bmd_port_mode_set(unit, port, mode, flags);
            }
        }
