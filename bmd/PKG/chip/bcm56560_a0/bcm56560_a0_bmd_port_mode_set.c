...
void
bcm56560_a0_xlbmac_drain_cell(unit, port)
{
    int ioerr = 0;
    int cnt;
    XLMAC_B0_TXFIFO_CELL_CNTr_t xlbcell_cnt;    
    
    /* Wait until TX fifo cell count is 0 */
    cnt = DRAIN_WAIT_MSEC / 10;
    while (--cnt >= 0) {
        ioerr += READ_XLMAC_B0_TXFIFO_CELL_CNTr(unit, port, &xlbcell_cnt);
        if (XLMAC_B0_TXFIFO_CELL_CNTr_CELL_CNTf_GET(xlbcell_cnt) == 0) {
            break;
        }
        BMD_SYS_USLEEP(10000);
    }
    if (cnt < 0) {
        CDK_WARN(("bcm56560_a0_bmd_port_mode_set[%d]: "
...
void
bcm56560_a0_xlmac_drain_cell(unit, port)
{
    int ioerr = 0;
    int cnt;
...
void
bcm56560_a0_clmac_drain_cell(unit, port)
{
    int ioerr = 0;
    int cnt;
...
int
bcm56560_a0_bmd_port_mode_set(int unit, int port, 
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    XLMAC_B0_EEE_CTRLr_t xlbmac_eee_ctrl;
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        if (mac_lb || phy_lb) {
            if ((speed_max >= 100000) || CDK_PBMP_MEMBER(clpbmp, port)) {
                bcm56560_a0_clmac_drain_cell(unit, port);
         ...
         */
        rv = bcm56560_a0_bmd_port_mode_get(unit, port, 
                                           &cur_mode, &cur_flags);
        ...
        if (CDK_SUCCESS(rv) && 
            ...
            if ((speed_max >= 100000) || CDK_PBMP_MEMBER(clpbmp, port)) {
                rv = bcm56560_a0_clport_init(unit, port);
            } else {
                rv = bcm56560_a0_xlport_init(unit, port);
            }
        }
    }
    ...
    if ((speed_max >= 100000) || CDK_PBMP_MEMBER(clpbmp, port)) {
        ...
        XLMAC_B0_CTRLr_SOFT_RESETf_SET(xlbmac_ctrl, 1);
        ioerr += WRITE_XLMAC_B0_CTRLr(unit, port, xlbmac_ctrl);
    
        /* Disable MACs (Rx only) */
        ioerr += READ_XLMAC_B0_CTRLr(unit, port, &xlbmac_ctrl);
        XLMAC_B0_CTRLr_RX_ENf_SET(xlbmac_ctrl, 0);
        ioerr += WRITE_XLMAC_B0_CTRLr(unit, port, xlbmac_ctrl);
    }
    ...
    if (mode == bmdPortModeDisabled) {
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
            ...
            if ((speed_max >= 100000) || CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                ioerr += READ_XLMAC_B0_MODEr(unit, port, &xlbmac_mode);
                ioerr += READ_XLPORT_CONFIGr(unit, port, &xlport_cfg);
            }
            ...
            if ((speed_max >= 100000) || CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                XLMAC_B0_MODEr_HDR_MODEf_SET(xlbmac_mode, hg2 ? 2 : hg);
            }
            ...
            if ((speed_max >= 100000) || CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                XLPORT_CONFIGr_HIGIG2_MODEf_SET(xlport_cfg, hg2);
                ioerr += WRITE_XLMAC_B0_MODEr(unit, port, xlbmac_mode);
                ioerr += WRITE_XLPORT_CONFIGr(unit, port, xlport_cfg);
            }
            ...
            if ((speed_max >= 100000) || CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                /* Configure MAC mode */
                ioerr += READ_XLMAC_B0_MODEr(unit, port, &xlbmac_mode);
                XLMAC_B0_MODEr_SPEED_MODEf_SET(xlbmac_mode, sp_sel);
                ioerr += WRITE_XLMAC_B0_MODEr(unit, port, xlbmac_mode);

                /* Configure EEE */
                ioerr += READ_XLMAC_B0_EEE_CTRLr(unit, port, &xlbmac_eee_ctrl);
                if (flags & BMD_PORT_MODE_F_EEE) {
                    /* Enable IEEE 802.3az EEE */
                    XLMAC_B0_EEE_CTRLr_EEE_ENf_SET(xlbmac_eee_ctrl, 1);
                } else {
                    XLMAC_B0_EEE_CTRLr_EEE_ENf_SET(xlbmac_eee_ctrl, 0);
                }
                ioerr += WRITE_XLMAC_B0_EEE_CTRLr(unit, port, xlbmac_eee_ctrl);
            }
            ...
            ioerr += bcm56560_a0_egr_buf_reset(unit, port, 0);
            ...
            if ((speed_max >= 100000) || CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                ioerr += READ_XLMAC_B0_CTRLr(unit, port, &xlbmac_ctrl);
                XLMAC_B0_CTRLr_SOFT_RESETf_SET(xlbmac_ctrl, 0);
                XLMAC_B0_CTRLr_RX_ENf_SET(xlbmac_ctrl, 1);
                XLMAC_B0_CTRLr_TX_ENf_SET(xlbmac_ctrl, 1);
                XLMAC_B0_CTRLr_LOCAL_LPBKf_SET(xlbmac_ctrl, mac_lb);
                if (flags & HG_FLAGS) {
                    XLMAC_B0_CTRLr_XGMII_IPG_CHECK_DISABLEf_SET(xlbmac_ctrl, 1);
                } else {
                    XLMAC_B0_CTRLr_XGMII_IPG_CHECK_DISABLEf_SET(xlbmac_ctrl, 0);
                }
                ioerr += WRITE_XLMAC_B0_CTRLr(unit, port, xlbmac_ctrl);
            }
        }
