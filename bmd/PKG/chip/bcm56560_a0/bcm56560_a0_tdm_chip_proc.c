...
#include "bcm56560_a0_tdm_chip_vec.h"
#include <cdk/cdk_debug.h>

int tdm_ap_core_vec_load(unsigned short **map, int bw, int port_speed, int yy, int len, int num_ext_ports);
 ...
 */
int
tdm_ap_lls_scheduler(struct ap_ll_node *pgw_tdm, tdm_ap_chip_legacy_t *ap_chip, ap_pgw_pntrs_t *pntrs_pkg, ap_pgw_scheduler_vars_t *vars_pkg, int *pgw_tdm_tbl[AP_LR_LLS_LEN], int *ovs_tdm_tbl[AP_OS_LLS_GRP_LEN], int op_flags[3])
{
  ...
  l1_tdm_len = (num_lr_1+(num_lr_10*2)+(num_lr_12*3)+(num_lr_20*4)+(num_lr_25*5)+(num_lr_40*8)+(num_lr_50*10)+(num_lr_100*20));
   ...
   /*Falcon core*/
              if( (wc_array[(*pntrs_pkg).cur_idx][(*vars_pkg).subport]!=17  && wc_array[(*pntrs_pkg).cur_idx][(*vars_pkg).subport]!=53) &&                 ((*ap_chip).tdm_globals.speed[wc_array[(*pntrs_pkg).cur_idx][(*vars_pkg).subport]] == SPEED_100G ||
   ...
   /*Falcon core*/
            ...
            if((wc_array[(*pntrs_pkg).cur_idx][(*vars_pkg).subport]!=17  && wc_array[(*pntrs_pkg).cur_idx][(*vars_pkg).subport]!=53) &&
...
int i, j, k,cnt=0,tmp_div=0, cnt1=0, size=AP_OS_LLS_GRP_LEN;
...
int token=port;
 ...
 for(j=0; j<AP_OS_LLS_GRP_LEN; j++) { uniq_elements_tbl[j] = AP_NUM_EXT_PORTS;}
   ...
   size = i;
    ...
    for(i= ((size/tmp_div)-1); i<size; i+= (size/tmp_div) ) {
      for(k=size-1; k>=i; k--){
