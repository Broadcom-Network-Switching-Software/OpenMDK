...
_cpu_port_enable_set(int unit, int enable)
{
    ...
    EPC_LINK_BMAPm_PORT_BITMAPf_GET(epc_link, epc_pbm);
    if (enable) {
        PBM_PORT_ADD(epc_pbm, CMIC_PORT);    
    } else {
        PBM_PORT_REMOVE(epc_pbm, CMIC_PORT); 
    }
    EPC_LINK_BMAPm_PORT_BITMAPf_SET(epc_link, epc_pbm);
...
int
bcm56560_a0_bmd_rx_start(int unit, bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    dma_addr_t bdcb;
...
int
bcm56560_a0_bmd_rx_poll(int unit, bmd_pkt_t **ppkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    bmd_pkt_t *pkt;
...
int
bcm56560_a0_bmd_rx_stop(int unit)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    int rv = CDK_E_NONE;
