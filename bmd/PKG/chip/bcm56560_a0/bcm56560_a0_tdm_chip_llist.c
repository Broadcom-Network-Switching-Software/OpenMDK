  ...
  if ((speed[0] == SPEED_25G) && (speed[1]== SPEED_25G)) {
   ...
   temp_port=cl_token;
    ...
    while (pool>0 && (--timeout)>0) {
     ...
     if ((speed[0] == SPEED_25G) && (speed[1]== SPEED_25G)) {
      tdm_ap_ll_append_25(list,port[1]);
     }
      tdm_ap_ll_append_25(list,cl_token);
   }
...
void
tdm_ap_ll_append_25(struct ap_ll_node *llist, int token)
{
    int i, cnt=0;
    struct ap_ll_node *list = llist;
  int port=token;
...
/*average out the spacing of 10G ports for cut thru support*/
   if((speed[0] == SPEED_100G || speed[0] == SPEED_50G || speed[0] == SPEED_25G ) ||
      (speed[1] == SPEED_100G || speed[1] == SPEED_50G || speed[1] == SPEED_25G )) {
    for( i = 0 ; i<len; i++) {
          pos=0;
          token=0;
      ...
      ((*ap_chip).tdm_globals.speed[tdm_ap_ll_get(list,i)] >= SPEED_10G && (*ap_chip).tdm_globals.speed[tdm_ap_ll_get(list,i)] <= SPEED_40G)) {
          ...
          avg = ((*ap_chip).tdm_globals.speed[token]== SPEED_40G)? (len/8):((*ap_chip).tdm_globals.speed[token]== SPEED_20G) ? (len/4) : (len/2);
        for (j=i+1; j<len; j++) {
