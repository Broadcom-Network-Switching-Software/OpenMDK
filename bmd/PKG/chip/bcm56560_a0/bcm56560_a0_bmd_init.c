...
_total_bandwidth(int unit)
{
    ...
    bcm56560_a0_all_pbmp_get(unit, &pbmp);
    CDK_PBMP_ITER(pbmp, port) {
        total_bw += bcm56560_a0_port_speed_max(unit, port);
    }
...
_core_bandwidth(int unit)
{
    ...
    int core_freq = bcm56560_a0_get_core_frequency(unit);
...
_port_map_init(int unit)
{
    ...
    for (lport = 0; lport < num_lport; lport++) {
        ...
        IFP_GM_LOGICAL_TO_PHYSICAL_MAPPINGr_PHYSICAL_PORT_NUMf_SET(ifp_l2p, 
                                                (P2M(unit, port) & 0x7f));
...
int
bcm56560_a0_port_is_falcon(int unit, int port)
{
...
_tdm_init(int unit)
{
    ...
    bcm56560_a0_all_pbmp_get(unit, &all_pbmp);
    CDK_PBMP_ITER(all_pbmp, port) {
        phy_port = L2P(unit, port);    
        speed = bcm56560_a0_port_speed_max(unit, phy_port);
        if (speed == 0) {
            continue;
        }
        ...
        if (bcm56560_a0_port_is_falcon(unit, phy_port)) { /*  Falcons  */
    ...
    bcm56560_a0_sel_tdm(chip_pkg, tdm_pkg);
    if (NULL == bcm56560_a0_set_tdm_tbl(tdm_pkg)) {
        CDK_ERR(("bcm56560_a0_init[%d]: Unable to configure TDM\n", unit));
        return CDK_E_FAIL;
    }
    ...
    rv = bcm56560_a0_set_iarb_tdm_table(tcfg->tdm_bw,
                                        tcfg->pipe_ovs_state[0],
                                        &tcfg->iarb_tdm_wrap_ptr,
                                        tcfg->iarb_tdm[0]);
    if (rv == 0) {
        CDK_ERR(("bcm56560_a0_init[%d]: Unable to configure IARB TDM\n", unit));
        return CDK_E_FAIL;
    }
 ...
 */
        ...
        for (base = 0; base < PGW_TDM_OVS_SIZE; base += PGW_TDM_SLOTS_PER_REG) {
            ...
            if (bcm56560_a0_port_speed_max(unit, port) >= 40000) {
                ...
                for (pgw_cl = 0; pgw_cl < 2; pgw_cl++) {
                    ...
                    PGW_HSP_CONFIGr_ENTRIES_IN_CALf_SET(pgw_hsp_cfg, 
                        bcm56560_a0_port_speed_max(unit, port)/1000);
    ...
    for (group = 0; group < MMU_OVS_GROUP_COUNT; group++) {
        ...
        speed = bcm56560_a0_port_speed_max(unit, port);
    ...
    core_freq = bcm56560_a0_get_core_frequency(unit);
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ...
        if (oversub) {
            ...
            if (mport < 32) {          /* mport in [0, 32)  */
                values[0] |= (1 << mport);
            } else if (mport < 64) {   /* mport in [32, 64) */ 
                values[1] |= (1 << (mport - 32));
            } else {                      /* mport in [64, 74)  */
                values[2] |= (1 << (mport - 64));
            }
    ...
    *mode = 2;
    ...
    for (lane = 0; lane < PORTS_PER_XLP; lane++) {
        ...
        if (lport == -1) {
            ...
            num_lanes[lane] = bcm56560_a0_port_lanes_get(unit, port);
...
_port_blk_ca_mode_set(int unit, int flexport, int xlp)
{
    ...
    blk = bcm56560_a0_pgw_clport_block_index_get(unit, xlp, &obm, &subport);  
...
_idb_init(int unit)
{
    ...
    /* Per-port settings */
    bcm56560_a0_all_front_pbmp_get(unit, &all_pbmp);
    CDK_PBMP_ITER(all_pbmp, port) {
...
int
bcm56560_a0_egr_buf_reset(int unit, int port, int reset)
{
    int ioerr = 0;
...
_port_speed_update(int unit, int port, int speed)
{
    ...
    if (bcm56560_a0_port_in_eq_bmp(unit, port) && core_freq < 500) {
        ioerr += READ_TOQ_MC_CFG1r(unit, &toq_mc_cfg1);
        fval = TOQ_MC_CFG1r_IS_MC_T2OQ_PORT0f_GET(toq_mc_cfg1);
        if (speed >= 30000) {
            fval |= 1 << (mport & 0x3f);
         ...
         */
        if (speed < 30000) {
            qnum = bcm56560_a0_mmu_port_mc_queues(unit, port);
            for (idx = 0; idx < qnum; idx++) {
                cosq = bcm56560_a0_mc_queue_num(unit, port, idx);
    ...
    if (bcm56560_a0_port_in_eq_bmp(unit, port)) {
...
_default_lossless_pg_headroom(int unit, int port, int *headroom, int *guarantee)
{
    ...
    bcm56560_a0_ovrs_pbmp_get(unit, &oversub_pbm);
...
_mmu_config_init(int unit, int input_port)
{
    ...
    CFAPFULLRESETPOINTr_CFAPFULLRESETPOINTf_SET(cfap_reset_point, 
                                CFAPFULLSETPOINT - 2 * jumbo_frame_cells);
    ...
    MMU_THDM_DB_POOL_YELLOW_SHARED_LIMITr_SET(db_ysh_lim, (pool_limit + 7)/8);
    ioerr += WRITE_MMU_THDM_DB_POOL_YELLOW_SHARED_LIMITr(unit, 0, db_ysh_lim);    
    
    MMU_THDM_DB_POOL_RED_SHARED_LIMITr_SET(db_rsh_lim, (pool_limit + 7)/8);
    ioerr += WRITE_MMU_THDM_DB_POOL_RED_SHARED_LIMITr(unit, 0, db_rsh_lim); 

    MMU_THDM_DB_POOL_RESUME_LIMITr_SET(db_rsm_lim, (pool_limit + 7)/8);
    ioerr += WRITE_MMU_THDM_DB_POOL_RESUME_LIMITr(unit, 0, db_rsm_lim);    

    MMU_THDM_DB_POOL_YELLOW_RESUME_LIMITr_SET(db_yrsm_lim, (pool_limit + 7)/8);
    ioerr += WRITE_MMU_THDM_DB_POOL_YELLOW_RESUME_LIMITr(unit, 0, db_yrsm_lim);

    MMU_THDM_DB_POOL_RED_RESUME_LIMITr_SET(db_rrsm_lim, (pool_limit + 7)/8);
    ...
    MMU_THDU_XPIPE_CONFIG_QGROUPm_LIMIT_YELLOW_CELLf_SET(cfg_qgrp_x, (pool_limit + 7)/8);
    MMU_THDU_XPIPE_CONFIG_QGROUPm_LIMIT_RED_CELLf_SET(cfg_qgrp_x, (pool_limit + 7)/8);

    MMU_THDU_XPIPE_OFFSET_QGROUPm_CLR(off_qgrp_x);
    pool_resume = default_mtu_cells * 2;
    MMU_THDU_XPIPE_OFFSET_QGROUPm_RESET_OFFSET_CELLf_SET(off_qgrp_x, pool_resume/8);
    MMU_THDU_XPIPE_OFFSET_QGROUPm_RESET_OFFSET_YELLOW_CELLf_SET(off_qgrp_x, pool_resume/8);
    MMU_THDU_XPIPE_OFFSET_QGROUPm_RESET_OFFSET_RED_CELLf_SET(off_qgrp_x, pool_resume/8);
 ...
 */
    CDK_PBMP_ITER(all_pbmp, port) {
        ...
        for (idx = 0; idx < numq; idx++) {
            ...
            MMU_THDM_DB_QUEUE_CONFIG_0m_YELLOW_SHARED_LIMITf_SET(db_q_cfg_x, (pool_limit + 7) / 8);
            MMU_THDM_DB_QUEUE_CONFIG_0m_RED_SHARED_LIMITf_SET(db_q_cfg_x, (pool_limit + 7) / 8);
            MMU_THDM_DB_QUEUE_CONFIG_0m_Q_SPIDf_SET(db_q_cfg_x, 0);
        
            MMU_THDM_DB_QUEUE_OFFSET_0m_CLR(db_q_off_x);
            MMU_THDM_DB_QUEUE_OFFSET_0m_RESUME_OFFSETf_SET(db_q_off_x, (default_mtu_cells * 2)/8);
            ...
            MMU_THDM_MCQE_QUEUE_CONFIG_0m_Q_SHARED_LIMITf_SET(mcqe_q_cfg_x, mcqe_limit/4);
            MMU_THDM_MCQE_QUEUE_CONFIG_0m_YELLOW_SHARED_LIMITf_SET(mcqe_q_cfg_x, (mcqe_limit + 7)/8);
            MMU_THDM_MCQE_QUEUE_CONFIG_0m_RED_SHARED_LIMITf_SET(mcqe_q_cfg_x, (mcqe_limit + 7)/8);
        ...
        CDK_PBMP_ITER(all_pbmp, port) {
            ...
            ioerr += WRITE_MMU_THDM_MCQE_PORTSP_CONFIG_0m(unit, port, mcqe_psp_cfg_x);
        }
    }
    ...
    CDK_PBMP_ITER(all_pbmp, port) {
        ...
        MMU_THDU_XPIPE_CONFIG_QUEUEm_LIMIT_RED_CELLf_SET(cfg_q_x, (pool_limit + 7)/8);
        MMU_THDU_XPIPE_CONFIG_QUEUEm_LIMIT_YELLOW_CELLf_SET(cfg_q_x, (pool_limit + 7)/8);
    
        MMU_THDU_XPIPE_OFFSET_QUEUEm_CLR(off_q_x);
        MMU_THDU_XPIPE_OFFSET_QUEUEm_RESET_OFFSET_CELLf_SET(off_q_x, pool_resume / 8);
        
        MMU_THDU_XPIPE_OFFSET_QUEUEm_RESET_OFFSET_YELLOW_CELLf_SET(off_q_x, pool_resume / 8);
        MMU_THDU_XPIPE_OFFSET_QUEUEm_RESET_OFFSET_RED_CELLf_SET(off_q_x, pool_resume / 8);
    ...
    MMU_THDR_DB_RESET_OFFSET_COLOR_PRIQr_RESET_OFFSET_YELLOWf_SET(db_lcol_pq, (default_mtu_cells * 2)/8);
    MMU_THDR_DB_RESET_OFFSET_COLOR_PRIQr_RESET_OFFSET_REDf_SET(db_lcol_pq, (default_mtu_cells * 2)/8);
...
_mmu_lls_init(int unit)
{
    ...
    if (idx >= LLS_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56560_a0_bmd_init[%d]: LLS INIT timeout\n", unit));
                ...
                if (idx_0 <= 5) {
                    ...
                    if ((idx < LB_MPORT) && (idx_0 >= 8)) {
                        ...
                        LLS_L1_CONFIGm_P_START_UC_SPRIf_SET(l1_config, 
                            bcm56560_a0_mmu_port_uc_queue_index(unit, L2P(unit, lport)));
                        LLS_L1_CONFIGm_P_START_MC_SPRIf_SET(l1_config, 
                            MC_Q_BASE + P2M(unit, idx_0 - 8 + 1) * 10);
            ...
            for (idx_0 = 0; idx_0 <= index_max; idx_0++) { 
                ...
                if ((idx_0 >= MC_Q_BASE_CMIC_PORT) && 
                    ...
                    if (idx_0 < MC_Q_BASE) {
                        ...
                        txq = bcm56560_a0_mmu_port_uc_queues(unit, 1);
                        mport = idx_0 / txq;
                        if (idx_0 < (LB_MPORT * txq + 1)) {
                            fval = M2P(unit, mport) + (8 - 1);
...
_xl_firmware_helper(void *ctx, uint32_t offset, uint32_t size, void *data)
{
    ...
    for (idx = 0; idx < fw_size; idx += 16) {
        ...
        for (wdx = 0; wdx <= 3; wdx++) {
            ...
            XLPORT_WC_UCMEM_DATAm_SET(xl_ucmem_data, wdx^3, wdata);
        }
...
_cxx_firmware_helper(void *ctx, uint32_t offset, uint32_t size, void *data)
{
    ...
    /* Enable parallel bus access */
    if (port >= 17 && port <= 20) {
        ioerr += READ_CXXPORT_WC_UCMEM_CTRLr(unit, 17, 0, &cxx_ucmem_ctrl);
        CXXPORT_WC_UCMEM_CTRLr_ACCESS_MODEf_SET(cxx_ucmem_ctrl, 1);
        ioerr += WRITE_CXXPORT_WC_UCMEM_CTRLr(unit, 17, 0, cxx_ucmem_ctrl);
    } else if (port >= 21 && port <= 24) {
        ioerr += READ_CXXPORT_WC_UCMEM_CTRLr(unit, 17, 1, &cxx_ucmem_ctrl);
        CXXPORT_WC_UCMEM_CTRLr_ACCESS_MODEf_SET(cxx_ucmem_ctrl, 1);
        ioerr += WRITE_CXXPORT_WC_UCMEM_CTRLr(unit, 17, 1, cxx_ucmem_ctrl);
    } else if (port >= 25 && port <= 28) {
        ioerr += READ_CXXPORT_WC_UCMEM_CTRLr(unit, 17, 2, &cxx_ucmem_ctrl);
        CXXPORT_WC_UCMEM_CTRLr_ACCESS_MODEf_SET(cxx_ucmem_ctrl, 1);
        ioerr += WRITE_CXXPORT_WC_UCMEM_CTRLr(unit, 17, 2, cxx_ucmem_ctrl);
    } else if (port >= 53 && port <= 56) {
        ioerr += READ_CXXPORT_WC_UCMEM_CTRLr(unit, 53, 0, &cxx_ucmem_ctrl);
        CXXPORT_WC_UCMEM_CTRLr_ACCESS_MODEf_SET(cxx_ucmem_ctrl, 1);
        ioerr += WRITE_CXXPORT_WC_UCMEM_CTRLr(unit, 53, 0, cxx_ucmem_ctrl);
    } else if (port >= 57 && port <= 60) {
        ioerr += READ_CXXPORT_WC_UCMEM_CTRLr(unit, 53, 1, &cxx_ucmem_ctrl);
        CXXPORT_WC_UCMEM_CTRLr_ACCESS_MODEf_SET(cxx_ucmem_ctrl, 1);
        ioerr += WRITE_CXXPORT_WC_UCMEM_CTRLr(unit, 53, 1, cxx_ucmem_ctrl);
    } else {
        ioerr += READ_CXXPORT_WC_UCMEM_CTRLr(unit, 53, 2, &cxx_ucmem_ctrl);
        CXXPORT_WC_UCMEM_CTRLr_ACCESS_MODEf_SET(cxx_ucmem_ctrl, 1);
        ioerr += WRITE_CXXPORT_WC_UCMEM_CTRLr(unit, 53, 2, cxx_ucmem_ctrl);
    }
    ...
    for (idx = 0; idx < fw_size; idx += 16) {
        ...
        for (wdx = 0; wdx <= 3; wdx++) {
            ...
            if ((port >= 17 && port <= 20) || (port >= 53 && port <= 56)) {
                CXXPORT_WC_UCMEM_DATA0m_SET(cxx_ucmem_data0, wdx^3, wdata);
            } else if ((port >= 21 && port <= 24) || (port >= 57 && port <= 60)) {
        ...
        if (port >= 17 && port <= 20) {
            WRITE_CXXPORT_WC_UCMEM_DATA0m(unit, idx >> 4, cxx_ucmem_data0, 17);
        } else if (port >= 21 && port <= 24) {
            WRITE_CXXPORT_WC_UCMEM_DATA1m(unit, idx >> 4, cxx_ucmem_data1, 17);
        } else if (port >= 25 && port <= 28) {
            WRITE_CXXPORT_WC_UCMEM_DATA2m(unit, idx >> 4, cxx_ucmem_data2, 17);
        } else if (port >= 53 && port <= 56) {
            WRITE_CXXPORT_WC_UCMEM_DATA0m(unit, idx >> 4, cxx_ucmem_data0, 53);
        } else if (port >= 57 && port <= 60) {
            WRITE_CXXPORT_WC_UCMEM_DATA1m(unit, idx >> 4, cxx_ucmem_data1, 53);
        } else {
            WRITE_CXXPORT_WC_UCMEM_DATA2m(unit, idx >> 4, cxx_ucmem_data2, 53);
        }
    }
...
_phy_default_ability_set(int unit, int port)
{
    ...
    speed_max = bcm56560_a0_port_speed_max(unit, port);    
    if (speed_max == 10000) {
...
int
bcm56560_a0_clport_init(int unit, int port)
{
    int ioerr = 0;
    int speed;
    uint32_t val;
...
int
bcm56560_a0_xlport_init(int unit, int port)
{
    int ioerr = 0;
    int speed;
    uint32_t val;
    ...
    XLMAC_B0_TX_CTRLr_t xlp_tx_ctrl;
    XLMAC_RX_CTRLr_t rx_ctrl;
    XLMAC_B0_RX_CTRLr_t xlp_rx_ctrl;
    XLMAC_RX_MAX_SIZEr_t rx_max_size;
    XLMAC_B0_RX_MAX_SIZEr_t xlp_rx_max_size;
    XLMAC_RX_LSS_CTRLr_t rx_lss_ctrl;
    XLMAC_B0_RX_LSS_CTRLr_t xlp_rx_lss_ctrl;
    XLMAC_CTRLr_t mac_ctrl;
    XLMAC_B0_CTRLr_t xlb_mac_ctrl;
    XLMAC_MODEr_t mac_mode;
    XLMAC_B0_MODEr_t xlp_mac_mode;
    EGR_PORT_CREDIT_RESETm_t egr_port_credit_reset;
    cdk_pbmp_t xlpbmp;
    ...
    if (CDK_PBMP_MEMBER(xlpbmp, port)) {
        ...
        XLMAC_B0_CTRLr_CLR(xlb_mac_ctrl);
        XLMAC_B0_CTRLr_SOFT_RESETf_SET(xlb_mac_ctrl, 1);
        ioerr += WRITE_XLMAC_B0_CTRLr(unit, port, xlb_mac_ctrl);
    }
    /* Reset EP credit before de-assert SOFT_RESET */
    ioerr += READ_EGR_PORT_CREDIT_RESETm(unit, port, &egr_port_credit_reset);
    EGR_PORT_CREDIT_RESETm_VALUEf_SET(egr_port_credit_reset, 1);
    ...
    if (CDK_PBMP_MEMBER(xlpbmp, port)) {
        ...
        XLMAC_B0_CTRLr_SOFT_RESETf_SET(xlb_mac_ctrl, 0);
        XLMAC_B0_CTRLr_RX_ENf_SET(xlb_mac_ctrl, 0);
        XLMAC_B0_CTRLr_TX_ENf_SET(xlb_mac_ctrl, 0);
        val = (BMD_PORT_PROPERTIES(unit, port) == BMD_PORT_HG) ? 1 : 0;
        XLMAC_B0_CTRLr_XGMII_IPG_CHECK_DISABLEf_SET(xlb_mac_ctrl, val);
        ioerr += WRITE_XLMAC_B0_CTRLr(unit, port, xlb_mac_ctrl);
    
        /* Configure Rx (strip CRC, strict preamble, IEEE header) */
        ioerr += READ_XLMAC_B0_RX_CTRLr(unit, port, &xlp_rx_ctrl);
        XLMAC_B0_RX_CTRLr_STRIP_CRCf_SET(xlp_rx_ctrl, 0);
        if (speed >= 10000 && BMD_PORT_PROPERTIES(unit, port) == BMD_PORT_XE) {
            XLMAC_B0_RX_CTRLr_STRICT_PREAMBLEf_SET(xlp_rx_ctrl, 1);
        } else {
            XLMAC_B0_RX_CTRLr_STRICT_PREAMBLEf_SET(xlp_rx_ctrl, 0);
        }
        ioerr += WRITE_XLMAC_B0_RX_CTRLr(unit, port, xlp_rx_ctrl);
    
        /* Configure Tx (Inter-Packet-Gap, recompute CRC mode, IEEE header) */
        ioerr += READ_XLMAC_B0_TX_CTRLr(unit, port, &xlp_tx_ctrl);
        XLMAC_B0_TX_CTRLr_TX_PREAMBLE_LENGTHf_SET(xlp_tx_ctrl, 0x8);
        XLMAC_B0_TX_CTRLr_PAD_THRESHOLDf_SET(xlp_tx_ctrl, 0x40);
        XLMAC_B0_TX_CTRLr_AVERAGE_IPGf_SET(xlp_tx_ctrl, 0xc);
        XLMAC_B0_TX_CTRLr_CRC_MODEf_SET(xlp_tx_ctrl, 0x2);
        ioerr += WRITE_XLMAC_B0_TX_CTRLr(unit, port, xlp_tx_ctrl);

        /* Set max Rx frame size */
        XLMAC_B0_RX_MAX_SIZEr_CLR(xlp_rx_max_size);
        XLMAC_B0_RX_MAX_SIZEr_RX_MAX_SIZEf_SET(xlp_rx_max_size, JUMBO_MAXSZ);
        ioerr += WRITE_XLMAC_B0_RX_MAX_SIZEr(unit, port, xlp_rx_max_size);
        
        XLMAC_B0_MODEr_CLR(xlp_mac_mode);
        XLMAC_B0_MODEr_HDR_MODEf_SET(xlp_mac_mode, 0);
        XLMAC_B0_MODEr_SPEED_MODEf_SET(xlp_mac_mode, 4);
        if (speed == 1000) {
            XLMAC_B0_MODEr_SPEED_MODEf_SET(xlp_mac_mode, 2);
        } else if (speed == 2500) {
            XLMAC_B0_MODEr_SPEED_MODEf_SET(xlp_mac_mode, 3);
        }
        ioerr += WRITE_XLMAC_B0_MODEr(unit, port, xlp_mac_mode);

        ioerr += READ_XLMAC_B0_RX_LSS_CTRLr(unit, port, &xlp_rx_lss_ctrl);
        XLMAC_B0_RX_LSS_CTRLr_DROP_TX_DATA_ON_LOCAL_FAULTf_SET(xlp_rx_lss_ctrl, 1);
        XLMAC_B0_RX_LSS_CTRLr_DROP_TX_DATA_ON_REMOTE_FAULTf_SET(xlp_rx_lss_ctrl, 1);
        ioerr += WRITE_XLMAC_B0_RX_LSS_CTRLr(unit, port, xlp_rx_lss_ctrl);

        /* Disable loopback and bring XLMAC out of reset */
        XLMAC_B0_CTRLr_LOCAL_LPBKf_SET(xlb_mac_ctrl, 0);
        XLMAC_B0_CTRLr_RX_ENf_SET(xlb_mac_ctrl, 1);
        XLMAC_B0_CTRLr_TX_ENf_SET(xlb_mac_ctrl, 1);
        ioerr += WRITE_XLMAC_B0_CTRLr(unit, port, xlb_mac_ctrl);
...
void *
bcm56560_a0_tdm_malloc(int unit, int size)
{
...
#else
    CDK_WARN(("bcm56560_a0_tdm_malloc[%d]: BMD_CONFIG_INCLUDE_DMA "
              "must be defined in bcm56560 to support "
              "bcm56560_a0_tdm_malloc.\n", unit));
    return NULL;
#endif
}

void
bcm56560_a0_tdm_free(int unit, void *addr)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
...
#define PGW_NUM_CLP 2
    ...
    CDK_PBMP_ITER(pbm, port) {
        ...
        CELL_ASM_CUT_THRU_THRESHOLD0r_XLP0_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
        CELL_ASM_CUT_THRU_THRESHOLD0r_XLP1_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
        CELL_ASM_CUT_THRU_THRESHOLD0r_XLP2_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
        CELL_ASM_CUT_THRU_THRESHOLD0r_XLP3_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
        CELL_ASM_CUT_THRU_THRESHOLD0r_XLP4_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
        CELL_ASM_CUT_THRU_THRESHOLD0r_XLP5_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
        CELL_ASM_CUT_THRU_THRESHOLD0r_XLP6_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
        CELL_ASM_CUT_THRU_THRESHOLD0r_CLP0_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
        CELL_ASM_CUT_THRU_THRESHOLD0r_CLP1_CELL_ASM_CUT_THRU_THRESHOLDf_SET(cell_asm_cut_thru_th0, asm_ct_thresh);
...
_edb_init(int unit)
{
    ...
    bcm56560_a0_all_front_pbmp_get(unit, &pbm);
...
_port_asf_speed_set(int unit, int port, int speed)
{
    ...
    switch (speed) {
        case 10:
            asf_speed_mode = APACHE_PORT_CT_SPEED_10M_FULL;
            break;
        case 100:
            asf_speed_mode = APACHE_PORT_CT_SPEED_100M_FULL;
            break;
        case 1000:
...
_portctrl_port_init(int unit)
{
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ...
        if (SPEED_MAX(unit, port) >= 100000) {
            bcm56560_a0_egr_buf_reset(unit, port, 1);

            _port_speed_update(unit, port, SPEED_MAX(unit, port));

            bcm56560_a0_egr_buf_reset(unit, port, 0);
...
void
bcm56560_a0_idb_buf_reset(int unit, int port, int reset)
{
    ...
    pgw = bcm56560_a0_pgw_clport_block_index_get(unit, port, &obm, &subport);  
    if (obm == 0) {
...
_ledup_init(int unit)
{
    ...
    CMIC_LEDUP0_PORT_ORDER_REMAPr_CLR(ledup0_order_remap);
    for (idx = 0; idx < 9; idx++) {
        CMIC_LEDUP0_PORT_ORDER_REMAPr_REMAP_PORT_0f_SET(ledup0_order_remap, _p2l_value(unit, ix, uP1_off, skip_slot[ix]));
        ++ix;
        CMIC_LEDUP0_PORT_ORDER_REMAPr_REMAP_PORT_1f_SET(ledup0_order_remap, _p2l_value(unit, ix, uP1_off, skip_slot[ix]));
        ++ix;
        CMIC_LEDUP0_PORT_ORDER_REMAPr_REMAP_PORT_2f_SET(ledup0_order_remap, _p2l_value(unit, ix, uP1_off, skip_slot[ix]));
        ++ix;
        CMIC_LEDUP0_PORT_ORDER_REMAPr_REMAP_PORT_3f_SET(ledup0_order_remap, _p2l_value(unit, ix, uP1_off, skip_slot[ix]));
        ++ix;
        ioerr += WRITE_CMIC_LEDUP0_PORT_ORDER_REMAPr(unit, idx, ledup0_order_remap);
    }
...
static int
_hash_bank_count_get(int unit, BCM56560_A0_ENUM_t mem, int *num_banks)
{
    int count;
...
int
bcm56560_a0_bmd_init(int unit)
{
    int ioerr = 0;
    int rv;
    int port, lport;
    int cfg_port;
    cdk_pbmp_t pbmp, cxxpbmp, clpbmp;
    ...
    PGW_ING_HW_RESET_CONTROL_2r_t pgw_ing_hw_reset_ctrl2;
    ...
    int lanes;
    XLPORT_MAC_CONTROLr_t xlb_mac_ctrl;
    CLPORT_MAC_CONTROLr_t cp_mac_ctrl;
    int qnum;
    ...
    L3_ENTRY_CONTROL_10r_t l3_ctrl_10;
    int blk_idx;
    int count, cmp;
     ...
     */
    PGW_ING_HW_RESET_CONTROL_2r_CLR(pgw_ing_hw_reset_ctrl2);
    PGW_ING_HW_RESET_CONTROL_2r_RESET_ALLf_SET(pgw_ing_hw_reset_ctrl2, 1);
    PGW_ING_HW_RESET_CONTROL_2r_VALIDf_SET(pgw_ing_hw_reset_ctrl2, 1);
    for (blk_idx = 0; blk_idx < 2; blk_idx++) {
        ioerr += WRITE_PGW_ING_HW_RESET_CONTROL_2r(unit, blk_idx, 
            pgw_ing_hw_reset_ctrl2);
    }
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56560_a0_bmd_init[%d]: ING_HW_RESET timeout\n", unit));
        return ioerr ? CDK_E_IO : CDK_E_TIMEOUT;
    }
    ...
    if (idx >= PIPE_RESET_TIMEOUT_MSEC) {
        CDK_WARN(("bcm56560_a0_bmd_init[%d]: EGR_HW_RESET timeout\n", unit));
        return ioerr ? CDK_E_IO : CDK_E_TIMEOUT;
    }
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ioerr += WRITE_EGR_VLAN_CONTROL_1m(unit, port, egr_vlan_ctrl_1);
        ioerr += WRITE_EGR_VLAN_CONTROL_2m(unit, port, egr_vlan_ctrl_2);
        ioerr += WRITE_EGR_VLAN_CONTROL_3m(unit, port, egr_vlan_ctrl_3);
        ioerr += WRITE_EGR_PVLAN_EPORT_CONTROLr(unit, port, egr_pvlan_ctrl);
        ioerr += WRITE_EGR_SF_SRC_MODID_CHECKr(unit, port, egr_sf_chk);
        ioerr += WRITE_EGR_1588_LINK_DELAY_64r(unit, port, egr_1588_link_delay);
        ioerr += WRITE_EGR_FCOE_INVALID_CRC_FRAMESr(unit, port, egr_fcoe_inv_crc);
        ioerr += WRITE_EGR_FCOE_DELIMITER_ERROR_FRAMESr(unit, port, egr_fcoe_errf);
        ioerr += WRITE_EGR_IPMC_CFG2m(unit, port, egr_ipmc_cfg2);
        ioerr += WRITE_ING_TRILL_ADJACENCYr(unit, port, ing_trill_adj);
        ioerr += WRITE_STORM_CONTROL_METER_CONFIGr(unit, port, storm_ctrl);
        ioerr += WRITE_SFLOW_ING_THRESHOLDr(unit, port, sflow_ing_threshold);
    }
    ...
    CDK_PBMP_ITER(all_pbmp, port) {
        ...
        ioerr += WRITE_EGR_ING_PORTm(unit, L2P(unit, LB_LPORT), egr_ing_port);
    }
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ...
        inst_nums = 1;
        if (bcm56560_a0_port_speed_max(unit, port >= 100000)) {
            port_field = 0xf;
            inst_nums = 3;
        }
        ...
        for (idx = 0; idx < inst_nums; idx++) {
            ...
            ioerr += READ_XLPORT_MAC_CONTROLr(unit, &xlb_mac_ctrl, cfg_port);
            XLPORT_MAC_CONTROLr_XMAC0_RESETf_SET(xlb_mac_ctrl, 0);
            ioerr += WRITE_XLPORT_MAC_CONTROLr(unit, xlb_mac_ctrl, cfg_port);
        }
    ...
    READ_L3_TABLE_HASH_CONTROLr(unit, &l3_hash_ctrl);
    ...
    WRITE_L3_TABLE_HASH_CONTROLr(unit, l3_hash_ctrl);
    ...
    ioerr += READ_L3_ENTRY_CONTROL_10r(unit, &l3_ctrl_10);
    L3_ENTRY_CONTROL_10r_DISABLE_L3_ENTRY_LPf_SET(l3_ctrl_10, 0xf);
    ioerr += WRITE_L3_ENTRY_CONTROL_10r(unit, l3_ctrl_10);
    ...
    ioerr += WRITE_EGR_VLAN_CONTROL_1m(unit, CMIC_PORT, egr_vlan_ctrl_1);
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ...
        if ((SPEED_MAX(unit, port) >= 100000) && CDK_PBMP_MEMBER(cxxpbmp, port)) {
            /* 12X10 */
            for (idx=0; idx < 3; idx++) {
                XLPORT_MAC_CONTROLr_CLR(xlb_mac_ctrl);
                ioerr += WRITE_XLPORT_MAC_CONTROLr(unit, xlb_mac_ctrl, 
                    port + idx * 4);
            }
            /* Clear MAC hard reset after warpcore is initialized */
            CLPORT_MAC_CONTROLr_CLR(cp_mac_ctrl);
            ioerr += WRITE_CLPORT_MAC_CONTROLr(unit, cp_mac_ctrl, port);
            /* Initialize CLPORTs after CLMAC is out of reset */
            ioerr += bcm56560_a0_clport_init(unit, port);
            ...
            if (XLPORT_SUBPORT(port) == 0) {
                XLPORT_MAC_CONTROLr_CLR(xlb_mac_ctrl);
                ioerr += WRITE_XLPORT_MAC_CONTROLr(unit, xlb_mac_ctrl, port);
            }
            /* Initialize XLPORTs after XLMAC is out of reset */
            ioerr += bcm56560_a0_xlport_init(unit, port);
        }
    }
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ...
        lanes = bcm56560_a0_port_lanes_get(unit, port);
        if (lanes == 4) {
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ...
        qnum = bcm56560_a0_mmu_port_uc_queue_index(unit, port);
