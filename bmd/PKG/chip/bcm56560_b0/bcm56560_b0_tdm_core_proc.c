...
#include "bcm56560_b0_tdm_core_vec.h"


/**
@name: tdm_b0_core_prealloc
@param:

Unless special exceptions are needed for certain port speeds, this is the standard procedure to presort ports by speed
 */
void
tdm_b0_core_prealloc(unsigned short stack[TDM_AUX_SIZE], int buffer[TDM_AUX_SIZE], short *x, char *boolstr, int j)
{
	int i;
 ...
 */
int
tdm_b0_core_postalloc(unsigned short **vector_map, int freq, unsigned short spd, short *yy, short *y, int lr_idx_limit, unsigned short lr_stack[TDM_AUX_SIZE], int token, const char* speed, int num_ext_ports)
{
	int v, load_status;
	...
	TDM_SML_BAR
	while (*y > 0) {
		load_status = tdm_b0_ap_b0_core_vec_load_b0(vector_map, freq, spd, *yy, lr_idx_limit, num_ext_ports);
		if (load_status==FAIL) {
 ...
 */
int
tdm_b0_core_postalloc_vmap_b0(unsigned short **vector_map, int freq, unsigned short spd, short *yy, short *y, int lr_idx_limit, unsigned short lr_stack[TDM_AUX_SIZE], int token, const char* speed, int num_ext_ports, int vmap_wid, int vmap_len)
{
	int v, load_status;
...
/**
@name: tdm_b0_arr_exists
@param:

Returns boolean on whether an element exists in a fixed length array
 */
int
tdm_b0_arr_exists(int element, int len, int arr[ARR_FIXED_LEN])
{
	int i, result=FAIL;
...
/**
@name: tdm_b0_arr_append
@param:

Appends to array without needing to track an index
Array must be initialized to NUM_EXT_PORTS
 */
/* int tdm_b0_arr_append(int element, int len, int arr[len]) { */
/* C90 not supported */
int
tdm_b0_arr_append(int element, int len, int arr[ARR_FIXED_LEN], int num_ext_ports)
{
...
/**
@name: tdm_b0_core_acc_alloc
@param:
 */
int
tdm_b0_core_acc_alloc( tdm_mod_t *_tdm )
{
	int g, j, l,
		*tdm_b0_pipe_main;
	...
	for (j=1; j<=_tdm->_chip_data.soc_pkg.tvec_size; j++) {
		g=tdm_b0_PQ((((10*_tdm->_core_data.vars_pkg.cap)/_tdm->_chip_data.soc_pkg.tvec_size)*j))+(j-1);
		for (l=(_tdm->_core_data.vmap_max_len-1); l>g; l--) {
			tdm_b0_pipe_main[l]=tdm_b0_pipe_main[l-1];
		}
		tdm_b0_pipe_main[g]=_tdm->_chip_data.soc_pkg.soc_vars.ancl_token;
	}
...
/**
@name: tdm_b0_core_vbs_scheduler
@param:

Allocate slots for linerate ports within the linerate capacity limit
 */
int
tdm_b0_core_vbs_scheduler( tdm_mod_t *_tdm )
{
	...
	_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt=0;
	if (_tdm->_core_data.vars_pkg.lr_enable) {
		for (i=0; i<_tdm->_core_data.vmap_max_wid; i++) {
			if (_tdm->_core_data.vmap[i][0]!=_tdm->_chip_data.soc_pkg.num_ext_ports) {
				_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt++;
			}
		}
	}
	for (i=0; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
		_tdm->_core_data.vars_pkg.port=_tdm->_core_data.vmap[i][0];
		if (tdm_b0_type_chk(_tdm)==3) {
	...
	for (i=0; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
		lr_slot_cnt+=tdm_b0_slots(_tdm->_chip_data.soc_pkg.speed[_tdm->_core_data.vmap[i][0]]/1000);
	}
	for (i=0; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
		_tdm->_core_data.vars_pkg.port=_tdm->_core_data.vmap[i][0];
		if ( !(tdm_b0_arr_exists(_tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm),ARR_FIXED_LEN,tsc_pipeline)) ) {
	...
	if (_tdm->_core_data.vars_pkg.lr_enable) {
		if (!triport_op) {
			tdm_b0_vector_dump(_tdm);
			for (i=0; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
				_tdm->_core_data.vars_pkg.port=_tdm->_core_data.vmap[i][0];
				_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.round=BOOL_FALSE;
				_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=i;
				if ( (tdm_b0_type_chk(_tdm)>1) && (tdm_b0_count_param_spd(_tdm)>=10) ) {
					timeout=TIMEOUT;
					do {
						x0=(x0>=xx)?(0):(x0);
						_tdm->_core_data.vars_pkg.m_tdm_b0_pick_vec.tsc_dq=tsc_pipeline[x0];
						_tdm->_core_data.vars_pkg.m_tdm_b0_pick_vec.prev_vec=i;
						vec_id=_tdm->_core_exec[TDM_CORE_EXEC__PICK_VEC](_tdm);
						_tdm->_core_data.vars_pkg.port=_tdm->_core_data.vmap[vec_id][0];
						if ( ((enum port_speed_e)tdm_b0_find_fastest_spd(_tdm)<=(_tdm->_chip_data.soc_pkg.speed[_tdm->_core_data.vmap[i][0]]-SPEED_DELIMITER)) ||
							 ((enum port_speed_e)tdm_b0_find_fastest_spd(_tdm)>=(_tdm->_chip_data.soc_pkg.speed[_tdm->_core_data.vmap[i][0]]+SPEED_DELIMITER)) ) {
							x0++;
						}
					} while ( /*(vec_id<=i) && */
							  ( ((enum port_speed_e)tdm_b0_find_fastest_spd(_tdm)<=(_tdm->_chip_data.soc_pkg.speed[_tdm->_core_data.vmap[i][0]]-SPEED_DELIMITER)) ||
								((enum port_speed_e)tdm_b0_find_fastest_spd(_tdm)>=(_tdm->_chip_data.soc_pkg.speed[_tdm->_core_data.vmap[i][0]]+SPEED_DELIMITER)) ) &&
								((--timeout)>0) );
					for (j=i; j<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; j++) {
						if (_tdm->_core_data.vmap[j][0]==tdm_b0_find_fastest_port(_tdm)) {
							vec_id=j;
							break;
						}
					}
					if (vec_id!=i) {
						_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.src=i;
						_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.dst=(_tdm->_core_data.vmap_max_wid-1);
						tdm_b0_mem_transpose(_tdm);
						_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.src=vec_id;
						_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.dst=i;
						tdm_b0_mem_transpose(_tdm);
						_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.src=(_tdm->_core_data.vmap_max_wid-1);
						_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.dst=vec_id;
						tdm_b0_mem_transpose(_tdm);
					}
				}
				if ((x0++)>=xx) {x0=0;}
		...
		/* MODIFIED: new added */
			...
			for (i=0; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
				...
				_tdm->_core_data.vars_pkg.port=_tdm->_core_data.vmap[i][0];
				_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=i;
				param_spd_i = tdm_b0_count_param_spd(_tdm);
				if (param_spd_i<10) {break;}
				if (param_spd_i!=param_spd_ii){
					param_spd_ii = param_spd_i;
					same_spd_num = 1;
					for (j=(i+1); j<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; j++) {
						_tdm->_core_data.vars_pkg.port=_tdm->_core_data.vmap[j][0];
						_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=j;
						param_spd_j = tdm_b0_count_param_spd(_tdm);
				...
				if (same_spd_num>2){
					...
					for (k=0; k<same_spd_num; k++) {
						...
						for (j=(i+1); ( (j<(i+same_spd_num)) && (j<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt) ); j++) {
							...
							if (check_transpose == PASS){
								_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.src=dst_idx;
								_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.dst=(_tdm->_core_data.vmap_max_wid-1);
								tdm_b0_mem_transpose(_tdm);
								_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.src=src_idx;
								_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.dst=dst_idx;
								tdm_b0_mem_transpose(_tdm);
								_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.src=(_tdm->_core_data.vmap_max_wid-1);
								_tdm->_core_data.vars_pkg.m_tdm_b0_mem_transpose.dst=src_idx;
								tdm_b0_mem_transpose(_tdm);
							}
		...
		if (_tdm->_core_data.vars_pkg.os_enable) {
			for (i=1; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
				_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.round=BOOL_FALSE;
				_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=i;
				pspd=tdm_b0_count_param_spd(_tdm);
				_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=(i-1);
				if ( ((pspd!=tdm_b0_count_param_spd(_tdm))) ) {
					_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.round=BOOL_TRUE;
					_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=(i-1);
					pspd=tdm_b0_count_param_spd(_tdm);
					_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=i;
					pspd1 = tdm_b0_count_param_spd(_tdm);
					if (pspd1>0 && pspd%pspd1==0) {
						cadence_blocks=1;
						cadence_ref_idx=i-1;
						_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.round=BOOL_FALSE;
						_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=cadence_ref_idx;
						pspd=tdm_b0_count_param_spd(_tdm);
						_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=(cadence_ref_idx-1);
						while(cadence_ref_idx>0 && (tdm_b0_count_param_spd(_tdm)==tdm_b0_count_param_spd(_tdm)) ) {
							cadence_ref_idx--;
						}
						...
						for (j=(i+cadence_step_size); j<(i+cadence_cnt); j+=cadence_step_size) {
							for (k=0; k<cadence_step_size; k++) {
								...
								do {
									_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.vector=(j+k);
									_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.size=_tdm->_chip_data.soc_pkg.lr_idx_limit;
									_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.step=1;
									tdm_b0_vector_rotate(_tdm);
									_tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.principle=0;
									_tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.idx=(j+k);
									if (_tdm->_core_data.vmap[cadence_ref_idx][tdm_b0_map_find_y_indx(_tdm)]!=_tdm->_chip_data.soc_pkg.num_ext_ports && _tdm->_core_data.vmap[j+k][tdm_b0_map_find_y_indx(_tdm)]!=_tdm->_chip_data.soc_pkg.num_ext_ports) {
										cadence_step++;
									}
		...
		do {
			for (i=1; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
				...
				for (k=0; k<_tdm->_chip_data.soc_pkg.lr_idx_limit; k++) {
					_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.vector=i;
					_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.size=_tdm->_chip_data.soc_pkg.lr_idx_limit;
					_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.step=rotation_step;
					tdm_b0_vector_rotate(_tdm);
					tdm_b0_vector_dump(_tdm);				
					_tdm->_core_data.vars_pkg.m_tdm_b0_fit_singular_col.node_x=i;
					_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.wid=i;
					_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.node_x=i;
					if ( (tdm_b0_fit_singular_col(_tdm)==PASS) && 
						 (tdm_b0_fit_prox(_tdm)==PASS) ) {
						dim_reduce=PASS;
						break;
					}
				}
				if (!dim_reduce) {
					_tdm->_core_data.vars_pkg.m_tdm_b0_count_nonsingular.x_idx=i;
					ns_min_cnt=tdm_b0_count_nonsingular(_tdm);
					for (k=0; k<_tdm->_chip_data.soc_pkg.lr_idx_limit; k++) {
						_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.vector=i;
						_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.size=_tdm->_chip_data.soc_pkg.lr_idx_limit;
						_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.step=rotation_step;
						...
						_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.wid=i;
						_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.node_x=i;
						if ( (tdm_b0_count_nonsingular(_tdm)<ns_min_cnt) &&
							 (tdm_b0_fit_prox(_tdm)==PASS) ) {
							ns_min_cnt=tdm_b0_count_nonsingular(_tdm);
						}
					}
					_tdm->_core_data.vars_pkg.m_tdm_b0_count_nonsingular.x_idx=i;
					while ( ns_min_cnt!=tdm_b0_count_nonsingular(_tdm) ) {
						_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.vector=i;
						_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.size=_tdm->_chip_data.soc_pkg.lr_idx_limit;
						_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.step=rotation_step;
						tdm_b0_vector_rotate(_tdm);
			...
			for (i=1; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
				_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.wid=_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt;
				_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.node_x=i;
				if (tdm_b0_fit_prox(_tdm)!=PASS) {
					for (j=0; j<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; j++) {
						_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.round=BOOL_FALSE;
						_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=j;
						pspd=tdm_b0_count_param_spd(_tdm);
						_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos=i;
						if (pspd==tdm_b0_count_param_spd(_tdm)) {
							_tdm->_core_data.vars_pkg.m_tdm_b0_sticky_transpose.src=i;
							_tdm->_core_data.vars_pkg.m_tdm_b0_sticky_transpose.dst=j;
							tdm_b0_sticky_transpose(_tdm);
							_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.wid=_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt;
							_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.node_x=i;
							fpnode=tdm_b0_fit_prox(_tdm);
							_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.node_x=j;
							if ( (fpnode==PASS) &&
								 (tdm_b0_fit_prox(_tdm)==PASS) ) {
								tdm_b0_vector_dump(_tdm);
								break;
			...
			if (!(_tdm->_core_data.vars_pkg.lr_1)) {
				for (i=0; i<_tdm->_chip_data.soc_pkg.lr_idx_limit; i++) {
					_tdm->_core_data.vars_pkg.m_tdm_b0_fit_row_min.y_idx=i;
					_tdm->_core_data.vars_pkg.m_tdm_b0_nsin_row.y_idx=i;
					if ( (!tdm_b0_nsin_row(_tdm)) || (!tdm_b0_fit_row_min(_tdm)) ) {
	...
	if (_tdm->_core_data.vars_pkg.HG1X106G_xX40G_OVERSUB==BOOL_TRUE) {
		...
		_tdm->_core_data.vars_pkg.m_tdm_b0_vector_clear.yy=0;
		tdm_b0_vector_clear(_tdm);
		...
		for (j=0; j<_tdm->_chip_data.soc_pkg.lr_idx_limit; j++) {
			_tdm->_core_data.vars_pkg.m_tdm_b0_nsin_row.y_idx=j;
			if (!tdm_b0_nsin_row(_tdm)) {
				_tdm->_core_data.vars_pkg.m_tdm_b0_vector_clear.yy=0;
				tdm_b0_vector_clear(_tdm);
				for (i=0; ( (i<(_tdm->_chip_data.soc_pkg.lr_idx_limit-4)) && ((++redis_100g_cnt)<=40) && i<(_tdm->_core_data.vmap_max_len-4) ); i+=5) {
					if (tdm_b0_empty_row(_tdm->_core_data.vmap,i,_tdm->_chip_data.soc_pkg.num_ext_ports,_tdm->_core_data.vmap_max_wid)) {
						_tdm->_core_data.vmap[0][i]=redis_100g_port;
					}
					else if (i>0 && (tdm_b0_empty_row(_tdm->_core_data.vmap,(i-1),_tdm->_chip_data.soc_pkg.num_ext_ports,_tdm->_core_data.vmap_max_wid)) && (tdm_b0_slice_size_2d(_tdm->_core_data.vmap,(i-1),_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.num_ext_ports,_tdm->_core_data.vmap_max_wid)<=tdm_b0_slice_size_2d(_tdm->_core_data.vmap,(i+1),_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.num_ext_ports,_tdm->_core_data.vmap_max_wid)) ) {
						_tdm->_core_data.vmap[0][i-1]=redis_100g_port;
					}
					else if (tdm_b0_empty_row(_tdm->_core_data.vmap,(i+1),_tdm->_chip_data.soc_pkg.num_ext_ports,_tdm->_core_data.vmap_max_wid)) {
						_tdm->_core_data.vmap[0][i+1]=redis_100g_port;
					}
				}
				break;
	...
	tdm_b0_print_vmap_vector(_tdm);
	TDM_PRINT0("\n\n\n");
	TDM_SML_BAR
	
	TDM_PRINT0("\n\tConstructing TDM linked list...\n");
	TDM_PRINT5("\t\t| Span=%0d | Length=%0d | Pivot=%0d | Slice Size=%0d | Retrace=%0d |\n",_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_core_data.vmap[0][0],tdm_b0_map_cadence_count(_tdm->_core_data.vmap[0],0,_tdm->_core_data.vmap_max_len),tdm_b0_map_retrace_count(_tdm->_core_data.vmap,0,0,_tdm->_core_data.vmap_max_len,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports));
	
    cal_len = _tdm->_chip_data.soc_pkg.lr_idx_limit;
	for (i=0; i<cal_len && i<VEC_MAP_LEN && i<_tdm->_core_data.vmap_max_len; i++) {
		singular_cnt = tdm_b0_fit_singular_cnt(_tdm->_core_data.vmap,i,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports);
		if (singular_cnt>1) { 
			for (j=0; j<(singular_cnt-1); j++){
				idx_up = -1; idx_dn = -1;
				for (k=(i-1); k>0; k--){
					if (vmap_bmp[k]==1 && 0==tdm_b0_fit_singular_cnt(_tdm->_core_data.vmap,k,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports)){
						idx_up = k; break;
					}
				}
	...
	for (i=0; i<cal_len && i<VEC_MAP_LEN && i<_tdm->_core_data.vmap_max_len; i++) {
		if (vmap_bmp[i]){
			singular_cnt = tdm_b0_fit_singular_cnt(_tdm->_core_data.vmap,i,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports);
			if (singular_cnt==0){
				tdm_b0_ll_append(&tdm_b0_ll,_tdm->_chip_data.soc_pkg.num_ext_ports);
			}
			else {
				for (j=0; j<_tdm->_core_data.vmap_max_wid; j++) {
					TOKEN_CHECK(_tdm->_core_data.vmap[j][i]) {
						tdm_b0_ll_append(&tdm_b0_ll,_tdm->_core_data.vmap[j][i]);
					}
				}
			}
		}
	...
	for (i=0; i<60 && i<_tdm->_core_data.vmap_max_len; i++) {
		for (j=0; j<_tdm->_core_data.vmap_max_wid; j++) {
			if (tdm_b0_fit_singular_cnt(_tdm->_core_data.vmap,i,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports)>0) {
				TOKEN_CHECK(_tdm->_core_data.vmap[j][i]) {
					tdm_b0_ll_append(&tdm_b0_ll,_tdm->_core_data.vmap[j][i]);
				}
			}
	...
	tdm_b0_ll_print(&tdm_b0_ll);
	tdm_b0_ll_deref(&tdm_b0_ll,tdm_b0_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit);
	for (j=_tdm->_chip_data.soc_pkg.lr_idx_limit; j<_tdm->_core_data.vmap_max_len; j++) {
		TOKEN_CHECK(tdm_b0_pipe_main[j]) {
			for (i=1; i<_tdm->_chip_data.soc_pkg.lr_idx_limit; i++) {
				if (tdm_b0_pipe_main[_tdm->_chip_data.soc_pkg.lr_idx_limit-i]==_tdm->_chip_data.soc_pkg.num_ext_ports) {
					tdm_b0_pipe_main[_tdm->_chip_data.soc_pkg.lr_idx_limit-i]=tdm_b0_pipe_main[j];
					tdm_b0_pipe_main[j]=_tdm->_chip_data.soc_pkg.num_ext_ports;
					break;
				}
			}
		}
	...
	if ( _tdm->_core_data.vars_pkg.cap>0 && (_tdm->_chip_data.soc_pkg.speed[tdm_b0_pipe_main[_tdm->_core_data.vars_pkg.cap-1]]>=SPEED_10G) ) {
		...
		for (j=0; j<_tdm->_chip_data.soc_pkg.pm_num_phy_modules; j++) {
			for (i=0; i<_tdm->_chip_data.soc_pkg.pmap_num_lanes; i++) {
				if (_tdm->_chip_data.soc_pkg.pmap[j][i]==tdm_b0_pipe_main[_tdm->_core_data.vars_pkg.cap-1]) {
					cap_tsc=j;
					break;
				}
			}
        ...
        if (cap_tsc<_tdm->_chip_data.soc_pkg.pm_num_phy_modules){
            ...
            cap_lkbk=1;
            for (j=(i-2); j>=0; j--) {
                if (tdm_b0_pipe_main[j]!=_tdm->_chip_data.soc_pkg.pmap[cap_tsc][0]&&tdm_b0_pipe_main[j]!=_tdm->_chip_data.soc_pkg.pmap[cap_tsc][1]&&tdm_b0_pipe_main[j]!=_tdm->_chip_data.soc_pkg.pmap[cap_tsc][2]&&tdm_b0_pipe_main[j]!=_tdm->_chip_data.soc_pkg.pmap[cap_tsc][3]) {
                    cap_lkbk++;
                }
                else {
                    break;
	...
	if (_tdm->_core_data.vars_pkg.os_enable && !_tdm->_core_data.vars_pkg.lr_enable) {
		for (j=0; j<_tdm->_core_data.vars_pkg.cap; j++) {
			if (tdm_b0_pipe_main[j]==_tdm->_chip_data.soc_pkg.num_ext_ports) {
				tdm_b0_pipe_main[j] = _tdm->_chip_data.soc_pkg.soc_vars.ovsb_token;
			}
		}
	}
	else {
		for (j=0; j<_tdm->_core_data.vars_pkg.cap; j++) {
			if (tdm_b0_pipe_main[j]==_tdm->_chip_data.soc_pkg.num_ext_ports) {
				if (_tdm->_core_data.vars_pkg.os_enable) {
					...
					if (j%2==0) {
						tdm_b0_pipe_main[j] = _tdm->_chip_data.soc_pkg.soc_vars.idl1_token;
					}
					else {
						tdm_b0_pipe_main[j] = _tdm->_chip_data.soc_pkg.soc_vars.idl2_token;
					}		
				}
			}
		}
	...
	if ( _tdm->_core_data.vars_pkg.lr_enable && _tdm->_core_data.vars_pkg.os_enable && ((((_tdm->_chip_data.soc_pkg.lr_idx_limit-lr_slot_cnt)*BW_QUANTA)/10)<100) && ((_tdm->_core_data.vars_pkg.lr_10+_tdm->_core_data.vars_pkg.lr_20+_tdm->_core_data.vars_pkg.lr_40+_tdm->_core_data.vars_pkg.lr_50+_tdm->_core_data.vars_pkg.lr_100+_tdm->_core_data.vars_pkg.lr_25)==1) ) {
		if (tdm_b0_core_filter_refactor(_tdm)) {
			_tdm->_core_data.vars_pkg.refactor_done=BOOL_TRUE;
		}
	}
	
	tdm_b0_ll_free(&tdm_b0_ll);
	
	if (tdm_b0_core_acc_alloc(_tdm)==FAIL) {
		return (TDM_EXEC_CORE_SIZE+1);
	}
...
/**
@name: tdm_b0_core_vbs_scheduler_lr_b0
@param:

Allocate slots for linerate ports within the overall TDM calendar capacity limit 
 */
int
tdm_b0_core_vbs_scheduler_lr_b0( tdm_mod_t *_tdm )
{
	unsigned short dim_reduce;
					...
					if (pspd1>0 && pspd%pspd1==0) {
						...
						for (j=(i+cadence_step_size); j<(i+cadence_cnt); j+=cadence_step_size) {
							for (k=0; k<cadence_step_size; k++) {
								...
								do {
									_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.vector=(j+k);
									_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.size=cal_len;
									_tdm->_core_data.vars_pkg.m_tdm_b0_vector_rotate.step=1;
									tdm_b0_vector_rotate(_tdm);
									_tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.principle=0;
									_tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.idx=(j+k);
	...
	for (i=0; i<cal_len && i<_tdm->_core_data.vmap_max_len && i<VEC_MAP_LEN; i++) {
		...
		if (singular_cnt>1) {
            /* get last column */
            col_last = _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt;
			...
			for (j=1; j<singular_cnt; j++){
                ...
                for (k=(i-1); k>0 && i>0; k--){
					if (vmap_bmp[k]>0 && 0==tdm_b0_fit_singular_cnt(_tdm->_core_data.vmap,k,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports)){
                        check_dist = PASS;
                        for (w=0; w<col_last; w++){
                            ...
                            for (v=k; v>=0; v--){
                                if (vmap_bmp[v]>0){
                                    cnt_tmp = tdm_b0_fit_singular_cnt(_tdm->_core_data.vmap,v,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports);
	...
	TDM_PRINT0("\n\tConstructing TDM linked list...\n");
	TDM_PRINT5("\t\t| Span=%0d | Length=%0d | Pivot=%0d | Slice Size=%0d | Retrace=%0d |\n",_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt,cal_len,_tdm->_core_data.vmap[0][0],tdm_b0_map_cadence_count(_tdm->_core_data.vmap[0],0,_tdm->_core_data.vmap_max_len),tdm_b0_map_retrace_count(_tdm->_core_data.vmap,0,0,_tdm->_core_data.vmap_max_len,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports));
	...
	tdm_b0_ll_print(&tdm_b0_ll);
	tdm_b0_ll_deref(&tdm_b0_ll,tdm_b0_pipe_main,cal_len);
	for (j=cal_len; j<_tdm->_core_data.vmap_max_len; j++) {
		TOKEN_CHECK(tdm_b0_pipe_main[j]) {
			for (i=1; i<cal_len; i++) {
				if (tdm_b0_pipe_main[cal_len-i]==_tdm->_chip_data.soc_pkg.num_ext_ports) {
					tdm_b0_pipe_main[cal_len-i]=tdm_b0_pipe_main[j];
					tdm_b0_pipe_main[j]=_tdm->_chip_data.soc_pkg.num_ext_ports;
					break;
				}
			}
 ...
 */
int
tdm_b0_core_vbs_scheduler_wrapper_b0( tdm_mod_t *_tdm )
{   
