...
/**
@name: tdm_b0_find_fastest_port
@param:

Returns the fastest port number from within a tsc
 */
int
tdm_b0_find_fastest_port( tdm_mod_t *_tdm )
{
...
/**
@name: tdm_b0_find_fastest_spd
@param:

Returns the speed of the fastest lane from within a tsc
 */
int
tdm_b0_find_fastest_spd( tdm_mod_t *_tdm )
{
...
/**
@name: tdm_b0_find_fastest_triport
@param:

Returns whether the current port is the fastest port from within a triport
 */
int
tdm_b0_find_fastest_triport( tdm_mod_t *_tdm )
{
    int i, tsc_id=_tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
...
/**
@name: tdm_b0_pick_vec
@param:

Select vector index on x axis to rotate based on priority of TSC pipeline
 */
int
tdm_b0_pick_vec( tdm_mod_t *_tdm )
{
    int i, vec_sel=1, port=_tdm->_core_data.vars_pkg.port;

    for (i=_tdm->_core_data.vars_pkg.m_tdm_b0_pick_vec.prev_vec; i<_tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.lr_vec_cnt; i++) {
        _tdm->_core_data.vars_pkg.port=_tdm->_core_data.vmap[i][0];
        if ( (_tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm)==_tdm->_core_data.vars_pkg.m_tdm_b0_pick_vec.tsc_dq) ) {
            if (_tdm->_core_data.vars_pkg.m_tdm_b0_pick_vec.triport_priority) {
                if (tdm_b0_find_fastest_triport(_tdm)) {
                    vec_sel=i;
                    _tdm->_core_data.vars_pkg.m_tdm_b0_pick_vec.triport_priority=BOOL_FALSE;
                    break;
                }
                else {
                    continue;
...
/**
@name: tdm_b0_map_find_y_indx
@param:

Scans vector map Y axis for Y index of any non-principle node
 */
int
tdm_b0_map_find_y_indx( tdm_mod_t *_tdm )
{
    int i, id=_tdm->_chip_data.soc_pkg.num_ext_ports;

    if (_tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.principle!=_tdm->_chip_data.soc_pkg.num_ext_ports && _tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.idx<_tdm->_core_data.vmap_max_wid) {
        for (i=0; i<_tdm->_chip_data.soc_pkg.lr_idx_limit; i++) {
            if (_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.idx][i]!=_tdm->_chip_data.soc_pkg.num_ext_ports && _tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.idx][i]!=_tdm->_core_data.vars_pkg.m_tdm_b0_map_find_y_indx.principle) {
                id = i;
                break;
            }
        }
...
/**
@name: tdm_b0_nsin_row
@param:

Checks if current row is singular
 */
int
tdm_b0_nsin_row( tdm_mod_t *_tdm )
{
    int i, found_port=_tdm->_chip_data.soc_pkg.num_ext_ports, check=PASS;

    for (i=0; i<_tdm->_core_data.vmap_max_wid; i++) {
        TOKEN_CHECK(_tdm->_core_data.vmap[i][_tdm->_core_data.vars_pkg.m_tdm_b0_nsin_row.y_idx]) {
            if (found_port==_tdm->_chip_data.soc_pkg.num_ext_ports) {
                found_port=_tdm->_core_data.vmap[i][_tdm->_core_data.vars_pkg.m_tdm_b0_nsin_row.y_idx];
            }
            else {
                check=FAIL;
                break;
...
/**
@name: tdm_b0_check_blank
@param:

Checks if current row is blank
 */
int
tdm_b0_empty_row(unsigned short **map, unsigned short y_idx, int num_ext_ports, int vec_map_wid)
{
    int i, check=PASS;
...
/**
@name: tdm_b0_slice_size_2d
@param:

Returns size of proximate 2-D slice in vector map or 0 if the passed row was blank
 */
int
tdm_b0_slice_size_2d(unsigned short **map, unsigned short y_idx, int lim, int num_ext_ports, int vec_map_wid)
{
    int i, slice_size=1;

    if (tdm_b0_empty_row(map,y_idx,num_ext_ports,vec_map_wid)) {
        return 0;
    }
    else {
        for (i=1; (y_idx-i)>=0; i++) {
            if (tdm_b0_empty_row(map,i,num_ext_ports,vec_map_wid)) {
                break;
            }
            slice_size++;
        }
...
/**
@name: tdm_b0_fit_singular_cnt
@param:

Given y index, count number of nodes
 */
int
tdm_b0_fit_singular_cnt(unsigned short **map, int node_y, int vec_map_wid, int num_ext_ports)
{
    int v, cnt=0;
...
/**
@name: tdm_b0_map_cadence_count
@param:

Returns size of port sequence at index
 */
int
tdm_b0_map_cadence_count(unsigned short *vector, int idx, int vec_map_len)
{
    int i=idx, cnt=0;
...
/**
@name: tdm_b0_map_retrace_count
@param:

Returns number of retraceable slots within cadence at index
 */
int
tdm_b0_map_retrace_count(unsigned short **map, int x_idx, int y_idx, int vec_map_len, int vec_map_wid, int num_ext_ports)
{
    int i=y_idx, cnt=0;
    unsigned short port=map[x_idx][y_idx];

    while (map[x_idx][++i]!=port && i<vec_map_len) {
        if ( tdm_b0_fit_singular_cnt(map,i,vec_map_wid,num_ext_ports)==0 ) {
            cnt++;
        }
    }
...
/**
@name: tdm_b0_fit_singular_col
@param:

Given x index, determines fit based on if current column is reducible
 */
int
tdm_b0_fit_singular_col( tdm_mod_t *_tdm )
{
    int v, result=PASS;

    for (v=0; v<_tdm->_chip_data.soc_pkg.lr_idx_limit; v++) {
        TOKEN_CHECK(_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_b0_fit_singular_col.node_x][v]) {
            if ( tdm_b0_fit_singular_cnt(_tdm->_core_data.vmap,v,_tdm->_core_data.vmap_max_wid,_tdm->_chip_data.soc_pkg.num_ext_ports)>1 ) {
                result=FAIL;
                break;
            }
        }
...
/**
@name: tdm_b0_count_nonsingular
@param:

Counts number of unreduced rows at current rotation
 */
int
tdm_b0_count_nonsingular( tdm_mod_t *_tdm )
{
    int i, j, ns_cnt=0;

    for (i=0; i<_tdm->_chip_data.soc_pkg.lr_idx_limit; i++) {
        TOKEN_CHECK(_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_b0_count_nonsingular.x_idx][i]) {
            for (j=0; j<_tdm->_core_data.vmap_max_wid; j++) {
                if ( (j!=_tdm->_core_data.vars_pkg.m_tdm_b0_count_nonsingular.x_idx) && (_tdm->_core_data.vmap[j][i]!=_tdm->_chip_data.soc_pkg.num_ext_ports) ) {
                    ns_cnt++;
                }
            }
        }
...
/**
@name: tdm_b0_fit_row_min
@param:

Checks if current row in vmap has a min spacing violation
 */
int
tdm_b0_fit_row_min( tdm_mod_t *_tdm )
{
    int i, j, port=_tdm->_chip_data.soc_pkg.num_ext_ports,
        pm_0, pm_1;

    for (i=0; i<_tdm->_core_data.vmap_max_wid; i++) {
        TOKEN_CHECK(_tdm->_core_data.vmap[i][_tdm->_core_data.vars_pkg.m_tdm_b0_fit_row_min.y_idx]) {
            port=_tdm->_core_data.vmap[i][_tdm->_core_data.vars_pkg.m_tdm_b0_fit_row_min.y_idx];
            break;
        }
    }
    TOKEN_CHECK(port) {
        for (i=0; i<_tdm->_core_data.vmap_max_wid; i++) {
            for (j=0; j<VBS_MIN_SPACING; j++) {
                TOKEN_CHECK(_tdm->_core_data.vmap[i][_tdm->_core_data.vars_pkg.m_tdm_b0_fit_row_min.y_idx+j]) {
                    _tdm->_core_data.vars_pkg.port = _tdm->_core_data.vmap[i][_tdm->_core_data.vars_pkg.m_tdm_b0_fit_row_min.y_idx+j];
                    pm_0 = _tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
                    _tdm->_core_data.vars_pkg.port = port;
                    pm_1 = _tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
                    if (_tdm->_core_data.vmap[i][_tdm->_core_data.vars_pkg.m_tdm_b0_fit_row_min.y_idx+j]!=port && pm_0==pm_1) {
                        return FAIL;
                    }
                }
            }
...
tdm_b0_fit_prox( tdm_mod_t *_tdm )
{
    ...
    for (i=(_tdm->_core_data.rule__prox_port_min-1); i<_tdm->_chip_data.soc_pkg.lr_idx_limit; i++) {
        TOKEN_CHECK(_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.node_x][i]) {
            _tdm->_core_data.vars_pkg.port=_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.node_x][i];
            wc=_tdm->_core_exec[TDM_CORE_EXEC__PM_SCAN](_tdm);
            for (v=0; v<_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.wid; v++) {
                if (v!=_tdm->_core_data.vars_pkg.m_tdm_b0_fit_prox.node_x) {
...
tdm_b0_count_param_spd( tdm_mod_t *_tdm )
{
    ...
    for (v=0; v<cal_len; v++) {
        if (_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos][v]!=_tdm->_chip_data.soc_pkg.num_ext_ports) {
            y_pos = (y_pos==0) ? (v):(y_pos);
            k++;
        }
    }
    _tdm->_core_data.vars_pkg.port = _tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.x_pos][y_pos];
    switch(k) {
    ...
    if (param_spd>=10) {
        while (_tdm->_core_data.vars_pkg.m_tdm_b0_count_param_spd.round && param_spd%5!=0) {
            param_spd--;
        }
    }
...
/**
@name: tdm_b0_cmn_chk_port_is_fp
@param:
@desc: Return BOOL_TRUE if given port is a front panel port,
       otherwise return BOOL_FALSE.
 */
int
tdm_b0_cmn_chk_port_is_fp(tdm_mod_t *_tdm, int port_token)
{
...
/**
@name: tdm_b0_cmn_chk_pipe_os_spd_types
@param:
 */
int
tdm_b0_cmn_chk_pipe_os_spd_types(tdm_mod_t *_tdm)
{
...
/**
@name: tdm_b0_cmn_get_port_pm
@param:
 */
int
tdm_b0_cmn_get_port_pm(tdm_mod_t *_tdm, int port_token)
{
...
/**
@name: tdm_b0_cmn_get_port_speed
@param:
 */
int
tdm_b0_cmn_get_port_speed(tdm_mod_t *_tdm, int port_token)
{
    if (tdm_b0_cmn_chk_port_is_fp(_tdm, port_token) == BOOL_TRUE) {
...
/**
@name: tdm_b0_cmn_get_port_speed_eth
@param:

Return port speed in ethernet mode. This converts HiGig speed to ethernet.
 */
int
tdm_b0_cmn_get_port_speed_eth(tdm_mod_t *_tdm, int port_token)
{
    int port_speed, port_speed_eth;

    port_speed = tdm_b0_cmn_get_port_speed(_tdm, port_token);
    switch(port_speed) {
        case SPEED_1G:
        case SPEED_1p25G:
        case SPEED_2p5G:
...
/**
@name: tdm_b0_cmn_get_port_speed_idx
@param:

Return speed class of the given port according.
 */
int
tdm_b0_cmn_get_port_speed_idx(tdm_mod_t *_tdm, int port_token)
{
    int port_speed, speed_idx = 0;

    port_speed = tdm_b0_cmn_get_port_speed_eth(_tdm, port_token);
    switch(port_speed) {
...
/**
@name: tdm_b0_cmn_get_speed_slots
@param:
 */
int
tdm_b0_cmn_get_speed_slots(tdm_mod_t *_tdm, int port_speed)
{
    int speed_slots = 0;
    int slot_unit;

    slot_unit = tdm_b0_cmn_get_slot_unit(_tdm);
...
/**
@name: tdm_b0_cmn_get_port_slots
@param:
 */
int
tdm_b0_cmn_get_port_slots(tdm_mod_t *_tdm, int port_token)
{
    int port_slots, port_speed;

    port_speed = tdm_b0_cmn_get_port_speed_eth(_tdm, port_token);
    port_slots = tdm_b0_cmn_get_speed_slots(_tdm, port_speed);
...
/**
@name: tdm_b0_cmn_get_pipe_cal
@param:

Return calendar pointer of the current pipe.
 */
tdm_b0_calendar_t *
tdm_b0_cmn_get_pipe_cal(tdm_mod_t *_tdm)
{
    tdm_b0_calendar_t * cal = NULL;
 ...
 */
tdm_b0_calendar_t *
tdm_b0_cmn_get_pipe_cal_prev(tdm_mod_t *_tdm)
{
    tdm_b0_calendar_t * cal = NULL;
