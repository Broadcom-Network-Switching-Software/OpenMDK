...
/**
@name: tdm_b0_ap_b0_print_tbl
@param:

Prints PGW TDM main calendar
 */
void
tdm_b0_ap_b0_print_tbl(int *cal, int len, const char* name, int id)
{
    int idx;
...
/**
@name: tdm_b0_ap_b0_print_tbl_ovs
@param:

Prints PGW TDM calendar oversub scheduling groups
 */
void
tdm_b0_ap_b0_print_tbl_ovs(int *cal, int *spc, int len, const char* name, int id)
{
    int idx;
...
/**
@name: tdm_b0_ap_b0_print_quad
@param:
 */
void
tdm_b0_ap_b0_print_quad(enum port_speed_e *speed, enum port_state_e *state, int len, int idx_start, int idx_end)
{
    int iter, iter2;
...
/**
@name: tdm_b0_ap_b0_parse_mmu_tdm_b0_tbl
@param:
 */
int
tdm_b0_ap_b0_parse_mmu_tdm_b0_tbl( tdm_mod_t *_tdm )
{
    const char *name;
    int j, k=512, m=0,
        *mmu_tdm_b0_tbl, *mmu_tdm_b0_ovs_1, *mmu_tdm_b0_ovs_2, *mmu_tdm_b0_ovs_3, *mmu_tdm_b0_ovs_4, *mmu_tdm_b0_ovs_5, *mmu_tdm_b0_ovs_6,
    ...
    mmu_tdm_b0_tbl = cal->cal_main;
    mmu_tdm_b0_ovs_1 = cal->cal_grp[0];
    mmu_tdm_b0_ovs_2 = cal->cal_grp[1];
    mmu_tdm_b0_ovs_3 = cal->cal_grp[2];
    mmu_tdm_b0_ovs_4 = cal->cal_grp[3];
    mmu_tdm_b0_ovs_5 = cal->cal_grp[4];
    mmu_tdm_b0_ovs_6 = cal->cal_grp[5];

/*for (j=0; j<16; j++) {
        if (mmu_tdm_b0_ovs_1[j]!=TD2P_NUM_EXT_PORTS || mmu_tdm_b0_ovs_2[j]!=TD2P_NUM_EXT_PORTS || mmu_tdm_b0_ovs_3[j]!=TD2P_NUM_EXT_PORTS || mmu_tdm_b0_ovs_4[j]!=TD2P_NUM_EXT_PORTS) {
            oversub=BOOL_TRUE;
        }
    }*/
    /* Allocate slots for MGMT, CMIC/LPBK, ANCL and RERESH among available AP_ANCL_TOKENs */
    for (j=0; j<k; j++) {
        if (mmu_tdm_b0_tbl[j]!=AP_ANCL_TOKEN) {
            TDM_PRINT3("PIPE: %s, MMU TDM TABLE, element #%0d, contains physical port #%0d\n", name, j, mmu_tdm_b0_tbl[j]);
        }
        else {
