...
void
bcm56560_b0_xlmac_drain_cell(unit, port)
{
    int ioerr = 0;
    int cnt;
    ...
    if (cnt < 0) {
        CDK_WARN(("bcm56560_b0_bmd_port_mode_set[%d]: "
...
void
bcm56560_b0_clmac_drain_cell(unit, port)
{
    int ioerr = 0;
    int cnt;
...
void
bcm56560_b0_clg2mac_drain_cell(unit, port)
{
    int ioerr = 0;
    int cnt;
    CLG2MAC_TXFIFO_CELL_CNTr_t clg2cell_cnt;

    cnt = DRAIN_WAIT_MSEC / 10;
    while (--cnt >= 0) {
        ioerr += READ_CLG2MAC_TXFIFO_CELL_CNTr(unit, port, &clg2cell_cnt);
        if (CLG2MAC_TXFIFO_CELL_CNTr_CELL_CNTf_GET(clg2cell_cnt) == 0) {
            break;
        }
        BMD_SYS_USLEEP(10000);
    }
...
int
bcm56560_b0_bmd_port_mode_set(int unit, int port,
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    CLG2MAC_EEE_CTRLr_t clg2mac_eee_ctrl;
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
         ...
         */
        rv = bcm56560_b0_bmd_port_mode_get(unit, port,
                                           &cur_mode, &cur_flags);
        if (CDK_SUCCESS(rv) &&
            ...
            if (CDK_PBMP_MEMBER(clpbmp, port)) {
                rv = bcm56560_b0_clport_init(unit, port);
            } else if (CDK_PBMP_MEMBER(xlpbmp, port)) {
                rv = bcm56560_b0_xlport_init(unit, port);
            } else {
                rv = bcm56560_b0_clg2port_init(unit, port);
            }
        }
    }
    ...
    if (CDK_PBMP_MEMBER(clpbmp, port)) {
        ...
        ioerr += READ_CLG2MAC_CTRLr(unit, port, &clg2mac_ctrl);
        CLG2MAC_CTRLr_SOFT_RESETf_SET(clg2mac_ctrl, 1);
        ioerr += WRITE_CLG2MAC_CTRLr(unit, port, clg2mac_ctrl);

        /* Disable CLMACs (Rx only) */
        ioerr += READ_CLG2MAC_CTRLr(unit, port, &clg2mac_ctrl);
        CLG2MAC_CTRLr_RX_ENf_SET(clg2mac_ctrl, 0);
        ioerr += WRITE_CLG2MAC_CTRLr(unit, port, clg2mac_ctrl);
    }
    ...
    if (mode == bmdPortModeDisabled) {
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
            ...
            if (CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                ioerr += READ_CLG2MAC_MODEr(unit, port, &clg2mac_mode);
                ioerr += READ_CLG2PORT_CONFIGr(unit, port, &clg2port_cfg);
            }
            ...
            if (CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                CLG2MAC_MODEr_HDR_MODEf_SET(clg2mac_mode, hg2 ? 2 : hg);
            }
            ...
            if (CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                CLG2PORT_CONFIGr_HIGIG_MODEf_SET(clg2port_cfg, hg);
            }
            ...
            if (CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                CLG2PORT_CONFIGr_HIGIG2_MODEf_SET(clg2port_cfg, hg2);
                ioerr += WRITE_CLG2MAC_MODEr(unit, port, clg2mac_mode);
                ioerr += WRITE_CLG2PORT_CONFIGr(unit, port, clg2port_cfg);
            }
            ...
            if (CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                /* Configure MAC mode */
                ioerr += READ_CLG2MAC_MODEr(unit, port, &clg2mac_mode);
                CLG2MAC_MODEr_SPEED_MODEf_SET(clg2mac_mode, sp_sel);
                ioerr += WRITE_CLG2MAC_MODEr(unit, port, clg2mac_mode);

                /* Configure EEE */
                ioerr += READ_CLG2MAC_EEE_CTRLr(unit, port, &clg2mac_eee_ctrl);
                if (flags & BMD_PORT_MODE_F_EEE) {
                    /* Enable IEEE 802.3az EEE */
                    CLG2MAC_EEE_CTRLr_EEE_ENf_SET(clg2mac_eee_ctrl, 1);
                } else {
                    CLG2MAC_EEE_CTRLr_EEE_ENf_SET(clg2mac_eee_ctrl, 0);
                }
                ioerr += WRITE_CLG2MAC_EEE_CTRLr(unit, port, clg2mac_eee_ctrl);
            }
            ...
            ioerr += bcm56560_b0_egr_buf_reset(unit, port, 0);
            ...
            if (CDK_PBMP_MEMBER(clpbmp, port)) {
                ...
                ioerr += READ_CLG2MAC_CTRLr(unit, port, &clg2mac_ctrl);
                CLG2MAC_CTRLr_SOFT_RESETf_SET(clg2mac_ctrl, 0);
                CLG2MAC_CTRLr_RX_ENf_SET(clg2mac_ctrl, 1);
                CLG2MAC_CTRLr_TX_ENf_SET(clg2mac_ctrl, 1);
                CLG2MAC_CTRLr_LOCAL_LPBKf_SET(clg2mac_ctrl, mac_lb);
                if (flags & HG_FLAGS) {
                    CLG2MAC_CTRLr_XGMII_IPG_CHECK_DISABLEf_SET(clg2mac_ctrl, 1);
                } else {
                    CLG2MAC_CTRLr_XGMII_IPG_CHECK_DISABLEf_SET(clg2mac_ctrl, 0);
                }
                ioerr += WRITE_CLG2MAC_CTRLr(unit, port, clg2mac_ctrl);
            }
        }
