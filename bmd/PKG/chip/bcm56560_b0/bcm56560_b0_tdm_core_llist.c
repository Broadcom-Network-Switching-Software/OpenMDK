...
/**
@name: tdm_b0_ll_append
@param:

Appends to TDM linked list
 */
void
tdm_b0_ll_append(struct node *llist, unsigned short port_append)
{
    struct node *mbox,
...
/**
@name: tdm_b0_ll_insert
@param:

Inserts to TDM linked list
 */
void
tdm_b0_ll_insert(struct node *llist, unsigned short port_insert, int idx)
{
    int i;
    struct node *mbox,
                *right=llist,
    ...
    if (right==NULL) {
        tdm_b0_ll_append(llist,port_insert);
    }
    else {
        for (i=1; i<=idx; i++) {
            if(right != NULL){
...
/**
@name: tdm_b0_ll_delete
@param:

Delete element from TDM linked list
 */
int
tdm_b0_ll_delete(struct node *llist, int idx)
{
    int i;
...
/**
@name: tdm_b0_ll_print
@param:

Prints elements from TDM linked list
 */
void
tdm_b0_ll_print(struct node *llist)
{
    int i=0;
...
/**
@name: tdm_b0_ll_deref
@param:

Dereferences entire TDM linked list into unlinked array
 */
void
tdm_b0_ll_deref(struct node *llist, int *tdm, int lim)
{
    int i=0;
    struct node *list = llist;
 ...
 */
int
tdm_b0_ll_get(struct node *llist, int idx, int num_ext_ports)
{
    int i;
    struct node *list = llist;
 ...
 */
int
tdm_b0_ll_dist(struct node *llist, int idx)
{
    int i, dist=0;
...
/**
@name: tdm_b0_ll_strip
@param:

Strips all of one tokenized value from TDM linked list
 */
void
tdm_b0_ll_strip(struct node *llist, tdm_mod_t *_tdm, int cadence_start_idx, int *pool, int *s_idx, int token)
{
    ...
    while (list!=NULL) {
        if ( ((*s_idx)>cadence_start_idx) && ((list->port)==start_port) ) {
            ...
            tdm_b0_ll_delete(llist,(*s_idx));
            (*s_idx)--;
            (*pool)++;
        }
        if ( ((list->port)!=token) ) {
            if ( safe && (tdm_b0_ll_tsc_dist(llist,_tdm,(*s_idx))<=_tdm->_core_data.rule__prox_port_min) ) {
                /* Following region is not safe to strip */
                safe = BOOL_FALSE;
                store_dist=tdm_b0_ll_tsc_dist(llist,_tdm,(*s_idx));
                safe_count=0;
            }
        }
...
/**
@name: tdm_b0_ll_retrace
@param:

Retraces all null slots in TDM linked list inside specified cadence subject to spacing rules
 */
int
tdm_b0_ll_retrace(struct node *llist, tdm_mod_t *_tdm, int cadence_start_idx)
{
    int i, timeout, store_idx=cadence_start_idx, oversub_pool=0, starting_dist, ending_dist, relinked, num_ins=0, cadence_step;

    starting_dist=tdm_b0_ll_dist(llist,cadence_start_idx);
    tdm_b0_ll_strip(llist,_tdm,cadence_start_idx,&oversub_pool,&store_idx,_tdm->_chip_data.soc_pkg.num_ext_ports);
    if ( (tdm_b0_ll_dist(llist,cadence_start_idx)>oversub_pool) && (oversub_pool>0) ) {
        cadence_step=tdm_b0_ll_dist(llist,cadence_start_idx)/oversub_pool;
        for (i=(store_idx-1+num_ins); i>cadence_start_idx; i-=cadence_step) {
            if (tdm_b0_ll_get(llist,i,_tdm->_chip_data.soc_pkg.num_ext_ports)!=_tdm->_chip_data.soc_pkg.num_ext_ports) {
                tdm_b0_ll_insert(llist,_tdm->_chip_data.soc_pkg.num_ext_ports,(i+1));
                oversub_pool--;
            }
        }
        timeout=32000;
        ...
        for (i=(store_idx-1+num_ins); i>cadence_start_idx; i--) {
            if (tdm_b0_ll_get(llist,i,_tdm->_chip_data.soc_pkg.num_ext_ports)!=_tdm->_chip_data.soc_pkg.num_ext_ports) {
                if ( (tdm_b0_ll_dist(llist,i)<_tdm->_core_data.rule__same_port_min) ||
                     (tdm_b0_ll_tsc_dist(llist,_tdm,i)<_tdm->_core_data.rule__prox_port_min) ) {
                    break;
                }
            }
        }
        while (oversub_pool>0) {
            ...
            if (!relinked) {
                for (i=(cadence_start_idx+1); i<tdm_b0_ll_dist(llist,cadence_start_idx); i++) {
                    if (tdm_b0_ll_get(llist,i,_tdm->_chip_data.soc_pkg.num_ext_ports)!=_tdm->_chip_data.soc_pkg.num_ext_ports && tdm_b0_ll_get(llist,(i-1),_tdm->_chip_data.soc_pkg.num_ext_ports)!=_tdm->_chip_data.soc_pkg.num_ext_ports) {
                        tdm_b0_ll_insert(llist,_tdm->_chip_data.soc_pkg.num_ext_ports,i);
                        relinked=BOOL_TRUE; oversub_pool--;
                        break;
                    }
                }
                if (!relinked) {
                    tdm_b0_ll_insert(llist,_tdm->_chip_data.soc_pkg.num_ext_ports,(cadence_start_idx+1));
                    oversub_pool--;
                    continue;
                }
            }
    ...
    ending_dist=tdm_b0_ll_dist(llist,cadence_start_idx);

    return tdm_b0_abs(ending_dist-starting_dist);
}
 ...
 */
int
tdm_b0_ll_free(struct node *llist)
{
