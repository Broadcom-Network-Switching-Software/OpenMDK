...
_config_port(int unit, int port, uint32_t vlan_flags, uint32_t port_mode_flags)
{
    ...
    rv = bcm56560_b0_bmd_vlan_port_add(unit, BMD_CONFIG_DEFAULT_VLAN,
                                       port, vlan_flags);
    if (CDK_SUCCESS(rv)) {
        rv = bcm56560_b0_bmd_port_stp_set(unit, port,
    ...
    if (CDK_SUCCESS(rv)) {
        ...
        if (speed_max == 100000) {
            rv = bcm56560_b0_bmd_port_mode_set(unit, port, bmdPortMode100000fd,
                                               port_mode_flags);
        } else if (speed_max == 40000) {
            rv = bcm56560_b0_bmd_port_mode_set(unit, port, bmdPortMode40000fd,
                                               port_mode_flags);
        } else if (speed_max == 10000) {
            rv = bcm56560_b0_bmd_port_mode_set(unit, port, bmdPortMode10000fd,
                                               port_mode_flags);
        } else if (speed_max == 1000) {
            rv = bcm56560_b0_bmd_port_mode_set(unit, port, bmdPortModeAuto,
                                               port_mode_flags);
        }
    }
...
int
bcm56560_b0_bmd_switching_init(int unit)
{
    int ioerr = 0;
    int rv;
    int port, lport;
    ...
    rv = bcm56560_b0_bmd_reset(unit);
    if (CDK_SUCCESS(rv)) {
        rv = bcm56560_b0_bmd_init(unit);
    }

    if (CDK_SUCCESS(rv)) {
        rv = bcm56560_b0_bmd_vlan_create(unit, BMD_CONFIG_DEFAULT_VLAN);
    }
    vlan_flags = BMD_VLAN_PORT_F_UNTAGGED;

    bcm56560_b0_all_front_pbmp_get(unit, &pbmp);
    CDK_PBMP_ITER(pbmp, port) {
        if (bcm56560_b0_port_speed_max(unit, port) <= 0) {
            /* Disabled flexport*/
            continue;
        }
        port_mode_flags = 0;
