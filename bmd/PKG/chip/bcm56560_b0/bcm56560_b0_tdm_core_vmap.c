...
/**
@name: tdm_b0_vmap_print_vmap
@param:
 */
void
tdm_b0_vmap_print_vmap(tdm_mod_t *_tdm, unsigned short **vmap)
{
    ...
    vmap_wid = tdm_b0_vmap_get_vmap_wid(_tdm, vmap);
    vmap_len = tdm_b0_vmap_get_vmap_len(_tdm, vmap);
    /* print vmap */
    TDM_PRINT0("\t TDM vmap \n");
    TDM_PRINT1("%6s\t", " ");
    TDM_PRINT1("%6s\t", " ");
...
/**
@name: tdm_b0_vmap_print_pmlist
@param:
 */
void
tdm_b0_vmap_print_pmlist(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist, int pmlist_size)
{
    int i, j;
    if (_tdm != NULL && pmlist != NULL) {
        for (i = 0; i < pmlist_size; i++) {
            ...
            TDM_PRINT0("], speed [");
            for (j = 0; j < pmlist[i].subport_cnt; j++) {
                TDM_PRINT1("%3d", (tdm_b0_cmn_get_port_speed(_tdm,
...
/**
@name: tdm_b0_core_vmap_alloc_mix
@param:
 */
int
tdm_b0_core_vmap_alloc_mix(tdm_mod_t *_tdm)
{
    return (tdm_b0_core_vmap_alloc(_tdm));
}


/**
@name: tdm_b0_vmap_chk_pipe_bandwidth
@param:
 */
int
tdm_b0_vmap_chk_pipe_bandwidth(tdm_mod_t *_tdm, int *lr_buff, int lr_buff_size)
{
    ...
    if (param_lr_en == BOOL_TRUE) {
        for (i = 0; i < lr_buff_size; i++) {
            slot_num = tdm_b0_cmn_get_port_slots(_tdm, lr_buff[i]);
            if (slot_num > 0) {
...
/**
@name: tdm_b0_vmap_chk_singularity
@param:
 */
int
tdm_b0_vmap_chk_singularity(tdm_mod_t *_tdm, unsigned short **vmap)
{
...
/**
@name: tdm_b0_vmap_chk_port_space
@param:
 */
int
tdm_b0_vmap_chk_port_space(tdm_mod_t *_tdm, int *cal, int cal_len, int espace,
                        int idx, int dir)
{
    ...
    if (_tdm != NULL && cal != NULL && idx >= 0 && idx < cal_len) {
        ...
        if (tdm_b0_cmn_chk_port_is_fp(_tdm, cal[idx])) {
            d1 = tdm_b0_vmap_calc_port_dist_same(_tdm, cal, cal_len, espace,
                                              idx, dir);
            d2 = tdm_b0_vmap_calc_port_dist_sister(_tdm, cal, cal_len, espace,
                                                idx, dir);
...
/**
@name: tdm_b0_vmap_chk_sister
@param:
 */
int
tdm_b0_vmap_chk_sister(tdm_mod_t *_tdm)
{
    ...
    if (param_lr_en == BOOL_TRUE && param_space_sister > 0) {
        for (i = 0; i < param_cal_len; i++) {
            if (tdm_b0_cmn_chk_port_is_fp(_tdm, param_cal_main[i])) {
                dist = tdm_b0_vmap_calc_port_dist_sister(_tdm, param_cal_main,
...
/**
@name: tdm_b0_vmap_chk_same
@param:
 */
int
tdm_b0_vmap_chk_same(tdm_mod_t *_tdm)
{
    ...
    if (param_lr_en == BOOL_TRUE && param_space_same > 0) {
        for (i = 0; i < param_cal_len; i++) {
            if (tdm_b0_cmn_chk_port_is_fp(_tdm, param_cal_main[i])) {
                dist = tdm_b0_vmap_calc_port_dist_same(_tdm, param_cal_main,
                            param_cal_len, 0, i, TDM_DIR_DN);
                if (dist != 0) {
                    result = FAIL;
...
/**
@name: tdm_b0_vmap_chk_vmap_sister_xy
@param:
 */
int
tdm_b0_vmap_chk_vmap_sister_xy(tdm_mod_t *_tdm, unsigned short **vmap, int col, int row)
{
    ...
    if ( !(col >= 0 && col < param_vmap_wid &&
        ...
        if (tdm_b0_cmn_chk_port_is_fp(_tdm, port_token)) {
            vmap_len = tdm_b0_vmap_get_vmap_len(_tdm, vmap);
            vmap_wid = tdm_b0_vmap_get_vmap_wid(_tdm, vmap);
            port_pm  = tdm_b0_cmn_get_port_pm(_tdm, port_token);
            min_sister_space = tdm_b0_vmap_get_port_space_sister(_tdm, port_token);
            for (i = 0; i < vmap_wid; i++) {
                for (k = 1; k < min_sister_space; k++) {
                    /* TDM_DIR_DN */
                    j = (row + k) % vmap_len;
                    if (tdm_b0_cmn_chk_port_is_fp(_tdm, vmap[i][j])) {
                        pm_tmp = tdm_b0_cmn_get_port_pm(_tdm, vmap[i][j]);
                        if (pm_tmp == port_pm) {
                            result = FAIL;
                            break;
                        }
...
/**
@name: tdm_b0_vmap_chk_vmap_sister_col
@param:
 */
int
tdm_b0_vmap_chk_vmap_sister_col(tdm_mod_t *_tdm, unsigned short **vmap, int col)
{
    ...
    if ( !(col >= 0 && col < param_vmap_wid)) {
        ...
        if (col < vmap_wid) {
            for (row = 0; row < vmap_len; row++) {
                if (tdm_b0_cmn_chk_port_is_fp(_tdm, vmap[col][row])) {
                    if (PASS != tdm_b0_vmap_chk_vmap_sister_xy(_tdm, vmap,
                                    col, row)) {
                        result = FAIL;
                        break;
                    }
...
/**
@name: tdm_b0_vmap_chk_vmap_sister
@param:
 */
int
tdm_b0_vmap_chk_vmap_sister(tdm_mod_t *_tdm, unsigned short **vmap)
{
    ...
    if (param_space_sister > 0) {
        vmap_wid = tdm_b0_vmap_get_vmap_wid(_tdm, vmap);
        for (i = 0; i < vmap_wid; i++) {
            if (tdm_b0_vmap_chk_vmap_sister_col(_tdm, vmap, i) != PASS) {
                result = FAIL;
                break;
            }
        }
...
/**
@name: tdm_b0_vmap_chk_lr_ports
@param:
 */
int
tdm_b0_vmap_chk_lr_ports(tdm_mod_t *_tdm, int *lr_buff, int lr_buff_size)
{
    int result = PASS;
    ...
    if (lr_buff != NULL && lr_buff_size > 0) {
        for (i = 0; i < lr_buff_size; i++) {
            ...
            if (tdm_b0_cmn_chk_port_is_fp(_tdm, port_phynum)) {
...
/**
@name: tdm_b0_vmap_op_pm_reset
@param:
 */
void
tdm_b0_vmap_op_pm_reset(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pm)
{
    int j;
    int param_token_empty;
...
/**
@name: tdm_b0_vmap_op_pm_update
@param:
 */
void
tdm_b0_vmap_op_pm_update(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pm)
{
    int j, subport_cnt;
    ...
    if (_tdm != NULL && pm != NULL) {
        if ((*pm).pm_en) {
            ...
            /* pm_num */
            (*pm).pm_num = tdm_b0_cmn_get_port_pm(_tdm, (*pm).subport_phy[0]);
            /* pm_slots */
            (*pm).pm_slots = 0;
            for (j = 0; j < TDM_VMAP_PM_LANES && j < subport_cnt; j++) {
            ...
            tdm_b0_vmap_op_pm_reset(_tdm, pm);
        }
    }
}
...
/**
@name: tdm_b0_vmap_op_pm_swap
@param:
 */
void
tdm_b0_vmap_op_pm_swap(tdm_b0_vmap_pm_t *pmlist, int pmlist_size,
                    int src_idx, int dst_idx)
{
    int i, j, n;
    tdm_b0_vmap_pm_t tmp;
...
/**
@name: tdm_b0_vmap_op_pm_migrate
@param:
 */
void
tdm_b0_vmap_op_pm_migrate(tdm_b0_vmap_pm_t *pmlist, int pmlist_size,
                       int src_idx, int dst_idx)
{
    int i, j, n;
...
/**
@name: tdm_b0_vmap_op_pm_addPort
@param:
 */
int
tdm_b0_vmap_op_pm_addPort(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pm,
...
/**
@name: tdm_b0_vmap_op_pmlist_init
@param:
 */
void
tdm_b0_vmap_op_pmlist_init(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist, int pmlist_size)
{
    int i;

    if (_tdm != NULL && pmlist != NULL) {
        for (i = 0; i < pmlist_size; i++) {
            tdm_b0_vmap_op_pm_reset(_tdm, &(pmlist[i]));
        }
    }
}
...
/**
@name: tdm_b0_vmap_op_pmlist_gen
@param:
 */
int
tdm_b0_vmap_op_pmlist_gen(tdm_mod_t *_tdm, int *lr_buff, int lr_buff_size,
    ...
    if (_tdm != NULL && lr_buff != NULL && pmlist != NULL) {
        ...
        for (i = 0; i < lr_buff_size; i++) {
            ...
            for (j = lr_buff_size - 1; j > i; j--) {
                ...
                slots1 = tdm_b0_cmn_get_port_slots(_tdm, lr_buff[j-1]);
                slots2 = tdm_b0_cmn_get_port_slots(_tdm, lr_buff[j]);
        ...
        for (i = 0; i < lr_buff_size; i++) {
            ...
            port_pm = tdm_b0_cmn_get_port_pm(_tdm, port_phynum);
            ...
            if (pm_idx < pmlist_size) {
                port_slots = tdm_b0_cmn_get_port_slots(_tdm, port_phynum);
                if (tdm_b0_vmap_op_pm_addPort(_tdm, &(pmlist[pm_idx]),
        ...
        /* print pm list */
        tdm_b0_vmap_print_pmlist(_tdm, pmlist, pmlist_size);
    }
...
/**
@name: tdm_b0_vmap_op_pmlist_sort
@param:
 ...
 */
void
tdm_b0_vmap_op_pmlist_sort(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                        int pmlist_size)
{
    int i, j, swap_happen;
        ...
        for (i = 0; i < pmlist_size; i++) {
            ...
            for (j = pmlist_size - 1; j > i; j--) {
                ...
                if (pmlist[j].pm_slots >
                    pmlist[j-1].pm_slots) {
                    tdm_b0_vmap_op_pm_swap(pmlist, pmlist_size, j, j-1);
 ...
 */
void
tdm_b0_vmap_op_pmlist_sort_2(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                        int pmlist_size)
{
    int i, j, swap_happen;
...
/**
@name: tdm_b0_vmap_op_pmlist_shift
@param:

@desc: shift all pms starting from pm_idx forward by 1 step.
 */
void
tdm_b0_vmap_op_pmlist_shift(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                         int pmlist_size, int pm_idx)
{
    ...
    if (_tdm != NULL && pmlist != NULL) {
        ...
        for (i = pm_idx_max; i >= pm_idx && i > 0 ; i--) {
            tdm_b0_vmap_op_pm_migrate(pmlist, pmlist_size, i, i+1);
        }

        /* clear pm at pm_idx */
        if (pm_idx < pmlist_size) {
            tdm_b0_vmap_op_pm_reset(_tdm, &(pmlist[pm_idx]));
        }
    }
}
...
/**
@name: tdm_b0_vmap_op_pmlist_adjust_lr_4lanes
@param:
 ...
 */
int
tdm_b0_vmap_op_pmlist_adjust_lr_4lanes(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                                    int pmlist_size)
{
        ...
        for (i = 0; i < pmlist_size; i++) {
            ...
            if (subport_cnt==2 && !(s[0]==s[1])) {
                ...
                if (split_en_dual == BOOL_TRUE) {
                    tdm_b0_vmap_op_pm_addPort(_tdm, &(pmlist[pm_cnt]),
...
/**
@name: tdm_b0_vmap_op_pmlist_adjust_lr
@param:
 */
int
tdm_b0_vmap_op_pmlist_adjust_lr(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                             int pmlist_size)
{
    ...
    /* case 1: 4-lane PM */
    tdm_b0_vmap_op_pmlist_adjust_lr_4lanes(_tdm, pmlist, pmlist_size);
...
/**
@name: tdm_b0_vmap_op_pmlist_adjust_os_4lanes
@param:
 ...
 */
int
tdm_b0_vmap_op_pmlist_adjust_os_4lanes(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                                    int pmlist_size)
{
    int i, n, result = PASS;
        ...
        if (os_2_lr_r > 0) {
            for (i = 0; i < (pmlist_size - 1); i++) {
                ...
                if (pm_slot_num > 0 && empty_slot_left > 0) {
                    ...
                    empty_pm_num = tdm_b0_math_div_ceil(slot_allc, pm_slot_num);
                    for (n=0; n<empty_pm_num && i<(pmlist_size-1); n++) {
                        ...
                        tdm_b0_vmap_op_pmlist_shift(_tdm, pmlist, pmlist_size,
                                               i+1);
                        tdm_b0_vmap_op_pm_addPort(_tdm, &(pmlist[i+1]),
            ...
            if (empty_slot_left > 0) {
                ...
                if (pmlist[i].subport_phy[0] == param_token_unused && i > 1) {
                    if (pmlist[i-1].subport_phy[0] != param_token_unused &&
                        ...
                        tdm_b0_vmap_op_pm_swap(pmlist, pmlist_size, i, i-1);
...
/**
@name: tdm_b0_vmap_op_pmlist_adjust_os
@param:
 */
int
tdm_b0_vmap_op_pmlist_adjust_os(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                             int pmlist_size)
{
    int param_num_pm_lanes;
    ...
    /* case 1: 4-lane PM */
    tdm_b0_vmap_op_pmlist_adjust_os_4lanes(_tdm, pmlist, pmlist_size);
...
/**
@name: tdm_b0_vmap_get_vmap_wid
@param:
 */
int
tdm_b0_vmap_get_vmap_wid(tdm_mod_t *_tdm, unsigned short **vmap)
{
    int i, j, vmap_wid = 0;
...
/**
@name: tdm_b0_vmap_get_vmap_len
@param:
 */
int
tdm_b0_vmap_get_vmap_len(tdm_mod_t *_tdm, unsigned short **vmap)
{
    int vmap_len = 0;
...
/**
@name: tdm_b0_vmap_get_port_space_sister
@param:
@desc: Return sister port space limit of the given port.
 */
int
tdm_b0_vmap_get_port_space_sister(tdm_mod_t *_tdm, int port_token)
{
...
/**
@name: tdm_b0_vmap_get_port_space_same
@param:
@desc: Return same port space limit of the given port.
 */
int
tdm_b0_vmap_get_port_space_same(tdm_mod_t *_tdm, int port_token)
{
...
/**
@name: tdm_b0_vmap_calc_port_dist_sister
@param:
    -- cal    : slot array
 ...
 */
int
tdm_b0_vmap_calc_port_dist_sister(tdm_mod_t *_tdm, int *cal, int cal_len,
                              int espace, int idx, int dir)
{
    ...
    if (cal != NULL && cal_len > 0 && idx >= 0 && idx < cal_len) {
        if (tdm_b0_cmn_chk_port_is_fp(_tdm, cal[idx])) {
            space = tdm_b0_vmap_get_port_space_sister(_tdm, cal[idx]) + espace;
            port_pm = tdm_b0_cmn_get_port_pm(_tdm, cal[idx]);
            for (n = 1; n < space; n++) {
                k = (dir == TDM_DIR_DN) ? ((idx + n) % cal_len) :
                                          ((cal_len + idx - n) % cal_len);
                if (tdm_b0_cmn_chk_port_is_fp(_tdm, cal[k])) {
                    if (port_pm == tdm_b0_cmn_get_port_pm(_tdm, cal[k])) {
                        distance = n;
                        break;
                    }
                }
...
/**
@name: tdm_b0_vmap_calc_port_dist_same
@param:
    -- cal    : slot array
    -- cal_len: size of slot array
    -- espace : extra space added into space limit, default is 0.
 ...
 */
int
tdm_b0_vmap_calc_port_dist_same(tdm_mod_t *_tdm, int *cal, int cal_len,
                            int espace, int idx, int dir)
{
    ...
    if (cal != NULL && cal_len > 0 && idx >= 0 && idx < cal_len) {
        if (tdm_b0_cmn_chk_port_is_fp(_tdm, cal[idx])) {
            space = tdm_b0_vmap_get_port_space_same(_tdm, cal[idx]) + espace;
            for (n = 1; n < space; n++) {
                k = (dir == TDM_DIR_DN) ? ((idx + n) % cal_len) :
                                          ((cal_len + idx - n) % cal_len);
                if (cal[idx] == cal[k]) {
...
/**
@name: tdm_b0_vmap_gen_lr_buff
@param:
 */
int
tdm_b0_vmap_gen_lr_buff(int *src_buff, int src_buff_size,
...
/**
@name: tdm_b0_vmap_switch_vmap_col
@param:
 ...
 */
int
tdm_b0_vmap_switch_vmap_col(tdm_mod_t *_tdm, unsigned short **vmap,
                         int col_x, int col_y)
{
...
/**
@name: tdm_b0_vmap_rotate_vmap
@param:
 */
int
tdm_b0_vmap_rotate_vmap(tdm_mod_t *_tdm, unsigned short **vmap)
{
    ...
    for (i = vmap_wid - 1; i > 0; i--) {
        if (tdm_b0_vmap_chk_vmap_sister_col(_tdm, vmap, i) != PASS) {
            ...
            for (j = i - 1; j >= 0; j--) {
                if(tdm_b0_vmap_switch_vmap_col(_tdm, vmap, i, j) == PASS) {
                    chk_i = tdm_b0_vmap_chk_vmap_sister_col(_tdm, vmap, i);
                    chk_j = tdm_b0_vmap_chk_vmap_sister_col(_tdm, vmap, j);
                    if (chk_i != PASS) {
                        /* switch back column i with j */
                        tdm_b0_vmap_switch_vmap_col(_tdm, vmap, i, j);
                    } else {
                        if (chk_j == PASS) {
...
/**
@name: tdm_b0_vmap_shift_port
@param:
 */
int
tdm_b0_vmap_shift_port(tdm_mod_t *_tdm, int port_token)
{
    ...
    for (n = 0; n < min_sister_space; n++) {
        ...
        for (i = 0; i < param_cal_len; i++) {
            if (param_cal_main[i] == port_token) {
                d_dn = tdm_b0_vmap_calc_port_dist_sister(_tdm, param_cal_main,
                        param_cal_len, 0, i, TDM_DIR_DN);
                d_up = tdm_b0_vmap_calc_port_dist_sister(_tdm, param_cal_main,
                ...
                if (d_dn > 0) {
                    for (j = 0; j < param_cal_len; j++) {
                        if (param_cal_main[j] == port_token) {
                            ...
                            d_x1 = tdm_b0_vmap_calc_port_dist_sister(_tdm,
                                        param_cal_main, param_cal_len, 0,
                                        x1, TDM_DIR_UP);
                            d_x2 = tdm_b0_vmap_calc_port_dist_sister(_tdm,
                                        param_cal_main, param_cal_len, 0,
                                        x2, TDM_DIR_DN);
                            if (d_x1 == 0 && d_x2 == 0) {
...
/**
@name: tdm_b0_vmap_filter_same
@param:
 */
int
tdm_b0_vmap_filter_same(tdm_mod_t *_tdm)
{
    ...
    for (i = 0; i < param_cal_len; i++) {
        ...
        if (tdm_b0_cmn_chk_port_is_fp(_tdm, port_i)) {
            dist_i = tdm_b0_vmap_calc_port_dist_same(_tdm, param_cal_main,
                        param_cal_len, 0, i, TDM_DIR_DN);
            if (dist_i != 0) {
                space_i = tdm_b0_vmap_get_port_space_same(_tdm, port_i);
                for (m = 1; m < (space_i - dist_i); m++) {
                    x = (param_cal_len + i - m) % param_cal_len;
                    y = (x + 1) % param_cal_len;
                    chk_x = tdm_b0_vmap_chk_port_space(_tdm, param_cal_main,
                                param_cal_len, 1, x, TDM_DIR_DN);
                    chk_y = tdm_b0_vmap_chk_port_space(_tdm, param_cal_main,
    ...
    result = tdm_b0_vmap_chk_same(_tdm);
    return result;
}


/**
@name: tdm_b0_vmap_filter_sister
@param:
 */
int
tdm_b0_vmap_filter_sister(tdm_mod_t *_tdm)
{
        ...
        for (i=0; i<param_cal_len; i++) {
            ...
            if (tdm_b0_cmn_chk_port_is_fp(_tdm, port_i)) {
                ...
                if (dist != 0) {
                    ...
                    if (tdm_b0_cmn_chk_port_is_fp(_tdm, port_j)) {
                        port = (param_speeds[port_i] < param_speeds[port_j]) ?
                               port_i : port_j;
                        tdm_b0_vmap_shift_port(_tdm, port);
    ...
    result = tdm_b0_vmap_chk_sister(_tdm);
    return result;
}


/**
@name: tdm_b0_vmap_calc_slot_pos
@param:
 */
int
tdm_b0_vmap_calc_slot_pos(int vmap_idx, int slot_idx, int vmap_idx_max, int slot_idx_max, int **r_a_arr)
{
    ...
    if (r_a_arr!=NULL &&
        ...
        if (check == PASS) {
            ...
            for (j=n; j>0; j--) {
                ...
                b = r_a_arr[j-1][1];
                if (j==n) {
                    pos = 1 + tdm_b0_math_div_floor(((i-1)*b), a);
                }
                else{
                    pos = pos + tdm_b0_math_div_ceil((pos*a), (b-a));
                }
            }
        }
    }
...
/**
@name: tdm_b0_vmap_alloc_one_port
@param:
 */
int
tdm_b0_vmap_alloc_one_port(int port_token, int slot_req, int slot_left, int cal_len, int vmap_idx, int vmap_wid_max, int vmap_len_max, unsigned short **vmap, int **r_a_arr)
{
    ...
    if (vmap != NULL  && r_a_arr != NULL &&
        ...
        for (i=0; i<slot_req; i++) {
            pos = tdm_b0_vmap_calc_slot_pos(vmap_idx, i, vmap_wid_max, cal_len,
                                         r_a_arr);
...
/**
@name: tdm_b0_vmap_part_pm_subports
@param:
 */
int
tdm_b0_vmap_part_pm_subports(tdm_mod_t *_tdm, int port_token,
tdm_b0_vmap_pm_t *pm, int vmap_idx, unsigned short **vmap)
{
    ...
    for (i = 0; i < tmp_wid; i++) {
        ...
        tmp_result = tdm_b0_vmap_alloc_one_port(
...
/**
@name: tdm_b0_vmap_patch_special_case
@param:
 */
void
tdm_b0_vmap_patch_special_case(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                            int pmlist_size)
{
    int i, j;
...
/**
@name: tdm_b0_vmap_gen_vmap
@param:
 */
int
tdm_b0_vmap_gen_vmap(tdm_mod_t *_tdm, tdm_b0_vmap_pm_t *pmlist,
                  int pmlist_size, unsigned short **vmap)
{
    ...
    for (i = 0; i < pmlist_size; i++) {
        ...
        if (port_cnt > 0 && slot_req > 0) {
            ...
            result_tmp = tdm_b0_vmap_alloc_one_port(port_token, slot_req,
            ...
            /* if (port_cnt > 1) { */
            if (1) {
                tdm_b0_vmap_part_pm_subports(_tdm, port_token,
...
/**
@name: tdm_b0_vmap_alloc
@param:
 */
int
tdm_b0_vmap_alloc(tdm_mod_t *_tdm)
{
    int i, j, result = PASS;
    int *lr_buff, lr_buff_size;
    unsigned short **vmap;
    int pmlist_size;
    tdm_b0_vmap_pm_t *pmlist;
    ...
    /* print stack usage */
    /* TDM_PRINT_STACK_SIZE("tdm_b0_core_vmap_alloc"); */
    ...
    /* allocate memory */
    pmlist  = (tdm_b0_vmap_pm_t *) TDM_ALLOC(
                        pmlist_size * sizeof(tdm_b0_vmap_pm_t), "pmlist");
    ...
    /* initialize memory */
    tdm_b0_vmap_op_pmlist_init(_tdm, pmlist, pmlist_size);
    for (i = 0; i < lr_buff_size; i++) {
    ...
    if (param_lr_en == BOOL_TRUE) {
        /* construct lr_buff */
        tdm_b0_vmap_gen_lr_buff(param_lr_buff, TDM_AUX_SIZE,
                             lr_buff, lr_buff_size, param_token_empty);
        /* check all ports in lr_buff */
        if (tdm_b0_vmap_chk_lr_ports(_tdm, lr_buff, lr_buff_size) != PASS) {
            result = FAIL;
        }
    }

    /* print/check pipe bandwidth */
    tdm_b0_vmap_chk_pipe_bandwidth(_tdm, lr_buff, lr_buff_size);

    if (param_lr_en == BOOL_TRUE && result == PASS) {
        /* construct pmlist based on ports in lr_buff */
        if (tdm_b0_vmap_op_pmlist_gen(_tdm, lr_buff, lr_buff_size, pmlist,
                                 pmlist_size) != PASS) {
            result = FAIL;
        }
        ...
        tdm_b0_vmap_op_pmlist_adjust_lr(_tdm, pmlist, pmlist_size);

        /* sort pmlist by max_pm_port_speed from highest to lowest */
        tdm_b0_vmap_op_pmlist_sort(_tdm, pmlist, pmlist_size);

        /* adjust pmlist for os: insert pm(s) for EMPTY slots (optional) */
        tdm_b0_vmap_op_pmlist_adjust_os(_tdm, pmlist, pmlist_size);

        /* apply patch for SPECIAL CASE */
        tdm_b0_vmap_patch_special_case(_tdm, pmlist, pmlist_size);

        /* allocate slots for each PM in pmlist */
        if (PASS != tdm_b0_vmap_gen_vmap(_tdm, pmlist, pmlist_size, vmap)) {
            result = FAIL;
        }

        /* check and fix vmap sister port spacing violation */
        if (tdm_b0_vmap_chk_vmap_sister(_tdm, vmap) != PASS) {
            tdm_b0_vmap_rotate_vmap(_tdm, vmap);
        }

        /* check vmap singularity */
        if (tdm_b0_vmap_chk_singularity(_tdm, vmap) != PASS) {
            result = FAIL;
        }

        /* print vmap */
        tdm_b0_vmap_print_vmap(_tdm, vmap);
    }
...
/**
@name: tdm_b0_core_vmap_alloc
@param:
 */
int
tdm_b0_core_vmap_alloc(tdm_mod_t *_tdm)
{
    int i, j, port_token;
    int param_lr_limit, param_ancl_num, param_cal_len,
    ...
    TDM_SEL_CAL(_tdm->_core_data.vars_pkg.cal_id,param_cal_main);
    if (param_lr_en == BOOL_TRUE) {
        if(tdm_b0_vmap_alloc(_tdm) != PASS) {
    ...
    if (tdm_b0_vmap_chk_sister(_tdm) != PASS) {
        if (tdm_b0_vmap_filter_sister(_tdm) != PASS) {
            _tdm->_core_data.vars_pkg.refactor_done = BOOL_FALSE;
        }
    }

    /* check/adjust same port space */
    if (tdm_b0_vmap_chk_same(_tdm) != PASS) {
        if (tdm_b0_vmap_filter_same(_tdm) != PASS) {
            _tdm->_core_data.vars_pkg.refactor_done = BOOL_FALSE;
        }
    }
...
/**
@name: tdm_b0_core_vmap_prealloc
@param:

Shim for TDM5
 */
int
tdm_b0_core_vmap_prealloc(tdm_mod_t *_tdm)
{
    ...
    for (i = 0; i < TDM_AUX_SIZE; i++) {
        ...
        if (tdm_b0_cmn_chk_port_is_fp(_tdm, port_phynum)) {
            if (param_states[port_phynum-1] == PORT_STATE__LINERATE ||
                ...
                spd_idx = tdm_b0_cmn_get_port_speed_idx(_tdm, port_phynum);
    ...
    _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.num_lr     = lr_cnt;
    _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.num_100g   = lr_cnt_100g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.num_40g    = lr_cnt_40g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.y8 = lr_cnt_1g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.y1 = lr_cnt_10g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.y2 = lr_cnt_20g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.y6 = lr_cnt_25g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.y3 = lr_cnt_40g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.y5 = lr_cnt_50g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.y4 = lr_cnt_100g;
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.y7 = lr_cnt_120g;
    ...
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.z8 = pipe_info->os_prt_cnt[TDM_SPEED_IDX_1G];
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.z1 = pipe_info->os_prt_cnt[TDM_SPEED_IDX_10G];
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.z2 = pipe_info->os_prt_cnt[TDM_SPEED_IDX_20G];
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.z6 = pipe_info->os_prt_cnt[TDM_SPEED_IDX_25G];
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.z3 = pipe_info->os_prt_cnt[TDM_SPEED_IDX_40G];
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.z5 = pipe_info->os_prt_cnt[TDM_SPEED_IDX_50G];
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.z4 = pipe_info->os_prt_cnt[TDM_SPEED_IDX_100G];
    _tdm->_core_data.vars_pkg.m_tdm_b0_core_vbs_scheduler.z7 = pipe_info->os_prt_cnt[TDM_SPEED_IDX_120G];
    _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.num_os = pipe_info->num_os_prt;
    for (i = 1; i < TDM_AUX_SIZE; i++) {
        _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.os1[i]   = pipe_info->os_prt[TDM_SPEED_IDX_1G][i-1];
        _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.os10[i]  = pipe_info->os_prt[TDM_SPEED_IDX_10G][i-1];
        _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.os20[i]  = pipe_info->os_prt[TDM_SPEED_IDX_20G][i-1];
        _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.os25[i]  = pipe_info->os_prt[TDM_SPEED_IDX_25G][i-1];
        _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.os40[i]  = pipe_info->os_prt[TDM_SPEED_IDX_40G][i-1];
        _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.os50[i]  = pipe_info->os_prt[TDM_SPEED_IDX_50G][i-1];
        _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.os100[i] = pipe_info->os_prt[TDM_SPEED_IDX_100G][i-1];
        _tdm->_core_data.vars_pkg.m_tdm_b0_vmap_alloc.os120[i] = pipe_info->os_prt[TDM_SPEED_IDX_120G][i-1];
    }
