...
int
bcm56560_b0_bmd_port_mode_get(int unit, int port,
                              bmd_port_mode_t *mode, uint32_t *flags)
{
    ...
    CLG2MAC_CTRLr_t clgmac_ctrl;
    CLG2PORT_CONFIGr_t clgport_cfg;
    cdk_pbmp_t clpbmp, clgpbmp, xlpbmp, cxxpbmp;
    ...
    *flags = 0;
    ...
    if (CDK_PBMP_MEMBER(clpbmp, port)) {
        ...
        *mode = bmdPortMode10000fd;
        ...
        ioerr += READ_CLG2MAC_CTRLr(unit, port, &clgmac_ctrl);
        *mode = bmdPortMode10000fd;
        if (CLG2MAC_CTRLr_LOCAL_LPBKf_GET(clgmac_ctrl) != 0) {
            *flags |= BMD_PORT_MODE_F_MAC_LOOPBACK;
        } else {
            get_phy_mode = 1;
        }

        ioerr += READ_CLG2PORT_CONFIGr(unit, port, &clgport_cfg);
        if (CLG2PORT_CONFIGr_HIGIG_MODEf_GET(clgport_cfg)) {
            if (CLG2PORT_CONFIGr_HIGIG2_MODEf_GET(clgport_cfg)) {
                *flags |= BMD_PORT_MODE_F_HIGIG2;
            } else {
                *flags |= BMD_PORT_MODE_F_HIGIG;
            }
