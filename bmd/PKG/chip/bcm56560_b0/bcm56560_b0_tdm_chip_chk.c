...
/**
@name: tdm_b0_ap_b0_chk_ethernet
 ...
 */
int tdm_b0_ap_b0_chk_ethernet(tdm_mod_t *_tdm)
{
    int result, chk[2];
    ...
    if(_tdm->_core_data.vars_pkg.pipe == AP_PIPE_X_ID){
        ...
        chk[0] = tdm_b0_ap_b0_check_ethernet(_tdm);
        _tdm->_core_data.vars_pkg.port = 37;
        chk[1] = tdm_b0_ap_b0_check_ethernet(_tdm);
    }
...
int tdm_b0_ap_b0_chk_struc(tdm_mod_t *_tdm)
{
    ...
    /* Ethernet */
    if ( tdm_b0_ap_b0_chk_ethernet(_tdm) ) {
        switch (freq) {
      /*AP*/
    ...
    for (i=1; i<(num_ext_ports-57); i++){
        ...
        if (  port_states[i-1]==PORT_STATE__LINERATE    ||
            ...
            _tdm->_core_data.vars_pkg.port = i;
              if(tdm_b0_ap_b0_which_tsc(_tdm)==num_ext_ports){
...
/**
@name: tdm_b0_ap_b0_chk_min
@param:

Checks min spacing in TDM (TSC sister port spacing)
 */
int tdm_b0_ap_b0_chk_min(tdm_mod_t *_tdm)
{
...
/*  for (i=1; i<cal_len; i++){
        AP_TOKEN_CHECK(mmu_mtbl[i]) {
            tsc0 = tdm_b0_ap_b0_legacy_which_tsc(mmu_mtbl[i],tsc);
            for (j=1; j<VBS_MIN_SPACING; j++) {
                AP_TOKEN_CHECK(mmu_mtbl[i+j]) {
                    tsc1 = tdm_b0_ap_b0_legacy_which_tsc(mmu_mtbl[i+j],tsc);
                    if (tsc0==tsc1){
                        result = FAIL;
                        TDM_ERROR8("%s MIN spacing violation port %0d, (#%03d | #%03d), TSC: [%0d, %0d, %0d, %0d]\n",table,mmu_mtbl[i],i,(i+j),
                            tsc[tdm_b0_ap_b0_legacy_which_tsc(mmu_mtbl[i+j],tsc)][0],
                            tsc[tdm_b0_ap_b0_legacy_which_tsc(mmu_mtbl[i+j],tsc)][1],
                            tsc[tdm_b0_ap_b0_legacy_which_tsc(mmu_mtbl[i+j],tsc)][2],
                            tsc[tdm_b0_ap_b0_legacy_which_tsc(mmu_mtbl[i+j],tsc)][3]);
                    }
                }
            }
        }
...
/**
@name: tdm_b0_ap_b0_chk_sub_lr
@param:

Checks that all line rate nodes were extracted from solution matrix
 */
int tdm_b0_ap_b0_chk_sub_lr( tdm_mod_t *_tdm)
{
...
/**
@name: tdm_b0_ap_b0_chk_sub_os
@param:

Checks that the oversub node is sorted into oversub speed groups
 */
int tdm_b0_ap_b0_chk_sub_os(tdm_mod_t *_tdm)
{
...
/**
@name: tdm_b0_ap_b0_chk_sub_mgmt
@param:

Checks that CPU bandwidth is satisfied
 */
int tdm_b0_ap_b0_chk_sub_cpu( tdm_mod_t *_tdm)
{
...
/**
@name: tdm_b0_ap_b0_chk_sub_lpbk
@param:

Checks that LOOPBACK bandwidth is satisfied
 */
int tdm_b0_ap_b0_chk_sub_lpbk( tdm_mod_t *_tdm)
{
 ...
 */
int tdm_b0_ap_b0_chk_sub_acc( tdm_mod_t *_tdm)
{
...
/**
@name: tdm_b0_ap_b0_chk_subscription
@param:

Check subscription status per port
 */
int tdm_b0_ap_b0_chk_subscription(tdm_mod_t *_tdm)
{
    int rst, result=PASS;

    /* 1. Check CPU port */
    if (FAIL==tdm_b0_ap_b0_chk_sub_cpu(_tdm)){
        result = FAIL;
    }
    /* 2. Check Loopback port */
    if (FAIL==tdm_b0_ap_b0_chk_sub_lpbk(_tdm)){
        result = FAIL;
    }
    /* 3. Check Agnostic accessory slots (must be equal to 8 or 10 in TD2/TD2+) */
    if (FAIL==tdm_b0_ap_b0_chk_sub_acc(_tdm)){
        result = FAIL;
    }

    /* 4. Check linerate ports */
    _tdm->_core_data.vars_pkg.pipe = AP_XPIPE_CAL_ID;
    rst  = tdm_b0_ap_b0_chk_sub_lr(_tdm);
    if(PASS!=rst){
    ...
    _tdm->_core_data.vars_pkg.pipe = AP_XPIPE_CAL_ID;
    rst  = tdm_b0_ap_b0_chk_sub_os(_tdm);
    if(PASS!=rst){
        result = (result==FAIL) ? (result): (rst);
    }
...
int tdm_b0_ap_b0_chk_os_jitter(tdm_mod_t *_tdm)
{
    ...
    if(clump_cnt!=0){
        ...
        ratio_var = tdm_b0_sqrt(ratio_sum2/clump_cnt);
        dist_ave  = dist_sum1/clump_cnt;
        dist_var  = tdm_b0_sqrt(dist_sum2/clump_cnt);
...
/**
@name: tdm_b0_ap_b0_chk_lr_jitter
@param:

Checks jitter of line rate ports
 */
int tdm_b0_ap_b0_chk_lr_jitter(tdm_mod_t *_tdm, int fail[16])
{
...
/**
@name: tdm_b0_ap_b0_chk_lls
@param:

Checks linked list scheduler
 */
int tdm_b0_ap_b0_chk_lls(tdm_mod_t *_tdm)
{
...
/**
@name: tdm_b0_ap_b0_chk_ovs_space
@param:

Checks that no port is back to back in the OVS table
 */
int tdm_b0_ap_b0_chk_ovs_space( tdm_mod_t *_tdm)
{
...
/**
@name: tdm_b0_ap_b0_chk_pgw_min
@param:

Checks min spacing in  the PGW table
 */
int tdm_b0_ap_b0_chk_pgw_min( tdm_mod_t *_tdm)
{
...
/**
@name: tdm_b0_ap_b0_chk_chip_specific
@param:

Checks chip specific
 */
void tdm_b0_ap_b0_chk_chip_specific(tdm_mod_t *_tdm, int fail[16])
{
...
/**
@name: tdm_b0_ap_b0_chk
@param:
 */
int
tdm_b0_ap_b0_chk( tdm_mod_t *_tdm)
{
    ...
    if (fail[0]==2) {
        ...
        result  = tdm_b0_ap_b0_chk_struc(_tdm);
    ...
    if (fail[1]==2) {
        ...
        fail[1] = tdm_b0_ap_b0_chk_tsc(_tdm);
    }
    ...
    if (fail[2]==2) {
        ...
        /* X Pipe */
        _tdm->_core_data.vars_pkg.pipe = AP_XPIPE_CAL_ID;
        result  = tdm_b0_ap_b0_chk_min(_tdm);
    ...
    if (fail[3]==2) {
        ...
        fail[3] = tdm_b0_ap_b0_chk_subscription(_tdm);
    }
    ...
    if (fail[4]==2) {
        ...
        /* X Pipe */
        _tdm->_core_data.vars_pkg.pipe = AP_XPIPE_CAL_ID;
        result  = tdm_b0_ap_b0_chk_os_jitter(_tdm);
    ...
    if (fail[5]==2 || fail[6]==2) {
        ...
        /* X Pipe */
        _tdm->_core_data.vars_pkg.pipe = AP_XPIPE_CAL_ID;
        tdm_b0_ap_b0_chk_lr_jitter(_tdm, fail);
    }
...
/*#ifdef _LLS_SCHEDULER*/
    if (fail[14]==2 && _tdm->_chip_data.soc_pkg.clk_freq>=435) {
        ...
        /* X Pipe */
        _tdm->_core_data.vars_pkg.pipe = AP_XPIPE_CAL_ID;
        result = tdm_b0_ap_b0_chk_lls(_tdm);
...
/*#endif*/
    ...
    if (fail[15]==2) {
        ...
        fail[15] = tdm_b0_ap_b0_chk_pgw_min(_tdm) || tdm_b0_ap_b0_chk_ovs_space(_tdm);
    }
    ...
    if (fail[7]==2 || fail[8]==2 || fail[9]==2 || fail[10]==2 || fail[11]==2 || fail[12]==2 || fail[13]==2){
        ...
        tdm_b0_ap_b0_chk_chip_specific( _tdm, fail );
    }
