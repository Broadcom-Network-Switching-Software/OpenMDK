...
{
    int rv = 0;
    rv = bcm56450_a0_bmd_vlan_port_add(unit, BMD_CONFIG_DEFAULT_VLAN,
                                       port, vlan_flags);

    if (CDK_SUCCESS(rv)) {
        rv = bcm56450_a0_bmd_port_stp_set(unit, port, 
                                          bmdSpanningTreeForwarding);
    }
    if (CDK_SUCCESS(rv)) {
        rv = bcm56450_a0_bmd_port_mode_set(unit, port, bmdPortModeAuto,
                                           port_mode_flags);
    }
    return rv;
}
...
int
bcm56450_a0_bmd_switching_init(int unit)
{
    int ioerr = 0;
    int rv =0;
    int port, ppport;
    ...
    if (CDK_SUCCESS(rv)) {
        rv = bcm56450_a0_bmd_vlan_create(unit, BMD_CONFIG_DEFAULT_VLAN);
    }
