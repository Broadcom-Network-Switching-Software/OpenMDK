...
_dcb_init(int unit, TX_DCB_t *dcb, const bmd_pkt_t *pkt)
{
    ...
    if (pkt->port >= 0) {
        ...
        sob[0] = 0x81000000;
        sob[1] = P2PP(unit, pkt->port);
        sob[2] = (0x00100000 | ((pkt->port * 8 + 0x40) << 8));
    }
}
...
int
bcm56450_a0_bmd_tx(int unit, const bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    TX_DCB_t *dcb;
    dma_addr_t bdcb;
    ...
    pkt1 = &tx_pkt;
    pkt1->data = bmd_dma_alloc_coherent(unit, pkt->size, &pkt1->baddr);
    CDK_ASSERT(pkt1->data);
    CDK_MEMCPY(&pkt1->data[0], &pkt->data[0], hdr_size);
    CDK_MEMCPY(&pkt1->data[hdr_size], &pkt->data[hdr_offset], pkt->size - hdr_offset);
    CDK_MEMCPY(&pkt->data[0], &pkt1->data[0], pkt->size);
    bmd_dma_free_coherent(unit, pkt->size, pkt1->data, pkt1->baddr);
    ...
    TX_DCB_ADDRf_SET(dcb[1], pkt->baddr + hdr_size); 
    TX_DCB_BYTE_COUNTf_SET(dcb[1], pkt->size - hdr_offset); 
    TX_DCB_STATf_SET(dcb[1],1);

    /* Start DMA */
    BMD_DMA_CACHE_FLUSH(dcb, 2 * sizeof(*dcb));
    bmd_xgsm_dma_tx_start(unit, bdcb); 

    /* Poll for DMA completion */
    if (bmd_xgsm_dma_tx_poll(unit, BMD_CONFIG_DMA_MAX_POLLS) < 0) {
        rv = CDK_E_TIMEOUT;
    }
    BMD_DMA_CACHE_INVAL(dcb, 2 * sizeof(*dcb));
    bmd_xgsm_dump_tx_dcbs(unit, (uint32_t *)dcb, 2,
