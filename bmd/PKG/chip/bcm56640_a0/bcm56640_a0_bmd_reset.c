...
bcm56640_a0_bmd_reset(int unit)
{
    ...
    if (CDK_CHIP_CONFIG(unit) & DCFG_LCPLL_156) {
        CDK_VERB(("bcm56640_a0_bmd_reset[%d]: Configure LCPLL for 156.25 MHz\n",
                  unit));
        for (idx = 0; idx < 4; idx++) {
            ioerr += READ_TOP_XG_PLL_CTRL_0r(unit, idx, &pll_ctrl_0);
            TOP_XG_PLL_CTRL_0r_CH0_MDIVf_SET(pll_ctrl_0, 20);
            TOP_XG_PLL_CTRL_0r_CH4_MDIVf_SET(pll_ctrl_0, 125);
            TOP_XG_PLL_CTRL_0r_CH5_MDIVf_SET(pll_ctrl_0, 25);
            ioerr += WRITE_TOP_XG_PLL_CTRL_0r(unit, idx, pll_ctrl_0);

            ioerr += READ_TOP_XG_PLL_CTRL_4r(unit, idx, &pll_ctrl_4);
            TOP_XG_PLL_CTRL_4r_NDIV_INTf_SET(pll_ctrl_4, 40);
            ioerr += WRITE_TOP_XG_PLL_CTRL_4r(unit, idx, pll_ctrl_4);

            ioerr += READ_TOP_XG_PLL_CTRL_1r(unit, idx, &pll_ctrl_1);
            TOP_XG_PLL_CTRL_1r_PDIVf_SET(pll_ctrl_1, 2);
