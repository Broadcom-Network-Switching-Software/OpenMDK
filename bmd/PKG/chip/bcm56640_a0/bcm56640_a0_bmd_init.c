...
_mmu_tdm_init(int unit)
{
    ...
    tdm_seq_len = bcm56640_a0_tdm_default(unit, &default_tdm_seq);
...
static int
_clock_sync(int unit, int stop_clk)
{
    int ioerr = 0;
    ...
    TOP_CORE_PLL_CTRL2r_HOLD_CH1f_SET(pll_ctrl2, stop_clk ? 1 : 0);
...
bcm56640_a0_bmd_init(int unit)
{
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ...
        ioerr += _clock_sync(unit, STOP_CLK);
        ioerr += WRITE_PORT_ENABLE_REGr(unit, port_en, port);
        ioerr += _clock_sync(unit, START_CLK);
    }
