...
_cpu_port_enable_set(int unit, int enable)
{
    ...
    epc_pbm = EPC_LINK_BMAPm_PORT_BITMAP_W0f_GET(epc_link);
    if (enable) {
        epc_pbm |= LSHIFT32(1, CMIC_PORT);
    } else {
        epc_pbm &= ~LSHIFT32(1, CMIC_PORT);
    }
    EPC_LINK_BMAPm_PORT_BITMAP_W0f_SET(epc_link, epc_pbm);
...
int
bcm56640_a0_bmd_rx_start(int unit, bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    dma_addr_t bdcb;
    ...
    bmd_xgsm_dma_rx_start(unit, bdcb); 
...
int
bcm56640_a0_bmd_rx_poll(int unit, bmd_pkt_t **ppkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    bmd_pkt_t *pkt;
    ...
    if (bmd_xgsm_dma_rx_poll(unit, 1) < 0) {
        return CDK_E_TIMEOUT;
    }
    BMD_DMA_CACHE_INVAL(dcb, sizeof(*dcb));
    bmd_xgsm_dump_rx_dcb(unit, (uint32_t *)dcb,
    ...
    bmd_xgsm_parse_higig2(unit, pkt, RX_DCB_MODULE_HEADERf_PTR(*dcb));
...
int
bcm56640_a0_bmd_rx_stop(int unit)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    int rv = CDK_E_NONE;
    ...
    rv = bmd_xgsm_dma_rx_abort(unit, BMD_CONFIG_DMA_MAX_POLLS);
    if (CDK_FAILURE(rv)) {
        return rv;
    }
