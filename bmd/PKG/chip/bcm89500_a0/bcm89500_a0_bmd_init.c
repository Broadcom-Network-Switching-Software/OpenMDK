...
bcm89500_a0_p4_br_phy_set(int unit)
{
    ...
    for (port = 0; port <= max_port; port++) {
        ...
        if ((CDK_CHIP_CONFIG(unit) & DCFG_P4IEEE) && (port == 4)) {
            ...
            READ_BR_PHYIDLr(unit, port, &phyidl);
            /* b0 need to change these steps */
            if (BR_PHYIDLr_REVISIONf_GET(phyidl) == 0x4) {
                /* Disable AGC freeze feature */
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x1c, 0xac01);

                /* Enable SM_DSP clock */
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x18, 0x0c00);

                /* Set the startup PLL bandwidth and gamma for IEEE mode */
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x17, 0x000a);
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x15, 0x010b);
                /* Set MSE threshold for IEEE mode */
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x17, 0x000e);
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x15, 0xa352);
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x17, 0x000f);
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x15, 0xa04c);
            
                /* Disable SM_DSP clock */
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x18, 0x0400);
            
                /* Disable link check timer */
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x17, 0x0f9a);
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x15, 0x20c0);
            ...
            if (BR_PHYIDLr_REVISIONf_GET(phyidl) == 0x4) {
            /* Disable BR functinos, Provide default ability, and Set 100TX, 
               auto-on, and full duplex */
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x04, 0x01e1);
                ioerr += cdk_robo_miim_write(unit, phy_addr, 0x00, 0x3100);
            }
        }
    }
