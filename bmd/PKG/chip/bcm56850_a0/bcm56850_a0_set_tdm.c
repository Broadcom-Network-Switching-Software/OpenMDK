 ...
 */
static void
TSC_port_transcription(int wc_array[NUM_WC][4], int speed[NUM_EXT_PORTS], int port_state_array[128])
{
    int i,j,last_port=NUM_EXT_PORTS;
    /*char tmp_str[16];*/
    ...
    for (i = 1; i < NUM_EXT_PORTS-1; i+=4) {
        ...
        else if (speed[i] > SPEED_0) {
            if (speed[i] <= SPEED_42G) {
                for (j=0; j<4; j++) {
                    if (port_state_array[i-1+j]==1 || port_state_array[i-1+j]==2) {
                        wc_array[(i-1)/4][j] = (i+j);
                        last_port=(i+j);
                    }
                    else if (port_state_array[i-1+j]==3) {
 ...
 */
static void
print_port(int wc_array[NUM_WC][4]) 
{
    int i,j;
    for(i=0; i<NUM_WC; i++) {
        for (j=0; j<4; j++) {
 ...
 */
static void
parse_mmu_tdm_tbl(int bw, int mgmt_bw, int mmu_tdm_tbl[256], int mmu_tdm_ovs_1[16], int mmu_tdm_ovs_2[16], int mmu_tdm_ovs_3[16], int mmu_tdm_ovs_4[16], int pipe)
{
    int j, k=256, m=0;
    const char *name;
    int oversub=FALSE;
 ...
 */
static void
print_tdm_tbl(int pgw_tdm_tbl[32], const char* name)
{
    int j;
    for (j = 0; j < 32; j++)
        {
 ...
 */
static void
print_tbl_summary(int x0[32], int x1[32], int y0[32], int y1[32], int ox0[32], int ox1[32], int oy0[32], int oy1[32], int sx0[32], int sx1[32], int sy0[32], int sy1[32], int bw)
{
    int t;
 ...
 */
static int
TDM_scheduler(int wc_array[NUM_WC][4], int speed[NUM_EXT_PORTS], int *cur_idx, int *pgw_tdm_idx, int *ovs_tdm_idx, int subp, int pgw_tdm_tbl[32], int ovs_tdm_tbl[32], int upperlimit, int iter1, int bw, int port_state_map[128], int iter3, int stop1, int swap_array[32], int *z, int subport, int op_flags[1])
{
 ...
 */
static int
write_mmu_tdm_tbl(int pgw0[32], int pgw1[32], int ovs0[32], int ovs1[32], int mmu[256], int bucket1[16], int bucket2[16], int bucket3[16], int bucket4[16], int x_pipe, int port_state_map[128], int tsc[NUM_WC][4], int bw, int speed[NUM_EXT_PORTS], int (*op_flags)[1])
{
    long int clk;
    int temp_10G[4], temp_1G[4], temp_20G[4];
    long int s1, s2, s3, limiter;
    static int a, x;
    int q, p, b, c=0, d, e, f, g, i=0, j, k=0, l, m, v, w, y, microsubbed10G=FALSE, overload10G=0, lr_idx_limit, proxcheck;
    ...
    int ovs01[64], ovs10[64], ovs20[32], ovs40[16], ovs100[8], ovs120[8];
    ...
    */
                    ...
                    for (v=0; v<4; v++)
                        {
                            ...
                            i = 0; tsc_swap[0] = ovs01[z6]; ovs01[z6] = NUM_EXT_PORTS;
                            for (a=0; a<NUM_WC; a++) for (b=0; b<4; b++) if (tsc[a][b] == tsc_swap[0]) c=a;
                            for (b=0; b<4; b++) if (tsc[c][b]!=tsc_swap[0] && tsc[c][b]!=NUM_EXT_PORTS) for (a=1; a<(z6+1); a++) if (ovs01[a]==tsc[c][b])
                                                                                                                                     {
                                                                                                                                         d=a;
                                                                                                                                         i++;
                                                                                                                                         tsc_swap[i] = tsc[c][b];
                                                                                                                                         for (f=d; f<z6; f++) ovs01[f] = ovs01[f+1];
                                                                                                                                     }
                            z6-=(1+i);
                    ...
                    for (v=0; v<4; v++)
                        {
                            ...
                            z1-=(1+i);
                            /* dequeue into buckets subject to given constraints */
                            if (speed[tsc[c][0]]>SPEED_16G || speed[tsc[c][2]]>SPEED_16G || speed[tsc[c][1]]>SPEED_16G || speed[tsc[c][3]]>SPEED_16G) {
                                /* TDM_VERBOSE0("TDM: __________VERBOSE: triport mode in 10G speed bins mode 0\n"); */
                                if (z11 < 8)
                                    {
                                        for (m = 0; m < 2; m++) /* if (tsc_swap[m] != NUM_EXT_PORTS) */ bucket1[z11+(m*8)] = tsc_swap[m];
                                        z11++;
                                    }
                                else if (z22 < 8)
                                    {
                                        for (m = 0; m < 2; m++) /* if (tsc_swap[m] != NUM_EXT_PORTS) */ bucket2[z22+(m*8)] = tsc_swap[m];
                                        z22++;
                                    }
                                else if (z33 < 8)
                                    {
                                        for (m = 0; m < 2; m++) /* if (tsc_swap[m] != NUM_EXT_PORTS) */ bucket3[z33+(m*8)] = tsc_swap[m];
                                        z33++;
                                    }
                                else if (z44 < 8)
                                    {
                                        for (m = 0; m < 2; m++) /* if (tsc_swap[m] != NUM_EXT_PORTS) */ bucket4[z44+(m*8)] = tsc_swap[m];
                                        z44++;
                                    }
                            }
                            else {
            ...
            while (z1<16 && z1>0)
                {
                    ...
                    /* dequeue into buckets subject to given constraints */
                    if (speed[tsc[c][0]]>SPEED_16G || speed[tsc[c][2]]>SPEED_16G || speed[tsc[c][1]]>SPEED_16G || speed[tsc[c][3]]>SPEED_16G) {
                        /* TDM_VERBOSE0("TDM: __________VERBOSE: triport mode in 10G speed bins mode 1\n"); */
                        if (z11 < 8)
                            {
                                for (m = 0; m < 2; m++) /* if (tsc_swap[m] != NUM_EXT_PORTS) */ bucket1[z11+(m*8)] = tsc_swap[m];
                                z11++;
    ...
    while (z2 > 0)
        {
            for (a=0; a<NUM_WC; a++) for (b=0; b<4; b++) if (tsc[a][b] == ovs20[z2]) c=a;
            ...
            if (speed[tsc[c][0]]<SPEED_20G || speed[tsc[c][2]]<SPEED_20G || speed[tsc[c][1]]<SPEED_20G || speed[tsc[c][3]]<SPEED_20G) {
                tsc_swap[0] = ovs20[z2]; 
                ovs20[z2] = NUM_EXT_PORTS;
                z2--;
                if (z11 < 16) {
                    q=0;
                    for (p=0; p<16; p++) {
                        if (bucket1[p]==NUM_EXT_PORTS) {
                            z11=p;
                            break;
                        }
                        else {
 ...
 */
static void
TDM_OVS_spacer(int wc[NUM_WC][4], int ovs_tdm_tbl[32], int ovs_spacing[32])
{
 ...
 */
static int
TDM_scheduler_wrap(int wc_array[NUM_WC][4], int speed[NUM_EXT_PORTS], int pgw_tdm_tbl[32], int ovs_tdm_tbl[32], int pgw_num, int bw, int port_state_map[128], int iter2, int stop, int (*op_flags)[1])
{
    ...
    /* TDM_MEMCPY(&op_flags, op_flags_str, 1* sizeof(int) );*/
