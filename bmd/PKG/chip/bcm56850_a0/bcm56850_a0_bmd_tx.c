...
_dcb_init(int unit, TX_DCB_t *dcb, const bmd_pkt_t *pkt)
{
    ...
    if (pkt->port >= 0) {
        /* Get transmit queue */
        txq = bcm56850_a0_uc_queue_num(unit, pkt->port, 0);
        if (PORT_IN_Y_PIPE(pkt->port)) {
            txq -= 1480;
        }
        ...
        sob[0] = 0x81000000;
        sob[1] = ((PORT_IN_Y_PIPE(pkt->port)) ? 0x80000000 : 0) | P2L(unit, pkt->port);
        sob[2] = LSHIFT32(txq, 8) | 0x00040000;
...
int
bcm56850_a0_bmd_tx(int unit, const bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    TX_DCB_t *dcb;
    dma_addr_t bdcb;
