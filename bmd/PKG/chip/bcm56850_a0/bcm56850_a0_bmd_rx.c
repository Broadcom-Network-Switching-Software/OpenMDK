...
_cpu_port_enable_set(int unit, int enable)
{
    ...
    epc_pbm = EPC_LINK_BMAPm_GET(epc_link, 0);
    if (enable) {
        epc_pbm |= LSHIFT32(1, CMIC_PORT);
    } else {
        epc_pbm &= ~LSHIFT32(1, CMIC_PORT);
    }
    EPC_LINK_BMAPm_SET(epc_link, 0, epc_pbm);
...
int
bcm56850_a0_bmd_rx_start(int unit, bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    dma_addr_t bdcb;
...
int
bcm56850_a0_bmd_rx_poll(int unit, bmd_pkt_t **ppkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    bmd_pkt_t *pkt;
...
int
bcm56850_a0_bmd_rx_stop(int unit)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    int rv = CDK_E_NONE;
