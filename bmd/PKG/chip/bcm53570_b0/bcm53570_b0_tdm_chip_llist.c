...
{
	int i, pool=0, skew=0, slices=0, split, count, set=BOOL_FALSE, temp_div, timeout, original_len;
	struct gh2_ll_node *list = llist;
	...
	if (list!=NULL && tdm_gh2_ll_len(list)>0) {
		...
		while (pool>0 && (--timeout)>0) {
			...
			if (set) {
				...
				for (i=((original_len/temp_div)-1); i<original_len; i+=(original_len/temp_div)) {
					tdm_gh2_ll_insert(list,token,i);
					if ((--pool)<=0) {
						break;
					}
