...
tdm_mem_transpose( tdm_mod_t *_tdm )
{
	...
	for (i=0; i<_tdm->_core_data.vmap_max_len; i++) {
		TDM_COPY( &(_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_mem_transpose.dst][i]), &(_tdm->_core_data.vmap[_tdm->_core_data.vars_pkg.m_tdm_mem_transpose.src][i]), sizeof(unsigned short) );
	}
	/* Clear source vector */
	for (i=0; i<_tdm->_core_data.vmap_max_len; i++) {
 ...
 */
void
tdm_gh2_col_transpose( tdm_mod_t *_tdm )
{
