...
_tdm_calculation(int unit)
{
    ...
    bcm53570_b0_all_pbmp_get(unit, &all_pbmp);
    CDK_PBMP_PORT_ADD(all_pbmp, CMIC_PORT);

    CDK_PBMP_ITER(all_pbmp, port) {
        if (SPEED_MAX(unit, port) == 0) {
            continue;
        }

        if ((SPEED_MAX(unit, port) > 0) && \
            (SPEED_MAX(unit, port) < 1000)) {
            tcfg->speed[port] = 1000;
        } else if (SPEED_MAX(unit, port) == 11000) {
            tcfg->speed[port] = 10000;
        } else if (SPEED_MAX(unit, port) == 21000) {
            tcfg->speed[port] = 20000;
        } else if (SPEED_MAX(unit, port) == 42000) {
            tcfg->speed[port] = 40000;
        } else {
            tcfg->speed[port] = SPEED_MAX(unit, port);
        }

        tcfg->port_state[port] = PORT_STATE_LINERATE;
    }

    tcfg->speed[CMIC_PORT] = 1000;
    ...
    /* enable cpu port */
    chip_pkg->state[CMIC_PORT] = PORT_STATE_LINERATE;
    for (idx = 0; idx < NUM_EXT_PORTS; idx ++) {
    ...
    bcm53570_b0_sel_tdm(chip_pkg, tdm_pkg);
    if (NULL == bcm53570_b0_set_tdm_gh2_tbl(tdm_pkg)) {
        CDK_ERR(("bcm53570_b0_init[%d]: Unable to configure TDM\n", unit));
        return CDK_E_FAIL;
    }
    tdm_size = tdm_pkg->_chip_data.cal_0.cal_len;
    CDK_MEMCPY(tcfg->idb_tdm_tbl_0, tdm_pkg->_chip_data.cal_0.cal_main, \
                   sizeof(int)*GH2_LR_VBS_LEN);
    CDK_VVERB(("\n"));
    CDK_VVERB(("tdm size: %d", tdm_size));
    CDK_VVERB(("\n"));
    CDK_VVERB(("tdm table:"));
    for (idx = 0; idx < tdm_size; idx++) {
        ...
        CDK_VVERB(("  %d", (tcfg->idb_tdm_tbl_0[idx]<0) ? 127 : tcfg->idb_tdm_tbl_0[idx]));
    }
    CDK_VVERB(("\n"));
    ...
    for (i = 0; i < tdm_size; i++) {
        ...
        val = (tcfg->idb_tdm_tbl_0[i] != 127) ? \
              P2M(unit, tcfg->idb_tdm_tbl_0[i]) : 127;
...
bcm53570_b0_tdm_malloc(int unit, int size)
{
    ...
    return (void*)bmd_dma_alloc_coherent(unit, size, &dma_addr);
#else
    CDK_WARN(("bcm53570_b0_tdm_malloc[%d]: BMD_CONFIG_INCLUDE_DMA "
              "must be defined in bcm53570 to support "
              "bcm53570_b0_tdm_malloc.\n", unit));
    return NULL;
#endif
}
...
_mmu_init_helper_lossy(int unit)
{
    ...
    total_physical_memory = 0x7fff + 1;
...
_mmu_init(int unit)
{
    ...
    _tdm_calculation(unit);

    READ_CFAPCONFIGr(unit, &cfapconfig);
    oval = CFAPCONFIGr_GET(cfapconfig);
    CFAPCONFIGr_CFAPPOOLSIZEf_SET(cfapconfig, 0x5fff);
    if (oval != CFAPCONFIGr_GET(cfapconfig)) {
...
_mac_cl_init(int unit, int port)
{
    ...
    if(CLMAC_CTRLr_SOFT_RESETf_GET(clmac_ctrl)){
        bcm53570_b0_cl_port_credit_reset(unit, port);
    }
    CLMAC_CTRLr_SOFT_RESETf_SET(clmac_ctrl, 0);
