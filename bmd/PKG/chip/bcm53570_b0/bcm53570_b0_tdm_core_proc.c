...
#include "bcm53570_b0_tdm_core_vec.h"
...
tdm_core_postalloc(unsigned short **vector_map, int freq, unsigned short spd, short *yy, short *y, int lr_idx_limit, unsigned short lr_stack[TDM_AUX_SIZE], int token, const char* speed, int num_ext_ports)
{
	...
	TDM_SML_BAR
	while (*y > 0) {
		load_status = tdm_gh2_core_vec_load(vector_map, freq, spd, *yy, lr_idx_limit, num_ext_ports);
		if (load_status==FAIL) {
 ...
 */
int
tdm_gh2_core_postalloc_vmap(unsigned short **vector_map, int freq, unsigned short spd, short *yy, short *y, int lr_idx_limit, unsigned short lr_stack[TDM_AUX_SIZE], int token, const char* speed, int num_ext_ports, int vmap_wid, int vmap_len)
{
	int v, load_status;
			...
			for (i=0; i<_tdm->_core_data.vars_pkg.m_tdm_core_vbs_scheduler.lr_vec_cnt; i++) {
				...
				if ( (tdm_type_chk(_tdm)>1) && (tdm_count_param_spd(_tdm)>=10) ) {
					...
					do {
						...
						if ( ((enum port_speed_e)tdm_find_fastest_spd(_tdm)<=(_tdm->_chip_data.soc_pkg.speed[_tdm->_core_data.vmap[i][0]]-SPEED_DELIMITER)) ||
							 ((enum port_speed_e)tdm_find_fastest_spd(_tdm)>=(_tdm->_chip_data.soc_pkg.speed[_tdm->_core_data.vmap[i][0]]+SPEED_DELIMITER)) ) {
							x0++;
						}
					} while ( /*(vec_id<=i) && */
							  ( ((enum port_speed_e)tdm_find_fastest_spd(_tdm)<=(_tdm->_chip_data.soc_pkg.speed[_tdm->_core_data.vmap[i][0]]-SPEED_DELIMITER)) ||
 ...
 */
int
tdm_gh2_core_vbs_scheduler_wrapper( tdm_mod_t *_tdm )
{   
