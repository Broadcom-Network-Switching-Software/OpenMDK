...
/**
@name: tdm_gh2_filter_check_pipe_sister_min
@param:
 */
int
tdm_gh2_filter_check_pipe_sister_min( tdm_mod_t *_tdm )
{
	...
	for (i=0; i<tdm_pipe_main_len; i++) {
		GH2_TOKEN_CHECK(tdm_pipe_main[i]) {
			tsc_i = tdm_gh2_scan_which_tsc(tdm_pipe_main[i],_tdm);
			for (j=1; j<VBS_MIN_SPACING; j++){
				idx0 = ((i+j)<tdm_pipe_main_len)? (i+j): (i+j-tdm_pipe_main_len);
				GH2_TOKEN_CHECK(tdm_pipe_main[idx0]) {
					tsc0 = tdm_gh2_scan_which_tsc(tdm_pipe_main[idx0],_tdm);
					if ( tsc_i == tsc0){
						result=FAIL;
						break;
					}
...
/**
@name: tdm_gh2_filter_check_port_sister_min
@param:
 */
int
tdm_gh2_filter_check_port_sister_min( tdm_mod_t *_tdm, int port )
{
	...
	GH2_TOKEN_CHECK(port){
		tsc_i = tdm_gh2_scan_which_tsc(port,_tdm);
		for (i=0; i<tdm_pipe_main_len; i++) {
...
/**
@name: tdm_gh2_filter_check_migrate_lr_slot
@param:

Migrate Linerate slot from src to dst in an array.
 */
int
tdm_gh2_filter_check_migrate_lr_slot(int idx_src, int idx_dst, int *tdm_tbl, int tdm_tbl_len, int **tsc)
{
	...
	if (check_pass==BOOL_TRUE){
		GH2_TOKEN_CHECK(tdm_tbl[idx_src]){
			GH2_TOKEN_CHECK(tdm_tbl[idx_dst]){
				tsc0 = tdm_gh2_legacy_which_tsc(tdm_tbl[idx_src],tsc);
				tsc1 = tdm_gh2_legacy_which_tsc(tdm_tbl[idx_dst],tsc);
				if ( tsc0 == tsc1 ){
					check_pass = BOOL_FALSE;
	...
	if (check_pass==BOOL_TRUE){
		i     = idx_dst;
		tsc_i = tdm_gh2_legacy_which_tsc(tdm_tbl[i],tsc);
		
		for (j=1; j<VBS_MIN_SPACING; j++){
			idx0 = ((i+j)<tdm_tbl_len)? (i+j): (i+j-tdm_tbl_len);
			idx1 = ((i-j)>=0)? (i-j): (i-j+tdm_tbl_len);
			GH2_TOKEN_CHECK(tdm_tbl[idx0]){
...
/**
@name: tdm_gh2_filter_chk_slot_shift_cond
@param:
 */
int
tdm_gh2_filter_chk_slot_shift_cond(int slot, int dir, int cal_len, int *cal_main, tdm_mod_t *_tdm)
{
    ...
    if (slot < cal_len) {
        ...
        idx     = src;
        port    = cal_main[idx];
        port_pm = tdm_gh2_filter_get_port_pm(port, _tdm);
        if (port >= param_phy_lo && port <= param_phy_hi) {
            ...
            for (k=1; k<=param_space_sister; k++) {
                ...
                if (port_k >= param_phy_lo && port_k <= param_phy_hi) {
                    if (tdm_gh2_filter_get_port_pm(port_k, _tdm) == port_pm) {
                        result = FAIL;
                        break;
                    }
                }
...
/**
@name: tdm_gh2_filter_calc_jitter
@param:
 */
int
tdm_gh2_filter_calc_jitter(int speed, int cal_len, int *space_min, int *space_max)
{
...
/**
@name: tdm_gh2_filter_get_same_port_dist
@param:
 */
int
tdm_gh2_filter_get_same_port_dist(int slot, int dir, int *cal_main, int cal_len)
{
    int n, k, dist = 0;
...
/**
@name: tdm_gh2_filter_get_port_pm
@param:

Return Port Macro number of the given port
 */
int
tdm_gh2_filter_get_port_pm(int port_token, tdm_mod_t *_tdm)
{
    int port_pm;
...
/**
@name: tdm_gh2_filter_get_port_speed
@param:
 */
static int
tdm_gh2_filter_get_port_speed(int port_token, tdm_mod_t *_tdm)
{
...
/**
@name: tdm_gh2_filter_migrate_lr_slot
@param:

Migrate Linerate slot from src to dst in an array.
 */
int
tdm_gh2_filter_migrate_lr_slot(int idx_src, int idx_dst, int *tdm_tbl, int tdm_tbl_len, int **tsc)
{
	int j, port=GH2_NUM_EXT_PORTS, filter_result=FAIL, check_pass=BOOL_TRUE;
	
	if (PASS==tdm_gh2_filter_check_migrate_lr_slot(idx_src, idx_dst, tdm_tbl, tdm_tbl_len, tsc)){
		if (check_pass==BOOL_TRUE){
			GH2_TOKEN_CHECK(tdm_tbl[idx_dst]){
...
/**
@name: tdm_gh2_filter_migrate_lr_slot_up
@param:

Migrate Linerate slot from src to dst in an array.
 */
int
tdm_gh2_filter_migrate_lr_slot_up(tdm_mod_t *_tdm)
{
				...
				GH2_TOKEN_CHECK(tdm_pipe_main[idx0]) {
					...
					if (tsc_i == tsc0){
						...
						for (k=0; k<VBS_MIN_SPACING; k++){
							...
							if (PASS==tdm_gh2_filter_check_migrate_lr_slot(i, idx_dst, tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap)){
								tdm_gh2_filter_migrate_lr_slot(i, idx_dst, tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap);
								mig_cnt++;
								break;
							}
						}
...
/**
@name: tdm_gh2_filter_migrate_lr_slot_dn
@param:

Migrate Linerate slot from src to dst in an array.
 */
int
tdm_gh2_filter_migrate_lr_slot_dn(tdm_mod_t *_tdm)
{
 	int i, j, k, idx0, tsc_i, tsc0, idx_dst, mig_cnt=0,
	    *tdm_pipe_main, tdm_pipe_main_len, **tdm_port_pmap;
				...
				GH2_TOKEN_CHECK(tdm_pipe_main[idx0]) {
					...
					if (tsc_i == tsc0){
						...
						for (k=0; k<VBS_MIN_SPACING; k++){
							...
							if (PASS==tdm_gh2_filter_check_migrate_lr_slot(idx0, idx_dst, tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap)){
								tdm_gh2_filter_migrate_lr_slot(idx0, idx_dst, tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap);
								mig_cnt++;
								break;
							}
						}
...
/**
@name: tdm_gh2_filter_sister_min
@param:
 */
int
tdm_gh2_filter_sister_min( tdm_mod_t *_tdm )
{
	...
	timeout = 20;
	do{
		min_space_check = tdm_gh2_filter_check_pipe_sister_min(_tdm);
		if (min_space_check==FAIL){
			mig_cnt = tdm_gh2_filter_migrate_lr_slot_dn(_tdm);
		}
	}
...
/* 	timeout = 10;
	do{
		min_space_check = tdm_gh2_filter_check_pipe_sister_min(_tdm);
		if (min_space_check==FAIL){
			mig_cnt = tdm_gh2_filter_migrate_lr_slot_up(_tdm);
		}
	}
	while ( (min_space_check==FAIL) && (mig_cnt>0) && (timeout--)>0 ); */
	
	if (tdm_gh2_filter_check_pipe_sister_min(_tdm)==PASS) {result = PASS;}
...
/**
@name: tdm_gh2_filter_dither
@param:
 */
int
tdm_gh2_filter_dither(int *tdm_tbl, int lr_idx_limit, int accessories, int **tsc, int threshold, enum port_speed_e *speed)
{
	int g, i, j, k=1, l=GH2_NUM_EXT_PORTS, slice_idx, shift_cnt=0;
	...
	for (i=0; i<(lr_idx_limit+accessories); i++) {
		if (tdm_tbl[i]==GH2_OVSB_TOKEN && tdm_gh2_scan_slice_size_local(i,tdm_tbl,(lr_idx_limit+accessories), &slice_idx)==tdm_gh2_slice_size(GH2_OVSB_TOKEN,tdm_tbl,(lr_idx_limit+accessories))) {
			g=i; while( (tdm_tbl[g]==GH2_OVSB_TOKEN) && (g<(lr_idx_limit+accessories)) ) {g++;}
			if ( tdm_gh2_slice_prox_up(g,tdm_tbl,tsc,speed) &&
				 tdm_gh2_slice_prox_dn((tdm_gh2_slice_idx(GH2_OVSB_TOKEN,tdm_tbl,(lr_idx_limit+accessories))-1),tdm_tbl,(lr_idx_limit+accessories),tsc,speed) ) {
				l=tdm_tbl[i];
				for (j=i; j<255; j++) {
					tdm_tbl[j]=tdm_tbl[j+1];
				}
	...
	if (dither_shift_done) {
		for (i=1; i<(lr_idx_limit+accessories); i++) {
			...
			while (tdm_tbl[i]!=GH2_OVSB_TOKEN && tdm_gh2_scan_slice_size_local(i,tdm_tbl,(lr_idx_limit+accessories),&slice_idx)==tdm_gh2_slice_size(1,tdm_tbl,(lr_idx_limit+accessories)) && i<(lr_idx_limit+accessories) ) {
				if (++dither_slice_counter>=threshold && tdm_tbl[i-1]==GH2_OVSB_TOKEN) {
					for (j=255; j>i; j--) {
						tdm_tbl[j]=tdm_tbl[j-1];
					}
					tdm_tbl[i+1]=GH2_OVSB_TOKEN;
					dither_done=BOOL_TRUE;
					break;
				}
				i++; if (tdm_tbl[i]==GH2_OVSB_TOKEN) {i++;}
			}
			if (dither_done) {
				break;
			}
...
/**
@name: tdm_gh2_filter_fine_dither
@param:
 */
int
tdm_gh2_filter_fine_dither(int port, int *tdm_tbl, int lr_idx_limit, int accessories, int **tsc)
{
	...
	for (j=2; j<(lr_idx_limit+accessories-4); j++) {
		if ( tdm_tbl[j]!=GH2_OVSB_TOKEN && tdm_tbl[j-2]!=GH2_OVSB_TOKEN && tdm_tbl[j+2]!=GH2_OVSB_TOKEN && tdm_tbl[j+4]!=GH2_OVSB_TOKEN && 
			 tdm_gh2_scan_slice_size_local((j-2), tdm_tbl, (lr_idx_limit+accessories), &slice_idx)==1 && 
			 tdm_gh2_scan_slice_size_local( j,    tdm_tbl, (lr_idx_limit+accessories), &slice_idx)==1 && 
			 tdm_gh2_scan_slice_size_local((j+2), tdm_tbl, (lr_idx_limit+accessories), &slice_idx)==1 && 
			 tdm_gh2_scan_slice_size_local((j+4), tdm_tbl, (lr_idx_limit+accessories), &slice_idx)==1 &&
			 tdm_gh2_slice_prox_local( j,    tdm_tbl, (lr_idx_limit+accessories), tsc)>min_prox )
		{
...
/**
@name: tdm_gh2_filter_shift_lr_port
@param:
 ...
 */
int
tdm_gh2_filter_shift_lr_port(unsigned short port, int *tdm_tbl, int tdm_tbl_len, int dir)
{
...
/**
@name: tdm_gh2_filter_migrate_os_slot
@param:

Migrate OVSB slot from src to dst in an array.
 */
int
tdm_gh2_filter_migrate_os_slot(int idx_src, int idx_dst, int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	...
	if ( tdm_tbl[idx_src]!=GH2_OVSB_TOKEN && tdm_tbl[idx_src]!=GH2_ANCL_TOKEN) {
		check_pass = BOOL_FALSE;
	}
	/* Check sister port spacing */
	if (check_pass==BOOL_TRUE){
...
/**
@name: tdm_gh2_filter_smooth_idle_slice
@param:

Smooth MMU TDM calendar by migrating IDLE slots
 */
int
tdm_gh2_filter_smooth_idle_slice(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	...
	idle1_token = GH2_IDL1_TOKEN;
	idle2_token = GH2_IDL2_TOKEN;
	...
	if(idle_slot_cnt>0){
		...
		for (k=0; k<idle_slot_cnt; k++){
			...
			idle_slice_max = tdm_gh2_scan_slice_max(idle1_token, tdm_tbl,tdm_tbl_len, &idle_slice_max_idx,0);
			lr_slice_max   = tdm_gh2_scan_mix_slice_max(1,tdm_tbl,tdm_tbl_len, &lr_slice_max_idx,pos);
			...
			if ( (idle_slice_max>1 && lr_slice_max>0) &&
				...
				tsc_x = tdm_gh2_legacy_which_tsc(tdm_tbl[idx_x],tsc);
				tsc_y = tdm_gh2_legacy_which_tsc(tdm_tbl[idx_y],tsc);
				tsc1  = tdm_gh2_legacy_which_tsc(tdm_tbl[idx1],tsc);
				tsc2  = tdm_gh2_legacy_which_tsc(tdm_tbl[idx2],tsc);
...
/**
@name: tdm_gh2_filter_smooth_os_slice
@param:
 ...
 */
int
tdm_gh2_filter_smooth_os_slice(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed, int dir)
{
	...
	for (i=1; i<tdm_tbl_len; i++) {
		...
		lr_clump_max = tdm_gh2_scan_mix_slice_max(1,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
		lr_clump_min = tdm_gh2_scan_mix_slice_min(1,tdm_tbl,tdm_tbl_len, &slice_idx, 0);
		if ( (lr_clump_max<=1) || 
		     (lr_clump_max==2 && lr_clump_min==1)){
			break;
		}
		if ( FAIL==tdm_gh2_check_shift_cond_pattern(filter_port, tdm_tbl, tdm_tbl_len, tsc, dir) ) {
			continue;
		}
		else if ( FAIL==tdm_gh2_check_shift_cond_local_slice(filter_port, tdm_tbl, tdm_tbl_len, tsc, dir) ) {
			continue;
		}
		else {
			tdm_gh2_filter_shift_lr_port(filter_port, tdm_tbl, tdm_tbl_len, shift_dir);
...
/**
@name: tdm_gh2_filter_smooth_os_slice_fine
@param:

Smooth MMU TDM calendar by migrating OVSB slot
 */
int
tdm_gh2_filter_smooth_os_slice_fine(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	...
	for (k=0; k<tdm_tbl_len; k++){
		...
		slice_max= tdm_gh2_scan_slice_max(ovsb_token,tdm_tbl,tdm_tbl_len, &slice_max_idx,k);
		slice_min= tdm_gh2_scan_slice_min(ovsb_token,tdm_tbl,tdm_tbl_len, &slice_min_idx,pos);
...
/**
@name: tdm_gh2_filter_smooth_os_os_up
@param:

Smooth MMU TDM calendar by transforming:
        --- _x_y_ovsb_ovsb_ -> _x_ovsb_y_ovsb_
 */
int tdm_gh2_filter_smooth_os_os_up(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	int i, slot_tmp, filter_cnt=0;
	
	for (i=2; i<(tdm_tbl_len-1); i++) {
		if (tdm_tbl[i  ]==GH2_OVSB_TOKEN &&
			tdm_tbl[i+1]==GH2_OVSB_TOKEN ){
			GH2_TOKEN_CHECK(tdm_tbl[i-1]){
				GH2_TOKEN_CHECK(tdm_tbl[i-2]){
					if ( PASS==tdm_gh2_check_slot_swap_cond((i-1), tdm_tbl, tdm_tbl_len, tsc, speed) ){
...
/**
@name: tdm_gh2_filter_smooth_os_os_dn
@param:

Smooth MMU TDM calendar by transforming:
		--- _ovsb_ovsb_x_y_ -> _ovsb_x_ovsb_y_
 */
int tdm_gh2_filter_smooth_os_os_dn(int *tdm_tbl, int tdm_tbl_len, int **tsc, enum port_speed_e *speed)
{
	int i, slot_tmp, filter_cnt=0;
...
/**
@name: tdm_gh2_filter_smooth_ancl
@param:

Smooth MMU TDM calendar with ANCILLARY slots
 */
int
tdm_gh2_filter_smooth_ancl(int ancl_token, int *tdm_tbl, int tdm_tbl_len, int ancl_space_min)
{
	int i, j, g, l, pool, ancl_num=0, ancl_dist_up, ancl_dist_dn, lr_slice_size, lr_slice_min, slice_idx, timeout;
	
	lr_slice_min = tdm_gh2_scan_slice_min(1, tdm_tbl, tdm_tbl_len, &slice_idx,0);
	...
	while( (pool>0) && ((--timeout)>0) ){
		for (i=1; i<tdm_tbl_len; i++) {
			...
			GH2_TOKEN_CHECK(tdm_tbl[i]) {
				lr_slice_size = tdm_gh2_scan_slice_size_local(i,tdm_tbl,tdm_tbl_len,&slice_idx);
				ancl_dist_up  = tdm_gh2_check_same_port_dist_up_port(ancl_token,i,tdm_tbl,tdm_tbl_len);
				ancl_dist_dn  = tdm_gh2_check_same_port_dist_dn_port(ancl_token,i,tdm_tbl,tdm_tbl_len);
...
/**
@name: tdm_gh2_filter_chain
@param:
 */
int
tdm_gh2_filter_tdm5( tdm_mod_t *_tdm )
{
    ...
    if (_tdm->_core_data.vars_pkg.lr_enable==BOOL_TRUE){
        filter_result = tdm_gh2_filter_sister_min(_tdm );
	...
	if(_tdm->_core_data.vars_pkg.lr_enable && !(_tdm->_core_data.vars_pkg.os_enable) && !(_tdm->_core_data.vars_pkg.refactor_done)){
		filter_result = tdm_gh2_filter_smooth_idle_slice(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed);
	...
	if (_tdm->_core_data.vars_pkg.lr_enable && _tdm->_core_data.vars_pkg.os_enable && !(_tdm->_core_data.vars_pkg.refactor_done)) {
		...
		filter_cnt= 0;
		max_size = tdm_gh2_scan_mix_slice_max(ovsb_token,tdm_pipe_main,tdm_pipe_main_len, &max_slice_idx, 0);
		min_size = tdm_gh2_scan_mix_slice_min(ovsb_token,tdm_pipe_main,tdm_pipe_main_len, &min_slice_idx, 0);
		if( (max_size-min_size)>=6 ){
			for (k=1; k<max_size-min_size; k++){
				filter_result = tdm_gh2_filter_migrate_os_slot(max_slice_idx, min_slice_idx, tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed );
				if(filter_result==FAIL){break;}
				filter_cnt++;
				max_size_k = tdm_gh2_scan_mix_slice_max(ovsb_token,tdm_pipe_main,tdm_pipe_main_len, &max_slice_idx, 0);
				min_size_k = tdm_gh2_scan_mix_slice_min(ovsb_token,tdm_pipe_main,tdm_pipe_main_len, &min_slice_idx, 0);
		...
		do{
			filter_result = tdm_gh2_filter_smooth_os_slice(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed, DN);
		...
		timeout = _tdm->_chip_data.soc_pkg.lr_idx_limit;
		do{
			filter_result = tdm_gh2_filter_smooth_os_slice(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed, UP);
			TDM_PRINT2("\nFilter done: ---OVSB MAX SLICE REDUCTION (2) Shift Up (%2d), filter applied <%d> times\n", ++filter_cnt, filter_result);
			TDM_SML_BAR
		} while ( filter_result>0 && (timeout--)>0 );

		/* Smooth oversub distribution */
		filter_result = tdm_gh2_filter_smooth_os_os_up(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed);
		TDM_PRINT1("\nFilter done: ---OVSB LOCAL SLICE REDUCTION (1) slot UP, X_Y_OVSB_OVSB, filter applied <%d> times \n", filter_result);
		TDM_SML_BAR
		filter_result = tdm_gh2_filter_smooth_os_os_dn(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed);
		TDM_PRINT1("\nFilter done: ---OVSB LOCAL SLICE REDUCTION (2) slot DOWN, OVSB_OVSB_X_Y, filter applied <%d> times \n", filter_result);
		TDM_SML_BAR

		/* Smooth oversub distribution */
		filter_result = tdm_gh2_filter_smooth_os_slice_fine(tdm_pipe_main, tdm_pipe_main_len, tdm_port_pmap, tdm_port_speed);
		...
		filter_result = 0;
		timeout=DITHER_PASS;
		while ( tdm_gh2_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>1 && 
		        tdm_gh2_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>=tdm_gh2_slice_size(1,tdm_pipe_main,tdm_pipe_main_len) && 
		       ((--timeout)>0) ) {			
			filter_result += tdm_gh2_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,tdm_port_pmap,DITHER_THRESHOLD,tdm_port_speed);
		}
		timeout=DITHER_PASS;
		while ( tdm_gh2_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>1 && 
		        tdm_gh2_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>=tdm_gh2_slice_size(1,tdm_pipe_main,tdm_pipe_main_len) && 
				((--timeout)>0) ) {			
			filter_result += tdm_gh2_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,tdm_port_pmap,(DITHER_THRESHOLD-DITHER_SUBPASS_STEP_SIZE),tdm_port_speed);
		}
		timeout=DITHER_PASS;
		while ( tdm_gh2_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>1 && 
		        tdm_gh2_slice_size(ovsb_token,tdm_pipe_main,tdm_pipe_main_len)>=tdm_gh2_slice_size(1,tdm_pipe_main,tdm_pipe_main_len) && 
				((--timeout)>0) ) {			
			filter_result += tdm_gh2_filter_dither(tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,tdm_port_pmap,(DITHER_THRESHOLD-DITHER_SUBPASS_STEP_SIZE-DITHER_SUBPASS_STEP_SIZE),tdm_port_speed);
		}
		...
		/* Smooth oversub distribution */
		filter_result = 0;
		if (tdm_gh2_slice_size(1, tdm_pipe_main, tdm_pipe_main_len)==2) {
			for (i=3; i<tdm_pipe_main_len; i++) {
				if (tdm_pipe_main[i-3]!=ovsb_token && 
					...
					tdm_gh2_scan_slice_size_local((i-3), tdm_pipe_main, tdm_pipe_main_len, &slice_idx)==2 && 
					tdm_gh2_scan_slice_size_local( i,    tdm_pipe_main, tdm_pipe_main_len, &slice_idx)==2 && 
					tdm_gh2_scan_slice_size_local((i+3), tdm_pipe_main, tdm_pipe_main_len, &slice_idx)==2 && 
					tdm_gh2_scan_slice_size_local((i+6), tdm_pipe_main, tdm_pipe_main_len, &slice_idx)==2) {
					if (tdm_gh2_filter_fine_dither(i,tdm_pipe_main,_tdm->_chip_data.soc_pkg.lr_idx_limit,_tdm->_chip_data.soc_pkg.tvec_size,tdm_port_pmap)==FAIL) {
						break;
					}
					filter_result++;
				}
...
/**
@name: tdm_gh2_filter_ovsb_p1
@param:
 */
int
tdm_gh2_filter_ovsb_p1(tdm_mod_t *_tdm)
{
...
/**
@name: tdm_gh2_filter_ovsb_5x
@param:
 */
int
tdm_gh2_filter_ovsb_5x(tdm_mod_t *_tdm)
{
...
/**
@name: tdm_gh2_filter_ovsb_4x
@param:
 */
int
tdm_gh2_filter_ovsb_4x(tdm_mod_t *_tdm)
{
    int i, lr_slot_cnt, os_slot_cnt, filter_cnt=0,
        x, y, port_x, port_y, tsc_x, tsc_y, sister_prox;
    int param_lr_limit, param_ancl_num, param_cal_len, 
...
/**
@name: tdm_gh2_filter_ovsb_3x
@param:
 */
int
tdm_gh2_filter_ovsb_3x(tdm_mod_t *_tdm)
{
    int i, lr_slot_cnt, os_slot_cnt, filter_cnt=0,
        x, y, port_x, port_y, tsc_x, tsc_y, sister_prox;
    int param_lr_limit, param_ancl_num, param_cal_len, 
...
/**
@name: tdm_gh2_filter_ovsb_2x
@param:
 */
int
tdm_gh2_filter_ovsb_2x(tdm_mod_t *_tdm)
{
    int i, lr_slot_cnt, os_slot_cnt, filter_cnt=0,
        x, y, port_x, port_y, tsc_x, tsc_y, sister_prox;
    int param_lr_limit, param_ancl_num, param_cal_len, 
...
/**
@name: tdm_gh2_filter_ovsb_1x
@param:
 */
int
tdm_gh2_filter_ovsb_1x(tdm_mod_t *_tdm)
{
    int i, j, k, lr_slot_cnt, os_slot_cnt, filter_cnt=0,
        x, y, port_x, port_y, tsc_x, tsc_y, sister_prox,
...
/**
@name: tdm_gh2_filter_ovsb
@param:
 */
int
tdm_gh2_filter_ovsb(tdm_mod_t *_tdm)
{
    int filter_cnt;
    
    /* Z_Y_OVSB_X_OVSB_OVSB */
    tdm_gh2_filter_ovsb_p1(_tdm);
    /* OVSB_OVSB_OVSB_OVSB_OVSB */
    tdm_gh2_filter_ovsb_5x(_tdm);
    /* OVSB_OVSB_OVSB_OVSB */
    tdm_gh2_filter_ovsb_4x(_tdm);
    /* OVSB_OVSB_OVSB */
    tdm_gh2_filter_ovsb_3x(_tdm);
    /* OVSB_OVSB */
    tdm_gh2_filter_ovsb_2x(_tdm);
    /* OVSB */
    filter_cnt = tdm_gh2_filter_ovsb_1x(_tdm);
    if (filter_cnt>0){
        tdm_gh2_filter_ovsb_1x(_tdm);
    }
...
/**
@name: tdm_gh2_filter_lr_jitter
@param:
 */
int
tdm_gh2_filter_lr_jitter(tdm_mod_t *_tdm, int min_speed)
{
    int i, j, k, filter_cnt=0, x, y, chk_x, chk_y, spd_x, spd_y;
    int m, port, port_speed, idx_curr,
        space_min, space_max,
        dist_up, dist_dn, dist_mv;
    int port_bmp[GH2_NUM_EXT_PORTS];
    int param_lr_limit, param_ancl_num, param_cal_len,
        param_phy_lo, param_phy_hi,
        param_lr_en;
    int *param_cal_main;
                    ...
                    if (param_cal_main[k]==port) {
                        ...
                        if (dist_up>space_max || dist_dn<space_min) {
                            ...
                            for (m=1; m<=dist_mv; m++) {
                                ...
                                y = (x + 1) % param_cal_len;
                                chk_x = tdm_gh2_filter_chk_slot_shift_cond(x, TDM_DIR_DN,
                                            param_cal_len, param_cal_main, _tdm);
                                chk_y = tdm_gh2_filter_chk_slot_shift_cond(y, TDM_DIR_UP,
                                            param_cal_len, param_cal_main, _tdm);
                                spd_x = tdm_gh2_filter_get_port_speed(param_cal_main[x], _tdm);
                                spd_y = tdm_gh2_filter_get_port_speed(param_cal_main[y], _tdm);
                                /* shift port */
                        ...
                        /* filter port if space violation is detected: down */
                            ...
                            for (m=1; m<=dist_mv; m++) {
                                ...
                                chk_x = tdm_gh2_filter_chk_slot_shift_cond(x, TDM_DIR_UP,
                                            param_cal_len, param_cal_main, _tdm);
                                chk_y = tdm_gh2_filter_chk_slot_shift_cond(y, TDM_DIR_DN,
                                            param_cal_len, param_cal_main, _tdm);
                                spd_x = tdm_gh2_filter_get_port_speed(param_cal_main[x], _tdm);
                                spd_y = tdm_gh2_filter_get_port_speed(param_cal_main[y], _tdm);
                                /* shift port */
...
/**
@name: tdm_gh2_filter_lr
@param:
 */
int
tdm_gh2_filter_lr(tdm_mod_t *_tdm)
{
    if (tdm_gh2_filter_lr_jitter(_tdm, SPEED_10G) == 0){
        return PASS;
    }
    if (tdm_gh2_filter_lr_jitter(_tdm, SPEED_20G) == 0) {
        return PASS;
    }
    if (tdm_gh2_filter_lr_jitter(_tdm, SPEED_40G) == 0) {
        return PASS;
    }
    return PASS;
}
...
/**
@name: tdm_gh2_filter_ovsb_4x_mix
@param:
 */
int
tdm_gh2_filter_ovsb_4x_mix(tdm_mod_t *_tdm)
{
    int i, lr_slot_cnt, os_slot_cnt, filter_cnt=0,
            ...
            for (i=0; i<(param_cal_len-6); i++){
                ...
                /* ovsb_ovsb_ovsb_ovsb_x_y -> ovsb_ovsb_x_ovsb_ovsb_y */
                    ...
                    if (port_x>=param_phy_lo && port_x<=param_phy_hi &&
                        port_y>=param_phy_lo && port_y<=param_phy_hi) {
                        spd_x  = tdm_gh2_filter_get_port_speed(port_x, _tdm);
                        spd_y  = tdm_gh2_filter_get_port_speed(port_y, _tdm);
                        tsc_y  = tdm_gh2_filter_get_port_pm(port_y, _tdm);
                        tsc_x  = tdm_gh2_filter_get_port_pm(port_x, _tdm);
                        if (tsc_x == tsc_y ||
                            spd_x >= SPEED_100G || spd_y >= SPEED_100G) {
                            sister_prox = FAIL;
                        }
...
/**
@name: tdm_gh2_filter_ovsb_3x_mix
@param:
 */
int
tdm_gh2_filter_ovsb_3x_mix(tdm_mod_t *_tdm)
{
    int i, lr_slot_cnt, os_slot_cnt, filter_cnt=0,
        x, y, port_x, port_y, tsc_x, tsc_y, spd_x, spd_y, sister_prox;
    int param_lr_limit, param_ancl_num, param_cal_len, 
...
/**
@name: tdm_gh2_filter_ovsb_2x_mix
@param:
 */
int
tdm_gh2_filter_ovsb_2x_mix(tdm_mod_t *_tdm)
{
    int i, lr_slot_cnt, os_slot_cnt, filter_cnt=0,
 ...
 */
int
tdm_gh2_filter_ovsb_mix(tdm_mod_t *_tdm)
{
    tdm_gh2_filter_ovsb_4x_mix(_tdm);
    tdm_gh2_filter_ovsb_3x_mix(_tdm);
    tdm_gh2_filter_ovsb_2x_mix(_tdm);
 ...
 */
int
tdm_gh2_filter_mix( tdm_mod_t *_tdm )
{
    int param_lr_en, param_os_en, param_cal_id;
