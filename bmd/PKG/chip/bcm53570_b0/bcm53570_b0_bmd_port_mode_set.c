...
int
bcm53570_b0_bmd_port_mode_set(int unit, int port,
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        if (cnt < 0) {
            CDK_WARN(("bcm53570_b0_bmd_port_mode_set[%d]: "
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
            ...
            */
            ...
            rv = bcm53570_b0_bmd_port_mode_get(unit, port,
                                            &cur_mode, &cur_flags);
    ...
    if (mode == bmdPortModeDisabled) {
        ...
        if (CDK_PBMP_MEMBER(xlpbmp, port)) {
            ...
            /* For this chip, QTCE ports need phy linkup signal to 
               activate MAC loopback.
            */
            if (IS_TSCQ(port) && (mac_lb == 1)) {
                bmd_phy_loopback_set(unit, port, 1);
                BMD_SYS_USLEEP(1000);

                bmd_phy_loopback_set(unit, port, 0);
            }
