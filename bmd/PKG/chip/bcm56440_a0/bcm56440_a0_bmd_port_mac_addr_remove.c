...
l2x_delete(int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr)
{
    ...
    schan_msg.gencmd.address = cdk_xgsm_blockport_addr(unit, ipipe_blk, -1, L2Xm, 0);

    /* Write header word + L2X entry */
    return cdk_xgsm_schan_op(unit, &schan_msg, 4, 0);
}

int
bcm56440_a0_bmd_port_mac_addr_remove(int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr)
{
    BMD_CHECK_UNIT(unit);
