...
int
bcm56820_a0_bmd_port_mode_set(int unit, int port, 
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if (speed == 1000) {
        ...
        case bmdPortMode2500fd:
            /* 2.5 Gbps supported only on XE ports */
    ...
    if (speed > bcm56820_a0_port_speed_max(unit, port)) {
        return CDK_E_PARAM;
    }
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        if (cnt < 0) {
            CDK_WARN(("bcm56820_a0_bmd_port_mode_set[%d]: "
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
             ...
             */
            rv = bcm56820_a0_bmd_port_mode_get(unit, port, 
                                               &cur_mode, &cur_flags);
            if (CDK_SUCCESS(rv) && 
                ...
                rv = bcm56820_a0_gxport_reset(unit, port);
                if (CDK_SUCCESS(rv)) {
                    rv = bcm56820_a0_gxport_init(unit, port);
                }
            }
        }
#endif
