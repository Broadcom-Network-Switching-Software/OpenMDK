...
static int
bcm56334_a0_unicore_phy_init(int unit, int port)
{
    int ioerr = 0;
...
int
bcm56334_a0_xport_reset(int unit, int port)
{
    int ioerr = 0;
    int msec;
    int idx;
    ...
    if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
        ioerr += bcm56334_a0_unicore_phy_init(unit, port);    
    }
#endif
    ...
    if (msec >= PLL_LOCK_MSEC) {
        CDK_WARN(("bcm56334_a0_xport_reset[%d]: "
...
int
bcm56334_a0_bmd_reset(int unit)
{
    int ioerr = 0;
    int wait_usec = 10000;
    int port;
    ...
    CMIC_SOFT_RESET_REGr_CMIC_XG1_PLL_RST_Lf_SET(cmic_sreset, 1);
    ...
    CMIC_SOFT_RESET_REGr_CMIC_GX8_SERDES_0_RST_Lf_SET(cmic_sreset, 1);
    CMIC_SOFT_RESET_REGr_CMIC_GX8_SERDES_1_RST_Lf_SET(cmic_sreset, 1);
    CMIC_SOFT_RESET_REGr_CMIC_GX8_SERDES_2_RST_Lf_SET(cmic_sreset, 1);
    ioerr += WRITE_CMIC_SOFT_RESET_REGr(unit, cmic_sreset);
    BMD_SYS_USLEEP(50);
 ...
 */
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ioerr += bcm56334_a0_xport_reset(unit, port);
    }
