...
int
bcm56334_a0_bmd_port_mode_set(int unit, int port, 
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    OP_PORT_TOTAL_COUNT_CELLr_t tot_cnt_cell;
    ...
    if (port == 26 || port == 27) {
        if (CDK_XGS_FLAGS(unit) & CHIP_FLAG_HL25) {
            hglite_port = 1;
        }
    }
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        while (--cnt >= 0) {
            ioerr += READ_OP_PORT_TOTAL_COUNT_CELLr(unit, port, &tot_cnt_cell);
            if (OP_PORT_TOTAL_COUNT_CELLr_GET(tot_cnt_cell) == 0) {
                break;
            }
            BMD_SYS_USLEEP(10000);
        }
        if (cnt < 0) {
            CDK_WARN(("bcm56334_a0_bmd_port_mode_set[%d]: "
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
             ...
             */
            rv = bcm56334_a0_bmd_port_mode_get(unit, port, 
                                               &cur_mode, &cur_flags);
            ...
            if ((CDK_SUCCESS(rv) && 
                ...
                rv = bcm56334_a0_xport_reset(unit, port);
                if (CDK_SUCCESS(rv)) {
                    rv = bcm56334_a0_xport_init(unit, port);
                }
            }
        }
#endif
