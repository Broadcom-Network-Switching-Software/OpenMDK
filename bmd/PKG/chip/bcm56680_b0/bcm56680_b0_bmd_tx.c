...
int
bcm56680_b0_bmd_tx(int unit, const bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    TX_DCB_t *dcb;
    dma_addr_t bdcb;
    ...
    CDK_VVERB(("Tx DMA ctrl = %08"PRIx32" %08"PRIx32"\n", 
               TX_DCB_GET(dcb[0], 1), TX_DCB_GET(dcb[1], 1)));
    CDK_VVERB(("Tx DMA mhdr = %08"PRIx32" %08"PRIx32" %08"PRIx32" %08"PRIx32"\n", 
               TX_DCB_GET(dcb[0], 2), TX_DCB_GET(dcb[0], 3), 
               TX_DCB_GET(dcb[0], 4), TX_DCB_GET(dcb[0], 5)));
    ...
    CDK_VVERB(("Tx DMA stat = %08"PRIx32" %08"PRIx32"\n", 
               TX_DCB_GET(dcb[0], 10), 
               (pkt->flags & BMD_PKT_F_UNTAGGED) ? TX_DCB_GET(dcb[1], 10) : 0));
