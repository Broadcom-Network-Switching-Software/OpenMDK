#include <bmd_config.h>
#if CDK_CONFIG_INCLUDE_BCM53134_A0 == 1

/*
 * 
 * This software is governed by the Broadcom Switch APIs license.
 * This license is set out in https://raw.githubusercontent.com/Broadcom-Network-Switching-Software/OpenMDK/master/Legal/LICENSE file.
 * 
 * Copyright 2007-2020 Broadcom Inc. All rights reserved.
 */

#include <bmd/bmd.h>

#include <cdk/chip/bcm53134_a0_defs.h>
#include <cdk/cdk_debug.h>

#include "bcm53134_a0_bmd.h"

int
bcm53134_a0_bmd_vlan_port_add(int unit, int vlan, int port, uint32_t flags)
{
    int ioerr = 0;
    VLAN_1Qm_t vlan_tab;
    uint32_t pbmp;
    BMD_CHECK_UNIT(unit);
    BMD_CHECK_VLAN(unit, vlan);
    BMD_CHECK_PORT(unit, port);

    ioerr += READ_VLAN_1Qm(unit, vlan, &vlan_tab);
    if (VLAN_1Qm_MSPT_ID_SWf_GET(vlan_tab) == 0) {
        return ioerr ? CDK_E_IO : CDK_E_NOT_FOUND;
    }
    pbmp = VLAN_1Qm_FORWARD_MAP_SWf_GET(vlan_tab);
    if (pbmp & (1 << port)) {
        return ioerr ? CDK_E_IO : CDK_E_EXISTS;
    }
    VLAN_1Qm_FORWARD_MAP_SWf_SET(vlan_tab, pbmp | (1 << port));
    if (flags & BMD_VLAN_PORT_F_UNTAGGED) {
        pbmp = VLAN_1Qm_UNTAG_MAP_SWf_GET(vlan_tab);
        VLAN_1Qm_UNTAG_MAP_SWf_SET(vlan_tab, pbmp | (1 << port));
    }
    pbmp = VLAN_1Qm_UNTAG_MAP_SWf_GET(vlan_tab);
    ioerr += WRITE_VLAN_1Qm(unit, vlan, vlan_tab);

    return ioerr ? CDK_E_IO : CDK_E_NONE;
}

#endif /* CDK_CONFIG_INCLUDE_BCM53134_A0 */
