...
int
bcm56634_a0_bmd_stat_get(int unit, int port, bmd_stat_t stat, bmd_counter_t *counter)
{
    int ioerr = 0;
    int collect_bigmac_counters = 0;
    RRPKTr_t rrpkt;
    RDBGC0r_t rdbgc0;
    ...
    if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
        ...
        /* Rx counter bug: skip reading BigMAC counters in GPORT mode */
        if (port >= 30) {
            ioerr += READ_XQPORT_MODE_REGr(unit, &xqport_mode, port);
            if (XQPORT_MODE_REGr_XQPORT_MODE_BITSf_GET(xqport_mode) == 2) {
                collect_bigmac_counters = 1;
            }
        } else {
            ioerr += READ_XPORT_MODE_REGr(unit, port, &xport_mode);
            if (XPORT_MODE_REGr_XPORT_MODE_BITSf_GET(xport_mode) == 1) {
                collect_bigmac_counters = 1;
            }
        }
    }
    ...
    case bmdStatRxDrops:
        ...
        counter->v[0] += RDBGC0r_GET(rdbgc0) & ((1L << 26) - 1);
        break;
    default:
        break;
    }
