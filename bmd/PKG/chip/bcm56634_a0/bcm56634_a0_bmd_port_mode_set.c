...
int
bcm56634_a0_bmd_port_mode_set(int unit, int port, 
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if ((flags & BMD_PORT_MODE_F_INTERNAL) == 0) {
        ...
        if (cnt < 0) {
            CDK_WARN(("bcm56634_a0_bmd_port_mode_set[%d]: "
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
             ...
             */
            rv = bcm56634_a0_bmd_port_mode_get(unit, port, 
                                               &cur_mode, &cur_flags);
            if (CDK_SUCCESS(rv) && 
                ...
                rv = bcm56634_a0_xport_reset(unit, port);
                if (CDK_SUCCESS(rv)) {
                    rv = bcm56634_a0_xport_init(unit, port);
                }
            }
        }
#endif
    ...
    if (mode == bmdPortModeDisabled) {
        ...
        if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
            ...
            ICONTROL_OPCODE_BITMAP_64r_t opcode_bmap;
            ...
            ICONTROL_OPCODE_BITMAP_64r_SET(opcode_bmap, 0, hg ? 0x1 : 0x0);
            /* Set HiGig vs. HiGig2 */
            ...
            ioerr += WRITE_ICONTROL_OPCODE_BITMAP_64r(unit, port, opcode_bmap);
            ...
            /* Configure 10G mode vs 2.5G mode */
            if (port >= 30) {
                ioerr += READ_XQPORT_MODE_REGr(unit, &xqport_mode, port);
                xmode = (speed >= 10000) ? 2 : 1;
                XQPORT_MODE_REGr_XQPORT_MODE_BITSf_SET(xqport_mode, xmode);
                ioerr += WRITE_XQPORT_MODE_REGr(unit, xqport_mode, port);
            } else {
                xmode = (speed >= 10000) ? 1 : 2;
                ioerr += READ_XPORT_MODE_REGr(unit, port, &xport_mode);
                XPORT_MODE_REGr_XPORT_MODE_BITSf_SET(xport_mode, xmode);
            ...
            MAC_CTRLr_RXENf_SET(mac_ctrl, (speed >= 10000) ? 1 : 0);
            ioerr += WRITE_MAC_CTRLr(unit, port, mac_ctrl);
#endif
        }
        if (speed < 10000) {
