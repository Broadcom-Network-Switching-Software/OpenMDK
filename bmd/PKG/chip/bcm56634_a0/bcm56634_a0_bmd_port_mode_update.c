...
int
bcm56634_a0_bmd_port_mode_update(int unit, int port)
{
    int rv = CDK_E_NONE;
    int ioerr = 0;
    int led_flags = 0;
    ...
    if (CDK_SUCCESS(rv) && status_change) {
        rv = bcm56634_a0_bmd_port_mode_get(unit, port, &mode, &flags);
        if (CDK_SUCCESS(rv)) {
            flags |= BMD_PORT_MODE_F_INTERNAL;
            rv = bcm56634_a0_bmd_port_mode_set(unit, port, mode, flags);
        }
