...
static int
bcm56634_a0_hyperlite_phy_init(int unit, int port)
{
    int ioerr = 0;
#if BMD_CONFIG_INCLUDE_PHY == 1
    int phy_addr = port - 29 + CDK_XGS_MIIM_INTERNAL + CDK_XGS_MIIM_BUS_2;
    uint32_t mreg_val;
...
static int
bcm56634_a0_unicore_phy_init(int unit, int port)
{
    int ioerr = 0;
#if BMD_CONFIG_INCLUDE_PHY == 1
...
int
bcm56634_a0_xport_reset(int unit, int port)
{
    int ioerr = 0;
    int msec;
    int idx;
    ...
    if (BMD_PORT_PROPERTIES(unit, port) & (BMD_PORT_HG | BMD_PORT_XE)) {
        if (port >= 30) {
            ioerr += bcm56634_a0_hyperlite_phy_init(unit, port);
        } else {
            ioerr += bcm56634_a0_unicore_phy_init(unit, port);
        }
    }
#endif
    ...
    if (msec >= PLL_LOCK_MSEC) {
        CDK_WARN(("bcm56634_a0_xport_reset[%d]: "
...
static int
bcm56634_a0_lcpll_init(int unit)
{
    int ioerr = 0;
    int msec;
    ...
    if (msec >= PLL_LOCK_MSEC) {
        CDK_WARN(("bcm56634_a0_bmd_reset[%d]: "
...
int
bcm56634_a0_bmd_reset(int unit)
{
    int ioerr = 0;
    int wait_usec = 10000;
    int port;
    ...
    /* Initialize LC PLLs */
    ioerr += bcm56634_a0_lcpll_init(unit);
    ...
    CDK_PBMP_ITER(pbmp, port) {
        ioerr += bcm56634_a0_xport_reset(unit, port);
    }
