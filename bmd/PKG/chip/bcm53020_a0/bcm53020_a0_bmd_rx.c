...
/* NSP specific design on Tx/Rx :
 * - Broadcom Tag in Packet for NSP is at the position in front of MAC_DA but 
 *   for Other Robo devices is at the position right after MAC_SA.
 */
...
int
bcm53020_a0_bmd_rx_start(int unit, bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    int rv;
...
int
bcm53020_a0_bmd_rx_poll(int unit, bmd_pkt_t **ppkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    int rv = CDK_E_NONE;
    ...
    if (pkt->size > (12 + ROBO_BRCM_HDR_SIZE)) {
        ...
        CDK_MEMCPY(brcm_hdr, &brcm_hdr[ROBO_BRCM_HDR_SIZE], 
                    pkt->size - ROBO_BRCM_HDR_SIZE);
    }
...
int
bcm53020_a0_bmd_rx_stop(int unit)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    int rv;
