...
_cpu_port_enable_set(int unit, int enable)
{
    ...
    epc_pbm = EPC_LINK_BMAPr_PORT_BITMAPf_GET(epc_link);
    if (enable) {
...
int
bcm56314_a0_bmd_rx_start(int unit, bmd_pkt_t *pkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    dma_addr_t bdcb;
...
int
bcm56314_a0_bmd_rx_poll(int unit, bmd_pkt_t **ppkt)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    bmd_pkt_t *pkt;
    ...
    bmd_xgs_dump_rx_dcb(unit, (uint32_t *)dcb,
                        CDK_BYTES2WORDS(RX_DCB_SIZE), CDK_HIGIG_WSIZE);
    ...
    bmd_xgs_parse_higig(unit, pkt, RX_DCB_MODULE_HEADERf_PTR(*dcb));
...
int
bcm56314_a0_bmd_rx_stop(int unit)
{
#if BMD_CONFIG_INCLUDE_DMA == 1
    RX_DCB_t *dcb;
    int rv = CDK_E_NONE;
