                              ...
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if(CDK_XGS_FLAGS(unit) & CHIP_FLAG_FE_ONLY) {
        return bcm56146_a0_bmd_port_mode_set(unit, port, mode, flags);
    }
    
    if (port > 25 && port < 30) {
        if (CDK_XGS_FLAGS(unit) & CHIP_FLAG_HD25_ONLY) {
            return bcm56146_a0_bmd_port_mode_set(unit, port, mode, flags);
    ...
    if(((CDK_XGS_FLAGS(unit) & CHIP_FLAG_HD127) && (port > 27)) ||
       ((CDK_XGS_FLAGS(unit) & CHIP_FLAG_HD25_HD127) && (port > 25))) {
        hgdual_port = 1;
    } else {
        if (((CDK_CHIP_CONFIG(unit) & DCFG_HGD) && ((port > 25) && (port < 30))) ||
            ((CDK_CHIP_CONFIG(unit) & DCFG_HG_HGD) && ((port > 27) && (port < 30)))) {
            hgdual_port = 1;
        }
    }
                              ...
                              bmd_port_mode_t mode, uint32_t flags)
{
    ...
    if (port > 25 && port < 30) {
        if ((CDK_XGS_FLAGS(unit) & CHIP_FLAG_HD25) ||
            ...
            (CDK_XGS_FLAGS(unit) & CHIP_FLAG_HD25_HD127)) {
            hglite_port = 1;
        } else {
            if (!CDK_CHIP_CONFIG(unit) ||
                (CDK_CHIP_CONFIG(unit) & DCFG_GE) ||
