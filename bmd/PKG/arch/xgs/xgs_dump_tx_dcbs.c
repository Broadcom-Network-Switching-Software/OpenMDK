 ...
 */
int
bmd_xgs_dump_tx_dcbs(int unit, uint32_t *dcbs, int dcb_cnt,
                     int dcb_size, int mh_size)
{
    ...
    if (CDK_DEBUG_CHECK(CDK_DBG_DMA | CDK_DBG_VVERBOSE)) {
        ...
        for (idx = 0; idx < dcb_cnt; idx++) {
            ...
            cdk_symbol_dump("TX_DCB", CDK_XGS_SYMBOLS(unit),
                            &dcbs[(idx*dcb_size)]);
        }
    }
#endif
    ...
    if (mh_size > 0 && (dcbs[2] >> 24) != 0xff &&
        ...
        cdk_symbol_dump(sym_name, &higig_symbols, &dcbs[2]); 
    }
#endif
