 ...
 */
int
bmd_xgs_dump_rx_dcb(int unit, uint32_t *dcb,
                    int dcb_size, int mh_size)
{
    /* Dump DMA descriptor */
    _dump_words("Rx DMA ctrl =", &dcb[1], 1);
    _dump_words("Rx DMA stat =", &dcb[dcb_size-1], 1);
    _dump_words("Rx pkt stat =", &dcb[2+mh_size], dcb_size - mh_size - 3);
    ...
    if (CDK_DEBUG_CHECK(CDK_DBG_DMA | CDK_DBG_VVERBOSE)) {
        ...
        cdk_symbol_dump("RX_DCB", CDK_XGS_SYMBOLS(unit), dcb); 
    }
#endif

    /* Dump module header if supplied */
    _dump_words("Rx DMA mhdr =", &dcb[2], mh_size);
    ...
    if (mh_size > 0 &&
        ...
        cdk_symbol_dump(sym_name, &higig_symbols, &dcb[2]); 
    }
#endif
