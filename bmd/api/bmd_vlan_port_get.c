...
extern int _bd##_bmd_vlan_port_get(int unit, int vlan, int *plist, int *utlist);
#include <bmdi/bmd_devlist.h>

/* Dispatch table for this function, which uses the externs above */
#define BMD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_fl,_cn,_pf,_pd,_r0,_r1) \
_bd##_bmd_vlan_port_get,
static int (*__dispatch_table[])(int unit, int vlan, int *plist, int *utlist) =
{
#include <bmdi/bmd_devlist.h>
0
};
    ...
    int *utlist)
{
    ...
    return __dispatch_table[dev_type](unit, vlan, plist, utlist);
}
