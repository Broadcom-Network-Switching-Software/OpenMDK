...
extern int _bd##_bmd_stat_get(int unit, int port, bmd_stat_t stat, bmd_counter_t *counter);
#include <bmdi/bmd_devlist.h>

/* Dispatch table for this function, which uses the externs above */
#define BMD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_fl,_cn,_pf,_pd,_r0,_r1) \
_bd##_bmd_stat_get,
static int (*__dispatch_table[])(int unit, int port, bmd_stat_t stat, bmd_counter_t *counter) =
{
#include <bmdi/bmd_devlist.h>
0
};
    ...
    bmd_counter_t *counter)
{
    ...
    return __dispatch_table[dev_type](unit, port, stat, counter);
}
