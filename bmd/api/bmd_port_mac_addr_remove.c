...
extern int _bd##_bmd_port_mac_addr_remove(int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr);
#include <bmdi/bmd_devlist.h>

/* Dispatch table for this function, which uses the externs above */
#define BMD_DEVLIST_ENTRY(_nm,_vn,_dv,_rv,_md,_pi,_bd,_bc,_fn,_fl,_cn,_pf,_pd,_r0,_r1) \
_bd##_bmd_port_mac_addr_remove,
static int (*__dispatch_table[])(int unit, int port, int vlan, const bmd_mac_addr_t *mac_addr) =
{
#include <bmdi/bmd_devlist.h>
0
};
    ...
    const bmd_mac_addr_t *mac_addr)
{
    ...
    return __dispatch_table[dev_type](unit, port, vlan, mac_addr);
}
